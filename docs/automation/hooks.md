...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be inspected with `aikaclaw hooks`, while hook-pack installation and updates now go through `aikaclaw plugins`.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in AikaClaw. See [Webhook Hooks](/automation/webhook) or use `aikaclaw webhooks` for Gmail helper commands.

Hooks can also be bundled inside plugins; see [Plugin hooks](/plugins/architecture#provider-runtime-hooks). `aikaclaw hooks list` shows both standalone hooks and plugin-managed hooks.
...
If you can write a small TypeScript function, you can write a hook. Managed and bundled hooks are trusted local code. Workspace hooks are discovered automatically, but AikaClaw keeps them disabled until you explicitly enable them via the CLI or config.
...
- Extend AikaClaw's behavior without modifying core code

## Getting Started

### Bundled Hooks

AikaClaw ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.aikaclaw/workspace/memory/`) when you issue `/new` or `/reset`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.aikaclaw/logs/commands.log`
...
```bash
aikaclaw hooks enable session-memory
```

Check hook status:

```bash
aikaclaw hooks check
```

Get detailed information:

```bash
aikaclaw hooks info session-memory
```

### Onboarding

During onboarding (`aikaclaw onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.

### Trust Boundary

Hooks run inside the Gateway process. Treat bundled hooks, managed hooks, and `hooks.internal.load.extraDirs` as trusted local code. Workspace hooks under `<workspace>/hooks/` are repo-local code, so AikaClaw requires an explicit enable step before loading them.

## Hook Discovery

Hooks are automatically discovered from these directories, in order of increasing override precedence:

1. **Bundled hooks**: shipped with AikaClaw; located at `<aikaclaw>/dist/hooks/bundled/` for npm installs (or a sibling `hooks/bundled/` for compiled binaries)
2. **Plugin hooks**: hooks bundled inside installed plugins (see [Plugin hooks](/plugins/architecture#provider-runtime-hooks))
3. **Managed hooks**: `~/.aikaclaw/hooks/` (user-installed, shared across workspaces; can override bundled and plugin hooks). **Extra hook directories** configured via `hooks.internal.load.extraDirs` are also treated as managed hooks and share the same override precedence.
...
Hook packs are standard npm packages that export one or more hooks via `aikaclaw.hooks` in
`package.json`. Install them with:

```bash
aikaclaw plugins install <path-or-spec>
```
...
those to a prerelease, AikaClaw stops and asks you to opt in explicitly with a
...
Hook packs can ship dependencies; they will be installed under `~/.aikaclaw/hooks/<id>`.
Each `aikaclaw.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `aikaclaw plugins install` installs hook-pack dependencies with `npm install --ignore-scripts`
...
The `metadata.aikaclaw` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before AikaClaw persists them.
...
- **Managed hooks** (`~/.aikaclaw/hooks/`): Shared across workspaces; can override bundled and plugin hooks

### 2. Create Directory Structure

```bash
mkdir -p ~/.aikaclaw/hooks/my-hook
cd ~/.aikaclaw/hooks/my-hook
```
...
metadata: { "aikaclaw": { "emoji": "🎯", "events": ["command:new"] } }
---
...
aikaclaw hooks list --eligible

# Verbose output (show missing requirements)
aikaclaw hooks list --verbose

# JSON output
aikaclaw hooks list --json
```
...
# JSON output
aikaclaw hooks info session-memory --json
```
...
# Disable a hook
aikaclaw hooks disable command-logger
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.aikaclaw/workspace`)
...
```bash
aikaclaw hooks enable bootstrap-extra-files
```
...
**Output**: `~/.aikaclaw/logs/commands.log`
...
tail -n 20 ~/.aikaclaw/logs/commands.log

# Pretty-print with jq
cat ~/.aikaclaw/logs/commands.log | jq .

# Filter by action
grep '"action":"new"' ~/.aikaclaw/logs/commands.log | jq .
```

**Enable**:

```bash
aikaclaw hooks enable command-logger
```
...
```yaml
metadata: { "aikaclaw": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "aikaclaw": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.aikaclaw/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.aikaclaw/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.aikaclaw/hooks/my-hook/handler.ts
   ```
