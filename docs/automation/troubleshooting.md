...
aikaclaw system heartbeat last
aikaclaw logs --follow
aikaclaw config get agents.defaults.heartbeat
aikaclaw channels status --probe
```
...
```bash
aikaclaw config get agents.defaults.heartbeat.activeHours
aikaclaw config get agents.defaults.heartbeat.activeHours.timezone
aikaclaw config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
aikaclaw cron list
aikaclaw logs --follow
```
