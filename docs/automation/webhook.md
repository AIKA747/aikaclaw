...
read_when:
  ...
  - Wiring external systems into AikaClaw
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your AikaClaw config directory (typically `~/.aikaclaw/hooks/transforms`).
...
- `aikaclaw webhooks gmail setup` writes `hooks.gmail` config for `aikaclaw webhooks gmail run`.
