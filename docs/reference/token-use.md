---
summary: "How AikaClaw builds prompt context and reports token usage + costs"
read_when:
...
AikaClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

AikaClaw assembles its own system prompt on every run. It includes:
...
For images, AikaClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from AikaClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `aikaclaw status --usage` and `aikaclaw channels list` show
...
`cacheWrite`. If pricing is missing, AikaClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. AikaClaw can
...
Anthropic's 1M context window is currently beta-gated. AikaClaw can inject the
...
AikaClaw skips the `context-1m-*` beta header because Anthropic currently
