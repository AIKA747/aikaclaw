...
AikaClaw has three public release lanes:
...
- Run `RELEASE_TAG=vYYYY.M.D node --import tsx scripts/aikaclaw-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/aikaclaw-npm-postpublish-verify.ts YYYY.M.D`
