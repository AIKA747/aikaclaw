...
This document explains how AikaClaw manages sessions end-to-end:
...
AikaClaw is designed around a single **Gateway process** that owns session state.
...
AikaClaw persists sessions in two layers:
...
- Transcripts: `~/.aikaclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

AikaClaw resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, AikaClaw reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.aikaclaw/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
AikaClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (AikaClaw consumes the events, but Pi decides when to compact).
...
AikaClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, AikaClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, AikaClaw leaves it alone.
...
- `aikaclaw status` (CLI)
- `aikaclaw sessions` / `sessions --json`
...
AikaClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- AikaClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, AikaClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
AikaClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but AikaClaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `aikaclaw status`.
