...
- With Anthropic API-key auth profiles, AikaClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, AikaClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
AikaClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.aikaclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$AIKACLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `AIKACLAW_CACHE_TRACE=1` enables cache tracing.
- `AIKACLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `AIKACLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `AIKACLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `AIKACLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
