...
This is the full reference for `aikaclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.aikaclaw/aikaclaw.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `aikaclaw doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.aikaclaw/credentials/oauth.json`; auth profiles live in `~/.aikaclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.aikaclaw/credentials/oauth.json` (or `$AIKACLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.aikaclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `aikaclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `aikaclaw configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `aikaclaw health`.
    - Tip: `aikaclaw status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export AIKACLAW_GATEWAY_TOKEN="your-token"
aikaclaw onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env AIKACLAW_GATEWAY_TOKEN
```
...
- Stores it under `~/.aikaclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.aikaclaw/aikaclaw.json`:
...
`aikaclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.aikaclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.aikaclaw/agents/<agentId>/sessions/`.
