...
AikaClaw features that can generate provider usage or paid API calls.
...
- `aikaclaw status --usage` and `aikaclaw channels list` show provider **usage windows**
...
AikaClaw can pick up credentials from:
...
- `aikaclaw status --usage`
- `aikaclaw models status --json`
...
`aikaclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
