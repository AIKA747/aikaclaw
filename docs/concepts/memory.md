---
title: "Memory"
summary: "How AikaClaw memory works (workspace files + automatic memory flush)"
read_when:
...
AikaClaw memory is **plain Markdown in the agent workspace**. The files are the
...
- `MEMORY.md` (optional)
  ...
  - If both `MEMORY.md` and `memory.md` exist at the workspace root, AikaClaw loads both (deduplicated by realpath so symlinks pointing to the same file are not injected twice).
  - **Only load in the main, private session** (never in group contexts).

These files live under the workspace (`agents.defaults.workspace`, default
`~/.aikaclaw/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.

## Memory tools

AikaClaw exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, AikaClaw triggers a **silent,
...
AikaClaw can build a small vector index over `MEMORY.md` and `memory/*.md` so
