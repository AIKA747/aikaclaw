...
read_when:
  - You want to understand how AikaClaw assembles model context
...
A **context engine** controls how AikaClaw builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

AikaClaw ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.aikaclaw/aikaclaw.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other AikaClaw plugin. Install
...
aikaclaw plugins install -l ./my-context-engine
```
...
Every time AikaClaw runs a model prompt, the context engine participates at
...
AikaClaw currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. AikaClaw
...
The built-in `legacy` engine preserves AikaClaw's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. AikaClaw uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. AikaClaw disables Pi's built-in
...
`ownsCompaction: false` does **not** mean AikaClaw automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `aikaclaw/plugin-sdk/core` to use
  AikaClaw's built-in compaction behavior.
...
AikaClaw resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to AikaClaw's built-in summarization. Plugin engines can implement
...
- Use `aikaclaw doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, AikaClaw
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `aikaclaw plugins install -l ./my-engine` to link a
