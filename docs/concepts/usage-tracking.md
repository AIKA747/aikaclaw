...
- `/usage cost` in chats: local cost summary aggregated from AikaClaw session logs.
- CLI: `aikaclaw status --usage` prints a full per-provider breakdown.
- CLI: `aikaclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
