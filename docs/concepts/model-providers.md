...
- CLI helpers: `aikaclaw onboard`, `aikaclaw models list`, `aikaclaw models set <provider/model>`.
- Provider plugins can inject model catalogs via `registerProvider({ catalog })`;
  AikaClaw merges that output into `models.providers` before writing
  `models.json`.
...
Provider plugins can now own most provider-specific logic while AikaClaw keeps
the generic inference loop.

Typical split:

- `auth[].run` / `auth[].runNonInteractive`: provider owns onboarding/login
  flows for `aikaclaw onboard`, `aikaclaw models auth`, and headless setup
...
That covers providers that still fit AikaClaw's normal transports. A provider
...
- Configure multiple keys via:
  - `AIKACLAW_LIVE_<PROVIDER>_KEY` (single live override, highest priority)
...
AikaClaw ships with the pi‑ai catalog. These providers require **no**
...
- Provider: `openai`
- Auth: `OPENAI_API_KEY`
- Optional rotation: `OPENAI_API_KEYS`, `OPENAI_API_KEY_1`, `OPENAI_API_KEY_2`, plus `AIKACLAW_LIVE_OPENAI_KEY` (single override)
- Example models: `openai/gpt-5.4`, `openai/gpt-5.4-pro`
- CLI: `aikaclaw onboard --auth-choice openai-api-key`
...
- `openai/gpt-5.3-codex-spark` is intentionally suppressed in AikaClaw because the live OpenAI API rejects it; Spark is treated as Codex-only
...
- Optional rotation: `ANTHROPIC_API_KEYS`, `ANTHROPIC_API_KEY_1`, `ANTHROPIC_API_KEY_2`, plus `AIKACLAW_LIVE_ANTHROPIC_KEY` (single override)
- Example model: `anthropic/claude-opus-4-6`
- CLI: `aikaclaw onboard --auth-choice token` (paste setup-token) or `aikaclaw models auth paste-token --provider anthropic`
- Direct API-key models support the shared `/fast` toggle and `params.fastMode`; AikaClaw maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
...
- CLI: `aikaclaw onboard --auth-choice openai-codex` or `aikaclaw models auth login --provider openai-codex`
- Default transport is `auto` (WebSocket-first, SSE fallback)
...
- Policy note: OpenAI Codex OAuth is explicitly supported for external tools/workflows like AikaClaw.
...
- CLI: `aikaclaw onboard --auth-choice opencode-zen` or `aikaclaw onboard --auth-choice opencode-go`
...
- Provider: `google`
- Auth: `GEMINI_API_KEY`
- Optional rotation: `GEMINI_API_KEYS`, `GEMINI_API_KEY_1`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY` fallback, and `AIKACLAW_LIVE_GEMINI_KEY` (single override)
- Example models: `google/gemini-3.1-pro-preview`, `google/gemini-3-flash-preview`
- Compatibility: legacy AikaClaw config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
- CLI: `aikaclaw onboard --auth-choice gemini-api-key`

### Google Vertex and Gemini CLI

- Providers: `google-vertex`, `google-gemini-cli`
- Auth: Vertex uses gcloud ADC; Gemini CLI uses its OAuth flow
- Caution: Gemini CLI OAuth in AikaClaw is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
- Gemini CLI OAuth is shipped as part of the bundled `google` plugin.
  - Enable: `aikaclaw plugins enable google`
  - Login: `aikaclaw models auth login --provider google-gemini-cli --set-default`
  - Note: you do **not** paste a client id or secret into `aikaclaw.json`. The CLI login flow stores
...
- CLI: `aikaclaw onboard --auth-choice zai-api-key`
...
- CLI: `aikaclaw onboard --auth-choice ai-gateway-api-key`
...
- CLI: `aikaclaw onboard --kilocode-api-key <key>`
...
- CLI: `aikaclaw onboard --auth-choice mistral-api-key`
- Groq: `groq` (`GROQ_API_KEY`)
- Cerebras: `cerebras` (`CEREBRAS_API_KEY`)
  - GLM models on Cerebras use ids `zai-glm-4.7` and `zai-glm-4.6`.
  - OpenAI-compatible base URL: `https://api.cerebras.ai/v1`.
- GitHub Copilot: `github-copilot` (`COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN`)
- Hugging Face Inference example model: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `aikaclaw onboard --auth-choice huggingface-api-key`. See [Hugging Face (Inference)](/providers/huggingface).
...
```bash
aikaclaw models auth login --provider qwen-portal --set-default
```
...
- CLI: `aikaclaw onboard --auth-choice volcengine-api-key`
...
- CLI: `aikaclaw onboard --auth-choice byteplus-api-key`
...
- CLI: `aikaclaw onboard --auth-choice synthetic-api-key`
...
`aikaclaw onboard` and the model picker. See [/providers/ollama](/providers/ollama)
...
- For custom providers, `reasoning`, `input`, `cost`, `contextWindow`, and `maxTokens` are optional.
  When omitted, AikaClaw defaults to:
  - `reasoning: false`
  - `input: ["text"]`
  - `cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }`
  - `contextWindow: 200000`
...
- For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), AikaClaw forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
- If `baseUrl` is empty/omitted, AikaClaw keeps the default OpenAI behavior (which resolves to `api.openai.com`).
- For safety, an explicit `compat.supportsDeveloperRole: true` is still overridden on non-native `openai-completions` endpoints.

## CLI examples

```bash
aikaclaw onboard --auth-choice opencode-zen
aikaclaw models set opencode/claude-opus-4-6
