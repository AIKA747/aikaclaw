---
summary: "Delegate architecture: running AikaClaw as a named agent on behalf of an organization"
...
Goal: run AikaClaw as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an AikaClaw agent that:
...
AikaClaw's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of AikaClaw's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.aikaclaw/workspace-delegate",
  tools: {
...
- Cron run history: `~/.aikaclaw/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.aikaclaw/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through AikaClaw's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
aikaclaw agents add delegate
```

This creates:

- Workspace: `~/.aikaclaw/workspace-delegate`
- State: `~/.aikaclaw/agents/delegate/agent`
- Sessions: `~/.aikaclaw/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.aikaclaw/workspace" },
      {
        id: "delegate",
        workspace: "~/.aikaclaw/workspace-delegate",
        tools: {
...
~/.aikaclaw/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.aikaclaw/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.aikaclaw/workspace-org",
        agentDir: "~/.aikaclaw/agents/org-assistant/agent",
