...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an AikaClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`aikaclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the AikaClaw gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
aikaclaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
aikaclaw acp --url wss://gateway-host:18789 --token-file ~/.aikaclaw/gateway.token

# Attach to an existing session key
aikaclaw acp --session agent:main:main

# Attach by label (must already exist)
aikaclaw acp --session-label "support inbox"

# Reset the session key before the first prompt
aikaclaw acp --session agent:main:main --reset-session
```
...
aikaclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.aikaclaw/gateway.token

# Override the server command (default: aikaclaw)
aikaclaw acp client --server "node" --server-args aikaclaw.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive an AikaClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `aikaclaw acp` over stdio.

Example config (persisted):

```bash
aikaclaw config set gateway.remote.url wss://gateway-host:18789
aikaclaw config set gateway.remote.token <token>
```
...
```bash
aikaclaw acp --session agent:main:main
aikaclaw acp --session agent:design:main
aikaclaw acp --session agent:qa:bug-123
```
...
AikaClaw bot over ACP, use `acpx` with its built-in `aikaclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx aikaclaw` at `aikaclaw acp`.
3. Target the AikaClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default AikaClaw ACP session
acpx aikaclaw exec "Summarize the active AikaClaw session state."

# Persistent named session for follow-up turns
acpx aikaclaw sessions ensure --name codex-bridge
acpx aikaclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my AikaClaw work agent for recent context relevant to this repo."
```

If you want `acpx aikaclaw` to target a specific Gateway and session key every
time, override the `aikaclaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "aikaclaw": {
      "command": "env AIKACLAW_HIDE_BANNER=1 AIKACLAW_SUPPRESS_NOTES=1 aikaclaw acp --url ws://127.0.0.1:18789 --token-file ~/.aikaclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local AikaClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env AIKACLAW_HIDE_BANNER=1 AIKACLAW_SUPPRESS_NOTES=1 node aikaclaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an AikaClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select “AikaClaw ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`AIKACLAW_GATEWAY_TOKEN`, `AIKACLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`AIKACLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `AIKACLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `aikaclaw acp client` sets `AIKACLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `aikaclaw`).
