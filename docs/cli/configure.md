---
summary: "CLI reference for `aikaclaw configure` (interactive configuration prompts)"
read_when:
...
Tip: `aikaclaw config` without a subcommand opens the same wizard. Use
`aikaclaw config get|set|unset` for non-interactive edits.
...
```bash
aikaclaw configure
aikaclaw configure --section model --section channels
```
