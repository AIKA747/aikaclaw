---
summary: "CLI reference for `aikaclaw onboard` (interactive onboarding)"
read_when:
...
```bash
aikaclaw onboard
aikaclaw onboard --flow quickstart
aikaclaw onboard --flow manual
aikaclaw onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`AIKACLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `aikaclaw gateway run`.
...
- Fastest first chat: `aikaclaw dashboard` (Control UI, no channel setup).
