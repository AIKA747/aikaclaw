---
summary: "CLI reference for `aikaclaw secrets` (reload, audit, configure, apply)"
read_when:
...
Use `aikaclaw secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
Scan AikaClaw state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `aikaclaw.json` refs)
...
```bash
aikaclaw secrets audit
aikaclaw secrets audit --check
aikaclaw secrets audit --json
aikaclaw secrets audit --allow-exec
```
...
```bash
aikaclaw secrets configure
aikaclaw secrets configure --plan-out /tmp/aikaclaw-secrets-plan.json
aikaclaw secrets configure --apply --yes
aikaclaw secrets configure --providers-only
aikaclaw secrets configure --skip-provider-setup
aikaclaw secrets configure --agent ops
aikaclaw secrets configure --json
```
...
- `configure` targets secret-bearing fields in `aikaclaw.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, AikaClaw fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
aikaclaw secrets apply --from /tmp/aikaclaw-secrets-plan.json --json
```
...
- `aikaclaw.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.aikaclaw/.env` known secret keys whose values were migrated
