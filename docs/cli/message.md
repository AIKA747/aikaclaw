---
summary: "CLI reference for `aikaclaw message` (send + channel actions)"
read_when:
...
```
aikaclaw message <subcommand> [flags]
```
...
- On cache miss, AikaClaw will attempt a live directory lookup when the provider supports it.
...
- `aikaclaw message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, AikaClaw does not force a `default` account SecretRef scope
...
```
aikaclaw message send --channel discord \
...
```
aikaclaw message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
aikaclaw message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
aikaclaw message poll --channel msteams \
...
```
aikaclaw message react --channel slack \
...
```
aikaclaw message react --channel signal \
...
```
aikaclaw message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
aikaclaw message send --channel telegram --target @mychat \
