---
summary: "CLI reference for `aikaclaw status` (diagnostics, probes, usage snapshots)"
read_when:
...
- Update info surfaces in the Overview; if an update is available, status prints a hint to run `aikaclaw update` (see [Updating](/install/updating)).
