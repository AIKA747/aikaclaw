---
summary: "AikaClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up AikaClaw on a Raspberry Pi
  - Running AikaClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# AikaClaw on Raspberry Pi

## Goal

Run a persistent, always-on AikaClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Check service
sudo systemctl status aikaclaw

# View logs
journalctl -u aikaclaw -f
```

## 9) Access the AikaClaw Dashboard
...
```bash
ssh user@gateway-host 'aikaclaw dashboard --no-open'
```
...
(or `AIKACLAW_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/aikaclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/aikaclaw-compile-cache
mkdir -p /var/tmp/aikaclaw-compile-cache
export AIKACLAW_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `AIKACLAW_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running AikaClaw, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit aikaclaw
```

```ini
[Service]
Environment=AIKACLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/aikaclaw-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
sudo systemctl restart aikaclaw
```

If possible, keep AikaClaw state/cache on SSD-backed storage to avoid SD-card
...
Most AikaClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled aikaclaw

# Enable if not
sudo systemctl enable aikaclaw

# Start on boot
sudo systemctl start aikaclaw
```
...
```bash
# Check logs
journalctl -u aikaclaw --no-pager -n 100

# Common fix: rebuild
cd ~/aikaclaw  # if using hackable install
npm run build
sudo systemctl restart aikaclaw
```
