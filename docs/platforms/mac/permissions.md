...
- Same path: run the app from a fixed location (for AikaClaw, `dist/AikaClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.aikaclaw.mac
sudo tccutil reset ScreenCapture ai.aikaclaw.mac
...
Workaround: move files into the AikaClaw workspace (`~/.aikaclaw/workspace`) if you want to avoid per-folder grants.
