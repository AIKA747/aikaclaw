...
service via the external `aikaclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.aikaclaw.gateway`
  (or `ai.aikaclaw.<profile>` when using `--profile`/`AIKACLAW_PROFILE`; legacy `com.aikaclaw.*` is supported).
