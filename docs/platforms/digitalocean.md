---
summary: "AikaClaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up AikaClaw on DigitalOcean
  - Looking for cheap VPS hosting for AikaClaw
title: "DigitalOcean (Platform)"
---

# AikaClaw on DigitalOcean

## Goal

Run a persistent AikaClaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status aikaclaw-gateway.service

# View logs
journalctl --user -u aikaclaw-gateway.service -f
```
...
```bash
aikaclaw config set gateway.bind tailnet
aikaclaw gateway restart
```
...
```bash
aikaclaw channels login whatsapp
# Scan QR code
```
...
- `~/.aikaclaw/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u aikaclaw --no-pager -n 50
```
