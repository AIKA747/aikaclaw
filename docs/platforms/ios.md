...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `aikaclaw.internal.`), **or**
...
Run `aikaclaw devices list` again before approval.

4. Verify connection:

```bash
aikaclaw nodes status
aikaclaw gateway call node.list --params "{}"
```
...
- `AIKACLAW_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine AikaClaw iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight AikaClaw builds.
...
```bash
export AIKACLAW_APNS_TEAM_ID="TEAMID"
export AIKACLAW_APNS_KEY_ID="KEYID"
export AIKACLAW_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_aikaclaw-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `aikaclaw.internal.`) and Tailscale split DNS.
...
```bash
aikaclaw nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__aikaclaw__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__aikaclaw__/canvas/` and `/__aikaclaw__/a2ui/`.
...
```bash
aikaclaw nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__aikaclaw; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
aikaclaw nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `aikaclaw devices list` and approve manually.
