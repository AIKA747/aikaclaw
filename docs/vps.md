---
summary: "Run AikaClaw on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for AikaClaw
title: "Linux Server"
...
Run the AikaClaw Gateway on any Linux server or cloud VPS. This page helps you
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/aikaclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `AIKACLAW_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  - `AIKACLAW_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/aikaclaw-compile-cache`
