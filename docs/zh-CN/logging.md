...
你可以在 `~/.aikaclaw/aikaclaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.aikaclaw/aikaclaw.json` 的 `logging` 下。
...
- AikaClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `aikaclaw plugins enable diagnostics-otel` 启用插件。
...
- `aikaclaw.tokens`（计数器，属性：`aikaclaw.token`、`aikaclaw.channel`、`aikaclaw.provider`、`aikaclaw.model`）
- `aikaclaw.cost.usd`（计数器，属性：`aikaclaw.channel`、`aikaclaw.provider`、`aikaclaw.model`）
- `aikaclaw.run.duration_ms`（直方图，属性：`aikaclaw.channel`、`aikaclaw.provider`、`aikaclaw.model`）
- `aikaclaw.context.tokens`（直方图，属性：`aikaclaw.context`、`aikaclaw.channel`、`aikaclaw.provider`、`aikaclaw.model`）

消息流：

- `aikaclaw.webhook.received`（计数器，属性：`aikaclaw.channel`、`aikaclaw.webhook`）
- `aikaclaw.webhook.error`（计数器，属性：`aikaclaw.channel`、`aikaclaw.webhook`）
- `aikaclaw.webhook.duration_ms`（直方图，属性：`aikaclaw.channel`、`aikaclaw.webhook`）
- `aikaclaw.message.queued`（计数器，属性：`aikaclaw.channel`、`aikaclaw.source`）
- `aikaclaw.message.processed`（计数器，属性：`aikaclaw.channel`、`aikaclaw.outcome`）
- `aikaclaw.message.duration_ms`（直方图，属性：`aikaclaw.channel`、`aikaclaw.outcome`）

队列 + 会话：

- `aikaclaw.queue.lane.enqueue`（计数器，属性：`aikaclaw.lane`）
- `aikaclaw.queue.lane.dequeue`（计数器，属性：`aikaclaw.lane`）
- `aikaclaw.queue.depth`（直方图，属性：`aikaclaw.lane` 或 `aikaclaw.channel=heartbeat`）
- `aikaclaw.queue.wait_ms`（直方图，属性：`aikaclaw.lane`）
- `aikaclaw.session.state`（计数器，属性：`aikaclaw.state`、`aikaclaw.reason`）
- `aikaclaw.session.stuck`（计数器，属性：`aikaclaw.state`）
- `aikaclaw.session.stuck_age_ms`（直方图，属性：`aikaclaw.state`）
- `aikaclaw.run.attempt`（计数器，属性：`aikaclaw.attempt`）
...
- `aikaclaw.webhook.error`
  - `aikaclaw.channel`、`aikaclaw.webhook`、`aikaclaw.chatId`、`aikaclaw.error`
- `aikaclaw.message.processed`
  - `aikaclaw.channel`、`aikaclaw.outcome`、`aikaclaw.chatId`、`aikaclaw.messageId`、`aikaclaw.sessionKey`、`aikaclaw.sessionId`、`aikaclaw.reason`
- `aikaclaw.session.stuck`
  - `aikaclaw.state`、`aikaclaw.ageMs`、`aikaclaw.queueDepth`、`aikaclaw.sessionKey`、`aikaclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `aikaclaw doctor`。
