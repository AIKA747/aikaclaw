...
read_when:
  - 你想管理智能体钩子
  - 你想安装或更新钩子
summary: CLI 参考：`aikaclaw hooks`（智能体钩子）
title: hooks
x-i18n:
...
通过将特定钩子添加到配置（`~/.aikaclaw/config.json`）来启用它。

**注意：** 由插件管理的钩子在 `aikaclaw hooks list` 中显示 `plugin:<id>`，
...
```bash
# 本地目录
aikaclaw hooks install ./my-hook-pack

# 本地压缩包
aikaclaw hooks install ./my-hook-pack.zip

# NPM 包
aikaclaw hooks install @aikaclaw/my-hook-pack

# 链接本地目录而不复制
aikaclaw hooks install -l ./my-hook-pack
```

## 更新钩子

```bash
aikaclaw hooks update <id>
aikaclaw hooks update --all
```
...
**输出：** `~/.aikaclaw/workspace/memory/YYYY-MM-DD-slug.md`
