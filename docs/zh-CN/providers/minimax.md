---
read_when:
  - 你想在 AikaClaw 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 AikaClaw 中使用 MiniMax M2.5
title: MiniMax
x-i18n:
...
```bash
aikaclaw plugins enable minimax  # 如果已加载则跳过。
aikaclaw gateway restart  # 如果 gateway 已在运行，则重启
...
- 使用 `aikaclaw models list` 和 `aikaclaw models set minimax/MiniMax-M2.5` 进行切换。
...
- 运行 `aikaclaw configure` 并选择 **MiniMax M2.5**，或者
