...
summary: 使用 Ansible、Tailscale VPN 和防火墙隔离进行自动化、加固的 AikaClaw 安装
title: Ansible
x-i18n:
...
将 AikaClaw 部署到生产服务器的推荐方式是通过 **[aikaclaw-ansible](https://github.com/aikaclaw/aikaclaw-ansible)** — 一个安全优先架构的自动化安装程序。
...
```bash
curl -fsSL https://raw.githubusercontent.com/aikaclaw/aikaclaw-ansible/main/install.sh | bash
```

> **📦 完整指南：[github.com/aikaclaw/aikaclaw-ansible](https://github.com/aikaclaw/aikaclaw-ansible)**
>
> aikaclaw-ansible 仓库是 Ansible 部署的权威来源。本页是快速概述。
...
# 查看实时日志
sudo journalctl -u aikaclaw -f

# 重启 Gateway 网关
sudo systemctl restart aikaclaw

# 提供商登录（以 aikaclaw 用户运行）
sudo -i -u aikaclaw
...
# 或直接运行（然后手动执行 /tmp/aikaclaw-setup.sh）
# ansible-playbook playbook.yml --ask-become-pass
```

## 更新 AikaClaw

Ansible 安装程序设置 AikaClaw 为手动更新。标准更新流程参见[更新](/install/updating)。
...
```bash
# 检查日志
sudo journalctl -u aikaclaw -n 100

# 验证权限
sudo ls -la /opt/aikaclaw
...
# 检查沙箱镜像
sudo docker images | grep aikaclaw-sandbox

# 如果缺失则构建沙箱镜像
cd /opt/aikaclaw/aikaclaw
sudo -u aikaclaw ./scripts/sandbox-setup.sh
```
...
- [安全架构](https://github.com/aikaclaw/aikaclaw-ansible/blob/main/docs/security.md)
- [技术详情](https://github.com/aikaclaw/aikaclaw-ansible/blob/main/docs/architecture.md)
- [故障排除指南](https://github.com/aikaclaw/aikaclaw-ansible/blob/main/docs/troubleshooting.md)

## 相关内容

- [aikaclaw-ansible](https://github.com/aikaclaw/aikaclaw-ansible) — 完整部署指南
