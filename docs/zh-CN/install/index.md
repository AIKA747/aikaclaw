...
read_when:
  ...
  - 你想部署到云平台
  - 你需要更新、迁移或卸载
summary: 安装 AikaClaw —— 安装脚本、npm/pnpm、从源码、Docker 等
title: 安装
x-i18n:
...
<Note>
在 Windows 上，我们强烈建议你在 [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) 下运行 AikaClaw。
</Note>

## 安装方法

<Tip>
**安装脚本** 是安装 AikaClaw 的推荐方式。它会一步完成 Node 检测、安装和新手引导。
</Tip>

<Warning>
对于 VPS/云主机，尽量避免使用第三方“一键式”市场镜像。优先选择干净的基础 OS 镜像（例如 Ubuntu LTS），然后使用安装脚本自行安装 AikaClaw。
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="npm / pnpm" icon="package">
    如果你已经自行管理 Node，我们推荐使用 Node 24。出于兼容性考虑，AikaClaw 仍支持 Node 22 LTS，目前为 `22.16+`：
    ...
    <Tabs>
      <Tab title="npm">
        ...
        <Accordion title="sharp 构建错误？">
          ...
          ```bash
          SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g aikaclaw@latest
          ```
      ...
      <Tab title="pnpm">
        ...
        pnpm approve-builds -g        # 批准 aikaclaw、node-llama-cpp、sharp 等
        aikaclaw onboard --install-daemon
        ```
    ...
    ```bash
    npm install -g github:aikaclaw/aikaclaw#main
    ```

    ```bash
    pnpm add -g github:aikaclaw/aikaclaw#main
    ```
  ...
  <Accordion title="从源码" icon="github">
    ...
    <Steps>
      ...
      <Step title="链接 CLI">
        ...
        或者，你也可以跳过链接，直接在仓库内通过 `pnpm aikaclaw ...` 运行命令。
      </Step>
      <Step title="运行新手引导">
        ```bash
        aikaclaw onboard --install-daemon
...
```bash
aikaclaw doctor         # 检查配置问题
aikaclaw status         # Gateway 网关状态
aikaclaw dashboard      # 打开浏览器 UI
```

如果你需要自定义运行时路径，请使用：

- `AIKACLAW_HOME` 用于基于主目录的内部路径
- `AIKACLAW_STATE_DIR` 用于可变状态位置
- `AIKACLAW_CONFIG_PATH` 用于配置文件位置
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不在**你的 `$PATH` 中，那么你的 shell 就找不到全局 npm 二进制文件（包括 `aikaclaw`）。
