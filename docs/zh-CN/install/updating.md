...
read_when:
  - 更新 AikaClaw
  - 更新后出现问题
summary: 安全更新 AikaClaw（全局安装或源码），以及回滚策略
title: 更新
x-i18n:
...
AikaClaw 发展迅速（尚未到"1.0"）。将更新视为发布基础设施：更新 → 运行检查 → 重启（或使用会重启的 `aikaclaw update`）→ 验证。
...
- 快照你的定制内容：
  - 配置：`~/.aikaclaw/aikaclaw.json`
  - 凭证：`~/.aikaclaw/credentials/`
  - 工作区：`~/.aikaclaw/workspace`
...
aikaclaw update --channel stable
```
...
- 如果你的 Gateway 网关作为服务运行，`aikaclaw gateway restart` 优于杀死 PID。
...
- 安装依赖、构建、构建控制 UI，并运行 `aikaclaw doctor`。
- 默认重启 Gateway 网关（使用 `--no-restart` 跳过）。

如果你通过 **npm/pnpm** 安装（没有 git 元数据），`aikaclaw update` 将尝试通过你的包管理器更新。如果无法检测到安装，请改用"更新（全局安装）"。

## 更新（控制 UI / RPC）

控制 UI 有**更新并重启**（RPC：`update.run`）。它：

1. 运行与 `aikaclaw update` 相同的源码更新流程（仅限 git checkout）。
...
## 始终运行：`aikaclaw doctor`

Doctor 是"安全更新"命令。它故意很无聊：修复 + 迁移 + 警告。

注意：如果你是**源码安装**（git checkout），`aikaclaw doctor` 会提供先运行 `aikaclaw update`。
...
- 检测并将旧版 Gateway 网关服务（launchd/systemd；旧版 schtasks）迁移到当前 AikaClaw 服务。
...
- macOS launchd（应用捆绑的 LaunchAgent）：`launchctl kickstart -k gui/$UID/bot.molt.gateway`（使用 `bot.molt.<profile>`；旧版 `com.aikaclaw.*` 仍然有效）
- Linux systemd 用户服务：`systemctl --user restart aikaclaw-gateway[-<profile>].service`
- Windows（WSL2）：`systemctl --user restart aikaclaw-gateway[-<profile>].service`
  - `launchctl`/`systemctl` 仅在服务已安装时有效；否则运行 `aikaclaw gateway install`。
...
```bash
pnpm add -g aikaclaw@<version>
```

提示：要查看当前发布的版本，运行 `npm view aikaclaw version`。
...
- 再次运行 `aikaclaw doctor` 并仔细阅读输出（它通常会告诉你修复方法）。
