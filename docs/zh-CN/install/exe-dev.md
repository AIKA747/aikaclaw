...
summary: 在 exe.dev 上运行 AikaClaw Gateway 网关（VM + HTTPS 代理）以实现远程访问
title: exe.dev
x-i18n:
...
目标：让 AikaClaw Gateway 网关运行在 exe.dev VM 上，并且可通过你的笔记本电脑访问：`https://<vm-name>.exe.xyz`
...
Shelley 是 [exe.dev](https://exe.dev) 的智能体，可以使用我们的提示词立即安装 AikaClaw。
使用的提示词如下：
...
提示：请让这个 VM 保持**有状态**。AikaClaw 会将状态存储在 `~/.aikaclaw/` 和 `~/.aikaclaw/workspace/` 下。
...
`gateway.auth.token` 中的令牌（可通过 `aikaclaw config get gateway.auth.token` 获取，或使用
`aikaclaw doctor --generate-gateway-token` 生成）。使用 `aikaclaw devices list` 和
`aikaclaw devices approve <requestId>` 批准设备。如果拿不准，请在浏览器中使用 Shelley！
