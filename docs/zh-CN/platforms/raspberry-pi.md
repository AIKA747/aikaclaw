---
read_when:
  - 在 Raspberry Pi 上设置 AikaClaw
  - 在 ARM 设备上运行 AikaClaw
  - 构建一个低成本、始终在线的个人 AI
summary: 在 Raspberry Pi 上运行 AikaClaw（低预算自托管方案）
title: Raspberry Pi
x-i18n:
...
# 在 Raspberry Pi 上运行 AikaClaw

## 目标

在 Raspberry Pi 上以 **约 35–80 美元**的一次性成本（无月费）运行一个持久化、始终在线的 AikaClaw Gateway 网关。
...
（或 `AIKACLAW_GATEWAY_TOKEN`）中的 token 粘贴到 Control UI 设置中。
...
- `AIKACLAW_NO_RESPAWN=1` 可避免 CLI 自重启带来的额外启动开销。
- 第一次运行会预热缓存；之后的运行收益最大。

### systemd 启动调优（可选）

如果这台 Pi 主要运行 AikaClaw，可以添加一个服务 drop-in，以减少重启抖动并保持稳定的启动环境：
...
如果可能，请将 AikaClaw 状态/缓存放在 SSD 支持的存储上，以避免冷启动期间 SD 卡随机 I/O 成为瓶颈。
...
AikaClaw 的大多数功能都可在 ARM64 上运行，但某些外部二进制文件可能需要 ARM 构建版本：
