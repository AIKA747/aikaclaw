---
read_when:
  - 在 DigitalOcean 上设置 AikaClaw
  - 寻找适合 AikaClaw 的低价 VPS 托管
summary: 在 DigitalOcean 上运行 AikaClaw（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 AikaClaw

## 目标

在 DigitalOcean 上以 **每月 6 美元**（或预留定价时每月 4 美元）运行一个持久化的 AikaClaw Gateway 网关。
...
- `~/.aikaclaw/` — 配置、凭证、会话数据
- `~/.aikaclaw/workspace/` — 工作区（`SOUL.md`、memory 等）
