...
<Steps>
  ...
  <Step title="渠道">
    ...
    - 私信安全：默认是配对。首次私信会发送一个代码；通过
      `aikaclaw pairing approve <channel> <code>` 批准，或使用 allowlist。
  </Step>
  <Step title="守护进程安装">
    - macOS：LaunchAgent
  ...
  <Step title="健康检查">
    - 启动 Gateway 网关（如有需要），并运行 `aikaclaw health`。
    - `aikaclaw status --deep` 会在状态输出中添加 Gateway 网关健康探测。
  </Step>
  <Step title="Skills">
    - 读取可用的 Skills 并检查要求。
...
- OAuth 凭证：`~/.aikaclaw/credentials/oauth.json`
- Auth profile（API 密钥 + OAuth）：`~/.aikaclaw/agents/<agentId>/agent/auth-profiles.json`
...
`~/.aikaclaw/credentials/oauth.json`（或 `$AIKACLAW_STATE_DIR/credentials/oauth.json`）
到 Gateway 网关主机。
</Note>
...
WhatsApp 凭证位于 `~/.aikaclaw/credentials/whatsapp/<accountId>/`。
...
- 下载适当的发布资源
- 将其存储在 `~/.aikaclaw/tools/signal-cli/<version>/`
...
- 命令参考：[`aikaclaw onboard`](/cli/onboard)
