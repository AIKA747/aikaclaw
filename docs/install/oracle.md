---
summary: "Host AikaClaw on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up AikaClaw on Oracle Cloud
  - Looking for free VPS hosting for AikaClaw
...
Run a persistent AikaClaw Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@aikaclaw`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=aikaclaw --reset` to re-authenticate.

**Gateway will not start** -- Run `aikaclaw doctor --non-interactive` and check logs with `journalctl --user -u aikaclaw-gateway -n 50`.
...
- [Updating](/install/updating) -- keep AikaClaw up to date
