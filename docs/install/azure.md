---
summary: "Run AikaClaw Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want AikaClaw running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on AikaClaw Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

# AikaClaw on Azure Linux VM

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs AikaClaw.
...
- Install AikaClaw with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-aikaclaw"
    LOCATION="westus2"
    VNET_NAME="vnet-aikaclaw"
    VNET_PREFIX="10.40.0.0/16"
    VM_SUBNET_NAME="snet-aikaclaw-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-aikaclaw-vm"
    VM_NAME="vm-aikaclaw"
    ADMIN_USERNAME="aikaclaw"
    BASTION_NAME="bas-aikaclaw"
    BASTION_PIP_NAME="pip-aikaclaw-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install AikaClaw (in the VM shell)">
    ...
    The installer installs Node LTS and dependencies if not already present, installs AikaClaw, and launches the onboarding wizard. See [Install](/install) for details.
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses. If that is your case, we recommend choosing the GitHub Copilot provider in the AikaClaw onboarding wizard. See [GitHub Copilot provider](/providers/github-copilot).
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The AikaClaw Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
...
- For more details on AikaClaw Azure deployment with the GitHub Copilot model provider: [AikaClaw on Azure with GitHub Copilot](https://github.com/johnsonshi/aikaclaw-azure-github-copilot)
