---
summary: "Automated, hardened AikaClaw installation with Ansible, Tailscale VPN, and firewall isolation"
read_when:
...
5. **AikaClaw** -- host-based, not containerized
...
<Steps>
  <Step title="Switch to the aikaclaw user">
    ```bash
    sudo -i -u aikaclaw
    ```
  </Step>
  <Step title="Run the onboarding wizard">
    The post-install script guides you through configuring AikaClaw settings.
  </Step>
...
# Provider login (run as aikaclaw user)
sudo -i -u aikaclaw
aikaclaw channels login
```
...
<Steps>
  ...
  <Step title="Run the playbook">
    ...
    # Then run: /tmp/aikaclaw-setup.sh
    ```
...
The Ansible installer sets up AikaClaw for manual updates. See [Updating](/install/updating) for the standard update flow.
...
<AccordionGroup>
  ...
  <Accordion title="Provider login fails">
    Make sure you are running as the `aikaclaw` user:
    ```bash
    sudo -i -u aikaclaw
    aikaclaw channels login
    ```
