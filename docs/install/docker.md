---
summary: "Optional Docker-based setup and onboarding for AikaClaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run AikaClaw on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export AIKACLAW_IMAGE="ghcr.io/aikaclaw/aikaclaw:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/aikaclaw/aikaclaw/pkgs/container/aikaclaw).
  ...
  <Step title="Complete onboarding">
    ...
    `aikaclaw-gateway` directly. `aikaclaw-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm aikaclaw-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm aikaclaw-cli channels login

    # Telegram
    docker compose run --rm aikaclaw-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm aikaclaw-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t aikaclaw:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node aikaclaw-gateway \
...
docker compose up -d aikaclaw-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `AIKACLAW_EXTRA_MOUNTS`
or `AIKACLAW_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `aikaclaw-cli` shares `aikaclaw-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d aikaclaw-gateway`, run onboarding
and setup-time config writes through `aikaclaw-gateway` with
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `AIKACLAW_IMAGE`               | Use a remote image instead of building locally                   |
| `AIKACLAW_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `AIKACLAW_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `AIKACLAW_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `AIKACLAW_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `AIKACLAW_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `AIKACLAW_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
```bash
docker compose exec aikaclaw-gateway node dist/index.js health --token "$AIKACLAW_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `AIKACLAW_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `AIKACLAW_CONFIG_DIR` to `/home/node/.aikaclaw` and
`AIKACLAW_WORKSPACE_DIR` to `/home/node/.aikaclaw/workspace`, so those paths
...
and rolling file logs under `/tmp/aikaclaw/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/aikaclaw/aikaclaw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
echo 'source ~/.clawdock/clawdock-helpers.sh' >> ~/.zshrc && source ~/.zshrc
```

Then use `clawdock-start`, `clawdock-stop`, `clawdock-dashboard`, etc. Run
`clawdock-help` for all commands.
See the [`ClawDock` Helper README](https://github.com/aikaclaw/aikaclaw/blob/main/scripts/shell-helpers/README.md).

<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ```bash
    export AIKACLAW_SANDBOX=1
    ./scripts/docker/setup.sh
    ```
    ...
    ```bash
    export AIKACLAW_SANDBOX=1
    export AIKACLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm aikaclaw-cli gateway probe
    docker compose run -T --rm aikaclaw-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `aikaclaw-cli` uses `network_mode: "service:aikaclaw-gateway"` so CLI
    ...
    `no-new-privileges` on `aikaclaw-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.aikaclaw`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/aikaclaw-config /path/to/aikaclaw-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export AIKACLAW_HOME_VOLUME="aikaclaw_home"`
    2. **Bake system deps**: `export AIKACLAW_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `AIKACLAW_HOME_VOLUME` or `AIKACLAW_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/aikaclaw/aikaclaw/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    AikaClaw runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm aikaclaw-cli dashboard --no-open
    docker compose run --rm aikaclaw-cli devices list
    docker compose run --rm aikaclaw-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm aikaclaw-cli config set gateway.mode local
    docker compose run --rm aikaclaw-cli config set gateway.bind lan
    docker compose run --rm aikaclaw-cli devices list --url ws://127.0.0.1:18789
    ```
