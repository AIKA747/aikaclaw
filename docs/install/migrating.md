---
summary: "Move (migrate) an AikaClaw install from one machine to another"
read_when:
  - You are moving AikaClaw to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating AikaClaw to a New Machine

This guide moves an AikaClaw gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.aikaclaw/` by default) and your **workspace**, you preserve:

- **Config** -- `aikaclaw.json` and all gateway settings
...
<Tip>
Run `aikaclaw status` on the old machine to confirm your state directory path.
Custom profiles use `~/.aikaclaw-<profile>/` or a path set via `AIKACLAW_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf aikaclaw-state.tgz .aikaclaw
    ```

    If you use multiple profiles (e.g. `~/.aikaclaw-work`), archive each separately.

  </Step>

  <Step title="Install AikaClaw on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.aikaclaw/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `AIKACLAW_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `aikaclaw doctor`.
  </Accordion>

  <Accordion title="Copying only aikaclaw.json">
  ...
  <Accordion title="Remote mode">
    ...
    Migrate the gateway host itself, not your local laptop. See [FAQ](/help/faq#where-does-aikaclaw-store-its-data).
  </Accordion>
...
- [ ] `aikaclaw status` shows the gateway running
