...
AikaClaw ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/aikaclaw`, override with
  `AIKACLAW_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
aikaclaw update --tag aikaclaw@2026.3.22
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `aikaclaw update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  AikaClaw prompts for confirmation (skip with `--yes`).

## Dry run

Preview what `aikaclaw update` would do without making changes:

```bash
aikaclaw update --dry-run
aikaclaw update --channel beta --dry-run
aikaclaw update --tag 2026.3.22 --dry-run
aikaclaw update --dry-run --json
```
...
When you switch channels with `aikaclaw update`, AikaClaw also syncs plugin
sources:
