---
summary: "Run AikaClaw Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want AikaClaw running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# AikaClaw on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent AikaClaw Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "AikaClaw 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the AikaClaw Gateway in Docker
- Persist `~/.aikaclaw` + `~/.aikaclaw/workspace` on the host (survives restarts/rebuilds)
...
<Steps>
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`AIKACLAW_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm aikaclaw-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
