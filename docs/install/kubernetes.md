---
summary: "Deploy AikaClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run AikaClaw on a Kubernetes cluster
  - You want to test AikaClaw in a Kubernetes environment
title: "Kubernetes"
---

# AikaClaw on Kubernetes

A minimal starting point for running AikaClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

AikaClaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/aikaclaw 18789:18789 -n aikaclaw
...
```bash
kubectl get secret aikaclaw-secrets -n aikaclaw -o jsonpath='{.data.AIKACLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: aikaclaw (configurable via AIKACLAW_NAMESPACE)
├── Deployment/aikaclaw        # Single pod, init container + gateway
├── Service/aikaclaw           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/aikaclaw-config  # aikaclaw.json + AGENTS.md
└── Secret/aikaclaw-secrets    # Gateway token + API keys
```
...
Edit `aikaclaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret aikaclaw-secrets -n aikaclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/aikaclaw -n aikaclaw
```

### Custom namespace

```bash
AIKACLAW_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
└── manifests/
    ...
    ├── configmap.yaml          # aikaclaw.json + AGENTS.md
