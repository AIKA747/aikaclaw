...
- `gateway`/`node` approvals are controlled by `~/.aikaclaw/exec-approvals.json`.
...
- AikaClaw sets `AIKACLAW_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  AikaClaw prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`aikaclaw security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `aikaclaw doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`aikaclaw security audit` and `aikaclaw doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
