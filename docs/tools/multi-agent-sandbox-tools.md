...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `aikaclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `aikaclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=aikaclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${AIKACLAW_STATE_DIR:-$HOME/.aikaclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
