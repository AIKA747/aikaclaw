---
title: Lobster
summary: "Typed workflow runtime for AikaClaw with resumable approval gates."
read_when:
...
Lobster is a workflow shell that lets AikaClaw run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: AikaClaw runs one Lobster tool call and gets a structured result.
...
AikaClaw launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | aikaclaw.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
aikaclaw.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In AikaClaw tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the AikaClaw Gateway (see the [Lobster repo](https://github.com/aikaclaw/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), AikaClaw keeps core tools enabled. To restrict core
...
→ aikaclaw calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ aikaclaw calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls AikaClaw tools that do.
