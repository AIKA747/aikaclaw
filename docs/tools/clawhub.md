---
summary: "ClawHub guide: public registry, native AikaClaw install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **AikaClaw skills and plugins**.

- Use native `aikaclaw` commands to search/install/update skills and install
...
```bash
aikaclaw plugins install clawhub:<package>
...
Native `aikaclaw` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for AikaClaw skills.
...
4. Users browse, download, and install skills in AikaClaw.
...
If you want to add new capabilities to your AikaClaw agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `aikaclaw skills install <skill-slug>`
3. Start a new AikaClaw session so it picks up the new skill.
...
## How it fits into AikaClaw

Native `aikaclaw skills install` installs into the active workspace `skills/`
directory. `aikaclaw plugins install clawhub:...` records a normal managed
...
current working directory. If an AikaClaw workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). AikaClaw loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.aikaclaw/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches AikaClaw how to perform a
...
Interested in becoming a moderator? Ask in the AikaClaw Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to AikaClaw workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/aikaclaw/skills` and `~/.aikaclaw/skills`). This is designed to find older skill installs without extra flags.
