...
read_when:
  - Running AikaClaw Gateway in WSL2 while Chrome lives on Windows
...
- AikaClaw Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/aikaclaw/aikaclaw/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- AikaClaw and Chrome are on the same machine
...
- AikaClaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, AikaClaw is not the problem yet.
...
Fix that before touching AikaClaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point AikaClaw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting AikaClaw to succeed
...
```bash
aikaclaw browser open https://example.com --browser-profile remote
aikaclaw browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `aikaclaw browser tabs` returns the target
...
3. AikaClaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug AikaClaw config or Control UI auth
