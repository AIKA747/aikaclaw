...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.aikaclaw/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so AikaClaw picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    aikaclaw agent --message "give me a greeting"
    ```
...
| `metadata.aikaclaw.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.aikaclaw.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.aikaclaw.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `aikaclaw agent --message "..."` to test before sharing
...
| `~/.aikaclaw/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with AikaClaw) | Lowest     | Global                |
