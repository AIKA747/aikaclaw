...
- **gateway host** → `aikaclaw` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, AikaClaw also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.aikaclaw/exec-approvals.sock",
...
When `tools.exec.strictInlineEval=true`, AikaClaw treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.aikaclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `aikaclaw approvals allowlist ...`).
- `aikaclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `aikaclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, AikaClaw still rejects the `env` builtin in safe-bin
...
`~/.aikaclaw/exec-approvals.json` directly.

CLI: `aikaclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, AikaClaw does not treat
...
topics, AikaClaw preserves the topic for the approval prompt and the post-approval follow-up.
