...
`aikaclaw agent` runs a single agent turn from the command line without needing
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    aikaclaw agent --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    aikaclaw agent --agent ops --message "Summarize logs"

    # Target a phone number (derives session key)
    aikaclaw agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    aikaclaw agent --session-id abc123 --message "Continue the task"
    ```
  ...
  <Step title="Deliver the reply to a channel">
    ...
    aikaclaw agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    aikaclaw agent --agent ops --message "Generate report" \
...
aikaclaw agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
