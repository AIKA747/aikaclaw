...
AikaClaw can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `aikaclaw configure --section web` enables the bundled Firecrawl plugin automatically.
...
AikaClaw always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
