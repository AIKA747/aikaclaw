...
<Steps>
  ...
  <Step title="Configure">
    ```bash
    aikaclaw configure --section web
    ```
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "AikaClaw plugin SDK" });
    ```
...
If no `provider` is set, AikaClaw checks for API keys in this order and uses
the first one found:
...
<Note>
  ...
  AikaClaw resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
<Tabs>
  <Tab title="Config file">
    Run `aikaclaw configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.aikaclaw/.env`.
