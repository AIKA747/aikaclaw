...
AikaClaw can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere AikaClaw can send audio.
...
AikaClaw defaults to Microsoft (unless disabled via
...
TTS config lives under `messages.tts` in `aikaclaw.json`.
...
- If `provider` is **unset**, AikaClaw prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `microsoft`.
...
`~/.aikaclaw/settings/tts.json`, override with `AIKACLAW_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, AikaClaw retries with MP3.

OpenAI/ElevenLabs output formats are fixed per channel (see above).

## Auto-TTS behavior

When enabled, AikaClaw:
...
Discord note: `/tts` is a built-in Discord command, so AikaClaw registers
