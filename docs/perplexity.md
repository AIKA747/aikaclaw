...
AikaClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, AikaClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `aikaclaw configure --section web`. It stores the key in
`~/.aikaclaw/aikaclaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.aikaclaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
