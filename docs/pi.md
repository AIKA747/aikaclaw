...
summary: "Architecture of AikaClaw's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in AikaClaw
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how AikaClaw integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

AikaClaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, AikaClaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in AikaClaw's local TUI mode)                                             |
...
├── pi-tools.ts                    # createAikaClawCodingTools()
...
├── aikaclaw-tools.ts              # AikaClaw-specific tools
...
Image injection is prompt-local: AikaClaw loads image refs from the current prompt and
...
2. **Custom Replacements**: AikaClaw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **AikaClaw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures AikaClaw's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, AikaClaw CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
AikaClaw wraps this with `guardSessionManager()` for tool result safety.
...
AikaClaw maintains an auth profile store with multiple API keys per provider:
...
AikaClaw loads custom pi extensions for specialized behavior:
...
AikaClaw also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | AikaClaw Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom AikaClaw tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.aikaclaw/agents/<agentId>/sessions/` (or `$AIKACLAW_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `AIKACLAW_LIVE_TEST=1`)
