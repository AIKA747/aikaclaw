...
summary: "Complete reference for every AikaClaw config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.aikaclaw/aikaclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — AikaClaw uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.aikaclaw/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `aikaclaw doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `aikaclaw doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["aikaclaw-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-aikaclaw",
          requireMention: false,
...
- AikaClaw additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `aikaclaw plugins install @aikaclaw/mattermost`.
...
- `commands.callbackUrl` must resolve to the AikaClaw gateway endpoint and be reachable from the Mattermost server.
...
AikaClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `aikaclaw channels add` (or channel onboarding) while still on a single-account top-level channel config, AikaClaw moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `aikaclaw doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@aikaclaw", "aikaclaw"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@aikaclaw"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `aikaclaw.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, AikaClaw auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/aikaclaw" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, AikaClaw assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before AikaClaw aborts it. Default: `900`.
    ...
    defaults: {
      sandbox: {
        ...
        docker: {
          ...
          apparmorProfile: "aikaclaw-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/aikaclaw-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "aikaclaw-sandbox-browser:bookworm-slim",
          network: "aikaclaw-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that AikaClaw materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.aikaclaw/sandboxes`
...
In `remote` mode, host-local edits made outside AikaClaw are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `aikaclaw.json`.
noVNC observer access uses VNC auth by default and AikaClaw emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `aikaclaw-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from AIKACLAW_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `AIKACLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `AIKACLAW_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `AIKACLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.aikaclaw/workspace",
        agentDir: "~/.aikaclaw/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@aikaclaw"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.aikaclaw/workspace-home" },
      { id: "work", workspace: "~/.aikaclaw/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, AikaClaw resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.aikaclaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.aikaclaw/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.aikaclaw/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.aikaclaw/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, AikaClaw starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.aikaclaw/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, AikaClaw treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:aikaclaw`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.aikaclaw/attachments/<uuid>/` with a `.manifest.json`.
...
AikaClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.aikaclaw/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `AIKACLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), AikaClaw forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `aikaclaw onboard --auth-choice opencode-zen` or `aikaclaw onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `aikaclaw onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `aikaclaw onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `aikaclaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `aikaclaw onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `aikaclaw onboard --auth-choice minimax-api`.
...
- Loaded from `~/.aikaclaw/extensions`, `<workspace>/.aikaclaw/extensions`, plus `plugins.load.paths`.
- Discovery accepts native AikaClaw plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native AikaClaw plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; AikaClaw applies those as sanitized agent settings, not as raw AikaClaw config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `aikaclaw plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      aikaclaw: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or AIKACLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `AIKACLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `AIKACLAW_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `AIKACLAW_APNS_RELAY_BASE_URL` / `AIKACLAW_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `AIKACLAW_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
AIKACLAW_CONFIG_PATH=~/.aikaclaw/a.json \
AIKACLAW_STATE_DIR=~/.aikaclaw-a \
aikaclaw gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.aikaclaw-dev` + port `19001`), `--profile <name>` (uses `~/.aikaclaw-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.aikaclaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-aikaclaw-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `AIKACLAW_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.aikaclaw/workspace/canvas",
    liveReload: true,
    // enabled: false, // or AIKACLAW_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__aikaclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__aikaclaw__/a2ui/`
...
- Also serves A2UI at `/__aikaclaw__/a2ui/`.
...
- Hostname defaults to `aikaclaw`. Override with `AIKACLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.aikaclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `aikaclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.aikaclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${AIKACLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `aikaclaw.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.aikaclaw/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/aikaclaw-vault-resolver",
...
- Legacy OAuth imports from `~/.aikaclaw/credentials/oauth.json`.
...
- Default log file: `/tmp/aikaclaw/aikaclaw-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one AikaClaw.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `AIKACLAW_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `aikaclaw doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `aikaclaw.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
