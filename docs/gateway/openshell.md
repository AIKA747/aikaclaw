---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for AikaClaw agents"
read_when:
...
OpenShell is a managed sandbox backend for AikaClaw. Instead of running Docker
containers locally, AikaClaw delegates sandbox lifecycle to the `openshell` CLI,
...
- AikaClaw Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, AikaClaw creates an OpenShell
...
- You edit files locally outside AikaClaw and want those changes visible in the
...
- When the sandbox is first created, AikaClaw seeds the remote workspace from
...
- AikaClaw does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside AikaClaw after the initial seed,
the remote sandbox does **not** see those changes. Use
`aikaclaw sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"aikaclaw"`  | Sandbox source for first-time create                  |
...
1. AikaClaw calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. AikaClaw calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `aikaclaw sandbox` commands
