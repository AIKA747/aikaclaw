...
- During heartbeat runs, AikaClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `aikaclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, AikaClaw skips the heartbeat run entirely (no model call).
...
```bash
aikaclaw system event --text "Check for urgent follow-ups" --mode now
```
