...
AikaClaw supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `AIKACLAW_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, AikaClaw runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). AikaClaw validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/AikaClaw QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/aikaclaw"],
...
- AikaClaw resolves these refs during sandbox activation, not lazily during each SSH call.
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `aikaclaw.json` refs)
...
When reload-time activation fails after a healthy state, AikaClaw enters degraded secrets state.
...
- Strict command paths (for example `aikaclaw memory` remote-memory paths and `aikaclaw qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `aikaclaw status`, `aikaclaw status --all`, `aikaclaw channels status`, `aikaclaw channels resolve`, `aikaclaw security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `aikaclaw secrets reload`.
...
```bash
aikaclaw secrets audit --check
aikaclaw secrets configure
aikaclaw secrets audit --check
```
...
- plaintext values at rest (`aikaclaw.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `aikaclaw.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `aikaclaw secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `aikaclaw.json` plus `auth-profiles.json` for one agent scope
...
- `aikaclaw secrets configure --providers-only`
- `aikaclaw secrets configure --skip-provider-setup`
- `aikaclaw secrets configure --agent <id>`
...
```bash
aikaclaw secrets apply --from /tmp/aikaclaw-secrets-plan.json
aikaclaw secrets apply --from /tmp/aikaclaw-secrets-plan.json --allow-exec
aikaclaw secrets apply --from /tmp/aikaclaw-secrets-plan.json --dry-run
aikaclaw secrets apply --from /tmp/aikaclaw-secrets-plan.json --dry-run --allow-exec
```
...
AikaClaw intentionally does not write rollback backups containing historical plaintext secret values.
