---
summary: "How AikaClaw sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
AikaClaw can run **tools inside sandbox backends** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`aikaclaw-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; AikaClaw emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
Use `backend: "ssh"` when you want AikaClaw to sandbox `exec`, file tools, and media reads on
...
- AikaClaw creates a per-scope remote root under `sandbox.ssh.workspaceRoot`.
- On first use after create or recreate, AikaClaw seeds that remote workspace from the local workspace once.
...
- AikaClaw does not sync remote changes back to the local workspace automatically.

Authentication material:

- `identityFile`, `certificateFile`, `knownHostsFile`: use existing local files and pass them through OpenSSH config.
- `identityData`, `certificateData`, `knownHostsData`: use inline strings or SecretRefs. AikaClaw resolves them through the normal secrets runtime snapshot, writes them to temp files with `0600`, and deletes them when the SSH session ends.
...
- Host-local edits made outside AikaClaw after the seed step are not visible remotely until you recreate the sandbox.
- `aikaclaw sandbox recreate` deletes the per-scope remote root and seeds again from local on next use.
- Browser sandboxing is not supported on the SSH backend.
- `sandbox.docker.*` settings do not apply to the SSH backend.

### OpenShell backend

Use `backend: "openshell"` when you want AikaClaw to sandbox tools in an
...
- `mirror` (default): local workspace stays canonical. AikaClaw syncs local files into OpenShell before exec and syncs the remote workspace back after exec.
- `remote`: OpenShell workspace is canonical after the sandbox is created. AikaClaw seeds the remote workspace once from the local workspace, then file tools and exec run directly against the remote sandbox without syncing changes back.

Remote transport details:

- AikaClaw asks OpenShell for sandbox-specific SSH config via `openshell sandbox ssh-config <name>`.
...
- Before `exec`, AikaClaw syncs the local workspace into the OpenShell sandbox.
- After `exec`, AikaClaw syncs the remote workspace back to the local workspace.
- File tools still operate through the sandbox bridge, but the local workspace remains the source of truth between turns.

Use this when:

- you edit files locally outside AikaClaw and want those changes to show up in the sandbox automatically
...
- When the sandbox is first created, AikaClaw seeds the remote workspace from the local workspace once.
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate directly against the remote OpenShell workspace.
- AikaClaw does **not** sync remote changes back into the local workspace after exec.
...
- If you edit files on the host outside AikaClaw after the seed step, the remote sandbox will **not** see those changes automatically.
...
- `aikaclaw sandbox list` shows OpenShell runtimes as well as Docker runtimes
- `aikaclaw sandbox recreate` deletes the current runtime and lets AikaClaw recreate it on next use
...
- `"none"` (default): tools see a sandbox workspace under `~/.aikaclaw/sandboxes`.
...
AikaClaw mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- AikaClaw blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
Default Docker image: `aikaclaw-sandbox:bookworm-slim`
...
`aikaclaw-sandbox-common:bookworm-slim`.
...
- The three graphics hardening flags (`--disable-3d-apis`,
  ...
  when containers lack GPU support. Set `AIKACLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0`
...
- `--disable-extensions` is enabled by default and can be disabled with
  `AIKACLAW_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
- `--renderer-process-limit=2` is controlled by
  `AIKACLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
Set `AIKACLAW_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can
override socket location with `AIKACLAW_DOCKER_SOCKET`. Full setup and env
...
- Use `aikaclaw sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
