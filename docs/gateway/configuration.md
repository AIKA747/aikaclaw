...
read_when:
  - Setting up AikaClaw for the first time
...
AikaClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.aikaclaw/aikaclaw.json`.

If the file is missing, AikaClaw uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `aikaclaw onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    aikaclaw onboard       # full onboarding flow
    aikaclaw configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.aikaclaw/aikaclaw.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
AikaClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`aikaclaw doctor`, `aikaclaw logs`, `aikaclaw health`, `aikaclaw status`)
- Run `aikaclaw doctor` to see exact issues
- Run `aikaclaw doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `aikaclaw.json`.
    ...
    ```bash
    aikaclaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `AIKACLAW_APNS_RELAY_BASE_URL` and `AIKACLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `AIKACLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.aikaclaw/aikaclaw.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `aikaclaw config set` for single keys.
    </Warning>
    ...
    ```bash
    aikaclaw gateway call config.get --params '{}'  # capture payload.hash
    aikaclaw gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.aikaclaw/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    aikaclaw gateway call config.patch --params '{
...
AikaClaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.aikaclaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, AikaClaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `AIKACLAW_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${AIKACLAW_GATEWAY_TOKEN}" } },
