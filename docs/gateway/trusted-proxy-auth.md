...
read_when:
  - Running AikaClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of AikaClaw
...
- You run AikaClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. AikaClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. AikaClaw extracts the user identity from the configured header
...
- AikaClaw can stay on loopback HTTP behind the proxy.
...
If AikaClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://aikaclaw.example.com
    to: http://aikaclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
aikaclaw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy aikaclaw:18789 {
...
location / {
    ...
    proxy_pass http://aikaclaw:18789;
...
`aikaclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update AikaClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `aikaclaw security audit` and review findings
