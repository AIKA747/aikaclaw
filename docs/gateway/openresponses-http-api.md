...
AikaClaw’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`aikaclaw agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "aikaclaw"`, `model: "aikaclaw/default"`, `model: "aikaclaw/<agentId>"`, or `x-aikaclaw-agent-id`
- use `x-aikaclaw-model` when you want to override the selected agent's backend model
- use `x-aikaclaw-session-key` for explicit session routing
- use `x-aikaclaw-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: AikaClaw reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
