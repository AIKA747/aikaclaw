...
`aikaclaw doctor` is the repair + migration tool for AikaClaw. It fixes stale
...
```bash
aikaclaw doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
aikaclaw doctor --repair --force
```
...
- Extra workspace dir detection (`~/aikaclaw`).
...
you to run `aikaclaw doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.aikaclaw/aikaclaw.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.aikaclaw/sessions/` to `~/.aikaclaw/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.aikaclaw/agent/` to `~/.aikaclaw/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.aikaclaw/credentials/*.json` (except `oauth.json`)
  - to `~/.aikaclaw/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `aikaclaw doctor`.

### 3b) Legacy cron store migrations

Doctor also checks the cron job store (`~/.aikaclaw/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.aikaclaw` folders exist across
  home directories or when `AIKACLAW_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.aikaclaw/aikaclaw.json` is
...
offers to remove them and install the AikaClaw service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named AikaClaw gateway services are considered first-class and are not
flagged as "extra."
...
- `aikaclaw doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `aikaclaw doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
- `aikaclaw doctor` prompts before rewriting supervisor config.
- `aikaclaw doctor --yes` accepts the default repair prompts.
- `aikaclaw doctor --repair` applies recommended fixes without prompts.
- `aikaclaw doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `aikaclaw gateway install --force`.
