...
read_when:
  - You see the AIKACLAW_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the AIKACLAW_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external AikaClaw plugin
---

# Plugin SDK Migration

AikaClaw has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`aikaclaw/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`aikaclaw/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`aikaclaw/plugin-sdk/\<subpath\>`)
...
<Steps>
  <Step title="Find deprecated imports">
    ...
    grep -r "aikaclaw/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "aikaclaw/plugin-sdk/compat";
    ...
    import { runEmbeddedPiAgent } from "aikaclaw/extension-api";
...
```bash
AIKACLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 aikaclaw gateway run
AIKACLAW_SUPPRESS_EXTENSION_API_WARNING=1 aikaclaw gateway run
```
