...
read_when:
  ...
  - You want a reference for all registration methods on AikaClawPluginApi
...
The `register(api)` callback receives an `AikaClawPluginApi` object with these
methods:
...
| `api.config`             | `AikaClawConfig`          | Current config snapshot                                   |
...
<Warning>
  Never import your own plugin through `aikaclaw/plugin-sdk/<your-plugin>`
