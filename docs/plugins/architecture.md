...
read_when:
  - Building or debugging native AikaClaw plugins
...
This page covers the internal architecture of the AikaClaw plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside AikaClaw. Every
native AikaClaw plugin registers against one or more capability types:
...
AikaClaw classifies every loaded plugin into a shape based on its actual
...
Use `aikaclaw plugins inspect <id>` to see a plugin's shape and capability
...
When you run `aikaclaw doctor` or `aikaclaw plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `aikaclaw status --all` and `aikaclaw plugins doctor`.

## Architecture overview

AikaClaw's plugin system has four layers:

1. **Manifest + discovery**
   AikaClaw finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `aikaclaw.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native AikaClaw plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of AikaClaw reads the registry to expose tools, channels, provider
...
That split lets AikaClaw validate config, explain missing/disabled plugins, and
...
normal chat actions. AikaClaw keeps one shared `message` tool in core, and
...
AikaClaw treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's AikaClaw-facing
  surfaces
...
So if AikaClaw adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If AikaClaw has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { AikaClawPluginDefinition } from "aikaclaw/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "aikaclaw/plugin-sdk";

const plugin: AikaClawPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
AikaClaw already treats image/audio/video understanding as one shared
...
If AikaClaw adds a new domain later, such as video generation, use the same
...
`AikaClawPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   AikaClaw can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that AikaClaw knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `AikaClawPluginApi` or
  `api.runtime`
...
Native AikaClaw plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the AikaClaw process

Compatible bundles are safer by default because AikaClaw currently treats them
...
name: `@aikaclaw/<id>` by default, or an approved typed suffix such as
...
AikaClaw exports capabilities, not implementation convenience.
...
At startup, AikaClaw does roughly this:
...
The manifest is the control-plane source of truth. AikaClaw uses it to:
...
AikaClaw keeps short in-process caches for:
...
- Set `AIKACLAW_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `AIKACLAW_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `AIKACLAW_PLUGIN_DISCOVERY_CACHE_MS` and
  `AIKACLAW_PLUGIN_MANIFEST_CACHE_MS`.
...
AikaClaw still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, AikaClaw calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | AikaClaw tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on AikaClaw's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before AikaClaw's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as AikaClaw adds those
...
- If AikaClaw adds a new capability such as video generation later, define the
...
- AikaClaw only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "AikaClaw plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `aikaclaw/plugin-sdk` import when
authoring plugins:

- `aikaclaw/plugin-sdk/plugin-entry` for plugin registration primitives.
- `aikaclaw/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `aikaclaw/plugin-sdk/channel-setup`,
  `aikaclaw/plugin-sdk/channel-pairing`,
  `aikaclaw/plugin-sdk/channel-contract`,
  `aikaclaw/plugin-sdk/channel-feedback`,
  `aikaclaw/plugin-sdk/channel-inbound`,
  `aikaclaw/plugin-sdk/channel-lifecycle`,
  `aikaclaw/plugin-sdk/channel-reply-pipeline`,
  `aikaclaw/plugin-sdk/command-auth`,
  `aikaclaw/plugin-sdk/secret-input`, and
  `aikaclaw/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `aikaclaw/plugin-sdk/channel-config-helpers`,
  `aikaclaw/plugin-sdk/allow-from`,
  `aikaclaw/plugin-sdk/channel-config-schema`,
  `aikaclaw/plugin-sdk/channel-policy`,
  `aikaclaw/plugin-sdk/config-runtime`,
  `aikaclaw/plugin-sdk/infra-runtime`,
  `aikaclaw/plugin-sdk/agent-runtime`,
  `aikaclaw/plugin-sdk/lazy-runtime`,
  `aikaclaw/plugin-sdk/reply-history`,
  `aikaclaw/plugin-sdk/routing`,
  `aikaclaw/plugin-sdk/status-helpers`,
  `aikaclaw/plugin-sdk/runtime-store`, and
  `aikaclaw/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `aikaclaw/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `aikaclaw/plugin-sdk/*` subpaths. AikaClaw core/test code may use the repo
...
- Avoid the root `aikaclaw/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `aikaclaw/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `aikaclaw/plugin-sdk/channel-actions`.
...
`aikaclaw/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape AikaClaw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to AikaClaw's
...
- if both `catalog` and `discovery` are registered, AikaClaw uses `catalog`
...
- Read-only command paths such as `aikaclaw status`, `aikaclaw status --all`,
  `aikaclaw channels status`, `aikaclaw channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `aikaclaw.extensions`:
...
Security guardrail: every `aikaclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `aikaclaw plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `aikaclaw.setupEntry` can point at a lightweight setup-only module.
When AikaClaw needs setup surfaces for a disabled channel plugin, or
...
Optional: `aikaclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let AikaClaw load the
...
Channel plugins can advertise setup/discovery metadata via `aikaclaw.channel` and
install hints via `aikaclaw.install`. This keeps the core catalog data-free.
...
AikaClaw can also merge **external channel catalogs** (for example, an MPM
...
Or point `AIKACLAW_PLUGIN_CATALOG_PATHS` (or `AIKACLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "aikaclaw": { "channel": {...}, "install": {...} } } ] }`.
...
```ts
import { delegateCompactionToRuntime } from "aikaclaw/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `AikaClawPluginApi` and/or `api.runtime` with the smallest useful
...
This is how AikaClaw stays opinionated without becoming hardcoded to one
