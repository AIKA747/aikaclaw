...
read_when:
  - You are building an AikaClaw plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (aikaclaw.plugin.json)

This page is for the **native AikaClaw plugin manifest** only.
...
AikaClaw auto-detects those bundle layouts too, but they are not validated
against the `aikaclaw.plugin.json` schema described here.

For compatible bundles, AikaClaw currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches AikaClaw runtime expectations.

Every native AikaClaw plugin **must** ship a `aikaclaw.plugin.json` file in the
**plugin root**. AikaClaw uses this manifest to validate configuration
...
`aikaclaw.plugin.json` is the metadata AikaClaw reads before it loads your
plugin code.
...
| `providerAuthEnvVars` | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that AikaClaw can inspect without loading plugin code.                                      |
...
AikaClaw reads this before provider runtime loads.
...
| `choiceLabel`      | No       | `string`                                        | User-facing label. If omitted, AikaClaw falls back to `choiceId`.                                        |
...
| File                   | Use it for                                                                                                         |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `aikaclaw.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs           |
| `package.json`         | npm metadata, dependency installation, and the `aikaclaw` block used for entrypoints and setup or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if AikaClaw must know it before loading plugin code, put it in `aikaclaw.plugin.json`
...
- The manifest is **required for native AikaClaw plugins**, including local filesystem loads.
