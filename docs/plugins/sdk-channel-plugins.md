...
summary: "Step-by-step guide to building a messaging channel plugin for AikaClaw"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect AikaClaw to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects AikaClaw to a
...
Channel plugins do not need their own send/edit/react tools. AikaClaw keeps one
...
<Steps>
    ...
    ```json package.json
    {
      ...
      "aikaclaw": {
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect AikaClaw to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Add a setup entry">
    ...
    AikaClaw loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    AikaClaw. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to AikaClaw.
...
├── package.json              # aikaclaw.channel metadata
├── aikaclaw.plugin.json      # Manifest with config schema
