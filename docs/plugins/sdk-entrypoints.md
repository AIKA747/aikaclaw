...
**Import:** `aikaclaw/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `AikaClawPluginConfigSchema \| () => AikaClawPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: AikaClawPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `aikaclaw.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.

## `defineChannelPluginEntry`

**Import:** `aikaclaw/plugin-sdk/core`
...
| `registerFull` | `(api: AikaClawPluginApi) => void`                               | No       | —                   |
...
AikaClaw loads this instead of the full entry when a channel is disabled,
...
AikaClaw classifies loaded plugins by their registration behavior:
...
Use `aikaclaw plugins inspect <id>` to see a plugin's shape.
