...
read_when:
  - You want to place an outbound voice call from AikaClaw
...
Voice calls for AikaClaw via a plugin. Supports outbound notifications and
...
- Use `aikaclaw voicecall ...` or the `voice_call` tool
...
```bash
aikaclaw plugins install ./extensions/voice-call
...
aikaclaw voicecall start --to "+15555550123"   # alias for call
...
aikaclaw voicecall latency                     # summarize turn latency from logs
