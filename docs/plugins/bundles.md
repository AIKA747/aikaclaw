---
summary: "Install and use Codex, Claude, and Cursor bundles as AikaClaw plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how AikaClaw maps bundle content into native features
...
AikaClaw can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
AikaClaw maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native AikaClaw plugins. Native plugins run
...
of requiring authors to rewrite them as native AikaClaw plugins, AikaClaw
...
## What AikaClaw maps from bundles

Not every bundle feature runs in AikaClaw today. Here is what works and what
...
| ------------- | ---------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal AikaClaw skills                                                    | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                           | Claude, Cursor |
| Hook packs    | AikaClaw-style `HOOK.md` + `handler.ts` layouts                                                      | Codex          |
...
These are recognized and shown in diagnostics, but AikaClaw does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit AikaClaw best when they use skill roots and AikaClaw-style
...
AikaClaw checks for native plugin format first:

1. `aikaclaw.plugin.json` or valid `package.json` with `aikaclaw.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, AikaClaw uses the native path. This prevents
...
- AikaClaw does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `aikaclaw plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. AikaClaw does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    AikaClaw hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
