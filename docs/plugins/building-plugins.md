...
summary: "Create your first AikaClaw plugin in minutes"
read_when:
  - You want to create a new AikaClaw plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to AikaClaw
---

# Building Plugins

Plugins extend AikaClaw with new capabilities: channels, model providers, speech,
image generation, web search, agent tools, or any combination.

You do not need to add your plugin to the AikaClaw repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`aikaclaw plugins install <package-name>`. AikaClaw tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect AikaClaw to a messaging platform (Discord, IRC, etc.)
  </Card>
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json aikaclaw.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to AikaClaw",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** publish to [ClawHub](/tools/clawhub) or npm, then install:
    ...
    AikaClaw checks ClawHub first, then falls back to npm.
...
Always import from focused `aikaclaw/plugin-sdk/<subpath>` paths:
...
import { ... } from "aikaclaw/plugin-sdk";
```
...
<Check>**package.json** has correct `aikaclaw` metadata</Check>
<Check>**aikaclaw.plugin.json** manifest is present and valid</Check>
