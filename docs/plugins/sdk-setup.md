...
read_when:
  ...
  - You are defining plugin config schemas or package.json aikaclaw metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`aikaclaw.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `aikaclaw` field that tells the plugin system what
...
```json
{
  "name": "@myorg/aikaclaw-my-channel",
  "version": "1.0.0",
  "type": "module",
  "aikaclaw": {
    "extensions": ["./index.ts"],
...
```json
{
  "name": "@myorg/aikaclaw-my-provider",
  "version": "1.0.0",
  "type": "module",
  "aikaclaw": {
    "extensions": ["./index.ts"],
...
When enabled, AikaClaw loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `aikaclaw.plugin.json` in the package root.
AikaClaw uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to AikaClaw",
  "configSchema": {
    "type": "object",
...
AikaClaw loads when it only needs setup surfaces (onboarding, config repair,
...
**When AikaClaw uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `aikaclaw/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that AikaClaw validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "aikaclaw/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `aikaclaw onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "aikaclaw/plugin-sdk/channel-setup";
...
helpers from `aikaclaw/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`aikaclaw/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `aikaclaw/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "aikaclaw/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/aikaclaw-my-channel",
...
```bash
aikaclaw plugins install @myorg/aikaclaw-my-plugin
```

AikaClaw tries ClawHub first and falls back to npm automatically. You can also
force a specific source:

```bash
aikaclaw plugins install clawhub:@myorg/aikaclaw-my-plugin   # ClawHub only
aikaclaw plugins install npm:@myorg/aikaclaw-my-plugin       # npm only
```
...
```bash
aikaclaw plugins search <query>
aikaclaw plugins install <package-name>
```

<Info>
  For npm-sourced installs, `aikaclaw plugins install` runs
