---
summary: "Use MiniMax models in AikaClaw"
read_when:
  - You want MiniMax models in AikaClaw
...
AikaClaw's MiniMax provider defaults to **MiniMax M2.7** and keeps
...
```bash
aikaclaw plugins enable minimax  # skip if already loaded.
aikaclaw gateway restart  # restart if gateway is already running
aikaclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax plugin README](https://github.com/aikaclaw/aikaclaw/tree/main/extensions/minimax) for details.
...
Configure manually via `aikaclaw.json`:
...
- Use `aikaclaw models list` and `aikaclaw models set minimax/MiniMax-M2.7` to switch.
...
- Running `aikaclaw configure` and selecting a **MiniMax** auth option, or
