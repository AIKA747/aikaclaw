---
summary: "Use Synthetic's Anthropic-compatible API in AikaClaw"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. AikaClaw registers it as the
...
```bash
aikaclaw onboard --auth-choice synthetic-api-key
```
...
Note: AikaClaw's Anthropic client appends `/v1` to the base URL, so use
