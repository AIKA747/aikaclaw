---
summary: "Use OpenCode Zen and Go catalogs with AikaClaw"
read_when:
...
OpenCode exposes two hosted catalogs in AikaClaw:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. AikaClaw keeps the runtime provider ids
...
aikaclaw onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
aikaclaw onboard --auth-choice opencode-go
aikaclaw onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
