---
summary: "Use Venice AI privacy-focused models in AikaClaw"
read_when:
  - You want privacy-focused inference in AikaClaw
...
```bash
aikaclaw onboard --auth-choice venice-api-key
```
...
```bash
aikaclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, AikaClaw shows all available Venice models. Pick based on your needs:
...
```bash
aikaclaw models set venice/kimi-k2-5
aikaclaw models set venice/claude-opus-4-6
```

List all available models:

```bash
aikaclaw models list | grep venice
```

## Configure via `aikaclaw configure`

1. Run `aikaclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
AikaClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
aikaclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
aikaclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
aikaclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
aikaclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
aikaclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `aikaclaw models list` to see currently available models. Some models may be temporarily offline.
