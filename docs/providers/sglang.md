---
summary: "Run AikaClaw with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run AikaClaw against a local SGLang server
...
AikaClaw can connect to SGLang using the `openai-completions` API.

AikaClaw can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, AikaClaw will query:
