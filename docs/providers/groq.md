...
read_when:
  - You want to use Groq with AikaClaw
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. AikaClaw connects
...
media-understanding provider, AikaClaw uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.aikaclaw/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `aikaclaw models list | grep groq`
