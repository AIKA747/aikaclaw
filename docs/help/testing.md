...
AikaClaw has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
- Pool note:
  ...
  - Opt back into Vitest file isolation with `AIKACLAW_TEST_ISOLATE=1 pnpm test`.
  - `AIKACLAW_TEST_NO_ISOLATE=0` or `AIKACLAW_TEST_NO_ISOLATE=false` also force isolated runs.
- Fast-local iteration note:
  ...
  - Opt out with `AIKACLAW_VITEST_FS_MODULE_CACHE=0` or `AIKACLAW_VITEST_FS_MODULE_CACHE=false` if you suspect stale transform cache behavior.
- Perf-debug note:
...
- Useful overrides:
  - `AIKACLAW_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `AIKACLAW_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises AikaClaw's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `AIKACLAW_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `AIKACLAW_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `AIKACLAW_LIVE_TEST=1`)
- Scope:
...
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `AIKACLAW_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `AIKACLAW_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `AIKACLAW_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Optional target overrides:
  - `AIKACLAW_ANDROID_NODE_ID` or `AIKACLAW_ANDROID_NODE_NAME`.
  - `AIKACLAW_ANDROID_GATEWAY_URL` / `AIKACLAW_ANDROID_GATEWAY_TOKEN` / `AIKACLAW_ANDROID_GATEWAY_PASSWORD`.
...
- How to enable:
  - `pnpm test:live` (or `AIKACLAW_LIVE_TEST=1` if invoking Vitest directly)
- Set `AIKACLAW_LIVE_MODELS=modern` (or `all`, alias for modern) to actually run this suite; otherwise it skips to keep `pnpm test:live` focused on gateway smoke
- How to select models:
  - `AIKACLAW_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet/Haiku 4.5, GPT-5.x + Codex, Gemini 3, GLM 4.7, MiniMax M2.7, Grok 4)
  - `AIKACLAW_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `AIKACLAW_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,..."` (comma allowlist)
- How to select providers:
  - `AIKACLAW_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `AIKACLAW_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
  - Separates “provider API is broken / key is invalid” from “gateway agent pipeline is broken”
  - Contains small, isolated regressions (example: OpenAI Responses/Codex Responses reasoning replay + tool-call flows)

### Layer 2: Gateway + dev agent smoke (what "@aikaclaw" actually does)
...
- How to select models:
  ...
  - `AIKACLAW_LIVE_GATEWAY_MODELS=all` is an alias for the modern allowlist
  - Or set `AIKACLAW_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
- How to select providers (avoid “OpenRouter everything”):
  - `AIKACLAW_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Enable:
  - `pnpm test:live` (or `AIKACLAW_LIVE_TEST=1` if invoking Vitest directly)
  - `AIKACLAW_LIVE_SETUP_TOKEN=1`
- Token sources (pick one):
  - Profile: `AIKACLAW_LIVE_SETUP_TOKEN_PROFILE=anthropic:setup-token-test`
  - Raw token: `AIKACLAW_LIVE_SETUP_TOKEN_VALUE=sk-ant-oat01-...`
- Model override (optional):
  - `AIKACLAW_LIVE_SETUP_TOKEN_MODEL=anthropic/claude-opus-4-6`

Setup example:

```bash
aikaclaw models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
AIKACLAW_LIVE_SETUP_TOKEN=1 AIKACLAW_LIVE_SETUP_TOKEN_PROFILE=anthropic:setup-token-test pnpm test:live src/agents/anthropic.setup-token.live.test.ts
```
...
- Enable:
  - `pnpm test:live` (or `AIKACLAW_LIVE_TEST=1` if invoking Vitest directly)
  - `AIKACLAW_LIVE_CLI_BACKEND=1`
- Defaults:
  - Model: `claude-cli/claude-sonnet-4-6`
  - Command: `claude`
  - Args: `["-p","--output-format","json","--permission-mode","bypassPermissions"]`
- Overrides (optional):
  - `AIKACLAW_LIVE_CLI_BACKEND_MODEL="claude-cli/claude-opus-4-6"`
  - `AIKACLAW_LIVE_CLI_BACKEND_MODEL="codex-cli/gpt-5.4"`
  - `AIKACLAW_LIVE_CLI_BACKEND_COMMAND="/full/path/to/claude"`
  - `AIKACLAW_LIVE_CLI_BACKEND_ARGS='["-p","--output-format","json","--permission-mode","bypassPermissions"]'`
  - `AIKACLAW_LIVE_CLI_BACKEND_CLEAR_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'`
  - `AIKACLAW_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt).
  - `AIKACLAW_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `AIKACLAW_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `AIKACLAW_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
- `AIKACLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` to keep Claude Code CLI MCP config enabled (default disables MCP config with a temporary empty file).

Example:

```bash
AIKACLAW_LIVE_CLI_BACKEND=1 \
  AIKACLAW_LIVE_CLI_BACKEND_MODEL="claude-cli/claude-sonnet-4-6" \
...
- Single model, direct (no gateway):
  - `AIKACLAW_LIVE_MODELS="openai/gpt-5.2" pnpm test:live src/agents/models.profiles.live.test.ts`

- Single model, gateway smoke:
  - `AIKACLAW_LIVE_GATEWAY_MODELS="openai/gpt-5.2" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `AIKACLAW_LIVE_GATEWAY_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google focus (Gemini API key + Antigravity):
  - Gemini (API key): `AIKACLAW_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
  - Antigravity (OAuth): `AIKACLAW_LIVE_GATEWAY_MODELS="google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-pro-high" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Gemini API vs Gemini CLI:
  - API: AikaClaw calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: AikaClaw shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
`AIKACLAW_LIVE_GATEWAY_MODELS="openai/gpt-5.2,openai-codex/gpt-5.4,anthropic/claude-opus-4-6,google/gemini-3.1-pro-preview,google/gemini-3-flash-preview,google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-flash,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
Include at least one image-capable model in `AIKACLAW_LIVE_GATEWAY_MODELS` (Claude/Gemini/OpenAI vision-capable variants, etc.) to exercise the image probe.

### Aggregators / alternate gateways

If you have keys enabled, we also support testing via:

- OpenRouter: `openrouter/...` (hundreds of models; use `aikaclaw models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `aikaclaw models list` / model selection.

- Profile store: `~/.aikaclaw/credentials/` (preferred; what “profile keys” means in the tests)
- Config: `~/.aikaclaw/aikaclaw.json` (or `AIKACLAW_CONFIG_PATH`)
...
- Current bundled providers covered:
  ...
  - `google`
- Optional narrowing:
  - `AIKACLAW_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google"`
  - `AIKACLAW_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-1,google/gemini-3.1-flash-image-preview"`
  - `AIKACLAW_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit"`
- Optional auth behavior:
  - `AIKACLAW_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
They also set `AIKACLAW_SKIP_CHANNELS=1` so gateway live probes do not start
...
`AIKACLAW_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
...
- `AIKACLAW_CONFIG_DIR=...` (default: `~/.aikaclaw`) mounted to `/home/node/.aikaclaw`
- `AIKACLAW_WORKSPACE_DIR=...` (default: `~/.aikaclaw/workspace`) mounted to `/home/node/.aikaclaw/workspace`
- `AIKACLAW_PROFILE_FILE=...` (default: `~/.profile`) mounted to `/home/node/.profile` and sourced before running tests
- External CLI auth dirs under `$HOME` are mounted read-only under `/host-auth/...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs inferred from `AIKACLAW_LIVE_PROVIDERS` / `AIKACLAW_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `AIKACLAW_DOCKER_AUTH_DIRS=all`, `AIKACLAW_DOCKER_AUTH_DIRS=none`, or a comma list like `AIKACLAW_DOCKER_AUTH_DIRS=.claude,.codex`
- `AIKACLAW_LIVE_GATEWAY_MODELS=...` / `AIKACLAW_LIVE_MODELS=...` to narrow the run
- `AIKACLAW_LIVE_GATEWAY_PROVIDERS=...` / `AIKACLAW_LIVE_PROVIDERS=...` to filter providers in-container
- `AIKACLAW_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
