---
summary: "Frequently asked questions about AikaClaw setup, configuration, and usage"
read_when:
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/aikaclaw/aikaclaw-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   aikaclaw health --json
   aikaclaw health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://aikaclaw.ai/install.sh | bash -s -- --install-method git
    ```

    This installs AikaClaw **from a git checkout**, so the agent can read the code + docs and
    ...
    [https://github.com/aikaclaw/aikaclaw/issues](https://github.com/aikaclaw/aikaclaw/issues)
    [https://github.com/aikaclaw/aikaclaw/pulls](https://github.com/aikaclaw/aikaclaw/pulls)

    Start with these commands (share outputs when asking for help):

    ```bash
    aikaclaw status
    aikaclaw models status
    aikaclaw doctor
    ```

    What they do:

    - `aikaclaw status`: quick snapshot of gateway/agent health + basic config.
    - `aikaclaw models status`: checks provider auth + model availability.
    - `aikaclaw doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `aikaclaw status --all`, `aikaclaw logs --follow`,
    `aikaclaw gateway status`, `aikaclaw health --verbose`.
  ...
  <Accordion title="Recommended way to install and set up AikaClaw">
    ...
    If you don't have a global install yet, run it via `pnpm aikaclaw onboard`.
  ...
  <Accordion title="How do I authenticate the dashboard (token) on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - If it asks for auth, paste the token from `gateway.auth.token` (or `AIKACLAW_GATEWAY_TOKEN`) into Control UI settings.
    - Retrieve it from the gateway host: `aikaclaw config get gateway.auth.token` (or generate one: `aikaclaw doctor --generate-gateway-token`).

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `aikaclaw gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
    - **Tailnet bind**: run `aikaclaw gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    1. Install AikaClaw on the new machine.
    2. Copy `$AIKACLAW_STATE_DIR` (default: `~/.aikaclaw`) from the old machine.
    3. Copy your workspace (default: `~/.aikaclaw/workspace`).
    4. Run `aikaclaw doctor` and restart the Gateway service.
    ...
    **Important:** if you only commit/push your workspace to GitHub, you're backing
    ...
    under `~/.aikaclaw/` (for example `~/.aikaclaw/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/aikaclaw/aikaclaw/blob/main/CHANGELOG.md](https://github.com/aikaclaw/aikaclaw/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.aikaclaw.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.aikaclaw.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.aikaclaw.ai`, then retry. More
    detail: [Troubleshooting](/help/faq#cannot-access-docsaikaclaw-ai-ssl-error).
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/aikaclaw/aikaclaw/tree/main/docs](https://github.com/aikaclaw/aikaclaw/tree/main/docs)
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://aikaclaw.ai/install.sh | bash -s -- --beta
    ```
    ...
    [https://aikaclaw.ai/install.ps1](https://aikaclaw.ai/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://aikaclaw.ai/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://aikaclaw.ai/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://aikaclaw.ai/install.sh | bash -s -- --install-method git --verbose
    ```
  ...
  <Accordion title="Windows install says git not found or aikaclaw not recognized">
    ...
    **2) aikaclaw is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest AikaClaw, track/report it in:

    - [Issue #30640](https://github.com/aikaclaw/aikaclaw/issues/30640)
  ...
  <Accordion title="How do I install AikaClaw on Linux?">
  ...
  <Accordion title="How do I install AikaClaw on a VPS?">
  ...
  <Accordion title="Can I ask AikaClaw to update itself?">
    ...
    ```bash
    aikaclaw update
    aikaclaw update status
    aikaclaw update --channel stable|beta|dev
    aikaclaw update --tag <dist-tag|version>
    aikaclaw update --no-restart
    ```

    If you must automate from an agent:

    ```bash
    aikaclaw update --yes --no-restart
    aikaclaw gateway restart
    ```
  ...
  <Accordion title="What does onboarding actually do?">
    `aikaclaw onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. You can run AikaClaw with **API keys** (Anthropic/OpenAI/others) or with
    **local-only models** so your data stays on your device. Subscriptions (Claude
    ...
    OpenAI Codex OAuth is explicitly supported for external tools like AikaClaw.
  ...
  <Accordion title="How does Anthropic setup-token auth work?">
    `claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in onboarding or paste it with `aikaclaw models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
  </Accordion>
  ...
  <Accordion title="Where do I find an Anthropic setup-token?">
    ...
    Copy the token it prints, then choose **Anthropic token (paste setup-token)** in onboarding. If you want to run it on the gateway host, use `aikaclaw models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `aikaclaw models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

  </Accordion>

  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    Yes - via **setup-token**. AikaClaw no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
  ...
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Tip: set a **fallback model** so AikaClaw can keep replying while a provider is rate-limited.
  ...
  <Accordion title="How does Codex auth work?">
    AikaClaw supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). Onboarding can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Onboarding (CLI)](/start/wizard).
  </Accordion>

  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. AikaClaw fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like AikaClaw. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `aikaclaw.json`.

    Steps:

    1. Enable the plugin: `aikaclaw plugins enable google`
    2. Login: `aikaclaw models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. AikaClaw needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. AikaClaw runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
  ...
  <Accordion title="If I buy a Mac mini to run AikaClaw, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `aikaclaw nodes status` / `aikaclaw nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Onboarding accepts `@username` input and resolves it to a numeric ID, but AikaClaw authorization uses numeric IDs only.

    Safer (no third-party bot):

    - DM your bot, then run `aikaclaw logs --follow` and read `from.id`.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different AikaClaw instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    If you run AikaClaw via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    ...
    This **does not delete your data** - it only changes the AikaClaw code install. Your state
    (`~/.aikaclaw`) and workspace (`~/.aikaclaw/workspace`) stay untouched.
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **AikaClaw-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run AikaClaw on a dedicated machine?">
  ...
  <Accordion title="What are the minimum VPS requirements and recommended OS?">
    AikaClaw is lightweight. For a basic Gateway + one chat channel:
  ...
  <Accordion title="Can I run AikaClaw in a VM and what are the requirements?">
...
<AccordionGroup>
  <Accordion title="What is AikaClaw, in one paragraph?">
    AikaClaw is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.
  </Accordion>

  <Accordion title="Value proposition">
    AikaClaw is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
  ...
  <Accordion title="What are the top five everyday use cases for AikaClaw?">
  ...
  <Accordion title="Can AikaClaw help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    AikaClaw draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    AikaClaw is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use AikaClaw when you
    ...
    Showcase: [https://aikaclaw.ai/showcase](https://aikaclaw.ai/showcase)
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.aikaclaw/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.aikaclaw/aikaclaw.json`). Precedence is `<workspace>/skills` > `~/.aikaclaw/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.aikaclaw/aikaclaw.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.aikaclaw/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which AikaClaw treats as `<workspace>/skills` on the next session.
  </Accordion>
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `AIKACLAW_SKIP_CRON` is not set.
    ...
    ```bash
    aikaclaw cron run <jobId> --force
    aikaclaw cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `aikaclaw skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://clawhub.com](https://clawhub.com).

    ```bash
    aikaclaw skills search "calendar"
    aikaclaw skills install <skill-slug>
  ...
  <Accordion title="Can AikaClaw run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.aikaclaw.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH).**
    Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. AikaClaw can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.aikaclaw/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "aikaclaw": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.aikaclaw/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with AikaClaw?">
    ...
    aikaclaw browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Persist `/home/node` with `AIKACLAW_HOME_VOLUME` so caches survive.
    - Bake system deps into the image with `AIKACLAW_DOCKER_APT_PACKAGES`.
  ...
  <Accordion title="How does memory work?">
    AikaClaw memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    AikaClaw also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, AikaClaw auto-selects a provider when it
    ...
    configured and present, AikaClaw
...
<AccordionGroup>
  <Accordion title="Is all data used with AikaClaw saved locally?">
    No - **AikaClaw's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.aikaclaw` + your workspace directory).
  ...
  <Accordion title="Where does AikaClaw store its data?">
    Everything lives under `$AIKACLAW_STATE_DIR` (default: `~/.aikaclaw`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$AIKACLAW_STATE_DIR/aikaclaw.json`                             | Main config (JSON5)                                                |
    | `$AIKACLAW_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
    | `$AIKACLAW_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
    | `$AIKACLAW_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
    | `$AIKACLAW_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
    | `$AIKACLAW_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
    | `$AIKACLAW_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
    | `$AIKACLAW_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
    | `$AIKACLAW_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

    Legacy single-agent path: `~/.aikaclaw/agent/*` (migrated by `aikaclaw doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.aikaclaw/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.aikaclaw`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md` (or legacy fallback `memory.md` when `MEMORY.md` is absent),
      `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
    - **State dir (`~/.aikaclaw`)**: config, credentials, auth profiles, sessions, logs,
      and shared skills (`~/.aikaclaw/skills`).

    Default workspace is `~/.aikaclaw/workspace`, configurable via:

    ```json5
    {
      agents: { defaults: { workspace: "~/.aikaclaw/workspace" } },
    }
    ```
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.aikaclaw` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall AikaClaw?">
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The AikaClaw repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    AikaClaw reads an optional **JSON5** config from `$AIKACLAW_CONFIG_PATH` (default: `~/.aikaclaw/aikaclaw.json`):

    ```
    $AIKACLAW_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.aikaclaw/workspace`).

  </Accordion>

  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `AIKACLAW_GATEWAY_TOKEN`).
  ...
  <Accordion title="Why do I need a token on localhost now?">
    AikaClaw enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

    If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `aikaclaw doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one AikaClaw.` every time.
    - `random`: rotating funny/seasonal taglines (default behavior).
    - If you want no banner at all, set env `AIKACLAW_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `aikaclaw configure --section web` and choose a provider.
    ...
    - Daemons read env vars from `~/.aikaclaw/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    - Restore from backup (git or a copied `~/.aikaclaw/aikaclaw.json`).
    - If you have no backup, re-run `aikaclaw doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `aikaclaw config set` for small changes.
    - Use `aikaclaw configure` for interactive edits.
  ...
  <Accordion title="Can the AikaClaw browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="How can my agent access my computer if the Gateway is hosted remotely?">
    ...
    5. Approve the node on the Gateway:
       ...
       ```bash
       aikaclaw devices list
       aikaclaw devices approve <requestId>
       ```
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `aikaclaw gateway status`
    - Gateway health: `aikaclaw status`
    - Channel health: `aikaclaw channels status`
  ...
  <Accordion title="Can two AikaClaw instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `aikaclaw agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    aikaclaw agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
  ...
  <Accordion title="How do I set up Tailscale on a VPS and connect from my Mac?">
    ...
    ```bash
    aikaclaw gateway --tailscale serve
    ```
...
<AccordionGroup>
  <Accordion title="How does AikaClaw load environment variables?">
    AikaClaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.aikaclaw/.env` (aka `$AIKACLAW_STATE_DIR/.env`)
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.aikaclaw/.env` so they're picked up even when the service doesn't inherit your shell env.
    ...
    `AIKACLAW_LOAD_SHELL_ENV=1`, `AIKACLAW_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `aikaclaw models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means AikaClaw won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.aikaclaw/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of AikaClaw instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset AikaClaw but keep it installed?">
    ...
    ```bash
    aikaclaw reset --scope full --yes --non-interactive
    ```
    ...
    - If you used profiles (`--profile` / `AIKACLAW_PROFILE`), reset each state dir (defaults are `~/.aikaclaw-<profile>`).
    - Dev reset: `aikaclaw gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    headers like `# Heading`), AikaClaw skips the heartbeat run to save API calls.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. AikaClaw runs on **your own account**, so if you're in the group, AikaClaw can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    aikaclaw directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does AikaClaw not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.aikaclaw/agents/<agentId>/sessions/`.
    ...
    - Use `aikaclaw doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    AikaClaw's default model is whatever you set as:

    ```
    agents.defaults.model.primary
    ```

    Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, AikaClaw currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    - `aikaclaw models set ...` (updates just model config)
    - `aikaclaw configure --section model` (interactive)
    - edit `agents.defaults.model` in `~/.aikaclaw/aikaclaw.json`

    Avoid `config.apply` with a partial object unless you intend to replace the whole config.
    If you did overwrite config, restore from backup or re-run `aikaclaw doctor` to repair.
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `aikaclaw onboard` and choose `Ollama`
    ...
    - for manual switching, use `aikaclaw models list` and `aikaclaw models set ollama/<model>`
  ...
  <Accordion title="What do AikaClaw, Flawd, and Krill use for models?">
    - These deployments can differ and may change over time; there is no fixed provider recommendation.
    - Check the current runtime setting on each gateway with `aikaclaw models status`.
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Upgrade to a current AikaClaw release (or run from source `main`), then restart the gateway.
  ...
  <Accordion title="Are opus / sonnet / gpt built-in shortcuts?">
    Yes. AikaClaw ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
  ...
  <Accordion title="How do I add models from other providers like OpenRouter or Z.AI?">
    ...
    **No API key found for provider after adding a new agent**
    ...
    ```
    ~/.aikaclaw/agents/<agentId>/agent/auth-profiles.json
    ```

    Fix options:

    - Run `aikaclaw agents add <id>` and configure auth during the wizard.
...
<AccordionGroup>
  <Accordion title="How does failover work?">
    ...
    Cooldowns apply to failing profiles (exponential backoff), so AikaClaw can keep responding even when a provider is rate-limited or temporarily failing.
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist:**
    ...
    - **Confirm where auth profiles live** (new vs legacy paths)
      - Current: `~/.aikaclaw/agents/<agentId>/agent/auth-profiles.json`
      - Legacy: `~/.aikaclaw/agent/*` (migrated by `aikaclaw doctor`)
    - **Confirm your env var is loaded by the Gateway**
      - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.aikaclaw/.env` or enable `env.shellEnv`.
    - **Make sure you're editing the correct agent**
      - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
    - **Sanity-check model/auth status**
      - Use `aikaclaw models status` to see configured models and whether providers are authenticated.
    ...
    **Fix checklist for "No credentials found for profile anthropic"**
    ...
    - **Use a setup-token**
      - Run `claude setup-token`, then paste it with `aikaclaw models auth setup-token --provider anthropic`.
      - If the token was created on another machine, use `aikaclaw models auth paste-token --provider anthropic`.
    - **If you want to use an API key instead**
      - Put `ANTHROPIC_API_KEY` in `~/.aikaclaw/.env` on the **gateway host**.
      - Clear any pinned order that forces a missing profile:

        ```bash
        aikaclaw models auth order clear --provider anthropic
        ```
  ...
  <Accordion title="Why did it also try Google Gemini and fail?">
    If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), AikaClaw will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
    ...
    **LLM request rejected: thinking signature required (Google Antigravity)**
    ...
    Fix: AikaClaw now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
<AccordionGroup>
  ...
  <Accordion title="What are typical profile IDs?">
    AikaClaw uses provider-prefixed IDs like:
  ...
  <Accordion title="Can I control which auth profile is tried first?">
    ...
    AikaClaw may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `aikaclaw models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.

    You can also set a **per-agent** order override (stored in that agent's `auth-profiles.json`) via the CLI:

    ```bash
    # Defaults to the configured default agent (omit --agent)
    aikaclaw models auth order get --provider anthropic

    # Lock rotation to a single profile (only try this one)
    aikaclaw models auth order set --provider anthropic anthropic:default

    # Or set an explicit order (fallback within provider)
    aikaclaw models auth order set --provider anthropic anthropic:work anthropic:default
    ...
    ```bash
    aikaclaw models auth order set --provider anthropic --agent main anthropic:default
    ```

  </Accordion>

  <Accordion title="OAuth vs API key - what is the difference?">
    AikaClaw supports both:
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```
    --port > AIKACLAW_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does aikaclaw gateway status say "Runtime: running" but "RPC probe: failed"?'>
    Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

    Use `aikaclaw gateway status` and trust these lines:
  ...
  <Accordion title='Why does aikaclaw gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `AIKACLAW_STATE_DIR` mismatch).

    Fix:

    ```bash
    aikaclaw gateway install --force
    ```
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    AikaClaw enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `aikaclaw gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run AikaClaw in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `aikaclaw gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `aikaclaw dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `aikaclaw doctor --generate-gateway-token`.
    - If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
    - Set `gateway.auth.token` (or `AIKACLAW_GATEWAY_TOKEN`) on the gateway host.
    - In the Control UI settings, paste the same token.
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `aikaclaw devices list`
      - `aikaclaw devices rotate --device <id> --role operator`
    - Still stuck? Run `aikaclaw status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - `AIKACLAW_CONFIG_PATH` (per-instance config)
    - `AIKACLAW_STATE_DIR` (per-instance state)
    - `agents.defaults.workspace` (workspace isolation)
    - `gateway.port` (unique ports)

    Quick setup (recommended):

    - Use `aikaclaw --profile <name> ...` per instance (auto-creates `~/.aikaclaw-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `aikaclaw --profile <name> gateway install`.

    Profiles also suffix service names (`ai.aikaclaw.<profile>`; legacy `com.aikaclaw.*`, `aikaclaw-gateway-<profile>.service`, `AikaClaw Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    aikaclaw tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    ```
    /tmp/aikaclaw/aikaclaw-YYYY-MM-DD.log
    ```
    ...
    - macOS: `$AIKACLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.aikaclaw/logs/...`; profiles use `~/.aikaclaw-<profile>/logs/...`)
    - Linux: `journalctl --user -u aikaclaw-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "AikaClaw Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `aikaclaw gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart AikaClaw?">
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `aikaclaw gateway status`
    2. Is the Gateway healthy? `aikaclaw status`
    3. Does the UI have the right token? `aikaclaw dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    aikaclaw channels status
    aikaclaw channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. AikaClaw already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: aikaclaw gateway restart vs aikaclaw gateway">
    - `aikaclaw gateway restart`: restarts the **background service** (launchd/systemd).
    - `aikaclaw gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `aikaclaw gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [AikaClaw assistant setup](/start/aikaclaw) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    aikaclaw message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose AikaClaw to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `aikaclaw pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `aikaclaw pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `aikaclaw doctor` to surface risky DM policies.
  ...
  <Accordion title="I ran /start in Telegram but did not get a pairing code">
    ...
    ```bash
    aikaclaw pairing list telegram
    ```
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. AikaClaw only replies to chats it receives or to explicit sends you trigger.

    Approve pairing with:

    ```bash
    aikaclaw pairing approve whatsapp <code>
    ```

    List pending requests:

    ```bash
    aikaclaw pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    AikaClaw blocks **cross-provider** messaging by default. If a tool call is bound
