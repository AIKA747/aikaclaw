...
This guide summarizes a sane workflow for working on the pi integration in AikaClaw.
...
```bash
AIKACLAW_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm aikaclaw agent --message "Hello" --thinking low`
...
State lives under the AikaClaw state directory. Default is `~/.aikaclaw`. If `AIKACLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `aikaclaw.json` for config
