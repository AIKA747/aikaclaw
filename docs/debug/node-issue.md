...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in AikaClaw
...
Running AikaClaw via Node with `tsx` fails at startup with:

```
[aikaclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `aikaclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch aikaclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node aikaclaw.mjs status` works on Node 25.
