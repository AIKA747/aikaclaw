...
```bash
aikaclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `aikaclaw tui --deliver`
...
- Local shell commands receive `AIKACLAW_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `AIKACLAW_THEME=light` before launching `aikaclaw tui`.
- To force the original dark palette instead, set `AIKACLAW_THEME=dark`.
...
- Check the Gateway logs: `aikaclaw logs --follow`.
- Confirm the agent can run: `aikaclaw status` and `aikaclaw models status`.
...
- No agents in picker: check `aikaclaw agents list` and your routing config.
