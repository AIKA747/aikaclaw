...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/aikaclaw`)
...
If the page fails to load, start the Gateway first: `aikaclaw gateway`.
...
created. Re-run `aikaclaw devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `aikaclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.aikaclaw/aikaclaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop aikaclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. AikaClaw
...
AikaClaw **blocks** Control UI connections without device identity.
