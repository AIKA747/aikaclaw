---
summary: "Synology Chat webhook setup and AikaClaw config"
read_when:
  - Setting up Synology Chat with AikaClaw
...
```bash
aikaclaw plugins install ./extensions/synology-chat
```
...
1. Install and enable the Synology Chat plugin.
   - `aikaclaw onboard` now shows Synology Chat in the same channel setup list as `aikaclaw channels add`.
   - Non-interactive setup: `aikaclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your AikaClaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in AikaClaw.
   - Guided: `aikaclaw onboard`
   - Direct: `aikaclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `aikaclaw pairing list synology-chat`
  - `aikaclaw pairing approve synology-chat <CODE>`
...
```bash
aikaclaw message send --channel synology-chat --target 123456 --text "Hello from AikaClaw"
aikaclaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. AikaClaw now rejects duplicate exact paths
