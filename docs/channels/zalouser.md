...
read_when:
  - Setting up Zalo Personal for AikaClaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside AikaClaw.
...
- Install via CLI: `aikaclaw plugins install @aikaclaw/zalouser`
- Or from a source checkout: `aikaclaw plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `aikaclaw channels login --channel zalouser`
...
```bash
aikaclaw directory self --channel zalouser
aikaclaw directory peers list --channel zalouser --query "name"
aikaclaw directory groups list --channel zalouser --query "work"
```
...
- `aikaclaw pairing list zalouser`
- `aikaclaw pairing approve zalouser <code>`
...
- On startup, AikaClaw resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, AikaClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in AikaClaw state. Example:
...
- AikaClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, AikaClaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `aikaclaw channels logout --channel zalouser && aikaclaw channels login --channel zalouser`
...
- The channel now runs fully in AikaClaw without external CLI binaries.
