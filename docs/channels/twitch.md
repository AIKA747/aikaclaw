...
read_when:
  - Setting up Twitch chat integration for AikaClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. AikaClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
aikaclaw plugins install @aikaclaw/twitch
```

Local checkout (when running from a git repo):

```bash
aikaclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `AIKACLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "aikaclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use AIKACLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
AIKACLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
