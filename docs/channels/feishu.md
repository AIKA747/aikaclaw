...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects AikaClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current AikaClaw releases, so no separate plugin install
is required.
...
```bash
aikaclaw plugins install @aikaclaw/feishu
```
...
If you just installed AikaClaw, run onboarding:
...
2. Configuring app credentials in AikaClaw
...
1. You already ran `aikaclaw channels add` for Feishu
2. The gateway is running (`aikaclaw gateway status`)
...
```bash
aikaclaw pairing approve feishu <CODE>
```
...
2. Run `aikaclaw logs --follow` and look for `chat_id`
...
2. Run `aikaclaw logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `aikaclaw gateway status`  | Show gateway status           |
| `aikaclaw gateway install` | Install/start gateway service |
| `aikaclaw gateway stop`    | Stop gateway service          |
| `aikaclaw gateway restart` | Restart gateway service       |
| `aikaclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `aikaclaw logs --follow`
...
5. Ensure the gateway is running: `aikaclaw gateway status`
6. Check logs: `aikaclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.aikaclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.aikaclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
