---
summary: "Mattermost bot setup and AikaClaw config"
read_when:
...
```bash
aikaclaw plugins install @aikaclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
aikaclaw plugins install ./extensions/mattermost
```
...
4. Configure AikaClaw and start the gateway.
...
Native slash commands are opt-in. When enabled, AikaClaw registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, AikaClaw derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as AikaClaw.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to AikaClaw.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from AikaClaw, not `404`.
...
- Approve via:
  - `aikaclaw pairing list mattermost`
  - `aikaclaw pairing approve mattermost <CODE>`
...
Use these target formats with `aikaclaw message send` or cron/webhooks:
...
AikaClaw resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), AikaClaw sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When AikaClaw sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, AikaClaw derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and AikaClaw run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="aikaclaw-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"aikaclaw-mattermost-interactions",
...
`aikaclaw message send` and cron/webhook deliveries.
