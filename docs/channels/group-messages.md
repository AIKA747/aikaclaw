...
Add a `groupChat` block to `~/.aikaclaw/aikaclaw.json` so display-name pings work even when WhatsApp strips the visual `@` in the text body:
    ...
    list: [
      {
        ...
        groupChat: {
          historyLimit: 50,
          mentionPatterns: ["@?aikaclaw", "\\+?15555550123"],
        },
      },
    ],
  },
...
1. Add your WhatsApp account (the one running AikaClaw) to the group.
2. Say `@aikaclaw …` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
...
- Manual smoke:
  - Send an `@aikaclaw` ping in the group and confirm a reply that references the sender name.
...
- Session store entries will appear as `agent:<agentId>:whatsapp:group:<jid>` in the session store (`~/.aikaclaw/agents/<agentId>/sessions/sessions.json` by default); a missing entry just means the group hasn’t triggered a run yet.
