...
```bash
aikaclaw plugins install @aikaclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
aikaclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "AikaClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `aikaclaw pairing list nextcloud-talk`
  - `aikaclaw pairing approve nextcloud-talk <CODE>`
