...
Matrix is the Matrix channel plugin for AikaClaw.
It uses the official `matrix-js-sdk` and supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Plugin required

Matrix is a plugin and is not bundled with core AikaClaw.

Install from npm:

```bash
aikaclaw plugins install @aikaclaw/matrix
```

Install from a local checkout:

```bash
aikaclaw plugins install ./extensions/matrix
```
...
```bash
aikaclaw channels add
aikaclaw configure --section channels
```
...
- To resolve room names before saving them, use `aikaclaw channels resolve --channel matrix "Project Room"`.
...
```json5
{
  channels: {
    matrix: {
      ...
      deviceName: "AikaClaw Gateway",
    },
  },
}
```

Matrix stores cached credentials in `~/.aikaclaw/credentials/matrix/`.
...
By default, Matrix messages from other configured AikaClaw Matrix accounts are ignored.
...
- AikaClaw still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix does not expose a native bot flag here; AikaClaw treats "bot-authored" as "sent by another configured Matrix account on this AikaClaw gateway".
...
```bash
aikaclaw matrix verify status --include-recovery-key --json
```
...
```bash
aikaclaw matrix verify bootstrap --verbose
```

Force a fresh cross-signing identity reset before bootstrapping:

```bash
aikaclaw matrix verify bootstrap --force-reset-cross-signing
```
...
```bash
aikaclaw matrix verify device "<your-recovery-key>" --verbose
```
...
```bash
aikaclaw matrix verify backup restore
```

Verbose restore diagnostics:

```bash
aikaclaw matrix verify backup restore --verbose
```
...
```bash
aikaclaw matrix verify status --account assistant
aikaclaw matrix verify backup restore --account assistant
aikaclaw matrix devices list --account assistant
```
...
AikaClaw treats this Matrix device as verified only when it is verified by your own cross-signing identity.
In practice, `aikaclaw matrix verify status --verbose` exposes three trust signals:
...
Local trust by itself is not enough for AikaClaw to treat the device as fully verified.

### What bootstrap does

`aikaclaw matrix verify bootstrap` is the repair and setup command for encrypted Matrix accounts.
...
If the homeserver requires interactive auth to upload cross-signing keys, AikaClaw tries the upload without auth first, then with `m.login.dummy`, then with `m.login.password` when `channels.matrix.password` is configured.
...
If you intentionally want to discard the current room-key backup and start a new backup baseline for future messages, use `aikaclaw matrix verify backup reset --yes`.
...
If startup finds broken bootstrap state and `channels.matrix.password` is configured, AikaClaw can attempt a stricter repair path.
If the current device is already owner-signed, AikaClaw preserves that identity instead of resetting it automatically.

Upgrading from the previous public Matrix plugin:

- AikaClaw automatically reuses the same Matrix account, access token, and device identity when possible.
- Before any actionable Matrix migration changes run, AikaClaw creates or reuses a recovery snapshot under `~/Backups/aikaclaw-migrations/`.
- If you use multiple Matrix accounts, set `channels.matrix.defaultAccount` before upgrading from the old flat-store layout so AikaClaw knows which account should receive that shared legacy state.
- If the previous plugin stored a Matrix room-key backup decryption key locally, startup or `aikaclaw doctor --fix` will import it into the new recovery-key flow automatically.
...
- If the Matrix access token changes later for the same account, homeserver, and user, AikaClaw now prefers reusing the most complete existing token-hash storage root instead of starting from an empty Matrix state directory.
- On the next gateway start, backed-up room keys are restored automatically into the new crypto store.
- If the old plugin had local-only room keys that were never backed up, AikaClaw will warn clearly. Those keys cannot be exported automatically from the previous rust crypto store, so some old encrypted history may remain unavailable until recovered manually.
...
`~/.aikaclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/`.
...
When the token changes but the account identity stays the same, AikaClaw reuses the best existing
...
AikaClaw currently provides that in Node by:
...
Under AikaClaw's security model, the gateway host and local AikaClaw state directory are already inside the trusted operator boundary, so this is primarily an operational durability concern rather than a separate remote trust boundary.

Planned improvement:

- add SecretRef support for persistent Matrix key material so recovery keys and related store-encryption secrets can be sourced from AikaClaw secrets providers instead of only local files
...
Incoming verification requests from another Matrix client are tracked and auto-accepted by AikaClaw.
For self-verification flows, AikaClaw also starts the SAS flow automatically when emoji verification becomes available and confirms its own side.
For verification requests from another Matrix user/device, AikaClaw auto-accepts the request and then waits for the SAS flow to proceed normally.
...
AikaClaw does not auto-accept self-initiated duplicate flows blindly. Startup skips creating a new request when a self-verification request is already pending.

Verification protocol/system notices are not forwarded to the agent chat pipeline, so they do not produce `NO_REPLY`.

### Device hygiene

Old AikaClaw-managed Matrix devices can accumulate on the account and make encrypted-room trust harder to reason about.
List them with:

```bash
aikaclaw matrix devices list
```

Remove stale AikaClaw-managed devices with:

```bash
aikaclaw matrix devices prune-stale
```

### Direct Room Repair

If direct-message state gets out of sync, AikaClaw can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer with:

```bash
aikaclaw matrix direct inspect --user-id @alice:example.org
```

Repair it with:

```bash
aikaclaw matrix direct repair --user-id @alice:example.org
```
...
Ack reactions use the standard AikaClaw resolution order:
...
```bash
aikaclaw pairing list matrix
aikaclaw pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, AikaClaw reuses the same pending pairing code and may send a reminder reply again after a short cooldown instead of minting a new code.
...
Set `defaultAccount` when you want AikaClaw to prefer one named Matrix account for implicit routing, probing, and CLI operations.
If you configure multiple named accounts, set `defaultAccount` or pass `--account <id>` for CLI commands that rely on implicit account selection.
Pass `--account <id>` to `aikaclaw matrix verify ...` and `aikaclaw matrix devices ...` when you want to override that implicit selection for one command.

## Private/LAN homeservers

By default, AikaClaw blocks private/internal Matrix homeservers for SSRF protection unless you
...
Matrix accepts these target forms anywhere AikaClaw asks you for a room or user target:
...
- `autoJoinAllowlist`: rooms/aliases allowed when `autoJoin` is `allowlist`. Alias entries are resolved to room IDs during invite handling; AikaClaw does not trust alias state claimed by the invited room.
