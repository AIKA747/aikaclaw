...
Tlon is a decentralized messenger built on Urbit. AikaClaw connects to your Urbit ship and can
...
```bash
aikaclaw plugins install @aikaclaw/tlon
```

Local checkout (when running from a git repo):

```bash
aikaclaw plugins install ./extensions/tlon
```
...
By default, AikaClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `aikaclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, AikaClaw replies in-thread.
