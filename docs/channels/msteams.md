...
```bash
aikaclaw plugins install @aikaclaw/msteams
```

Local checkout (when running from a git repo):

```bash
aikaclaw plugins install ./extensions/msteams
```

If you choose Teams during setup and a git checkout is detected,
AikaClaw will offer the local install path automatically.
...
3. Configure AikaClaw with those credentials.
...
- Talk to AikaClaw via Teams DMs, group chats, or channels.
...
- On startup, AikaClaw resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.aikaclaw/aikaclaw.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring AikaClaw, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `aikaclaw-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `aikaclaw plugins install @aikaclaw/msteams`
   - From a local checkout: `aikaclaw plugins install ./extensions/msteams`
...
```json5
{
  ...
  description: { short: "AikaClaw in Teams", full: "AikaClaw in Teams" },
...
AikaClaw handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, AikaClaw only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/AikaClawShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

AikaClaw sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `aikaclaw message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.aikaclaw/msteams-polls.json`.
...
```bash
aikaclaw message send --channel msteams \
...
aikaclaw message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
aikaclaw message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
aikaclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
aikaclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
