...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure AikaClaw HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `aikaclaw pairing approve slack <code>`.
...
- The interactive callback values are AikaClaw-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, AikaClaw falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while AikaClaw is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "AikaClaw",
    "description": "Slack connector for AikaClaw"
  },
  "features": {
    "bot_user": {
      "display_name": "AikaClaw",
      "always_online": false
    },
    "app_home": {
...
AikaClaw supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, AikaClaw falls back to normal delivery for remaining payloads.
