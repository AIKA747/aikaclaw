...
AikaClaw routes replies **back to the channel where a message came from**. The
...
AikaClaw infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, AikaClaw still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when AikaClaw would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.aikaclaw/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.aikaclaw`):

- `~/.aikaclaw/agents/<agentId>/sessions/sessions.json`
