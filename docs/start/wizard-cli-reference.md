...
read_when:
  - You need detailed behavior for aikaclaw onboard
...
This page is the full reference for `aikaclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.aikaclaw/aikaclaw.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `aikaclaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `aikaclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `aikaclaw health`.
    - `aikaclaw status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.aikaclaw/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.aikaclaw/agents/<agentId>/agent/auth-profiles.json`
...
`~/.aikaclaw/credentials/oauth.json` (or `$AIKACLAW_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`aikaclaw onboard`](/cli/onboard)
