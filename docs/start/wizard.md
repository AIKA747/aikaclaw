...
CLI onboarding is the **recommended** way to set up AikaClaw on macOS,
...
`aikaclaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`aikaclaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.aikaclaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `aikaclaw doctor` first.
</Note>
...
Use `aikaclaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.aikaclaw/workspace-<agentId>`.
...
- CLI command reference: [`aikaclaw onboard`](/cli/onboard)
