...
import type { AikaClawConfig } from "../../../extensions/discord/src/runtime-api.js";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: AikaClawConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): AikaClawConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("aikaclaw/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("aikaclaw/plugin-sdk/acp-runtime")>(
    "aikaclaw/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("aikaclaw/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("aikaclaw/plugin-sdk/command-auth")>(
...
vi.mock("aikaclaw/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("aikaclaw/plugin-sdk/reply-runtime")>(
    "aikaclaw/plugin-sdk/reply-runtime",
  );
  return {
    ...actual,
...
vi.mock("aikaclaw/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("aikaclaw/plugin-sdk/config-runtime")>(
...
vi.mock("aikaclaw/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("aikaclaw/plugin-sdk/runtime-env")>(
    "aikaclaw/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("aikaclaw/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("aikaclaw/plugin-sdk/infra-runtime")>(
