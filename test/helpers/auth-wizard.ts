...
export function requireAikaClawAgentDir(): string {
  const agentDir = process.env.AIKACLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("AIKACLAW_AGENT_DIR not set");
  }
  return agentDir;
}
