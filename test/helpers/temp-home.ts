...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked AIKACLAW_HOME.
  delete process.env.AIKACLAW_HOME;
  process.env.AIKACLAW_STATE_DIR = path.join(base, ".aikaclaw");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "aikaclaw-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".aikaclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
