...
describe("collectBundledExtensionManifestErrors", () => {
  it("flags invalid bundled extension install metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | aikaclaw.install.npmSpec must be a non-empty string",
    ]);
  });
      ...
      collectBundledExtensionManifestErrors([
        {
          ...
          packageJson: {
            aikaclaw: {
              install: { npmSpec: "@aikaclaw/broken", minHostVersion: "2026.3.14" },
            },
          },
        },
      ]),
    ).toEqual([
      "bundled extension 'broken' manifest invalid | aikaclaw.install.minHostVersion must use a semver floor in the form \">=x.y.z\"",
    ]);
  });
  ...
  it("flags non-object install metadata instead of throwing", () => {
    ...
    ).toEqual(["bundled extension 'broken' manifest invalid | aikaclaw.install must be an object"]);
  });
});
  ...
  it("allows bundled plugin runtime deps under dist/extensions but still blocks other node_modules", () => {
    expect(
      collectForbiddenPackPaths([
        ...
        "node_modules/.bin/aikaclaw",
      ]),
    ).toEqual(["extensions/tlon/node_modules/.bin/tlon", "node_modules/.bin/aikaclaw"]);
  });
});
  ...
  it("requires the shipped channel catalog, control ui, and optional bundled metadata", () => {
    ...
    expect(missing).toEqual(
      expect.arrayContaining([
        ...
        "dist/extensions/matrix/aikaclaw.plugin.json",
        "dist/extensions/matrix/package.json",
        "dist/extensions/whatsapp/light-runtime-api.js",
        "dist/extensions/whatsapp/runtime-api.js",
        "dist/extensions/whatsapp/aikaclaw.plugin.json",
...
describe("collectPackUnpackedSizeErrors", () => {
  it("accepts pack results within the unpacked size budget", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("aikaclaw-2026.3.14.tgz", 120_354_302)]),
    ).toEqual([]);
  });

  it("flags oversized pack results that risk low-memory startup failures", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("aikaclaw-2026.3.12.tgz", 224_002_564)]),
    ).toEqual([
      "aikaclaw-2026.3.12.tgz unpackedSize 224002564 bytes (213.6 MiB) exceeds budget 199229440 bytes (190.0 MiB). Investigate duplicate channel shims, copied extension trees, or other accidental pack bloat before release.",
    ]);
  });
  ...
  it("fails closed when npm pack output omits unpackedSize for every result", () => {
    expect(
      collectPackUnpackedSizeErrors([
        { filename: "aikaclaw-2026.3.14.tgz" },
        { filename: "aikaclaw-extra.tgz", unpackedSize: Number.NaN },
      ]),
    ).toEqual([
