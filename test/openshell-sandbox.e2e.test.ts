...
const AIKACLAW_OPENSHELL_E2E = process.env.AIKACLAW_E2E_OPENSHELL === "1";
const AIKACLAW_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const AIKACLAW_OPENSHELL_COMMAND =
  process.env.AIKACLAW_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "aikaclaw-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && AIKACLAW_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: AIKACLAW_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(AIKACLAW_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-openshell-e2e-"));
      ...
      const gatewayName = `aikaclaw-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `aikaclaw-policy-allow-${scopeSuffix}`;
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: AIKACLAW_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("aikaclaw-openshell-e2e");
