...
describe("resolveVitestIsolation", () => {
  ...
  it("restores isolate mode when explicitly requested", () => {
    expect(resolveVitestIsolation({ AIKACLAW_TEST_ISOLATE: "1" })).toBe(true);
    expect(resolveVitestIsolation({ AIKACLAW_TEST_NO_ISOLATE: "0" })).toBe(true);
    expect(resolveVitestIsolation({ AIKACLAW_TEST_NO_ISOLATE: "false" })).toBe(true);
  });
});
...
describe("scoped vitest configs", () => {
  ...
  it("loads channel include overrides from AIKACLAW_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-vitest-channels-"));
    try {
      ...
      const config = createChannelsVitestConfig({
        AIKACLAW_VITEST_INCLUDE_FILE: includeFile,
      });
