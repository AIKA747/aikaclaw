  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.AIKACLAW_HOME;
        delete env.AIKACLAW_STATE_DIR;
        delete env.AIKACLAW_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "aikaclaw.mjs");
