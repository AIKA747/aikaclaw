...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("aikaclaw-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@aikaclaw/whatsapp",
      version: "2026.3.23",
      description: "AikaClaw WhatsApp channel plugin",
      aikaclaw: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("aikaclaw-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@aikaclaw/whatsapp",
      aikaclaw: {
        channel: {
