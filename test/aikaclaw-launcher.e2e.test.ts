...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-launcher-"));
  fixtureRoots.push(fixtureRoot);
  await fs.copyFile(
    path.resolve(process.cwd(), "aikaclaw.mjs"),
    path.join(fixtureRoot, "aikaclaw.mjs"),
  );
...
describe("aikaclaw launcher", () => {
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      ...
      'import "missing-aikaclaw-launcher-dep";\nexport {};\n',
      "utf8",
    );

    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "aikaclaw.mjs"), "--help"], {
      cwd: fixtureRoot,
      encoding: "utf8",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("missing-aikaclaw-launcher-dep");
  ...
  it("explains how to recover from an unbuilt source install", async () => {
    ...
    expect(result.stderr).toContain("github:aikaclaw/aikaclaw#<ref>");
  });
});
