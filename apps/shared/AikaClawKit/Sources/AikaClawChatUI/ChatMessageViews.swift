...
struct ChatMessageBubble: View {
    let message: AikaClawChatMessage
    let style: AikaClawChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? AikaClawChatTheme.userText : AikaClawChatTheme.assistantText
    ...
    private var inlineAttachments: [AikaClawChatMessageContent] {
    ...
    private var toolCalls: [AikaClawChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [AikaClawChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? AikaClawChatTheme.userBubble
        }
        if self.style == .onboarding {
            return AikaClawChatTheme.onboardingAssistantBubble
        }
        return AikaClawChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return AikaClawChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: AikaClawChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? AikaClawChatTheme.userText : AikaClawChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: AikaClawChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(AikaClawChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [AikaClawChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: AikaClawChatTheme.assistantText)
            }
        }
    }
}
