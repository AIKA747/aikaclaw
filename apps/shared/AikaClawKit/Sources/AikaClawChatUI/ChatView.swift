...
@MainActor
public struct AikaClawChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: AikaClawChatViewModel
    ...
    public init(
        viewModel: AikaClawChatViewModel,
    ...
    private var visibleMessages: [AikaClawChatMessage] {
        let base: [AikaClawChatMessage]
    ...
    private func mergeToolResults(in messages: [AikaClawChatMessage]) -> [AikaClawChatMessage] {
        ...
        for message in messages {
            ...
            let merged = AikaClawChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: AikaClawChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: AikaClawChatMessage) -> Bool {
    ...
    private func primaryText(in message: AikaClawChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: AikaClawChatMessage) -> Bool {
    ...
    private func toolCalls(in message: AikaClawChatMessage) -> [AikaClawChatMessageContent] {
    ...
    private func inlineToolResults(in message: AikaClawChatMessage) -> [AikaClawChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: AikaClawChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: AikaClawChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(AikaClawChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
