...
internal fun aikaclawAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this AikaClaw Android gateway connection failure.

    Please:
    - pick one route only: same machine, same LAN, Tailscale, or public URL
    - classify this as pairing/auth, TLS trust, wrong advertised route, wrong address/port, or gateway down
    - quote the exact app status/error below
    - tell me whether `aikaclaw devices list` should show a pending pairing request
    - if more signal is needed, ask for `aikaclaw qr --json`, `aikaclaw devices list`, and `aikaclaw nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${aikaclawAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("AikaClaw gateway diagnostics", report))
