...
final class MotionService: MotionServicing {
    func activities(params: AikaClawMotionActivityParams) async throws -> AikaClawMotionActivityPayload {
        ...
        let mapped: [AikaClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return AikaClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: AikaClawPedometerParams) async throws -> AikaClawPedometerPayload {
        ...
        let payload: AikaClawPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = AikaClawPedometerPayload(
