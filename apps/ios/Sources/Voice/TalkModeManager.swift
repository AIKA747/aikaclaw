...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__AIKACLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<AikaClawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.aikaclaw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = AikaClawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> AikaClawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return AikaClawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> AikaClawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> AikaClawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return AikaClawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> AikaClawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = AikaClawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: AikaClawTalkPTTStopPayload) {
