...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [AikaClawCapability.canvas.rawValue, AikaClawCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(AikaClawCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(AikaClawCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = AikaClawLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(AikaClawCapability.location.rawValue) }

        caps.append(AikaClawCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(AikaClawCapability.watch.rawValue)
        }
        caps.append(AikaClawCapability.photos.rawValue)
        caps.append(AikaClawCapability.contacts.rawValue)
        caps.append(AikaClawCapability.calendar.rawValue)
        caps.append(AikaClawCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(AikaClawCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            AikaClawScreenCommand.record.rawValue,
            AikaClawSystemCommand.notify.rawValue,
            AikaClawChatCommand.push.rawValue,
        ...
        if caps.contains(AikaClawCapability.camera.rawValue) {
            commands.append(AikaClawCameraCommand.list.rawValue)
            commands.append(AikaClawCameraCommand.snap.rawValue)
            commands.append(AikaClawCameraCommand.clip.rawValue)
        }
        if caps.contains(AikaClawCapability.location.rawValue) {
            commands.append(AikaClawLocationCommand.get.rawValue)
        }
        if caps.contains(AikaClawCapability.device.rawValue) {
            commands.append(AikaClawDeviceCommand.status.rawValue)
            commands.append(AikaClawDeviceCommand.info.rawValue)
        }
        if caps.contains(AikaClawCapability.watch.rawValue) {
            commands.append(AikaClawWatchCommand.status.rawValue)
            commands.append(AikaClawWatchCommand.notify.rawValue)
        }
        if caps.contains(AikaClawCapability.photos.rawValue) {
            commands.append(AikaClawPhotosCommand.latest.rawValue)
        }
        if caps.contains(AikaClawCapability.contacts.rawValue) {
            commands.append(AikaClawContactsCommand.search.rawValue)
            commands.append(AikaClawContactsCommand.add.rawValue)
        }
        if caps.contains(AikaClawCapability.calendar.rawValue) {
            commands.append(AikaClawCalendarCommand.events.rawValue)
            commands.append(AikaClawCalendarCommand.add.rawValue)
        }
        if caps.contains(AikaClawCapability.reminders.rawValue) {
            commands.append(AikaClawRemindersCommand.list.rawValue)
            commands.append(AikaClawRemindersCommand.add.rawValue)
        }
        if caps.contains(AikaClawCapability.motion.rawValue) {
            commands.append(AikaClawMotionCommand.activity.rawValue)
            commands.append(AikaClawMotionCommand.pedometer.rawValue)
        }
