...
struct OnboardingWizardView: View {
    ...
    @State private var statusLine: String = "In your AikaClaw chat, run /pair qr, then scan the code here."
    ...
    private var introStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Turn this iPhone into a secure AikaClaw node for chat, voice, camera, and device tools.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                Label("Use AikaClaw from your phone", systemImage: "message.fill")
            }
            ...
            HStack(alignment: .top, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Security notice")
                        .font(.headline)
                    Text(
                        "The connected AikaClaw agent can use device capabilities you enable, such as camera, microphone, photos, contacts, calendar, and location. Continue only if you trust the gateway and agent you connect to.")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
    ...
    private var welcomeStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Scan a QR code from your AikaClaw gateway or continue with manual setup.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .padding(.horizontal, 32)
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text("How to pair")
                    .font(.headline)
                Text("In your AikaClaw chat, run")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
                Text("/pair qr")
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `aikaclaw devices list`."
                    }()
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `aikaclaw devices approve` (or `aikaclaw devices approve <requestId>`)\n"
                            + "2) `/pair approve` in your AikaClaw chat\n"
                            + "\(requestLine)\n"
                            + "AikaClaw will also retry automatically when you return to this app.")
                }
            }
    ...
    private func advanceFromIntro() {
        ...
        self.statusLine = "In your AikaClaw chat, run /pair qr, then scan the code here."
        self.step = .welcome
    }
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            if let last = GatewaySettingsStore.loadLastGatewayConnection() {
                ...
                case .discovered:
                    self.manualHost = "aikaclaw.local"
        ...
        if self.selectedMode == .developerLocal && self.manualHost == "aikaclaw.local" {
        ...
        if !hasSavedGateway, !hasToken, !hasPassword {
            self.statusLine = "No saved pairing found. In your AikaClaw chat, run /pair qr, then scan the code here."
        }
    }
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "aikaclaw.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "aikaclaw.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "aikaclaw.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
