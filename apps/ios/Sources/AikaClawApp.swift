...
@MainActor
final class AikaClawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.aikaclaw.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.aikaclaw.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.aikaclaw.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "aikaclaw.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "aikaclaw.watch.promptId"
    static let sessionKeyKey = "aikaclaw.watch.sessionKey"
    static let actionPrimaryIDKey = "aikaclaw.watch.action.primary.id"
    static let actionPrimaryLabelKey = "aikaclaw.watch.action.primary.label"
    static let actionSecondaryIDKey = "aikaclaw.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "aikaclaw.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "aikaclaw.watch.action.primary"
    static let actionSecondaryIdentifier = "aikaclaw.watch.action.secondary"
    static let actionIdentifierPrefix = "aikaclaw.watch.action."
    static let actionIDKeyPrefix = "aikaclaw.watch.action.id."
    static let actionLabelKeyPrefix = "aikaclaw.watch.action.label."
    static let categoryPrefix = "aikaclaw.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: AikaClawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> AikaClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AikaClawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "AikaClaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [AikaClawWatchAction]) -> [UNNotificationAction] {
...
@main
struct AikaClawApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(AikaClawAppDelegate.self) private var appDelegate
...
extension AikaClawApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("AikaClaw: installing uncaught exception handler")
