...
final class RemindersService: RemindersServicing {
    func list(params: AikaClawRemindersListParams) async throws -> AikaClawRemindersListPayload {
        ...
        let payload: [AikaClawReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return AikaClawReminderPayload(
        ...
        return AikaClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: AikaClawRemindersAddParams) async throws -> AikaClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = AikaClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return AikaClawRemindersAddPayload(reminder: payload)
    }
