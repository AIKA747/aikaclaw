...
final class ContactsService: ContactsServicing {
    ...
    func search(params: AikaClawContactsSearchParams) async throws -> AikaClawContactsSearchPayload {
        ...
        return AikaClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: AikaClawContactsAddParams) async throws -> AikaClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return AikaClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return AikaClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> AikaClawContactPayload {
        AikaClawContactPayload(
