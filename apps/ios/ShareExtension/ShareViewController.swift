...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.aikaclaw.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to AikaClaw", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to AikaClaw gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to AikaClaw.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "AikaClawShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "AikaClaw is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "AikaClaw Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("aikaclaw-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
