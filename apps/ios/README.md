# AikaClaw iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an AikaClaw Gateway as a `role: node`.
...
- Beta release uses canonical `ai.aikaclaw.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `AikaClawPushTransport=relay`, `AikaClawPushDistribution=official`, and `AikaClawPushAPNsEnvironment=production`.
...
- `AIKACLAW_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
- `apps/ios/Sources/AikaClaw.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `AikaClawPushTransport=direct` and `AikaClawPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- Debug builds default to `AikaClawPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `aikaclaw.json`. `AIKACLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine AikaClaw official builds and to ensure a
