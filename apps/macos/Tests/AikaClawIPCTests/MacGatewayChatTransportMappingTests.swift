...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: AikaClawProtocol.AnyCodable(["ok": AikaClawProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: AikaClawProtocol.AnyCodable(["ok": AikaClawProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = AikaClawProtocol.AnyCodable([
            "runId": AikaClawProtocol.AnyCodable("run-1"),
            "sessionKey": AikaClawProtocol.AnyCodable("main"),
            "state": AikaClawProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: AikaClawProtocol.AnyCodable(["a": AikaClawProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
