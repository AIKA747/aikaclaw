...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: AikaClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: AikaClawProtocol.AnyCodable] = [
            "presence": .init([AikaClawProtocol.AnyCodable(entry)]),
        ]
        let payload = AikaClawProtocol.AnyCodable(payloadMap)
