...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    private func voiceWakeChangedEvent(payload: AikaClawProtocol.AnyCodable) -> EventFrame {
        EventFrame(
            type: "event",
    ...
    @Test func `applies voice wake changed event to app state`() async {
        ...
        let evt = self.voiceWakeChangedEvent(payload: AikaClawProtocol.AnyCodable(["triggers": [
            "aikaclaw",
            "computer",
        ]]))
        ...
        #expect(updated == ["aikaclaw", "computer"])
    ...
    @Test func `ignores voice wake changed event with invalid payload`() async {
        let previous = await applyTriggersAndCapturePrevious(["before"])
        let evt = self.voiceWakeChangedEvent(payload: AikaClawProtocol.AnyCodable(["unexpected": 123]))
