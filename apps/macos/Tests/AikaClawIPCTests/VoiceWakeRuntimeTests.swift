...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["aikaclaw"]
        let text = "hey aikaclaw"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey aikaclaw do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["aikaclaw"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "aikaclaw", start: 0.2, duration: 0.1, range: transcript.range(of: "aikaclaw")),
