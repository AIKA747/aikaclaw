...
struct WebChatSwiftUISmokeTests {
    private struct TestTransport: AikaClawChatTransport {
        func requestHistory(sessionKey: String) async throws -> AikaClawChatHistoryPayload {
            let json = """
            {"sessionKey":"\(sessionKey)","sessionId":null,"messages":[],"thinkingLevel":"off"}
            """
            return try JSONDecoder().decode(AikaClawChatHistoryPayload.self, from: Data(json.utf8))
        }
            ...
            attachments _: [AikaClawChatAttachmentPayload]) async throws -> AikaClawChatSendResponse
        {
            ...
            """
            return try JSONDecoder().decode(AikaClawChatSendResponse.self, from: Data(json.utf8))
        }
