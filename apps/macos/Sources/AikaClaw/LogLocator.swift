...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["AIKACLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/aikaclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("aikaclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("aikaclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/aikaclaw/ (rolling or stdout), or nil if none exist.
