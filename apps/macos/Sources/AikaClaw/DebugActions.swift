...
enum DebugActions {
    private static let verboseDefaultsKey = "aikaclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "aikaclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = AikaClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "AikaClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = AikaClawPaths.configURL
        guard
