...
enum CommandResolver {
    private static let projectRootDefaultsKey = "aikaclaw.gatewayProjectRootPath"
    private static let helperName = "aikaclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let aikaclawEntry = root.appendingPathComponent("aikaclaw.mjs").path
        if FileManager().isReadableFile(atPath: aikaclawEntry) { return aikaclawEntry }
        let binEntry = root.appendingPathComponent("bin/aikaclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/aikaclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let aikaclawPaths = self.aikaclawManagedPaths(home: home)
        if !aikaclawPaths.isEmpty {
            extras.insert(contentsOf: aikaclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + aikaclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func aikaclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".aikaclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func aikaclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectAikaClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("aikaclaw.mjs").path,
            root.appendingPathComponent("bin/aikaclaw.js").path,
        ]
    ...
    static func hasAnyAikaClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.aikaclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func aikaclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let aikaclawPath = self.projectAikaClawExecutable(projectRoot: root) {
            return [aikaclawPath, subcommand] + extraArgs
        }
        if let aikaclawPath = self.aikaclawExecutable(searchPaths: searchPaths) {
            return [aikaclawPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "aikaclaw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            aikaclaw entrypoint missing (looked for dist/index.js or aikaclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real aikaclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/aikaclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v aikaclaw >/dev/null 2>&1; then
          CLI="$(command -v aikaclaw)"
          aikaclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/aikaclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/aikaclaw.mjs"
            node "$PRJ/aikaclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/aikaclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/aikaclaw.js"
            node "$PRJ/bin/aikaclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent aikaclaw"
          pnpm --silent aikaclaw \(quotedArgs);
        else
          echo "aikaclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? AikaClawConfigFile.loadDict()
