...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AikaClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.aikaclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AikaClawKit.AnyCodable]()) {
                $0[$1.key] = AikaClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AikaClawProtocol.AnyCodable?) -> [String: AikaClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AikaClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AikaClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AikaClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("aikaclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("aikaclaw.control.agent")
}
