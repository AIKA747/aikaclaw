...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.aikaclaw.mac`) even if the shipped
    ...
    {
        if environment["AIKACLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "aikaclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "aikaclaw.nixMode") { return true }
