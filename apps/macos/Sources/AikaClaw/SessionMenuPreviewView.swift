...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.aikaclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AikaClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AikaClawSessionPreviewEntry,
    ...
    private static func cache(payload: AikaClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: AikaClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AikaClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AikaClawKit.AnyCodable]) -> [AikaClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(AikaClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: AikaClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AikaClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AikaClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AikaClawChatMessage) -> String? {
