...
enum AikaClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = AikaClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AikaClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        AikaClawLogging.bootstrapIfNeeded()
        let label = AikaClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AikaClawOSLogHandler: AppLogLevelBackedHandler {
...
struct AikaClawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = AikaClawLogging.parseLabel(self.label)
