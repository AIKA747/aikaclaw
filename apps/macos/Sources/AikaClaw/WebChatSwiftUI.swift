...
private let webChatSwiftLogger = Logger(subsystem: "ai.aikaclaw", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "aikaclaw.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: AikaClawChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> AikaClawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(AikaClawChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            AikaClawChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return AikaClawChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> AikaClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                AikaClawGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    AikaClawAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: AikaClawProtocol.ModelChoice) -> AikaClawChatModelChoice {
        AikaClawChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<AikaClawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any AikaClawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = AikaClawChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: AikaClawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<AikaClawChatView>) -> NSViewController
    {
