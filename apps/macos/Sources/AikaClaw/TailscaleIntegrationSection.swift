...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.aikaclaw/aikaclaw.json. Prefer AIKACLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.aikaclaw/aikaclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.aikaclaw/aikaclaw.json. Restart the gateway to apply."
        }
