const SETTINGS_KEY_PREFIX = "aikaclaw.control.settings.v1:";
const LEGACY_SETTINGS_KEY = "aikaclaw.control.settings.v1";
const LEGACY_TOKEN_SESSION_KEY = "aikaclaw.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "aikaclaw.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    typeof window.__AIKACLAW_CONTROL_UI_BASE_PATH__ === "string" &&
    window.__AIKACLAW_CONTROL_UI_BASE_PATH__.trim();
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("aikaclaw.control.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;
