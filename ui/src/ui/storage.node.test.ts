  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __AIKACLAW_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window.__AIKACLAW_CONTROL_UI_BASE_PATH__;
    return;
  }
  Object.defineProperty(window, "__AIKACLAW_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", async () => {
    ...
    setControlUiBasePath(" /aikaclaw/ ");

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/aikaclaw"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", async () => {
    setTestLocation({
      ...
      pathname: "/apps/aikaclaw/chat",
    });

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/aikaclaw"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", async () => {
    ...
    sessionStorage.setItem("aikaclaw.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "aikaclaw.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/aikaclaw",
    ...
    const scopedKey = "aikaclaw.control.settings.v1:wss://gateway.example:8443/aikaclaw";
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      ...
      borderRadius: 50,
      sessionsByGateway: {
        "wss://gateway.example:8443/aikaclaw": {
          sessionKey: "agent",
  ...
  it("does not persist gateway tokens when saving settings", async () => {
    ...
    const scopedKey = `aikaclaw.control.settings.v1:${gwUrl}`;
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      gatewayUrl: gwUrl,
      theme: "claw",
      themeMode: "system",
  ...
  it("caps persisted session scopes to the most recent gateways", async () => {
    ...
    const { saveSettings } = await import("./storage.ts");
    const gwUrl = expectedGatewayUrl("");
    const scopedKey = `aikaclaw.control.settings.v1:wss://gateway.example:8443`;
