...
import { AikaClawApp } from "./app.ts";
...
function resetChatStateForSessionSwitch(state: AppViewState, sessionKey: string) {
  ...
  (state as unknown as AikaClawApp).chatStreamStartedAt = null;
  state.chatRunId = null;
  (state as unknown as AikaClawApp).resetToolStream();
  (state as unknown as AikaClawApp).resetChatScroll();
  state.applySettings({
    ...state.settings,
    sessionKey,
  ...
  return html`
    <div class="chat-controls">
      <button
        ...
        @click=${async () => {
          const app = state as unknown as AikaClawApp;
