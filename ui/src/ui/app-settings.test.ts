...
describe("applySettingsFromUrl", () => {
  ...
  it("hydrates query token params and strips them from the URL", () => {
    ...
    host.settings.gatewayUrl = "wss://control.example/aikaclaw";
  ...
  it("keeps query token params pending when a gatewayUrl confirmation is required", () => {
    setTestWindowUrl(
      "https://control.example/ui/overview?gatewayUrl=wss://other-gateway.example/aikaclaw&token=abc123",
    );
    const host = createHost("overview");
    host.settings.gatewayUrl = "wss://control.example/aikaclaw";
    ...
    expect(host.pendingGatewayUrl).toBe("wss://other-gateway.example/aikaclaw");
