...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/aikaclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/aikaclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/aikaclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__AIKACLAW_CONTROL_UI_BASE_PATH__ = "/aikaclaw";
    const app = mountApp("/aikaclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/aikaclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/aikaclaw/sessions");
  });
  ...
  it("hydrates token from query params and strips them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("aikaclaw.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "aikaclaw.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/aikaclaw" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("aikaclaw.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/aikaclaw",
    });
    expect(JSON.parse(localStorage.getItem("aikaclaw.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/aikaclaw";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/aikaclaw");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/aikaclaw#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/aikaclaw");
    expect(app.settings.token).toBe("");
