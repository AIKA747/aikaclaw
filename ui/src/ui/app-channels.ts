...
export async function handleWhatsAppStart(host: AikaClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: AikaClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: AikaClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: AikaClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: AikaClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: AikaClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: AikaClawApp): string | null {
...
function buildGatewayHttpHeaders(host: AikaClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: AikaClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: AikaClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: AikaClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: AikaClawApp) {
  const state = host.nostrProfileFormState;
