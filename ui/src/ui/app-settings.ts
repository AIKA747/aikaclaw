...
import type { AikaClawApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as AikaClawApp);
  }
  if (host.tab === "usage") {
    await loadUsage(host as unknown as AikaClawApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as AikaClawApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as AikaClawApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as AikaClawApp);
    await loadConfig(host as unknown as AikaClawApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as AikaClawApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as AikaClawApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as AikaClawApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as AikaClawApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as AikaClawApp);
    await loadDevices(host as unknown as AikaClawApp);
    await loadConfig(host as unknown as AikaClawApp);
    await loadExecApprovals(host as unknown as AikaClawApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as AikaClawApp);
    await loadConfig(host as unknown as AikaClawApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as AikaClawApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as AikaClawApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__AIKACLAW_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as AikaClawApp;
...
async function loadOverviewLogs(host: AikaClawApp) {
...
function buildAttentionItems(host: AikaClawApp) {
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as AikaClawApp, true),
    loadConfigSchema(host as unknown as AikaClawApp),
    loadConfig(host as unknown as AikaClawApp),
  ]);
}
