...
This document describes how the AikaClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`aikaclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
AikaClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing AikaClaw Gateway
...
- ACP client spawns `aikaclaw acp` and speaks ACP messages over stdio.
...
`aikaclaw acp` resolves the Gateway URL and auth from CLI flags or config:
