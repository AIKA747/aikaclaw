...
export default defineConfig({
  resolve: {
    // Keep this ordered: the base `aikaclaw/plugin-sdk` alias is a prefix match.
    alias: [
      {
        find: "aikaclaw/extension-api",
        replacement: path.join(repoRoot, "src", "extensionAPI.ts"),
      },
      ...pluginSdkSubpaths.map((subpath) => ({
        find: `aikaclaw/plugin-sdk/${subpath}`,
        replacement: path.join(repoRoot, "src", "plugin-sdk", `${subpath}.ts`),
      })),
      {
        find: "aikaclaw/plugin-sdk",
