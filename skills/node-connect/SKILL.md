---
name: node-connect
description: Diagnose AikaClaw node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify AikaClaw is advertising that route, then fix pairing/auth.
...
- whether `aikaclaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `aikaclaw qr --json`. It uses the same setup-code payload Android scans.

```bash
aikaclaw config get gateway.mode
aikaclaw config get gateway.bind
aikaclaw config get gateway.tailscale.mode
aikaclaw config get gateway.remote.url
aikaclaw config get gateway.auth.mode
aikaclaw config get gateway.auth.allowTailscale
aikaclaw config get plugins.entries.device-pair.config.publicUrl
aikaclaw qr --json
aikaclaw devices list
aikaclaw nodes status
```

If this AikaClaw instance is pointed at a remote gateway, also run:

```bash
aikaclaw qr --remote --json
```
...
`aikaclaw qr --json` success means:
...
If `aikaclaw qr --json` says `Gateway is only bound to loopback`:
...
```bash
aikaclaw devices list
aikaclaw devices approve --latest
```
...
- `aikaclaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run aikaclaw qr again, rescan, then approve the pending device pairing.`
