---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for AikaClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, AikaClaw cron scheduling for periodic checks, or version status checks on a machine running AikaClaw (laptop, workstation, Pi, VPS).
---

# AikaClaw Host Hardening

## Overview

Assess and harden the host running AikaClaw, then align it to a user-defined risk tolerance without breaking access. Use AikaClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim AikaClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. AikaClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if AikaClaw can access sensitive data.
...
### 2) Run AikaClaw security audits (read-only)

As part of the default read-only checks, run `aikaclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `aikaclaw security audit` (faster, non-probing)
2. `aikaclaw security audit --json` (structured output)

Offer to apply AikaClaw safe defaults (numbered):

1. `aikaclaw security audit --fix`

Be explicit that `--fix` only tightens AikaClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check AikaClaw version/update status (read-only)

As part of the default read-only checks, run `aikaclaw update status`.
...
- Credential hygiene notes (location of AikaClaw creds, prefer disk encryption)
...
- AikaClaw security audit (re-run)
...
After AikaClaw install or first hardening pass, run at least one baseline audit and version check:

- `aikaclaw security audit`
- `aikaclaw security audit --deep`
- `aikaclaw update status`

Ongoing monitoring is recommended. Use the AikaClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `aikaclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `aikaclaw update status`
...
Before creating, `aikaclaw cron list` and match on exact `name`. If found, `aikaclaw cron edit <id> ...`.
If not found, `aikaclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `aikaclaw update status` (preferred for source checkouts and channels)
2. `npm view aikaclaw version` (published npm version)

## AikaClaw command accuracy

Use only supported commands and flags:

- `aikaclaw security audit [--deep] [--fix] [--json]`
- `aikaclaw status` / `aikaclaw status --deep`
- `aikaclaw health --json`
- `aikaclaw update status`
- `aikaclaw cron add|list|runs|run`

Do not invent CLI flags or imply AikaClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by AikaClaw compaction:
