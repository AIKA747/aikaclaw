...
const liveEnabled = HAS_OPENAI_KEY && process.env.AIKACLAW_LIVE_TEST === "1";
...
const TEST_RUNTIME_MANIFEST = {
  name: "aikaclaw-memory-lancedb-runtime",
...
function createRuntimeLoader(
  ...
  return createLanceDbRuntimeLoader({
    ...
    resolveStateDir: () => "/tmp/aikaclaw-state",
...
describe("memory plugin e2e", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "aikaclaw-memory-test-" });
  ...
  test("passes configured dimensions to OpenAI embeddings API", async () => {
    ...
    vi.resetModules();
    vi.doMock("aikaclaw/plugin-sdk/infra-runtime", () => ({
    ...
    try {
      ...
      vi.doUnmock("aikaclaw/plugin-sdk/infra-runtime");
      vi.doUnmock("openai");
...
describe("lancedb runtime loader", () => {
  test("uses the bundled module when it is already available", async () => {
    ...
    const installRuntime = vi.fn(async () => "/tmp/aikaclaw-state/plugin-runtimes/lancedb.js");
  ...
  test("reuses an existing user runtime install before attempting a reinstall", async () => {
    ...
    const resolveRuntimeEntry = vi.fn(
      () => "/tmp/aikaclaw-state/plugin-runtimes/memory-lancedb/runtime-entry.js",
    );
    const installRuntime = vi.fn(
      async () => "/tmp/aikaclaw-state/plugin-runtimes/memory-lancedb/runtime-entry.js",
    );
    const loader = createRuntimeLoader({
      importResolved,
      resolveRuntimeEntry,
    ...
    expect(resolveRuntimeEntry).toHaveBeenCalledWith(
      expect.objectContaining({
        runtimeDir: "/tmp/aikaclaw-state/plugin-runtimes/memory-lancedb/lancedb",
      }),
    );
    expect(installRuntime).not.toHaveBeenCalled();
  });
  ...
  test("installs LanceDB into user state when the bundled runtime is unavailable", async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "installing runtime deps under /tmp/aikaclaw-state/plugin-runtimes/memory-lancedb/lancedb",
      ),
    );
  });
  ...
  test("fails fast in nix mode instead of attempting auto-install", async () => {
    ...
    const loader = createRuntimeLoader({
      env: { AIKACLAW_NIX_MODE: "1" } as NodeJS.ProcessEnv,
      installRuntime,
    });
  ...
  test("clears the cached failure so later calls can retry the install", async () => {
    ...
    const installRuntime = vi
      ...
      .mockResolvedValueOnce(
        "/tmp/aikaclaw-state/plugin-runtimes/memory-lancedb/lancedb/node_modules/@lancedb/lancedb/index.js",
      );
    const importResolved = vi.fn(async () => runtimeModule);
    const loader = createRuntimeLoader({
      installRuntime,
...
describeLive("memory plugin live tests", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "aikaclaw-memory-live-" });
