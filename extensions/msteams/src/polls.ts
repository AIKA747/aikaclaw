...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["aikaclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["aikaclaw", "pollId"]) ??
    readNestedString(value, ["aikaclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "aikaclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "aikaclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          aikaclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "aikaclaw poll vote",
          displayText: "Vote recorded",
          value: { aikaclawPollId: pollId, pollId },
        },
      },
    ],
  };
