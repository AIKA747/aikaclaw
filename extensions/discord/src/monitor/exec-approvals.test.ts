...
const STORE_PATH = path.join(os.tmpdir(), "aikaclaw-exec-approvals-test.json");
...
beforeEach(() => {
  ...
  mockResolveGatewayConnectionAuth.mockReset().mockImplementation(
    async (params: {
      ...
      const envToken = params.env.AIKACLAW_GATEWAY_TOKEN;
      const envPassword = params.env.AIKACLAW_GATEWAY_PASSWORD;
...
vi.mock("aikaclaw/plugin-sdk/gateway-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("aikaclaw/plugin-sdk/gateway-runtime")>();
  ...
  return {
    ...
    createOperatorApprovalsGatewayClient: async (
      ...
      const envUrl = process.env.AIKACLAW_GATEWAY_URL?.trim();
...
vi.mock("aikaclaw/plugin-sdk/text-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("aikaclaw/plugin-sdk/text-runtime")>();
  return {
    ...actual,
    logDebug: vi.fn(),
    logError: vi.fn(),
...
describe("DiscordExecApprovalHandler gateway auth", () => {
  ...
  it("prefers AIKACLAW_GATEWAY_TOKEN when config token is missing", async () => {
    vi.stubEnv("AIKACLAW_GATEWAY_TOKEN", "env-gateway-token");
    const handler = new DiscordExecApprovalHandler({
      token: "discord-bot-token",
      accountId: "default",
      config: { enabled: true, approvers: ["123"] },
...
describe("DiscordExecApprovalHandler gateway auth resolution", () => {
  ...
  it("passes env URL overrides to shared gateway auth resolver", async () => {
    const previousGatewayUrl = process.env.AIKACLAW_GATEWAY_URL;
    try {
      process.env.AIKACLAW_GATEWAY_URL = "wss://gateway-from-env.example/ws";
      const handler = new DiscordExecApprovalHandler({
        token: "test-token",
        accountId: "default",
        config: { enabled: true, approvers: ["123"] },
      ...
      if (typeof previousGatewayUrl === "string") {
        process.env.AIKACLAW_GATEWAY_URL = previousGatewayUrl;
      } else {
