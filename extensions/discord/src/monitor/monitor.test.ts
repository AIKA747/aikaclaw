...
import { buildPluginBindingApprovalCustomId } from "aikaclaw/plugin-sdk/conversation-runtime";
...
async function createInfraRuntimeMock(
  importOriginal: () => Promise<typeof import("aikaclaw/plugin-sdk/infra-runtime")>,
) {
...
vi.mock("aikaclaw/plugin-sdk/infra-runtime", createInfraRuntimeMock);
vi.mock("aikaclaw/plugin-sdk/infra-runtime.js", createInfraRuntimeMock);
...
describe("discord component interactions", () => {
  ...
  const createCfg = (): AikaClawConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/aikaclaw-sessions-test.json");
