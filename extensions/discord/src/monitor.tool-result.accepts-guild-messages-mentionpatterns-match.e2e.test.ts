...
describe("discord tool result dispatch", () => {
  it("accepts guild messages when mentionPatterns match", async () => {
    const cfg = createMentionRequiredGuildConfig({
      messages: {
        inbound: { debounceMs: 0 },
        groupChat: { mentionPatterns: ["\\baikaclaw\\b"] },
      },
    } as Partial<Config>);
    ...
    await handler(createGuildMessageEvent({ messageId: "m2", content: "aikaclaw: hello" }), client);
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "AikaClaw" },
          },
        },
      }),
      client,
