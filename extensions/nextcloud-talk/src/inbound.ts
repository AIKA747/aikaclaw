...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as AikaClawConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as AikaClawConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as AikaClawConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as AikaClawConfig);
