/**
 * Synology Chat Channel Plugin for AikaClaw.
 *
 * Implements the ChannelPlugin interface following the LINE pattern.
 */

import type { AikaClawConfig } from "aikaclaw/plugin-sdk/account-resolution";
import {
  createHybridChannelConfigAdapter,
  createScopedDmSecurityResolver,
} from "aikaclaw/plugin-sdk/channel-config-helpers";
import { waitUntilAbort } from "aikaclaw/plugin-sdk/channel-lifecycle";
import {
...
import { attachChannelToResult } from "aikaclaw/plugin-sdk/channel-send-result";
import { createChatChannelPlugin, type ChannelPlugin } from "aikaclaw/plugin-sdk/core";
import { createEmptyChannelDirectoryAdapter } from "aikaclaw/plugin-sdk/directory-runtime";
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "aikaclaw pairing approve synology-chat <code>",
...
> & {
  pairing: {
    ...
    notifyApproval: (params: { cfg: AikaClawConfig; id: string }) => Promise<void>;
  };
  security: {
    resolveDmPolicy: (params: { cfg: AikaClawConfig; account: ResolvedSynologyChatAccount }) => {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      meta: {
        ...
        blurb: "Connect your Synology NAS Chat to AikaClaw",
        order: 90,
      },
      capabilities: {
