...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): AikaClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as AikaClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): AikaClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as AikaClawConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: AikaClawConfig, accountId = "media") {
    await handleTelegramAction(
