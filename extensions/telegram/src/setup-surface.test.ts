...
async function runPrepare(cfg: AikaClawConfig, accountId: string) {
...
async function runFinalize(cfg: AikaClawConfig, accountId: string) {
...
function expectPreparedResult(
...
): { cfg: AikaClawConfig } & Exclude<Awaited<ReturnType<typeof runPrepare>>, void | undefined> {
  ...
  return prepared as { cfg: AikaClawConfig } & Exclude<
...
describe("telegramSetupWizard.finalize", () => {
  it("shows global config commands for the default account", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining('aikaclaw config set channels.telegram.dmPolicy "allowlist"'),
      "Telegram DM access warning",
    );
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(`aikaclaw config set channels.telegram.allowFrom '["YOUR_USER_ID"]'`),
      "Telegram DM access warning",
    );
  });
  ...
  it("shows account-scoped config commands for named accounts", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(
        'aikaclaw config set channels.telegram.accounts.alerts.dmPolicy "allowlist"',
      ),
      "Telegram DM access warning",
    );
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(
        `aikaclaw config set channels.telegram.accounts.alerts.allowFrom '["YOUR_USER_ID"]'`,
      ),
      "Telegram DM access warning",
    );
  });
