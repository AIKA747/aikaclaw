...
async function withIsolatedStateDirAsync<T>(fn: () => Promise<T>): Promise<T> {
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-telegram-state-"));
  return await withEnvAsync({ AIKACLAW_STATE_DIR: stateDir }, async () => {
    try {
      return await fn();
    } finally {
...
async function withConfigPathAsync<T>(cfg: unknown, fn: () => Promise<T>): Promise<T> {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-telegram-cfg-"));
  const configPath = path.join(dir, "aikaclaw.json");
  fs.writeFileSync(configPath, JSON.stringify(cfg), "utf-8");
  return await withEnvAsync({ AIKACLAW_CONFIG_PATH: configPath }, async () => {
    try {
      return await fn();
    } finally {
...
describe("createTelegramBot", () => {
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    await withIsolatedStateDirAsync(async () => {
      for (const [index, testCase] of cases.entries()) {
        ...
        expect(pairingText, testCase.name).toContain("aikaclaw pairing approve telegram");
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "aikaclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("honors routed group activation from session store", async () => {
    const storeDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-telegram-"));
