...
import { resetInboundDedupe } from "aikaclaw/plugin-sdk/reply-runtime";
import type { MsgContext } from "aikaclaw/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "aikaclaw/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "aikaclaw/plugin-sdk/reply-runtime";
import type { MockFn } from "aikaclaw/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("aikaclaw/plugin-sdk/config-runtime").loadConfig;
type ResolveStorePathFn = typeof import("aikaclaw/plugin-sdk/config-runtime").resolveStorePath;
type TelegramBotRuntimeForTest = NonNullable<
  Parameters<typeof import("./bot.js").setTelegramBotRuntimeForTest>[0]
>;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("aikaclaw/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/aikaclaw-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("aikaclaw/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("aikaclaw/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("aikaclaw/plugin-sdk/config-runtime", async (importOriginal) => {
...
vi.doMock("aikaclaw/plugin-sdk/conversation-runtime", async (importOriginal) => {
...
vi.doMock("aikaclaw/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("aikaclaw/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: AikaClawConfig): {
...
vi.doMock("aikaclaw/plugin-sdk/command-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("aikaclaw/plugin-sdk/command-auth")>();
  return {
    ...actual,
...
vi.doMock("aikaclaw/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("aikaclaw/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("aikaclaw/plugin-sdk/reply-runtime", async (importOriginal) => {
...
vi.doMock("aikaclaw/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("aikaclaw/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("aikaclaw/plugin-sdk/infra-runtime", async (importOriginal) => {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: AikaClawConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: AikaClawConfig) => {
