...
let isWSL2Sync: typeof import("aikaclaw/plugin-sdk/infra-runtime").isWSL2Sync;
...
async function loadModule() {
  vi.resetModules();
  ({ isWSL2Sync } = await import("aikaclaw/plugin-sdk/infra-runtime"));
  ({
  ...
  it.each([
    {
      ...
      expected: {
        value: true,
        source: "env:AIKACLAW_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "uses env disable when set",
      env: { AIKACLAW_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1" },
      expected: {
        value: false,
        source: "env:AIKACLAW_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "prefers env enable over config",
      env: { AIKACLAW_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1" },
  ...
  it.each([
    {
      ...
      env: { AIKACLAW_TELEGRAM_DNS_RESULT_ORDER: "verbatim" },
      nodeMajor: 22,
      expected: {
        value: "verbatim",
        source: "env:AIKACLAW_TELEGRAM_DNS_RESULT_ORDER",
      },
    },
    {
      name: "normalizes trimmed env values",
      env: { AIKACLAW_TELEGRAM_DNS_RESULT_ORDER: "  IPV4FIRST  " },
      nodeMajor: 20,
      expected: {
        value: "ipv4first",
        source: "env:AIKACLAW_TELEGRAM_DNS_RESULT_ORDER",
    ...
    },
    {
      ...
      env: { AIKACLAW_TELEGRAM_DNS_RESULT_ORDER: "bogus" },
      network: { dnsResultOrder: "ipv4first" },
      nodeMajor: 20,
      expected: { value: "ipv4first", source: "config" },
    },
