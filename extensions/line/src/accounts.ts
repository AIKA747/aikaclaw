...
import { resolveAccountEntry } from "aikaclaw/plugin-sdk/account-resolution";
...
export function listLineAccountIds(cfg: AikaClawConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: AikaClawConfig): string {
