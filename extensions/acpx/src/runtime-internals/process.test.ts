...
import { createWindowsCmdShimFixture } from "aikaclaw/plugin-sdk/testing";
...
async function createTempDir(): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), "aikaclaw-acpx-process-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("spawnAndCollect", () => {
  ...
  async function collectSpawnedEnvSnapshot(options?: {
    ...
    const result = await spawnAndCollect({
      ...
      args: [
        "-e",
        `process.stdout.write(JSON.stringify({openai:process.env.${openAiEnvKey},github:process.env.${githubEnvKey},hf:process.env.${hfEnvKey},aikaclaw:process.env.AIKACLAW_API_KEY,shell:process.env.AIKACLAW_SHELL}))`,
      ],
      cwd: process.cwd(),
  ...
  it("strips shared provider auth env vars from spawned acpx children", async () => {
    ...
    expect(parsed.aikaclaw).toBe("keep-me");
