...
metadata: { "aikaclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == AikaClaw `sessions_spawn`
- **File I/O** == AikaClaw `read`/`write`
- **Remote fetch** == AikaClaw `web_fetch` (or `exec` with curl when POST is required)
