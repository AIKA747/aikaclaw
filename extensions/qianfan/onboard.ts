...
function resolveQianfanPreset(cfg: AikaClawConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: AikaClawConfig) => {
...
export function applyQianfanProviderConfig(cfg: AikaClawConfig): AikaClawConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: AikaClawConfig): AikaClawConfig {
