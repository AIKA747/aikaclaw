...
const CONFIG_KEY = Symbol.for("aikaclaw:testConfigMock");
...
vi.mock("aikaclaw/plugin-sdk/state-paths", async (importOriginal) => {
  const actual = await importOriginal<typeof import("aikaclaw/plugin-sdk/state-paths")>();
  return {
    ...actual,
    resolveOAuthDir: () => "/tmp/aikaclaw-oauth",
  };
});
...
vi.mock("@whiskeysockets/baileys", async (importOriginal) => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aikaclaw:lastSocket")] =
    created.lastSocket;
  return {
    ...actual,
    ...created.mod,
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aikaclaw:lastSocket")];
  if (typeof getter === "function") {
