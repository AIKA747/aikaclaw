...
function mergeWhatsAppConfig(
  cfg: AikaClawConfig,
  patch: Partial<NonNullable<NonNullable<AikaClawConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: AikaClawConfig, dmPolicy: DmPolicy): AikaClawConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: AikaClawConfig, allowFrom?: string[]): AikaClawConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: AikaClawConfig, selfChatMode: boolean): AikaClawConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: AikaClawConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so AikaClaw can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for AikaClaw" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("aikaclaw channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
