...
import { VERSION } from "aikaclaw/plugin-sdk/cli-runtime";
import { danger, success } from "aikaclaw/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "aikaclaw/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "aikaclaw/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("aikaclaw channels login")}`,
              ),
            );
          }
        }
