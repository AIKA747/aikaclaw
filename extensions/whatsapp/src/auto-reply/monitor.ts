import { resolveInboundDebounceMs } from "aikaclaw/plugin-sdk/channel-inbound";
import { formatCliCommand } from "aikaclaw/plugin-sdk/cli-runtime";
import { waitForever } from "aikaclaw/plugin-sdk/cli-runtime";
import { hasControlCommand } from "aikaclaw/plugin-sdk/command-auth";
import { loadConfig } from "aikaclaw/plugin-sdk/config-runtime";
import { formatDurationPrecise } from "aikaclaw/plugin-sdk/infra-runtime";
...
import { getReplyFromConfig } from "aikaclaw/plugin-sdk/reply-runtime";
import { resolveAgentRoute } from "aikaclaw/plugin-sdk/routing";
import { logVerbose } from "aikaclaw/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "aikaclaw/plugin-sdk/runtime-env";
import { getChildLogger } from "aikaclaw/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "aikaclaw/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("aikaclaw channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("aikaclaw channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
