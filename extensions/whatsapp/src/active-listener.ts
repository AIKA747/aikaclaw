...
const WHATSAPP_ACTIVE_LISTENER_STATE_KEY = Symbol.for("aikaclaw.whatsapp.activeListenerState");
...
export function requireActiveWebListener(accountId?: string | null): {
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${id}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`aikaclaw channels login --channel whatsapp --account ${id}`)}.`,
    );
  }
