...
import { info, success } from "aikaclaw/plugin-sdk/runtime-env";
...
import { resolveOAuthDir } from "aikaclaw/plugin-sdk/state-paths";
import type { WebChannel } from "aikaclaw/plugin-sdk/text-runtime";
import { jidToE164, resolveUserPath } from "aikaclaw/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("aikaclaw channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
