...
import type { AikaClawPluginApi, AikaClawPluginToolContext } from "../runtime-api.js";
import {
...
function fakeApi(overrides: Partial<AikaClawPluginApi> = {}): AikaClawPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<AikaClawPluginToolContext> = {}): AikaClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: AikaClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
...
describe("resolveWindowsLobsterSpawn", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-lobster-win-spawn-"));
    setProcessPlatform("win32");
  });
