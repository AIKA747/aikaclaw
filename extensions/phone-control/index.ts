...
function normalizeDenyList(cfg: AikaClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AikaClawPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: AikaClawPluginApi) {
    ...
    const timerService: AikaClawPluginService = {
