...
export const imessageDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: AikaClawConfig) => cfg.channels?.imessage?.dmPolicy ?? "pairing",
  setPolicy: (cfg: AikaClawConfig, policy) =>
    setChannelDmPolicyWithAllowFrom({
      cfg,
      channel,
      dmPolicy: policy,
...
function resolveIMessageCliPath(params: { cfg: AikaClawConfig; accountId: string }) {
