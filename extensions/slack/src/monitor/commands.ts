import type { SlackSlashCommandConfig } from "aikaclaw/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "aikaclaw");
  const name = normalizedName || "aikaclaw";
  return {
