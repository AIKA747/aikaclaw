...
// Prefix for AikaClaw-generated action IDs to scope our handler
const AIKACLAW_ACTION_PREFIX = "aikaclaw:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${AIKACLAW_ACTION_PREFIX}`);

  // Handle AikaClaw modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
