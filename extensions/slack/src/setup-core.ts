...
function enableSlackAccount(cfg: AikaClawConfig, accountId: string): AikaClawConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: AikaClawConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: AikaClawConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: AikaClawConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: AikaClawConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: AikaClawConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.allow !== false && value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: AikaClawConfig; accountId: string }) =>
    ...
    disable: (cfg: AikaClawConfig) => setSetupChannelEnabled(cfg, channel, false),
