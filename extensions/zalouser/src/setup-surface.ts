...
function ensureZalouserPluginEnabled(cfg: AikaClawConfig): AikaClawConfig {
    ...
    if (parts.length === 0) {
      await prompter.note(
        [
          ...
          `Tip: use \`${formatCliCommand("aikaclaw directory peers list --channel zalouser")}\` to look up people after onboarding.`,
        ].join("\n"),
  ...
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ...
        : resolveDefaultZalouserAccountId(cfg as AikaClawConfig);
    return await promptZalouserAllowFrom({
      cfg: cfg as AikaClawConfig,
      prompter,
      accountId: id,
    });
  },
...
export const zalouserSetupWizard: ChannelSetupWizard = {
  ...
  groupAccess: {
    ...
    setPolicy: ({ cfg, accountId, policy }) =>
      setZalouserGroupPolicy(cfg as AikaClawConfig, accountId, policy),
    resolveAllowlist: async ({ cfg, accountId, entries, prompter }) => {
      if (entries.length === 0) {
        await prompter.note(
          [
            ...
            `Tip: use \`${formatCliCommand("aikaclaw directory groups list --channel zalouser")}\` after onboarding to find group IDs.`,
      ...
      const updatedAccount = resolveZalouserAccountSync({ cfg: cfg as AikaClawConfig, accountId });
      try {
    ...
    applyAllowlist: ({ cfg, accountId, resolved }) =>
      setZalouserGroupAllowlist(cfg as AikaClawConfig, accountId, resolved as string[]),
  },
