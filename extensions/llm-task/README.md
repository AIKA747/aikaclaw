...
`aikaclaw.invoke --each`) without adding new AikaClaw code per workflow.
...
This extension depends on AikaClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** AikaClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.aikaclaw/extensions` as a standalone plugin directory.
