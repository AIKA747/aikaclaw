...
): Promise<{ cfg: AikaClawConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var AIKACLAW_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
function resolveTwitchGroupPolicy(cfg: AikaClawConfig): "open" | "allowlist" | "disabled" {
  const account = getAccountConfig(cfg, DEFAULT_ACCOUNT_ID);
...
const twitchDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg) => {
    const account = getAccountConfig(cfg as AikaClawConfig, DEFAULT_ACCOUNT_ID);
    if (account?.allowedRoles?.includes("all")) {
      return "open";
    }
  ...
  setPolicy: (cfg, policy) => {
    ...
    return setTwitchAccessControl(cfg as AikaClawConfig, allowedRoles, true);
  },
  promptAllowFrom: async ({ cfg, prompter }) => {
    ...
    return setTwitchAccount(cfg as AikaClawConfig, {
      ...(account ?? undefined),
      allowFrom,
    });
  },
...
const twitchGroupAccess: NonNullable<ChannelSetupWizard["groupAccess"]> = {
  ...
  currentPolicy: ({ cfg }) => resolveTwitchGroupPolicy(cfg as AikaClawConfig),
  currentEntries: ({ cfg }) => {
    const account = getAccountConfig(cfg as AikaClawConfig, DEFAULT_ACCOUNT_ID);
  ...
  setPolicy: ({ cfg, policy }) => setTwitchGroupPolicy(cfg as AikaClawConfig, policy),
  resolveAllowlist: async () => [],
  applyAllowlist: ({ cfg }) => cfg as AikaClawConfig,
};
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.AIKACLAW_TWITCH_ACCESS_TOKEN?.trim();
    ...
    const cfgWithAccount = setTwitchAccount(cfg as AikaClawConfig, {
      username,
      accessToken: token,
      clientId,
      channel: channelName,
