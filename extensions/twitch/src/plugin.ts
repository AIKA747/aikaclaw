/**
 * Twitch channel plugin for AikaClaw.
 *
 ...
 * for AikaClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: AikaClawConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: AikaClawConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        isConfigured: (_account: unknown, cfg: AikaClawConfig): boolean =>
      ...
      resolver: {
        resolveTargets: async ({
          ...
          runtime: import("aikaclaw/plugin-sdk/runtime-env").RuntimeEnv;
