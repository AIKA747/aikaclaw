...
Read-only diff viewer plugin for **AikaClaw** agents.
...
Set plugin-wide defaults in `~/.aikaclaw/aikaclaw.json`:
...
Before:
AikaClaw supports plugins.

After:
AikaClaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/aikaclaw-diffs`).
