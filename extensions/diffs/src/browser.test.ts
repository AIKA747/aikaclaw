...
import type { AikaClawConfig } from "../api.js";
import type { AikaClawPluginApi, AikaClawPluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("aikaclaw-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<AikaClawPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: AikaClawPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<AikaClawPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as AikaClawPluginApi);
