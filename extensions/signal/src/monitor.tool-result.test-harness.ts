...
vi.mock("aikaclaw/plugin-sdk/config-runtime", async () => {
  ...
  return {
    ...actual,
    loadConfig: () => config,
    resolveStorePath: vi.fn(() => "/tmp/aikaclaw-sessions.json"),
...
vi.mock("aikaclaw/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("aikaclaw/plugin-sdk/security-runtime")>(
    "aikaclaw/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("aikaclaw/plugin-sdk/reply-runtime"),
      import("aikaclaw/plugin-sdk/infra-runtime"),
    ]);
    resetInboundDedupe();
    config = {
