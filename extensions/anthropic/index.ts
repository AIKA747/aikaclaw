...
import { parseDurationMs } from "aikaclaw/plugin-sdk/cli-runtime";
import {
  definePluginEntry,
...
import { normalizeModelCompat } from "aikaclaw/plugin-sdk/provider-models";
import { fetchClaudeUsage } from "aikaclaw/plugin-sdk/provider-usage";
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("aikaclaw doctor --yes")}"`,
  ].join("\n");
}
