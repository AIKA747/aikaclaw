...
Official Voice Call plugin for **AikaClaw**.
...
Docs: `https://docs.aikaclaw.ai/plugins/voice-call`
Plugin system: `https://docs.aikaclaw.ai/plugin`

## Install (local dev)

### Option A: install via AikaClaw (recommended)
...
```bash
mkdir -p ~/.aikaclaw/extensions
cp -R extensions/voice-call ~/.aikaclaw/extensions/voice-call
cd ~/.aikaclaw/extensions/voice-call && pnpm install
```
...
- advanced webhook, streaming, and tunnel notes: `https://docs.aikaclaw.ai/plugins/voice-call`

## Stale call reaper

See the plugin docs for recommended ranges and production examples:
`https://docs.aikaclaw.ai/plugins/voice-call#stale-call-reaper`

## TTS for calls

Voice Call uses the core `messages.tts` configuration for
streaming speech on calls. Override examples and provider caveats live here:
`https://docs.aikaclaw.ai/plugins/voice-call#tts-for-calls`

## CLI

```bash
aikaclaw voicecall call --to "+15555550123" --message "Hello from AikaClaw"
aikaclaw voicecall continue --call-id <id> --message "Any questions?"
aikaclaw voicecall speak --call-id <id> --message "One moment"
aikaclaw voicecall end --call-id <id>
aikaclaw voicecall status --call-id <id>
aikaclaw voicecall tail
