import type { AnyAgentTool, AikaClawPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: AikaClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<AikaClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
