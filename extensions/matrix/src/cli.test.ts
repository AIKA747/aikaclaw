...
describe("matrix CLI verification commands", () => {
  ...
  it("lists matrix devices", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith("- A7hWrQ70ea (current, AikaClaw Gateway)");
    expect(console.log).toHaveBeenCalledWith("  Last IP: 127.0.0.1");
    expect(console.log).toHaveBeenCalledWith("- BritdXC6iL (AikaClaw Gateway)");
  });
  ...
  it("prunes stale matrix gateway devices", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith("Deleted stale AikaClaw devices: BritdXC6iL");
  ...
  it("adds a matrix account and prints a binding hint", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "Bind this account to an agent: aikaclaw agents bind --agent <id> --bind matrix:ops",
    );
  });
  ...
  it("bootstraps verification for newly added encrypted accounts", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "Matrix device hygiene warning: stale AikaClaw devices detected (BritdXC6iL). Run 'aikaclaw matrix devices prune-stale --account ops'.",
    );
  });
  ...
  it("uses --name as fallback account id and prints account-scoped config path", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "Bind this account to an agent: aikaclaw agents bind --agent <id> --bind matrix:main-bot",
    );
  });
  ...
  it("shows explicit backup issue in default status output", async () => {
    ...
    expect(console.log).not.toHaveBeenCalledWith(
      "- Backup is present but not trusted for this device. Re-run 'aikaclaw matrix verify device <key>'.",
    );
  });
  ...
  it("includes backup reset guidance when the backup key does not match this device", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "- If you want a fresh backup baseline and accept losing unrecoverable history, run 'aikaclaw matrix verify backup reset --yes'.",
    );
  });
  ...
  it("prints resolved account-aware guidance when a named Matrix account is selected implicitly", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "- Run 'aikaclaw matrix verify device <key> --account assistant' to verify this device.",
    );
    expect(console.log).toHaveBeenCalledWith(
      "- Run 'aikaclaw matrix verify bootstrap --account assistant' to create a room key backup.",
    );
  });
