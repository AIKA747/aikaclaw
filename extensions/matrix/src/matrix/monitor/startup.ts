...
export async function runMatrixStartupMaintenance(params: {
  ...
  try {
    ...
    if (deviceHealth.staleAikaClawDevices.length > 0) {
      params.logger.warn(
        `matrix: stale AikaClaw devices detected for ${params.auth.userId}: ${deviceHealth.staleAikaClawDevices.map((device) => device.deviceId).join(", ")}. Run 'aikaclaw matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
