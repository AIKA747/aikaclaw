...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another AikaClaw runtime is using the same account. " +
    `Check 'aikaclaw matrix verify status --verbose --account ${accountId}' and 'aikaclaw matrix devices list --account ${accountId}'.`
  );
}
