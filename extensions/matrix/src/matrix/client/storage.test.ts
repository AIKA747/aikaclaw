...
const createBackupArchiveMock = vi.hoisted(() =>
  vi.fn(async (_params: unknown) => ({
    ...
    archiveRoot: "2026-03-17-aikaclaw-backup",
...
describe("matrix client storage paths", () => {
  ...
  function setupStateDir(
    ...
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-matrix-storage-"));
    const stateDir = path.join(homeDir, ".aikaclaw");
  ...
  function createMigrationEnv(stateDir: string): NodeJS.ProcessEnv {
    return {
      ...
      AIKACLAW_HOME: path.dirname(stateDir),
      AIKACLAW_STATE_DIR: stateDir,
      AIKACLAW_TEST_FAST: "1",
