...
import { isAikaClawManagedMatrixDevice } from "./matrix/device-health.js";
import {
...
function formatMatrixCliCommand(command: string, accountId?: string): string {
  ...
  return `aikaclaw matrix ${command}${suffix}`;
}
...
async function addMatrixAccount(params: {
  ...
  try {
    ...
    deviceHealth = {
      ...
      staleAikaClawDeviceIds: addedDevices
        .filter((device) => !device.current && isAikaClawManagedMatrixDevice(device.displayName))
...
export function registerMatrixCli(params: { program: Command }): void {
  const root = params.program
    ...
    .addHelpText("after", () => "\nDocs: https://docs.aikaclaw.ai/channels/matrix\n");
      ...
      async (options: {
        ...
        await runMatrixCliCommand({
          ...
          onText: (result) => {
            ...
            } else if (result.deviceHealth.staleAikaClawDeviceIds.length > 0) {
              console.log(
                `Matrix device hygiene warning: stale AikaClaw devices detected (${result.deviceHealth.staleAikaClawDeviceIds.join(", ")}). Run 'aikaclaw matrix devices prune-stale --account ${result.accountId}'.`,
              );
            }
            ...
            const bindHint = `aikaclaw agents bind --agent <id> --bind matrix:${result.accountId}`;
  ...
  devices
    .command("prune-stale")
    .description("Delete stale AikaClaw-managed devices for this account")
    .option("--account <id>", "Account ID (for multi-account setups)")
    .option("--verbose", "Show detailed diagnostics")
    .option("--json", "Output as JSON")
    .action(async (options: { account?: string; verbose?: boolean; json?: boolean }) => {
      ...
      await runMatrixCliCommand({
        ...
        onText: (result, verbose) => {
          printAccountLabel(accountId);
          console.log(
            `Deleted stale AikaClaw devices: ${result.deletedDeviceIds.length ? result.deletedDeviceIds.join(", ") : "none"}`,
          );
