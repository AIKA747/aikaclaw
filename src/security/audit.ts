...
import type { ConfigFileSnapshot, AikaClawConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: AikaClawConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your AikaClaw state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your AikaClaw state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.AIKACLAW_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.AIKACLAW_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("aikaclaw security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.AIKACLAW_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(env.AIKACLAW_GATEWAY_PASSWORD) ||
...
function collectLoggingFindings(cfg: AikaClawConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: AikaClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: AikaClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: AikaClawConfig): string[] {
  const channels = asRecord(cfg.channels);
  if (!channels) {
    return [];
  }
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("aikaclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("aikaclaw security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set AIKACLAW_GATEWAY_TOKEN/AIKACLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("aikaclaw security audit --deep")}".`,
    });
  }
