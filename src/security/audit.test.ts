...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: AikaClawConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: AikaClawConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: AikaClawConfig) => string[];
  isConfigured?: (account: unknown, cfg: AikaClawConfig) => boolean;
  isEnabled?: (account: unknown, cfg: AikaClawConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
async function runInstallMetadataAudit(
  ...
  return runSecurityAudit({
    ...
    includeChannelSecurity: false,
    stateDir,
    configPath: path.join(stateDir, "aikaclaw.json"),
...
describe("security audit", () => {
  ...
  let previousAikaClawHome: string | undefined;
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "aikaclaw.json");
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ AIKACLAW_STATE_DIR: sharedChannelSecurityStateDir }, () =>
      fn(sharedChannelSecurityStateDir),
    );
  };

  const runSharedExtensionsAudit = async (config: AikaClawConfig) => {
    return runSecurityAudit({
      config,
      includeFilesystem: true,
      includeChannelSecurity: false,
      stateDir: sharedExtensionsStateDir,
      configPath: path.join(sharedExtensionsStateDir, "aikaclaw.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  };
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        aikaclaw: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-security-audit-"));
    previousAikaClawHome = process.env.AIKACLAW_HOME;
    process.env.AIKACLAW_HOME = path.join(fixtureRoot, "home");
    await fs.mkdir(process.env.AIKACLAW_HOME, { recursive: true, mode: 0o700 });
  ...
  afterAll(async () => {
    if (previousAikaClawHome === undefined) {
      delete process.env.AIKACLAW_HOME;
    } else {
      process.env.AIKACLAW_HOME = previousAikaClawHome;
    }
    if (!fixtureRoot) {
      return;
    }
  ...
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    const cases = [
          ...
          withEnvAsync(
            {
              ...
              AIKACLAW_GATEWAY_PASSWORD: undefined,
            },
            async () =>
              audit({
                gateway: {
      ...
      },
      {
        ...
        run: async () => {
          const sourceConfig: AikaClawConfig = {
            gateway: {
              bind: "lan",
              auth: {
                token: {
          ...
          const resolvedConfig: AikaClawConfig = {
            gateway: {
              bind: "lan",
              auth: {},
            },
  ...
  it("evaluates safeBinTrustedDirs risk findings", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/aikaclaw-safe-bins"];
    const cases = [
      {
    ...
    const cases = [
      {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from(
                    "aikaclaw-sbx-browser-old\naikaclaw-sbx-browser-missing-hash\n",
                  ),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "aikaclaw-sbx-browser-old") {
                return {
              ...
              if (args[0] === "inspect" && args.at(-1) === "aikaclaw-sbx-browser-missing-hash") {
                return {
        ...
        assert: (res: SecurityAuditReport) => {
          ...
          expect(staleEpoch?.detail).toContain("aikaclaw-sbx-browser-old");
        },
      },
      {
        ...
        run: async () => {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from("aikaclaw-sbx-browser-exposed\n"),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "aikaclaw-sbx-browser-exposed") {
                return {
              ...
              if (args[0] === "port" && args.at(-1) === "aikaclaw-sbx-browser-exposed") {
                return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-aikaclaw.json");
    ...
    await Promise.all(
      cases
        ...
        .map(async (testCase) => {
          ...
          const configPath = path.join(fixture.stateDir, "aikaclaw.json");
          await fs.writeFile(configPath, "{}\n", "utf-8");
          if (!isWindows) {
            await fs.chmod(configPath, 0o600);
          }
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): AikaClawConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("keeps source-configured channel security findings when resolved inspection is incomplete", async () => {
    const cases = [
      ...
      },
      {
        ...
        plugin: (sourceConfig: AikaClawConfig) =>
          stubChannelPlugin({
            id: "slack",
            label: "Slack",
            inspectAccount: (cfg) => {
  ...
  it.each([
    {
      ...
      detailIncludes: [
        ...
        "~/.aikaclaw/credentials/discord-allowFrom.json:team.owner",
      ],
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: AikaClawConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("evaluates hooks ingress auth and routing findings", async () => {
    ...
    } satisfies NonNullable<AikaClawConfig["hooks"]>;
    ...
    const cases = [
      ...
      },
      {
        ...
        env: {
          AIKACLAW_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
        },
      ...
      },
      {
        ...
        cfg: { hooks: unrestrictedBaseHooks } satisfies AikaClawConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "warn" as const,
      },
      {
        name: "scores unrestricted hooks.allowedAgentIds by remote exposure",
        cfg: { gateway: { bind: "lan" }, hooks: unrestrictedBaseHooks } satisfies AikaClawConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "critical" as const,
      },
      {
        name: "scores hooks request sessionKey override by local exposure",
        cfg: { hooks: requestSessionKeyHooks } satisfies AikaClawConfig,
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.aikaclaw",
      configPath: "/Users/test/Dropbox/.aikaclaw/aikaclaw.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: AikaClawConfig = { logging: { redactSensitive: "off" } };
    ...
    const cases = [
          ...
          runInstallMetadataAudit(
            {
              plugins: {
                installs: {
                  "voice-call": {
                    source: "npm",
                    spec: "@aikaclaw/voice-call@1.2.3",
              ...
              hooks: {
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@aikaclaw/test-hooks@1.2.3",
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            path.join(pluginDir, "package.json"),
            JSON.stringify({ name: "@aikaclaw/voice-call", version: "9.9.9" }),
            "utf-8",
          );
          await fs.writeFile(
            path.join(hookDir, "package.json"),
            JSON.stringify({ name: "@aikaclaw/test-hooks", version: "8.8.8" }),
            "utf-8",
          );
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            ...
            JSON.stringify({
              name: "escape-plugin",
              aikaclaw: { extensions: ["../outside.js"] },
            }),
          );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.AIKACLAW_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.AIKACLAW_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
