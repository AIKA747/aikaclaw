...
type ChannelConfigAccessorParams<Config extends AikaClawConfig = AikaClawConfig> = {
  cfg: Config;
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends AikaClawConfig = AikaClawConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends AikaClawConfig = AikaClawConfig>(
...
> {
  const base = {
    resolveAllowFrom: ({ cfg, accountId }: { cfg: AikaClawConfig; accountId?: string | null }) =>
      mapAllowFromEntries(
...
function setTopLevelChannelEnabledInConfigSection<Config extends AikaClawConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends AikaClawConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends AikaClawConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
