...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.AIKACLAW_GATEWAY_PORT).toBe("18789");
    expect(env.AIKACLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.AIKACLAW_SERVICE_MARKER).toBe("aikaclaw");
    expect(env.AIKACLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.AIKACLAW_SERVICE_VERSION).toBe("string");
    expect(env.AIKACLAW_SYSTEMD_UNIT).toBe("aikaclaw-gateway.service");
    expect(env.AIKACLAW_WINDOWS_TASK_NAME).toBe("AikaClaw Gateway");
    if (process.platform === "darwin") {
      expect(env.AIKACLAW_LAUNCHD_LABEL).toBe("ai.aikaclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", AIKACLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.AIKACLAW_SYSTEMD_UNIT).toBe("aikaclaw-gateway-work.service");
    expect(env.AIKACLAW_WINDOWS_TASK_NAME).toBe("AikaClaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.AIKACLAW_LAUNCHD_LABEL).toBe("ai.aikaclaw.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through AIKACLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", AIKACLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.AIKACLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits AIKACLAW_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".aikaclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", AIKACLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".aikaclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", AIKACLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".aikaclaw"));
  });

  it("uses AIKACLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", AIKACLAW_STATE_DIR: "/var/lib/aikaclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/aikaclaw"));
  });

  it("expands ~ in AIKACLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", AIKACLAW_STATE_DIR: "~/aikaclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/aikaclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { AIKACLAW_STATE_DIR: "C:\\State\\aikaclaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\aikaclaw");
  });
});
