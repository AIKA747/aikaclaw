...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `aikaclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      AIKACLAW_LAUNCHD_LABEL: `ai.aikaclaw.launchd-int-${testId}`,
      AIKACLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
