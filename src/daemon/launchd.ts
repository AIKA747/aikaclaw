...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.AIKACLAW_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return envLabel;
  }
  return resolveGatewayLaunchAgentLabel(args?.env?.AIKACLAW_PROFILE);
}
...
export function resolveGatewayLogPaths(env: GatewayServiceEnv): {
  ...
  const prefix = env.AIKACLAW_LOG_PREFIX?.trim() || "gateway";
  return {
    logDir,
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromEnv = parseStrictPositiveInteger(env.AIKACLAW_GATEWAY_PORT ?? "");
  return fromEnv ?? null;
}
...
function throwBootstrapGuiSessionError(params: {
  ...
  throw new Error(
    [
      ...
      "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.aikaclaw.ai/gateway",
    ].join("\n"),
  );
}
...
export async function findLegacyLaunchAgents(env: GatewayServiceEnv): Promise<LegacyLaunchAgent[]> {
  ...
  for (const label of resolveLegacyGatewayLaunchAgentLabels(env.AIKACLAW_PROFILE)) {
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.AIKACLAW_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "aikaclaw gateway install --force",
  });
}
...
export async function restartLaunchAgent({
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    serviceTarget,
    plistPath,
    actionHint: "aikaclaw gateway restart",
  });
