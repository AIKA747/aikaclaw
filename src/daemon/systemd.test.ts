...
const TEST_MANAGED_HOME = "/tmp/aikaclaw-test-home";
const GATEWAY_SERVICE = "aikaclaw-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/aikaclaw gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/aikaclaw", "gateway", "run"]);
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/aikaclaw-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "aikaclaw-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "aikaclaw-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/aikaclaw-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled aikaclaw-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled aikaclaw-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when AIKACLAW_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/aikaclaw-gateway.service",
    },
    {
      name: "uses profile-specific service name when AIKACLAW_PROFILE is set to a custom value",
      env: { HOME: "/home/test", AIKACLAW_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/aikaclaw-gateway-jbphoenix.service",
    },
    {
      name: "prefers AIKACLAW_SYSTEMD_UNIT over AIKACLAW_PROFILE",
      env: {
        HOME: "/home/test",
        AIKACLAW_PROFILE: "jbphoenix",
        AIKACLAW_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles AIKACLAW_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        AIKACLAW_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from AIKACLAW_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        AIKACLAW_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/aikaclaw gateway start --name "My Bot"')).toEqual([
      "/usr/bin/aikaclaw",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('aikaclaw --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["aikaclaw", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('aikaclaw --path "C:\\\\Program Files\\\\AikaClaw"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["aikaclaw", "--path", "C:\\\\Program Files\\\\AikaClaw"]);

    expect(
      splitArgsPreservingQuotes('aikaclaw --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["aikaclaw", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/aikaclaw gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads AIKACLAW_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/aikaclaw gateway run", "EnvironmentFile=%h/.aikaclaw/.env"],
      { [`${TEST_SERVICE_HOME}/.aikaclaw/.env`]: "AIKACLAW_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.AIKACLAW_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/aikaclaw gateway run",
        "EnvironmentFile=%h/.aikaclaw/.env",
        'Environment="AIKACLAW_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.aikaclaw/.env`]: "AIKACLAW_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.AIKACLAW_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.AIKACLAW_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.aikaclaw/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/aikaclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/aikaclaw gateway run",
          'EnvironmentFile=%h/.aikaclaw/first.env "%h/.aikaclaw/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.aikaclaw/first.env") {
        return "AIKACLAW_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.aikaclaw/second env.env") {
        return 'AIKACLAW_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      ...
      AIKACLAW_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "AIKACLAW_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "AIKACLAW_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "AIKACLAW_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      AIKACLAW_GATEWAY_TOKEN: "override-token",
      AIKACLAW_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/aikaclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/aikaclaw gateway run",
          "EnvironmentFile=%h/.aikaclaw/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.aikaclaw/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'AIKACLAW_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "AIKACLAW_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      AIKACLAW_GATEWAY_TOKEN: "quoted token",
      AIKACLAW_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      AIKACLAW_GATEWAY_TOKEN: "file",
      AIKACLAW_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "aikaclaw-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ AIKACLAW_PROFILE: "work" });
  });
