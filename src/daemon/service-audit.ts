...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds AIKACLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `aikaclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service AIKACLAW_GATEWAY_TOKEN does not match gateway.auth.token in aikaclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.AIKACLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.AIKACLAW_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `aikaclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
