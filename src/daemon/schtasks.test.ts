...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when AIKACLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".aikaclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when AIKACLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", AIKACLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".aikaclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers AIKACLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        AIKACLAW_PROFILE: "rescue",
        AIKACLAW_STATE_DIR: "C:\\State\\aikaclaw",
      },
      expected: path.join("C:\\State\\aikaclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", AIKACLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".aikaclaw", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem AikaClaw Gateway",
          "cd /d C:\\Projects\\aikaclaw",
          "set NODE_ENV=production",
          "set AIKACLAW_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\aikaclaw",
          environment: {
            NODE_ENV: "production",
            AIKACLAW_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\aikaclaw\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\aikaclaw\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\AikaClaw Share\\node.exe" "\\\\fileserver\\AikaClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\AikaClaw Share\\node.exe",
            "\\\\fileserver\\AikaClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from AIKACLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ AIKACLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
