...
function listAgents(cfg: AikaClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: AikaClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<AikaClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: AikaClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: AikaClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: AikaClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<AikaClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  AikaClawConfig,
  {
    bindingsRef: AikaClawConfig["bindings"];
    agentsRef: AikaClawConfig["agents"];
    sessionRef: AikaClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: AikaClawConfig): Map<string, ResolvedAgentRoute> {
