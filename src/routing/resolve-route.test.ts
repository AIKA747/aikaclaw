...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: AikaClawConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: AikaClawConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/aikaclaw-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<AikaClawConfig["bindings"]>[number];
