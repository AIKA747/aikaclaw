...
function buildHooksReport(config: AikaClawConfig): HookStatusReport {
...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  if (hooks.length === 0) {
    const message = opts.eligible
      ? `No eligible hooks found. Run \`${formatCliCommand("aikaclaw hooks list")}\` to see all hooks.`
      : "No hooks found.";
    return message;
  }
...
export function formatHookInfo(
  ...
  if (!hook) {
    ...
    return `Hook "${hookName}" not found. Run \`${formatCliCommand("aikaclaw hooks list")}\` to see available hooks.`;
  }
...
export function registerHooksCli(program: Command): void {
  ...
  hooks
    .command("install")
    .description("Deprecated: install a hook pack via `aikaclaw plugins install`")
    ...
    .action(async (raw: string, opts: { link?: boolean; pin?: boolean }) => {
      defaultRuntime.log(
        theme.warn("`aikaclaw hooks install` is deprecated; use `aikaclaw plugins install`."),
      );
  ...
  hooks
    .command("update")
    .description("Deprecated: update hook packs via `aikaclaw plugins update`")
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      defaultRuntime.log(
        theme.warn("`aikaclaw hooks update` is deprecated; use `aikaclaw plugins update`."),
      );
