...
import type { ConfigFileSnapshot } from "../config/types.aikaclaw.js";

const loadConfigMock = vi.fn<() => AikaClawConfig>();
...
vi.mock("../commands/doctor/providers/matrix.js", () => ({
  cleanStaleMatrixPluginConfig: (cfg: AikaClawConfig) => cleanStaleMatrixPluginConfigMock(cfg),
}));
...
function makeSnapshot(overrides: Partial<ConfigFileSnapshot> = {}): ConfigFileSnapshot {
  return {
    ...
    raw: '{ "plugins": {} }',
    parsed: { plugins: {} },
    resolved: { plugins: {} } as AikaClawConfig,
    valid: false,
    config: { plugins: {} } as AikaClawConfig,
    hash: "abc",
...
describe("loadConfigForInstall", () => {
  const matrixNpmRequest = {
    rawSpec: "@aikaclaw/matrix",
    normalizedSpec: "@aikaclaw/matrix",
  };
  ...
  beforeEach(() => {
    ...
    cleanStaleMatrixPluginConfigMock.mockImplementation((cfg: AikaClawConfig) => ({
      config: cfg,
      changes: [],
    }));
  });
  ...
  it("returns the config directly when loadConfig succeeds", async () => {
    const cfg = { plugins: { entries: { matrix: { enabled: true } } } } as AikaClawConfig;
  ...
  it("does not run stale Matrix cleanup on the happy path", async () => {
    const cfg = { plugins: {} } as AikaClawConfig;
    loadConfigMock.mockReturnValue(cfg);
  ...
  it("allows explicit repo-checkout Matrix reinstall recovery", async () => {
    ...
    const snapshotCfg = { plugins: {} } as AikaClawConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        config: snapshotCfg,
  ...
  it("rejects non-Matrix install requests when config is invalid", async () => {
    ...
    ).rejects.toThrow("Config invalid; run `aikaclaw doctor --fix` before installing plugins.");
    expect(readConfigFileSnapshotMock).not.toHaveBeenCalled();
  });
  ...
  it("throws when loadConfig fails with INVALID_CONFIG and snapshot parsed is empty", async () => {
    ...
    await expect(loadConfigForInstall(matrixNpmRequest)).rejects.toThrow(
      "Config file could not be parsed; run `aikaclaw doctor` to repair it.",
    );
  });
