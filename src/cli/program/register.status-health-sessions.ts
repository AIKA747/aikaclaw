...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aikaclaw status", "Show channel health + session summary."],
          ["aikaclaw status --all", "Full diagnosis (read-only)."],
          ["aikaclaw status --json", "Machine-readable output."],
          ["aikaclaw status --usage", "Show model provider usage/quota snapshots."],
          [
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aikaclaw sessions", "List all sessions."],
          ["aikaclaw sessions --agent work", "List sessions for one agent."],
          ["aikaclaw sessions --all-agents", "Aggregate sessions across agents."],
          ["aikaclaw sessions --active 120", "Only last 2 hours."],
          ["aikaclaw sessions --json", "Machine-readable output."],
          ["aikaclaw sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aikaclaw sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "aikaclaw sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["aikaclaw sessions cleanup --enforce", "Apply maintenance now."],
          ["aikaclaw sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["aikaclaw sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "aikaclaw sessions cleanup --enforce --store ./tmp/sessions.json",
