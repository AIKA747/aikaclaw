...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.AIKACLAW_DISABLE_LAZY_SUBCOMMANDS;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.AIKACLAW_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.AIKACLAW_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
  ...
  it("registers only the primary placeholder and dispatches", async () => {
    const program = createRegisteredProgram(["node", "aikaclaw", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "aikaclaw"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "aikaclaw", "nodes", "list"], "aikaclaw");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "aikaclaw", "acp", "--help"], "aikaclaw");
