...
beforeEach(() => {
  ...
  originalHideBanner = process.env.AIKACLAW_HIDE_BANNER;
  ...
  delete process.env.AIKACLAW_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.AIKACLAW_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("aikaclaw");
    program
      .command("status")
      .option("--json")
      .action(() => {});
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "aikaclaw", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("aikaclaw-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "aikaclaw", "message", "send"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "aikaclaw", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "aikaclaw", "channels", "add"],
    });
  ...
  it("only allows invalid config for explicit Matrix reinstall requests", async () => {
    await runPreAction({
      parseArgv: ["plugins", "install", "@aikaclaw/matrix"],
      processArgv: ["node", "aikaclaw", "plugins", "install", "@aikaclaw/matrix"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "aikaclaw", "plugins", "install", "alpha"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "aikaclaw", "plugins", "install", "./extensions/matrix"],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@aikaclaw/matrix", "--marketplace", "local/repo"],
      processArgv: [
        "node",
        "aikaclaw",
        "plugins",
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "aikaclaw", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.AIKACLAW_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "aikaclaw", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "aikaclaw", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "aikaclaw", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "aikaclaw", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "aikaclaw", "agents", "list", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "aikaclaw", "config", "set", "gateway.auth.mode", "local", "--json"],
    });
    ...
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "aikaclaw", "agents", "list"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "aikaclaw", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "aikaclaw", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "aikaclaw", "backup", "create", "--json"],
    });
