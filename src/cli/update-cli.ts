...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update AikaClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["aikaclaw update", "Update a source checkout (git)"],
        ["aikaclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["aikaclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["aikaclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["aikaclaw update --tag main", "One-off package install from GitHub main"],
        ["aikaclaw update --dry-run", "Preview actions without changing anything"],
        ["aikaclaw update --no-restart", "Update without restarting the service"],
        ["aikaclaw update --json", "Output result as JSON"],
        ["aikaclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["aikaclaw update wizard", "Interactive update wizard"],
        ["aikaclaw --update", "Shorthand for aikaclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run aikaclaw update status to see the active channel and source
...
${theme.heading("Notes:")}
  ...
  - For global installs: auto-updates via detected package manager when possible
...
`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aikaclaw update status", "Show channel + version status."],
          ["aikaclaw update status --json", "JSON output."],
