...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# AikaClaw Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(aikaclaw completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# AikaClaw Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(aikaclaw completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $AIKACLAW_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "aikaclaw") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
