...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("aikaclaw-daemon-install-int-");
    configPath = path.join(tempHome, "aikaclaw.json");
    process.env.HOME = tempHome;
    process.env.AIKACLAW_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.AIKACLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});
