...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@aikaclaw/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@aikaclaw/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@aikaclaw/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@aikaclaw/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @aikaclaw/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@aikaclaw/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@aikaclaw/plugin-alpha@1.2.3",
        installPath: "/tmp/aikaclaw/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@aikaclaw/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @aikaclaw/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@aikaclaw/plugin-alpha@latest",
      true,
      "/tmp/aikaclaw/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@aikaclaw/plugin-alpha@latest",
      installPath: "/tmp/aikaclaw/extensions/alpha",
      version: "1.2.3",
