...
describe("tryRouteCli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.AIKACLAW_DISABLE_ROUTE_FIRST;
    delete process.env.AIKACLAW_DISABLE_ROUTE_FIRST;
    vi.resetModules();
  ...
  afterEach(() => {
    ...
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.AIKACLAW_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  });

  it("passes suppressDoctorStdout=true for routed --json commands", async () => {
    await expect(tryRouteCli(["node", "aikaclaw", "status", "--json"])).resolves.toBe(true);
  ...
  it("does not pass suppressDoctorStdout for routed non-json commands", async () => {
    await expect(tryRouteCli(["node", "aikaclaw", "status"])).resolves.toBe(true);
  ...
  it("routes logs to stderr during plugin loading in --json mode and restores after", async () => {
    ...
    await tryRouteCli(["node", "aikaclaw", "agents", "--json"]);
  ...
  it("does not route logs to stderr during plugin loading without --json", async () => {
    ...
    await tryRouteCli(["node", "aikaclaw", "agents"]);
  ...
  it("routes status when root options precede the command", async () => {
    await expect(tryRouteCli(["node", "aikaclaw", "--log-level", "debug", "status"])).resolves.toBe(
      true,
    );
