...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "aikaclaw config set secrets.providers.vault --provider-source file --provider-path /etc/aikaclaw/secrets.json --provider-mode json",
);
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("aikaclaw doctor")}\` ${message}`;
}
...
function collectDryRunSchemaErrors(config: AikaClawConfig): ConfigSetDryRunError[] {
...
export async function runConfigSet(opts: {
  ...
  try {
    ...
    const nextConfig = next as AikaClawConfig;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "aikaclaw.json";
...
export function registerConfigCli(program: Command) {
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing aikaclaw.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
