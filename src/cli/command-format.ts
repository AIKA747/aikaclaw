...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+aikaclaw\b|^aikaclaw\b/;
const CONTAINER_FLAG_RE = /(?:^|\s)--container(?:\s|=|$)/;
const PROFILE_FLAG_RE = /(?:^|\s)--profile(?:\s|=|$)/;
const DEV_FLAG_RE = /(?:^|\s)--dev(?:\s|$)/;
const UPDATE_COMMAND_RE =
  /^(?:pnpm|npm|bunx|npx)\s+aikaclaw\b.*(?:^|\s)update(?:\s|$)|^aikaclaw\b.*(?:^|\s)update(?:\s|$)/;
...
export function formatCliCommand(
  ...
  const container = env.AIKACLAW_CONTAINER_HINT?.trim();
  const profile = normalizeProfileName(env.AIKACLAW_PROFILE);
