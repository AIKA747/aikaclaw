...
import type { ConfigFileSnapshot, AikaClawConfig } from "../config/types.js";
import { createCliRuntimeCapture, mockRuntimeModule } from "./test-runtime-capture.js";

/**
 * Test for issue #6070:
 * `aikaclaw config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: AikaClawConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});
const mockResolveSecretRefValue = vi.fn();

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: AikaClawConfig, options?: { unsetPaths?: string[][] }) =>
...
function buildSnapshot(params: {
  resolved: AikaClawConfig;
  config: AikaClawConfig;
}): ConfigFileSnapshot {
  return {
    path: "/tmp/aikaclaw.json",
    exists: true,
...
function setSnapshot(resolved: AikaClawConfig, config: AikaClawConfig) {
...
function withRuntimeDefaults(resolved: AikaClawConfig): AikaClawConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
function makeInvalidSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/custom-aikaclaw.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: {},
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: AikaClawConfig = {
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__AIKACLAW_REDACTED__");
    });
  });
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      ...
      expect(payload.path).toBe("/tmp/custom-aikaclaw.json");
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: AikaClawConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
    ...
    it("shows --strict-json and keeps --json as a legacy alias in help", async () => {
      ...
      expect(helpText).toContain("aikaclaw config set gateway.port 19001 --strict-json");
      expect(helpText).toContain(
        "aikaclaw config set channels.discord.token --ref-provider default --ref-source",
      );
  ...
  describe("config set builders and dry-run", () => {
    ...
    it("rejects --allow-exec without --dry-run", async () => {
      const nonexistentBatchPath = path.join(
        os.tmpdir(),
        `aikaclaw-config-batch-nonexistent-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      await expect(
    ...
    it("supports batch-file mode", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `aikaclaw-config-batch-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
    ...
    it("rejects malformed batch-file payloads", async () => {
      const pathname = path.join(
        os.tmpdir(),
        `aikaclaw-config-batch-invalid-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(pathname, '{"path":"gateway.auth.mode","value":"token"}', "utf8");
      try {
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/aikaclaw.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });
    ...
    it("handles config file path with home directory", async () => {
      ...
      snapshot.path = "/home/user/.aikaclaw/aikaclaw.json";
      mockReadConfigFileSnapshot.mockResolvedValueOnce(snapshot);

      await runConfigCommand(["config", "file"]);

      expect(mockLog).toHaveBeenCalledWith("/home/user/.aikaclaw/aikaclaw.json");
    });
  });
});
