...
describe("registerQrCli", () => {
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("aikaclaw devices approve <requestId>");
  });
  ...
  it("uses AIKACLAW_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("AIKACLAW_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
