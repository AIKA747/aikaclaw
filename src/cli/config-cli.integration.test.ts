...
describe("config cli integration", () => {
  it("supports batch-file dry-run and then writes real config changes", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-config-cli-int-"));
    const configPath = path.join(tempDir, "aikaclaw.json");
  ...
  it("keeps file unchanged when real-file dry-run fails and reports JSON error payload", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-config-cli-int-fail-"));
    const configPath = path.join(tempDir, "aikaclaw.json");
    const envSnapshot = captureEnv([
      "AIKACLAW_CONFIG_PATH",
      "AIKACLAW_TEST_FAST",
  ...
  it("skips exec provider execution during dry-run by default", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-config-cli-int-exec-skip-"));
    const configPath = path.join(tempDir, "aikaclaw.json");
    const batchPath = path.join(tempDir, "batch.json");
    const markerPath = path.join(tempDir, "marker.txt");
    const envSnapshot = captureEnv(["AIKACLAW_CONFIG_PATH", "AIKACLAW_TEST_FAST"]);
    try {
      fs.writeFileSync(
        configPath,
        `${JSON.stringify(
  ...
  it("executes exec providers during dry-run when --allow-exec is set", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-config-cli-int-exec-allow-"));
    const configPath = path.join(tempDir, "aikaclaw.json");
    const batchPath = path.join(tempDir, "batch.json");
    const markerPath = path.join(tempDir, "marker.txt");
    const envSnapshot = captureEnv(["AIKACLAW_CONFIG_PATH", "AIKACLAW_TEST_FAST"]);
