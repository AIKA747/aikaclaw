...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-bundled-skills-test-"));
    process.env.AIKACLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
