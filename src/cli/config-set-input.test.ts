...
describe("config set input parsing", () => {
  ...
  it("parses valid --batch-file payloads", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-config-set-input-"));
  ...
  it("rejects malformed --batch-file payloads", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-config-set-input-invalid-"));
    const batchPath = path.join(tempDir, "batch.json");
