...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.aikaclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.AIKACLAW_PROFILE = profile;

  const stateDir = env.AIKACLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.AIKACLAW_STATE_DIR?.trim()) {
    env.AIKACLAW_STATE_DIR = stateDir;
  }

  if (!env.AIKACLAW_CONFIG_PATH?.trim()) {
    env.AIKACLAW_CONFIG_PATH = path.join(stateDir, "aikaclaw.json");
  }

  if (profile === "dev" && !env.AIKACLAW_GATEWAY_PORT?.trim()) {
    env.AIKACLAW_GATEWAY_PORT = "19001";
  }
}
