    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aikaclaw memory status", "Show index and provider status."],
          ["aikaclaw memory status --deep", "Probe embedding provider readiness."],
          ["aikaclaw memory index --force", "Force a full reindex."],
          ['aikaclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'aikaclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["aikaclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.aikaclaw.ai/cli/memory")}\n`,
    );
