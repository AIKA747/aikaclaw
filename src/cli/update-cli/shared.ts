...
export function normalizeTag(value?: string | null): string | null {
  return normalizePackageTagInput(value, ["aikaclaw", DEFAULT_PACKAGE_NAME]);
}
...
export function resolveGitInstallDir(): string {
  const override = process.env.AIKACLAW_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  ...
  if (home.startsWith("/")) {
    return path.posix.join(home, "aikaclaw");
  }
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveAikaClawPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", AIKACLAW_REPO_URL, params.dir],
      env: gitEnv,
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `AIKACLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set AIKACLAW_GIT_DIR to an empty folder or an aikaclaw checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`AIKACLAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "aikaclaw.mjs");
