...
vi.mock("../infra/path-env.js", () => ({
  ensureAikaClawCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "aikaclaw", "status"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    await runCli(["node", "aikaclaw", "--help"]);

    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "aikaclaw", "--help"]);
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "aikaclaw", "--container", "demo", "status"]);
