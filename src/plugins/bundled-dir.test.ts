...
const originalBundledDir = process.env.AIKACLAW_BUNDLED_PLUGINS_DIR;
...
afterEach(() => {
  ...
  if (originalBundledDir === undefined) {
    ...
    process.env.AIKACLAW_BUNDLED_PLUGINS_DIR = originalBundledDir;
  }
...
describe("resolveBundledPluginsDir", () => {
  it("prefers the staged runtime bundled plugin tree from the package root", () => {
    const repoRoot = makeRepoRoot("aikaclaw-bundled-dir-runtime-");
    ...
    fs.writeFileSync(
      ...
      `${JSON.stringify({ name: "aikaclaw" }, null, 2)}\n`,
      "utf8",
    );
  ...
  it("falls back to built dist/extensions in installed package roots", () => {
    const repoRoot = makeRepoRoot("aikaclaw-bundled-dir-dist-");
    fs.mkdirSync(path.join(repoRoot, "dist", "extensions"), { recursive: true });
    fs.writeFileSync(
      path.join(repoRoot, "package.json"),
      `${JSON.stringify({ name: "aikaclaw" }, null, 2)}\n`,
  ...
  it("prefers source extensions under vitest to avoid stale staged plugins", () => {
    const repoRoot = makeRepoRoot("aikaclaw-bundled-dir-vitest-");
  ...
  it("prefers source extensions in a git checkout even without vitest env", () => {
    const repoRoot = makeRepoRoot("aikaclaw-bundled-dir-git-");
    fs.mkdirSync(path.join(repoRoot, "extensions"), { recursive: true });
