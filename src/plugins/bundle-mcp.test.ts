...
describe("loadEnabledBundleMcpConfig", () => {
  it("loads enabled Claude bundle MCP config and absolutizes relative args", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "AIKACLAW_HOME", "AIKACLAW_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("aikaclaw-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("aikaclaw-bundle-mcp-workspace-");
  ...
  it("merges inline bundle MCP servers and skips disabled bundles", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "AIKACLAW_HOME", "AIKACLAW_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("aikaclaw-bundle-inline-home-");
      const workspaceDir = await tempHarness.createTempDir("aikaclaw-bundle-inline-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.AIKACLAW_HOME;
      delete process.env.AIKACLAW_STATE_DIR;

      const enabledRoot = path.join(homeDir, ".aikaclaw", "extensions", "inline-enabled");
      const disabledRoot = path.join(homeDir, ".aikaclaw", "extensions", "inline-disabled");
  ...
  it("resolves inline Claude MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "AIKACLAW_HOME", "AIKACLAW_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("aikaclaw-bundle-inline-placeholder-home-");
      const workspaceDir = await tempHarness.createTempDir(
        "aikaclaw-bundle-inline-placeholder-workspace-",
      );
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.AIKACLAW_HOME;
      ...
      const pluginRoot = path.join(homeDir, ".aikaclaw", "extensions", "inline-claude");
