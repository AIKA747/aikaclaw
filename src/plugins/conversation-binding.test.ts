...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-plugin-binding-"));
...
vi.mock("../infra/home-dir.js", async (importOriginal) => {
  ...
  return {
    ...
    expandHomePrefix: (value: string) => {
      if (value === "~/.aikaclaw/plugin-binding-approvals.json") {
...
describe("plugin conversation binding approvals", () => {
  ...
  it("migrates a legacy codex thread binding session key through the new approval flow", async () => {
    sessionBindingState.setRecord({
      ...
      targetSessionKey: "aikaclaw-app-server:thread:019ce411-6322-7db2-a821-1a61c530e7d9",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
