...
export async function runProviderPluginAuthMethod(params: {
  ...
  opts?: Partial<ProviderAuthOptionBag>;
}): Promise<{ config: AikaClawConfig; defaultModel?: string }> {
...
export async function applyAuthChoicePluginProvider(
  ...
  const agentDir =
    params.agentDir ??
    (agentId === defaultAgentId ? resolveAikaClawAgentDir() : resolveAgentDir(nextConfig, agentId));
  const workspaceDir =
