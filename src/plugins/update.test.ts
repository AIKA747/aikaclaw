...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("skips integrity drift checks for unpinned npm specs during dry-run updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "opik-aikaclaw",
      targetDir: "/tmp/opik-aikaclaw",
      version: "0.2.6",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-aikaclaw": {
              source: "npm",
              spec: "@opik/opik-aikaclaw",
              integrity: "sha512-old",
              installPath: "/tmp/opik-aikaclaw",
            },
          },
        },
      },
      pluginIds: ["opik-aikaclaw"],
      dryRun: true,
    });
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @aikaclaw/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @aikaclaw/missing.",
      },
    ]);
  });
  ...
  it("reuses a recorded npm dist-tag spec for id-based updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "aikaclaw-codex-app-server",
      targetDir: "/tmp/aikaclaw-codex-app-server",
        ...
        plugins: {
          installs: {
            "aikaclaw-codex-app-server": {
              ...
              resolvedSpec: "aikaclaw-codex-app-server@0.2.0-beta.3",
            },
          },
        },
      },
    ...
    expect(installPluginFromNpmSpecMock).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "aikaclaw-codex-app-server@beta",
        expectedPluginId: "aikaclaw-codex-app-server",
      }),
    );
    expect(result.config.plugins?.installs?.["aikaclaw-codex-app-server"]).toMatchObject({
      source: "npm",
      spec: "aikaclaw-codex-app-server@beta",
      installPath: "/tmp/aikaclaw-codex-app-server",
      version: "0.2.0-beta.4",
  ...
  it("uses and persists an explicit npm spec override during updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      extensions: ["index.ts"],
      npmResolution: {
        name: "aikaclaw-codex-app-server",
        version: "0.2.0-beta.4",
        resolvedSpec: "aikaclaw-codex-app-server@0.2.0-beta.4",
      },
    });
  ...
  it("skips recorded integrity checks when an explicit npm version override changes the spec", async () => {
      ...
      config: {
        plugins: {
          installs: {
            "aikaclaw-codex-app-server": {
              source: "npm",
              spec: "aikaclaw-codex-app-server@0.2.0-beta.3",
              integrity: "sha512-old",
              installPath: "/tmp/aikaclaw-codex-app-server",
            },
          },
    ...
    expect(installPluginFromNpmSpecMock).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "aikaclaw-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      }),
    );
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@aikaclaw/voice-call",
      targetDir: "/tmp/aikaclaw-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@aikaclaw/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@aikaclaw/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@aikaclaw/voice-call");
    expect(result.config.plugins?.entries?.["@aikaclaw/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@aikaclaw/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@aikaclaw/voice-call",
      installPath: "/tmp/aikaclaw-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    ...
    const env = { AIKACLAW_HOME: "/srv/aikaclaw-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/aikaclaw-home";
    resolveBundledPluginSourcesMock.mockReturnValue(
      new Map([
        [
          "feishu",
