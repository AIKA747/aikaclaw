...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AikaClawPluginDefinition;
  register?: AikaClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AikaClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AikaClawPluginDefinition;
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.AIKACLAW_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverAikaClawPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverAikaClawPlugins>["candidates"][number];
  right: ReturnType<typeof discoverAikaClawPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadAikaClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadAikaClawPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into AikaClaw yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: AikaClawPluginModule | null = null;
    try {
      mod = getJiti(safeSource)(safeSource) as AikaClawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
