...
function readBundledPluginRecords(): BundledPluginRecord[] {
  ...
  for (const dirName of fs.readdirSync(EXTENSIONS_ROOT).toSorted()) {
    ...
    const manifestPath = path.join(rootDir, "aikaclaw.plugin.json");
    if (!fs.existsSync(packagePath) || !fs.existsSync(manifestPath)) {
      continue;
    }

    const manifest = readJsonFile<PluginManifestShape>(manifestPath);
    const pkg = readJsonFile<AikaClawPackageShape>(packagePath);
    ...
    records.push({
      ...
      packageName,
      manifestId,
      installNpmSpec: normalizeText(pkg.aikaclaw?.install?.npmSpec),
      channelId: normalizeText(pkg.aikaclaw?.channel?.id),
    });
  }
  return records;
}
...
function resolveAllowedPackageNamesForId(pluginId: string): string[] {
  return ALLOWED_PACKAGE_SUFFIXES.map((suffix) => `@aikaclaw/${pluginId}${suffix}`);
}
...
describe("bundled plugin naming guardrails", () => {
  it("keeps bundled workspace package names anchored to the plugin id", () => {
    ...
    expect(
      mismatches,
      `Bundled extension package names must stay anchored to the manifest id via @aikaclaw/<id> or an approved suffix (${ALLOWED_PACKAGE_SUFFIXES.join(", ")}). Update the plugin naming docs and this invariant before adding a new naming form.\nFound: ${mismatches.join(", ") || "<none>"}`,
    ).toEqual([]);
  });
  ...
  it("keeps bundled workspace directories aligned with the plugin id unless explicitly allowlisted", () => {
    ...
    expect(
      mismatches,
      `Bundled extension directory names should match aikaclaw.plugin.json:id. If a legacy exception is unavoidable, add it to DIR_ID_EXCEPTIONS with a comment.\nFound: ${mismatches.join(", ") || "<none>"}`,
    ).toEqual([]);
  });

  it("keeps bundled aikaclaw.install.npmSpec aligned with the package name", () => {
    const mismatches = readBundledPluginRecords()
      .filter(
    ...
    expect(
      mismatches,
      `Bundled aikaclaw.install.npmSpec values must match the package name so install/update paths stay deterministic.\nFound: ${mismatches.join(", ") || "<none>"}`,
    ).toEqual([]);
  });
  ...
  it("keeps bundled channel ids aligned with the canonical plugin id", () => {
    ...
    expect(
      mismatches,
      `Bundled aikaclaw.channel.id values must match aikaclaw.plugin.json:id for the owning plugin.\nFound: ${mismatches.join(", ") || "<none>"}`,
    ).toEqual([]);
  });
});
