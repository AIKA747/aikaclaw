...
import { clearPluginDiscoveryCache, discoverAikaClawPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("aikaclaw-plugins", tempDirs);
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverAikaClawPlugins>[0],
) {
  return discoverAikaClawPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      aikaclaw: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverAikaClawPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".aikaclaw", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".aikaclaw", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverAikaClawPlugins({
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@aikaclaw/voice-call",
  ...
  it("strips provider suffixes from package-derived ids", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@aikaclaw/ollama-provider",
      extensions: ["./src/index.ts"],
    });
    fs.writeFileSync(
      path.join(globalExt, "src", "index.ts"),
  ...
  it("normalizes bundled speech package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@aikaclaw/elevenlabs-speech",
      extensions: ["./src/index.ts"],
    });
    writePluginPackageManifest({
      packageDir: microsoftDir,
      packageName: "@aikaclaw/microsoft-speech",
      extensions: ["./src/index.ts"],
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@aikaclaw/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("falls back to legacy index discovery when a scanned bundle sidecar is malformed", async () => {
    ...
    expect(legacy?.format).toBe("aikaclaw");
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@aikaclaw/escape-pack",
  ...
  it("skips missing package extension entries without escape diagnostics", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@aikaclaw/missing-entry-pack",
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverAikaClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        AIKACLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    });
    ...
    const second = discoverAikaClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        AIKACLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverAikaClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        AIKACLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
