...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "aikaclaw skills install ${pkg.name}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this AikaClaw runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires AikaClaw >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
  ...
  try {
    ...
    return {
      ...
      clawhub: {
        ...
        clawhubUrl:
          params.baseUrl?.trim() ||
          process.env.AIKACLAW_CLAWHUB_URL?.trim() ||
          "https://clawhub.ai",
