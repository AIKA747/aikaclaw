...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "aikaclaw-sdk-alias-"));
let tempDirIndex = 0;
...
function createPluginSdkAliasFixture(params?: {
  ...
  const packageJson: Record<string, unknown> = {
    name: params?.packageName ?? "aikaclaw",
    type: "module",
  };
  ...
  if (trustedRootIndicatorMode === "bin+marker") {
    fs.writeFileSync(path.join(root, "aikaclaw.mjs"), "export {};\n", "utf-8");
  }
...
function createExtensionApiAliasFixture(params?: { srcBody?: string; distBody?: string }) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "aikaclaw", type: "module" }, null, 2),
    "utf-8",
  );
  fs.writeFileSync(path.join(root, "aikaclaw.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(srcFile, params?.srcBody ?? "export {};\n", "utf-8");
...
describe("plugin sdk alias helpers", () => {
  it.each([
    ...
    },
    {
      ...
      modulePath: () => "/tmp/tsx-cache/aikaclaw-loader.js",
      argv1: (root: string) => path.join(root, "aikaclaw.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
  ...
  it("does not derive plugin-sdk subpaths from cwd fallback when package root is not an AikaClaw root", () => {
    const fixture = createPluginSdkAliasFixture({
      packageName: "moltbot",
  ...
  it("builds plugin-sdk aliases from the module being loaded, not the loader location", () => {
    ...
    expect(fs.realpathSync(sourceAliases["aikaclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(sourceAliases["aikaclaw/plugin-sdk/channel-runtime"] ?? "")).toBe(
    ...
    expect(fs.realpathSync(distAliases["aikaclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(distRootAlias),
    );
    expect(fs.realpathSync(distAliases["aikaclaw/plugin-sdk/channel-runtime"] ?? "")).toBe(
      fs.realpathSync(path.join(fixture.root, "dist", "plugin-sdk", "channel-runtime.js")),
    );
  });

  it("resolves plugin-sdk aliases for user-installed plugins via the running aikaclaw argv hint", () => {
    const fixture = createPluginSdkAliasFixture({
      srcFile: "channel-runtime.ts",
      distFile: "channel-runtime.js",
      packageExports: {
    ...
    const sourceRootAlias = path.join(fixture.root, "src", "plugin-sdk", "root-alias.cjs");
    fs.writeFileSync(sourceRootAlias, "module.exports = {};\n", "utf-8");
    const externalPluginRoot = path.join(makeTempDir(), ".aikaclaw", "extensions", "demo");
    ...
    const aliases = withCwd(externalPluginRoot, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(externalPluginEntry, path.join(fixture.root, "aikaclaw.mjs")),
      ),
    );

    expect(fs.realpathSync(aliases["aikaclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(aliases["aikaclaw/plugin-sdk/channel-runtime"] ?? "")).toBe(
      fs.realpathSync(path.join(fixture.root, "src", "plugin-sdk", "channel-runtime.ts")),
    );
  });

  it("does not resolve plugin-sdk alias files from cwd fallback when package root is not an AikaClaw root", () => {
    const fixture = createPluginSdkAliasFixture({
      srcFile: "channel-runtime.ts",
      distFile: "channel-runtime.js",
      packageName: "moltbot",
  ...
  it("loads source runtime shims through the non-native Jiti boundary", async () => {
    ...
    fs.writeFileSync(
      ...
      `import { resolveOutboundSendDep } from "aikaclaw/plugin-sdk/infra-runtime";
...
`,
    ...
    const withAlias = createJiti(jitiBaseUrl, {
      ...buildPluginLoaderJitiOptions({
        "aikaclaw/plugin-sdk/infra-runtime": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
