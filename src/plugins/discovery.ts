...
function resolveDiscoveryCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AIKACLAW_PLUGIN_DISCOVERY_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseDiscoveryCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.AIKACLAW_DISABLE_PLUGIN_DISCOVERY_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @aikaclaw/voice-call -> voice-call).
    ...
    addCandidate({
      ...
      manifest: {
        ...
        ...(!packageManifest?.description && entry.packageDescription
          ...
          : {}),
        ...(!packageManifest?.aikaclaw && entry.packageManifest
          ? { aikaclaw: entry.packageManifest }
          : {}),
      },
      packageDir: rootDir,
      bundledManifest: entry.manifest,
      bundledManifestPath: path.join(rootDir, "aikaclaw.plugin.json"),
    });
  }
...
export function discoverAikaClawPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ownershipUid?: number | null;
  cache?: boolean;
