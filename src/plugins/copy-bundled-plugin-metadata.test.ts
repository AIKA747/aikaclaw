...
const excludeOptionalEnv = { AIKACLAW_INCLUDE_OPTIONAL_BUNDLED: "0" } as const;
...
describe("copyBundledPluginMetadata", () => {
  it("copies plugin manifests, package metadata, and local skill directories", () => {
    const repoRoot = makeRepoRoot("aikaclaw-bundled-plugin-meta-");
    ...
    writeJson(path.join(pluginDir, "aikaclaw.plugin.json"), {
      id: "acpx",
    ...
    expect(
      fs.existsSync(path.join(repoRoot, "dist", "extensions", "acpx", "aikaclaw.plugin.json")),
    ).toBe(true);
    expect(
      fs.readFileSync(
    ...
    const bundledManifest = JSON.parse(
      fs.readFileSync(
        path.join(repoRoot, "dist", "extensions", "acpx", "aikaclaw.plugin.json"),
        "utf8",
      ),
    ) as { skills?: string[] };
    ...
    ) as { aikaclaw?: { extensions?: string[] } };
    expect(packageJson.aikaclaw?.extensions).toEqual(["./index.js"]);
  });

  it("relocates node_modules-backed skill paths into bundled-skills and rewrites the manifest", () => {
    const repoRoot = makeRepoRoot("aikaclaw-bundled-plugin-node-modules-");
    ...
    const bundledManifest = JSON.parse(
      fs.readFileSync(
        path.join(repoRoot, "dist", "extensions", "tlon", "aikaclaw.plugin.json"),
        "utf8",
      ),
    ) as { skills?: string[] };
  ...
  it("falls back to repo-root hoisted node_modules skill paths", () => {
    const repoRoot = makeRepoRoot("aikaclaw-bundled-plugin-hoisted-skill-");
    const pluginDir = path.join(repoRoot, "extensions", "tlon");
  ...
  it("omits missing declared skill paths and removes stale generated outputs", () => {
    const repoRoot = makeRepoRoot("aikaclaw-bundled-plugin-missing-skill-");
    const pluginDir = path.join(repoRoot, "extensions", "tlon");
    fs.mkdirSync(pluginDir, { recursive: true });
    writeJson(path.join(pluginDir, "aikaclaw.plugin.json"), {
      id: "tlon",
  ...
  it("retries transient skill copy races from concurrent runtime postbuilds", () => {
    const repoRoot = makeRepoRoot("aikaclaw-bundled-plugin-retry-");
  ...
  it("removes generated outputs for plugins no longer present in source", () => {
    const repoRoot = makeRepoRoot("aikaclaw-bundled-plugin-removed-");
    const staleBundledSkillDir = path.join(
      repoRoot,
      "dist",
      "extensions",
    ...
    writeJson(path.join(repoRoot, "dist", "extensions", "removed-plugin", "aikaclaw.plugin.json"), {
      id: "removed-plugin",
      configSchema: { type: "object" },
      skills: ["./bundled-skills/@scope/skill"],
    });
    writeJson(path.join(repoRoot, "dist", "extensions", "removed-plugin", "package.json"), {
      name: "@aikaclaw/removed-plugin",
    });
  ...
  it("removes stale dist outputs when a source extension directory no longer has a manifest", () => {
    const repoRoot = makeRepoRoot("aikaclaw-bundled-plugin-manifestless-source-");
    ...
    writeJson(path.join(staleDistDir, "aikaclaw.plugin.json"), {
      id: "google-gemini-cli-auth",
      configSchema: { type: "object" },
    });
    writeJson(path.join(staleDistDir, "package.json"), {
      name: "@aikaclaw/google-gemini-cli-auth",
    });
  ...
  it("skips metadata for optional bundled clusters only when explicitly disabled", () => {
    const repoRoot = makeRepoRoot("aikaclaw-bundled-plugin-optional-skip-");
    const pluginDir = path.join(repoRoot, "extensions", "acpx");
    fs.mkdirSync(pluginDir, { recursive: true });
    writeJson(path.join(pluginDir, "aikaclaw.plugin.json"), {
      id: "acpx",
  ...
  it("still bundles previously released optional plugins without the opt-in env", () => {
    const repoRoot = makeRepoRoot("aikaclaw-bundled-plugin-released-optional-");
    ...
    writeJson(path.join(pluginDir, "package.json"), {
      name: "@aikaclaw/whatsapp",
      aikaclaw: {
        extensions: ["./index.ts"],
        install: { npmSpec: "@aikaclaw/whatsapp" },
      },
    });
