...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("aikaclaw-bundle-commands-home-");
      const workspaceDir = await tempHarness.createTempDir("aikaclaw-bundle-commands-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.AIKACLAW_HOME;
      delete process.env.AIKACLAW_STATE_DIR;

      const pluginRoot = path.join(homeDir, ".aikaclaw", "extensions", "compound-bundle");
