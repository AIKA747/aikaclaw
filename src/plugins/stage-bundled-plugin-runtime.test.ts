...
import { discoverAikaClawPlugins } from "./discovery.js";
...
describe("stageBundledPluginRuntime", () => {
  it("stages bundled dist plugins as runtime wrappers and links staged dist node_modules", () => {
    const repoRoot = makeRepoRoot("aikaclaw-stage-bundled-runtime-");
  ...
  it("writes wrappers that forward plugin entry imports into canonical dist files", async () => {
    const repoRoot = makeRepoRoot("aikaclaw-stage-bundled-runtime-chunks-");
  ...
  it("stages root runtime sidecars that bundled plugin boundaries resolve directly", () => {
    const repoRoot = makeRepoRoot("aikaclaw-stage-bundled-runtime-sidecars-");
  ...
  it("keeps plugin command registration on the canonical dist graph when loaded from dist-runtime", async () => {
    const repoRoot = makeRepoRoot("aikaclaw-stage-bundled-runtime-commands-");
    ...
    fs.writeFileSync(
      ...
      [
        "const registry = globalThis.__aikaclawTestPluginCommands ??= new Map();",
  ...
  it("copies package metadata files but symlinks other non-js plugin artifacts into the runtime overlay", () => {
    const repoRoot = makeRepoRoot("aikaclaw-stage-bundled-runtime-assets-");
    const distPluginDir = path.join(repoRoot, "dist", "extensions", "diffs");
    fs.mkdirSync(path.join(distPluginDir, "assets"), { recursive: true });
    fs.writeFileSync(
      path.join(distPluginDir, "package.json"),
      JSON.stringify(
        { name: "@aikaclaw/diffs", aikaclaw: { extensions: ["./index.js"] } },
        null,
        2,
      ),
      "utf8",
    );
    fs.writeFileSync(path.join(distPluginDir, "aikaclaw.plugin.json"), "{}\n", "utf8");
  ...
  it("preserves package metadata needed for bundled plugin discovery from dist-runtime", () => {
    const repoRoot = makeRepoRoot("aikaclaw-stage-bundled-runtime-discovery-");
    const distPluginDir = path.join(repoRoot, "dist", "extensions", "demo");
    ...
    fs.writeFileSync(
      path.join(distPluginDir, "aikaclaw.plugin.json"),
      JSON.stringify(
        {
          id: "demo",
          channels: ["demo"],
    ...
    const env = {
      ...process.env,
      AIKACLAW_BUNDLED_PLUGINS_DIR: runtimeExtensionsDir,
    };
    const discovery = discoverAikaClawPlugins({
      env,
      cache: false,
    });
  ...
  it("removes stale runtime plugin directories that are no longer in dist", () => {
    const repoRoot = makeRepoRoot("aikaclaw-stage-bundled-runtime-stale-");
  ...
  it("removes dist-runtime when the built bundled plugin tree is absent", () => {
    const repoRoot = makeRepoRoot("aikaclaw-stage-bundled-runtime-missing-");
  ...
  it("tolerates EEXIST when an identical runtime symlink is materialized concurrently", () => {
    const repoRoot = makeRepoRoot("aikaclaw-stage-bundled-runtime-eexist-");
