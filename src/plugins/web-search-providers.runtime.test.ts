...
let loadAikaClawPluginsMock: ReturnType<typeof vi.fn>;
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadPluginManifestRegistryMock = vi
      ...
      .mockReturnValue({
        plugins: [
          {
            ...
            manifestPath: "/tmp/brave/aikaclaw.plugin.json",
            channels: [],
            providers: [],
            skills: [],
            hooks: [],
          ...
          },
          {
            ...
            manifestPath: "/tmp/noise/aikaclaw.plugin.json",
            channels: [],
            providers: [],
            skills: [],
            hooks: [],
        ...
        ? R
        : never);
    loadAikaClawPluginsMock = vi
      .spyOn(loaderModule, "loadAikaClawPlugins")
  ...
  it("loads bundled providers through the plugin loader in alphabetical order", () => {
    ...
    expect(loadAikaClawPluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("scopes plugin loading to manifest-declared web-search candidates", () => {
    ...
    expect(loadAikaClawPluginsMock).toHaveBeenCalledWith(
  ...
  it("memoizes snapshot provider resolution for the same config and env", () => {
    ...
    const env = { AIKACLAW_HOME: "/tmp/aikaclaw-home" } as NodeJS.ProcessEnv;
  ...
  it("invalidates the snapshot cache when config or env contents change in place", () => {
    ...
    const env = {
      AIKACLAW_HOME: "/tmp/aikaclaw-home-a",
    ...
    env.AIKACLAW_HOME = "/tmp/aikaclaw-home-b";
    resolvePluginWebSearchProviders({
      config,
      env,
      bundledAllowlistCompat: true,
    ...
    expect(loadAikaClawPluginsMock).toHaveBeenCalledTimes(2);
  });
  ...
  it("skips web-search snapshot memoization when plugin cache opt-outs are set", () => {
    ...
    const env = {
      AIKACLAW_HOME: "/tmp/aikaclaw-home",
  ...
  it("skips web-search snapshot memoization when discovery cache ttl is zero", () => {
    ...
    const env = {
      AIKACLAW_HOME: "/tmp/aikaclaw-home",
      AIKACLAW_PLUGIN_DISCOVERY_CACHE_MS: "0",
    } as NodeJS.ProcessEnv;
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const env = {
      AIKACLAW_HOME: "/tmp/aikaclaw-home",
      AIKACLAW_PLUGIN_DISCOVERY_CACHE_MS: "5",
      AIKACLAW_PLUGIN_MANIFEST_CACHE_MS: "20",
    } as NodeJS.ProcessEnv;
  ...
  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    ...
    const env = {
      AIKACLAW_HOME: "/tmp/aikaclaw-home",
      AIKACLAW_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    } as NodeJS.ProcessEnv;
    ...
    env.AIKACLAW_PLUGIN_DISCOVERY_CACHE_MS = "5";
