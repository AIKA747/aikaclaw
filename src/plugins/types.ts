 ...
 */
export type AikaClawPluginConfigSchema = {
...
export type AikaClawPluginToolContext = {
...
export type AikaClawPluginToolFactory = (
  ctx: AikaClawPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type AikaClawPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type AikaClawPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<AikaClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). AikaClaw uses this to expose
 ...
 *
 * Runs after AikaClaw resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common AikaClaw auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and AikaClaw wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before AikaClaw builds its generic
...
/**
 * Provider-owned stream wrapper hook after AikaClaw applies its generic
 ...
 *
 * Return `true` or `false` to override AikaClaw's built-in provider cache TTL
 ...
 *
 * Runs only after AikaClaw exhausts normal env/profile/config auth resolution
 ...
 *
 * Runs after AikaClaw loads the discovered model catalog and merges configured
...
export type ProviderPlugin = {
   ...
   *
   * AikaClaw calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic AikaClaw wrappers.
   *
   ...
   *
   * Called after AikaClaw resolves the raw configured credential but before the
   ...
   * hint than AikaClaw's generic auth-store guidance.
   */
   ...
   * `errorMessage` when AikaClaw should surface a provider-specific hint for
   ...
   * merging. AikaClaw deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * AikaClaw uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `aikaclaw doctor`.
   *
   ...
   *
   * AikaClaw calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep AikaClaw's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
  ...
  getConfiguredCredentialValue?: (config?: AikaClawConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: AikaClawConfig, value: unknown) => void;
  applySelectionConfig?: (config: AikaClawConfig) => AikaClawConfig;
...
export type AikaClawPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  commandBody: string;
  /** Current AikaClaw configuration */
  config: AikaClawConfig;
 ...
 */
export type AikaClawPluginCommandDefinition = {
...
export type AikaClawPluginHttpRouteAuth = "gateway" | "plugin";
export type AikaClawPluginHttpRouteMatch = "exact" | "prefix";

export type AikaClawPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type AikaClawPluginHttpRouteParams = {
  path: string;
  handler: AikaClawPluginHttpRouteHandler;
  auth: AikaClawPluginHttpRouteAuth;
  match?: AikaClawPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type AikaClawPluginCliContext = {
  program: Command;
  config: AikaClawConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type AikaClawPluginCliRegistrar = (ctx: AikaClawPluginCliContext) => void | Promise<void>;

/** Context passed to long-lived plugin services. */
export type AikaClawPluginServiceContext = {
  config: AikaClawConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type AikaClawPluginService = {
  id: string;
  start: (ctx: AikaClawPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: AikaClawPluginServiceContext) => void | Promise<void>;
};

export type AikaClawPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type AikaClawPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  register?: (api: AikaClawPluginApi) => void | Promise<void>;
  activate?: (api: AikaClawPluginApi) => void | Promise<void>;
};

export type AikaClawPluginModule =
  | AikaClawPluginDefinition
  | ((api: AikaClawPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime";

/** Main registration API injected into native plugin entry files. */
export type AikaClawPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | AikaClawPluginToolFactory,
    opts?: AikaClawPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: AikaClawPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: AikaClawPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: AikaClawPluginChannelRegistration | ChannelPlugin) => void;
  registerGatewayMethod: (method: string, handler: GatewayRequestHandler) => void;
  registerCli: (registrar: AikaClawPluginCliRegistrar, opts?: { commands?: string[] }) => void;
  registerService: (service: AikaClawPluginService) => void;
   ...
   */
  registerCommand: (command: AikaClawPluginCommandDefinition) => void;
...
export type PluginFormat = "aikaclaw" | "bundle";
