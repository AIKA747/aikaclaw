...
const DYNAMIC_ARCHIVE_TEMPLATE_PRESETS = [
  {
    ...
    packageJson: {
      name: "@evil/..",
      version: "0.0.1",
      aikaclaw: { extensions: ["./dist/index.js"] },
...
function ensureSuiteTempRoot() {
  ...
  suiteTempRoot = fs.mkdtempSync(path.join(bundleTempRoot, "aikaclaw-plugin-install-"));
  return suiteTempRoot;
}
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "aikaclaw.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@aikaclaw/voice-call" });
  ...
  it("installs from a zip archive", async () => {
    ...
    expectSuccessfulArchiveInstall({ result, stateDir, pluginId: "@aikaclaw/zipper" });
  });

  it("installs flat-root plugin archives from ClawHub-style downloads", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: {
        name: "@aikaclaw/rootless",
        version: "0.0.1",
        aikaclaw: { extensions: ["./dist/index.js"] },
      },
  ...
  it("rejects packages without aikaclaw.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@aikaclaw/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("aikaclaw.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_AIKACLAW_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when aikaclaw.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@aikaclaw/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing aikaclaw.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_AIKACLAW_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without aikaclaw.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.aikaclaw).toBeUndefined();
  ...
  it("rejects plugins whose minHostVersion is newer than the current host", async () => {
    vi.stubEnv("AIKACLAW_VERSION", "2026.3.21");
    const { pluginDir, extensionsDir } = setupInstallPluginFromDirFixture();
    const packageJsonPath = path.join(pluginDir, "package.json");
    const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
      aikaclaw?: { install?: Record<string, unknown> };
    };
    manifest.aikaclaw = {
      ...manifest.aikaclaw,
      install: {
        ...manifest.aikaclaw?.install,
    ...
    expect(result.error).toContain("requires AikaClaw >=2026.3.22, but this host is 2026.3.21");
  ...
  it("rejects plugins with invalid minHostVersion metadata", async () => {
    ...
    expect(result.error).toContain("invalid package.json aikaclaw.install.minHostVersion");
    expect(vi.mocked(runCommandWithTimeout)).not.toHaveBeenCalled();
  });

  it("reports unknown host versions distinctly for minHostVersion-gated plugins", async () => {
    vi.stubEnv("AIKACLAW_VERSION", "unknown");
    const { pluginDir, extensionsDir } = setupInstallPluginFromDirFixture();
    const packageJsonPath = path.join(pluginDir, "package.json");
    const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
      aikaclaw?: { install?: Record<string, unknown> };
  ...
  it("uses aikaclaw.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@aikaclaw/cognee-aikaclaw"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it("keeps scoped install ids aligned across manifest and package-name cases", async () => {
    const scenarios = [
      ...
      },
      {
        ...
        expectedPluginId: "@aikaclaw/test-plugin",
        install: (pluginDir: string, extensionsDir: string) =>
          installPluginFromDir({
            dirPath: pluginDir,
            extensionsDir,
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@aikaclaw/voice-call@0.0.1",
              name: "@aikaclaw/voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@aikaclaw/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@aikaclaw/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@aikaclaw/voice-call@0.0.1",
    });
  ...
  it("handles prerelease npm specs correctly", async () => {
    const prereleaseMetadata = {
      id: "@aikaclaw/voice-call@0.0.2-beta.1",
      name: "@aikaclaw/voice-call",
...

    {
      ...
      if (!result.ok) {
        ...
        expect(result.error).toContain('"@aikaclaw/voice-call@beta"');
      }
    }
...

    {
      ...
      expect(result.npmResolution?.resolvedSpec).toBe("@aikaclaw/voice-call@0.0.2-beta.1");
      expectSingleNpmPackIgnoreScriptsCall({
        calls: run.mock.calls,
        expectedSpec: "@aikaclaw/voice-call@beta",
      });
      expect(packTmpDir).not.toBe("");
    }
  });
