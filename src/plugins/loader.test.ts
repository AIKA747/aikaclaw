...
import { __testing, clearPluginLoaderCache, loadAikaClawPlugins } from "./loader.js";
...
type PluginLoadConfig = NonNullable<Parameters<typeof loadAikaClawPlugins>[0]>["config"];
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "aikaclaw-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.AIKACLAW_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "aikaclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.AIKACLAW_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadAikaClawPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          aikaclaw: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.AIKACLAW_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.AIKACLAW_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadAikaClawPlugins>) {
...
function useNoBundledPlugins() {
  process.env.AIKACLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadAikaClawPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadAikaClawPlugins>[0], "cache" | "config">,
) {
  return loadAikaClawPlugins({
    cache: false,
    ...options,
...
function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".aikaclaw", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ AIKACLAW_STATE_DIR: stateDir, ...params.env }, () =>
    loadAikaClawPlugins({
      workspaceDir,
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadAikaClawPlugins>,
  pluginId: string,
) {
  expect(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadAikaClawPlugins>;
  loadSecond: () => ReturnType<typeof loadAikaClawPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadAikaClawPlugins>;
  loadVariant: () => ReturnType<typeof loadAikaClawPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const aikaclawHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(aikaclawHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  ...
  const env = {
    ...process.env,
    AIKACLAW_HOME: aikaclawHome,
    HOME: ignoredHome,
    AIKACLAW_STATE_DIR: stateDir,
    AIKACLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.AIKACLAW_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.AIKACLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".aikaclaw", "extensions", "sample-bundle");
    ...
    const registry = withEnv({ AIKACLAW_STATE_DIR: stateDir }, () =>
      loadAikaClawPlugins({
        workspaceDir,
    ...
    const cases = [
      {
        ...
        assert: (registry: ReturnType<typeof loadAikaClawPlugins>) => {
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
      ...
      },
      {
        ...
        run: () => {
          ...
          const full = loadAikaClawPlugins(options);
          const scoped = loadAikaClawPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
          const scopedAgain = loadAikaClawPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadAikaClawPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadAikaClawPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadAikaClawPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadAikaClawPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadAikaClawPlugins(options);
          ...
          loadFirst: () =>
            loadAikaClawPlugins({
              ...
              env: {
                ...
                AIKACLAW_STATE_DIR: stateDir,
                AIKACLAW_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadAikaClawPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(aikaclawHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadAikaClawPlugins(options),
          loadVariant: () =>
            loadAikaClawPlugins({
              ...options,
              runtimeOptions: {
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadAikaClawPlugins({
      env: {
        ...
        HOME: homeDir,
        AIKACLAW_HOME: undefined,
        AIKACLAW_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers AIKACLAW_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const aikaclawHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "aikaclaw-home-demo",
      dir: path.join(aikaclawHome, "plugins", "aikaclaw-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "aikaclaw-home-demo", register() {} };`,
    });
    ...
    const registry = loadAikaClawPlugins({
      ...
      config: {
        plugins: {
          allow: ["aikaclaw-home-demo"],
          entries: {
            "aikaclaw-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/aikaclaw-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "aikaclaw-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadAikaClawPlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadAikaClawPlugins>) => {
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "aikaclaw.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const setupRegistry = loadAikaClawPlugins({
      cache: false,
      config,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@aikaclaw/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@aikaclaw/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@aikaclaw/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@aikaclaw/setup-runtime-not-preferred-test",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "aikaclaw.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "aikaclaw.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const stateDir = makeTempDir();
          return withEnv({ AIKACLAW_STATE_DIR: stateDir }, () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const workspaceDir = makeTempDir();
          const workspaceExtDir = path.join(workspaceDir, ".aikaclaw", "extensions", "shadowed");
          mkdirSafe(workspaceExtDir);
          writePlugin({
            id: "shadowed",
            body: `module.exports = { id: "shadowed", register() {} };`,
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("aikaclaw/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ AIKACLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadAikaClawPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__aikaclawLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("aikaclaw/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("aikaclaw/plugin-sdk");
