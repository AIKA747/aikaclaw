...
const APPROVALS_PATH = "~/.aikaclaw/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "aikaclaw-app-server:thread:",
  "aikaclaw-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("aikaclaw.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("aikaclaw.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to AikaClaw instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
