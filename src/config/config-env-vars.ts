...
function collectConfigEnvVarsByTarget(cfg?: AikaClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: AikaClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: AikaClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: AikaClawConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.AIKACLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
