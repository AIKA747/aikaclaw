...
export const GENERATED_BASE_CONFIG_SCHEMA = {
  ...
  uiHints: {
    ...
    update: {
      ...
      group: "Update",
      order: 25,
      help: "Update-channel and startup-check behavior for keeping AikaClaw runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
      tags: ["advanced"],
    },
    cli: {
      label: "CLI",
    ...
    hooks: {
      ...
      group: "Hooks",
      order: 110,
      help: "Inbound webhook automation surface for mapping external events into wake or agent actions in AikaClaw. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
      tags: ["advanced"],
    },
    ui: {
      label: "UI",
    ...
    meta: {
      label: "Metadata",
      help: "Metadata fields automatically maintained by AikaClaw to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
      tags: ["advanced"],
    },
    "meta.lastTouchedVersion": {
      label: "Config Last Touched Version",
      help: "Auto-set when AikaClaw writes the config.",
      tags: ["media"],
    },
    "meta.lastTouchedAt": {
    ...
    "env.vars": {
      ...
      help: "Explicit key/value environment variable overrides merged into runtime process environment for AikaClaw. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
      tags: ["advanced"],
    },
    "wizard.lastRunAt": {
    ...
    "wizard.lastRunVersion": {
      ...
      help: "AikaClaw version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes.",
      tags: ["advanced"],
    },
    ...
    "diagnostics.cacheTrace.filePath": {
      ...
      help: "JSONL output path for cache trace logs (default: $AIKACLAW_STATE_DIR/logs/cache-trace.jsonl).",
      tags: ["observability", "storage"],
    },
    ...
    "agents.list[].runtime": {
      label: "Agent Runtime",
      help: "Optional runtime descriptor for this agent. Use embedded for default AikaClaw execution or acp for external ACP harness defaults.",
      tags: ["advanced"],
    },
    "agents.list[].runtime.type": {
      label: "Agent Runtime Type",
      help: 'Runtime type for this agent: "embedded" (default AikaClaw runtime) or "acp" (ACP harness defaults).',
      tags: ["advanced"],
    },
    ...
    "agents.list[].runtime.acp.agent": {
      ...
      help: "Optional ACP harness agent id to use for this AikaClaw agent (for example codex, claude).",
      tags: ["advanced"],
    },
    ...
    "browser.profiles.*.driver": {
      ...
      help: 'Per-profile browser driver mode. Use "aikaclaw" (or legacy "clawd") for CDP-based profiles, or use "existing-session" for host-local Chrome DevTools MCP attachment.',
      tags: ["storage"],
    },
    ...
    "gateway.controlUi.basePath": {
      ...
      help: "Optional URL prefix where the Control UI is served (e.g. /aikaclaw).",
      placeholder: "/aikaclaw",
      tags: ["network", "storage"],
    },
    ...
    "gateway.nodes.allowCommands": {
      ...
      help: "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `aikaclaw security audit`.",
      tags: ["access", "network"],
    },
    ...
    "nodeHost.browserProxy.allowProfiles": {
      ...
      help: "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, AikaClaw enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
    ...
    "agents.defaults.memorySearch.store.path": {
      ...
      help: "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.aikaclaw/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
      tags: ["storage"],
    },
    ...
    "memory.backend": {
      ...
      help: 'Selects the global memory engine: "builtin" uses AikaClaw memory internals, while "qmd" uses the QMD sidecar pipeline. Keep "builtin" unless you intentionally operate QMD.',
      tags: ["storage"],
    },
    "memory.citations": {
    ...
    "models.providers.*.injectNumCtxForOpenAICompat": {
      ...
      help: "Controls whether AikaClaw injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
      tags: ["models"],
    },
    ...
    "agents.defaults.embeddedPi": {
      label: "Embedded Pi",
      help: "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in AikaClaw sessions.",
      tags: ["advanced"],
    },
    ...
    "agents.defaults.sandbox.browser.network": {
      ...
      help: "Docker network for sandbox browser containers (default: aikaclaw-sandbox-browser). Avoid bridge if you need stricter isolation.",
      tags: ["storage"],
    },
    ...
    "commands.mcp": {
      label: "Allow /mcp",
      help: "Allow /mcp chat command to manage AikaClaw MCP server config under mcp.servers (default: false).",
      tags: ["advanced"],
    },
    "commands.plugins": {
      label: "Allow /plugins",
    ...
    mcp: {
      label: "MCP",
      help: "Global MCP server definitions managed by AikaClaw. Embedded Pi and other runtime adapters can consume these servers without storing them inside Pi-owned project settings.",
      tags: ["advanced"],
    },
    "mcp.servers": {
      label: "MCP Servers",
      help: "Named MCP server definitions. AikaClaw stores them in its own config and runtime adapters decide which transports are supported at execution time.",
      tags: ["advanced"],
    },
    "ui.seamColor": {
      label: "Accent Color",
    ...
    "session.parentForkMaxTokens": {
      ...
      help: "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, AikaClaw starts a fresh thread session instead of forking; set 0 to disable this protection.",
    ...
    "discovery.wideArea.domain": {
      ...
      help: "Optional unicast DNS-SD domain for wide-area discovery, such as aikaclaw.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
      tags: ["network"],
    },
    "discovery.mdns": {
    ...
    "channels.irc": {
      label: "IRC",
      help: "IRC channel provider configuration and compatibility settings for classic IRC transport workflows. Use this section when bridging legacy chat infrastructure into AikaClaw.",
      tags: ["network", "channels"],
    },
    ...
    "channels.discord.eventQueue.listenerTimeout": {
      ...
      help: "Canonical Discord listener timeout control in ms for gateway normalization/enqueue handlers. Default is 120000 in AikaClaw; set per account via channels.discord.accounts.<id>.eventQueue.listenerTimeout.",
      tags: ["network", "performance", "channels"],
    },
    ...
    "agents.list[].identity.avatar": {
      ...
      placeholder: "avatars/aikaclaw.png",
      tags: ["advanced"],
    },
    ...
    "plugins.installs": {
      ...
      help: "CLI-managed install metadata (used by `aikaclaw plugins update` to locate install sources).",
      tags: ["advanced"],
    },
    ...
    "plugins.installs.*.installPath": {
      ...
      help: "Resolved install directory (usually ~/.aikaclaw/extensions/<id>).",
      tags: ["storage"],
    },
