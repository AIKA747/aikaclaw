...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): AikaClawConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: AikaClawConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        AIKACLAW_STATE_DIR: envStateDir,
      };
      const cfg: AikaClawConfig = {};
