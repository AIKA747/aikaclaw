...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("aikaclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "aikaclaw-sandbox-browser-global" },
      agentBrowser: { network: "aikaclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("aikaclaw-sandbox-browser-agent");
  });
