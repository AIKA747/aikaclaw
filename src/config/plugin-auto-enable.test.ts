...
function makeTempDir() {
  const dir = mkdtempSafe(path.join(os.tmpdir(), "aikaclaw-plugin-auto-enable-"));
  tempDirs.push(dir);
  return dir;
}
...
function writePluginManifestFixture(params: { rootDir: string; id: string; channels: string[] }) {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "aikaclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function makeRegistry(plugins: Array<{ id: string; channels: string[] }>): PluginManifestRegistry {
  return {
    plugins: plugins.map((p) => ({
      ...
      manifestPath: `/fake/${p.id}/aikaclaw.plugin.json`,
    })),
    diagnostics: [],
  };
}
...
describe("applyPluginAutoEnable", () => {
        ...
        entries: [
          {
            ...
            aikaclaw: {
              ...
              install: {
                npmSpec: "@aikaclaw/env-secondary",
              },
            },
          },
        ],
  ...
  describe("third-party channel plugins (pluginId ≠ channelId)", () => {
    it("uses the plugin manifest id, not the channel id, for plugins.entries", () => {
      // Reproduces: https://github.com/aikaclaw/aikaclaw/issues/25261
