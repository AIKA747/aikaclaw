...
import type { ConfigFileSnapshot } from "./types.aikaclaw.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.aikaclaw/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/aikaclaw/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/aikaclaw/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/aikaclaw/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/aikaclaw/nickserv-password.txt");
