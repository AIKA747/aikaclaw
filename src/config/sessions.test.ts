...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ AIKACLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-aikaclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-aikaclaw#general");
  });
  ...
  it("derives session transcripts dir from AIKACLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { AIKACLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when AIKACLAW_STATE_DIR differs from stored paths", () => {
