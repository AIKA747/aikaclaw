...
const ENV_CATALOG_PATHS = ["AIKACLAW_PLUGIN_CATALOG_PATHS", "AIKACLAW_MPM_CATALOG_PATHS"];

function collectModelRefs(cfg: AikaClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: AikaClawConfig, providerId: string): boolean {
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.aikaclaw) || !isRecord(entry.aikaclaw.channel)) {
      continue;
    }
    const channel = entry.aikaclaw.channel;
...
function collectCandidateChannelIds(cfg: AikaClawConfig): string[] {
...
function configMayNeedPluginManifestRegistry(cfg: AikaClawConfig): boolean {
  const configuredChannels = cfg.channels as Record<string, unknown> | undefined;
  if (!configuredChannels || typeof configuredChannels !== "object") {
    return false;
  }
...
function isPluginExplicitlyDisabled(cfg: AikaClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: AikaClawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: AikaClawConfig, pluginId: string): AikaClawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
