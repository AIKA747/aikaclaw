...
import type { AikaClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  aikaclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  aikaclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: AikaClawConfig,
  pathSegments: string[],
): { changed: boolean; next: AikaClawConfig } {
...
function coerceConfig(value: unknown): AikaClawConfig {
...
function stampConfigVersion(cfg: AikaClawConfig): AikaClawConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: AikaClawConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer AikaClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as AikaClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AikaClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AikaClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: AikaClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as AikaClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.AIKACLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.AIKACLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      argv: process.argv.slice(0, 8),
      execArgv: process.execArgv.slice(0, 8),
      watchMode: deps.env.AIKACLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.AIKACLAW_WATCH_SESSION === "string" &&
        deps.env.AIKACLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.AIKACLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.AIKACLAW_WATCH_COMMAND === "string" &&
        deps.env.AIKACLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.AIKACLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `AIKACLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: AikaClawConfig | null = null;
let runtimeConfigSourceSnapshot: AikaClawConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AIKACLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.AIKACLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): AikaClawConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): AikaClawConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: AikaClawConfig;
  candidate: AikaClawConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: AikaClawConfig): AikaClawConfig {
...
export function loadConfig(): AikaClawConfig {
...
export async function readBestEffortConfig(): Promise<AikaClawConfig> {
...
export async function writeConfigFile(
  ...
  // edits to aikaclaw.json remain visible (no stale snapshot).
}
