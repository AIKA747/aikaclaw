...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.AIKACLAW_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveAikaClawPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    AIKACLAW_STATE_DIR: path.join(os.tmpdir(), "aikaclaw-config-doc-baseline-state"),
    AIKACLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
  ...
  const channelPlugins =
    process.env.AIKACLAW_CONFIG_DOC_BASELINE_DEBUG === "1"
