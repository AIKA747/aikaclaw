...
import { buildWebSearchProviderConfig, withTempHome, writeAikaClawConfig } from "./test-helpers.js";
...
describe("$schema key in config (#14998)", () => {
  ...
  it("rejects non-string $schema", () => {
    const result = AikaClawSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = AikaClawSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeAikaClawConfig(home, {
        agents: { list: [{ id: "pi" }] },
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeAikaClawConfig(home, {
        gateway: { bind: "${AIKACLAW_BIND}" },
      });

      const prev = process.env.AIKACLAW_BIND;
      process.env.AIKACLAW_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.AIKACLAW_BIND;
        } else {
          process.env.AIKACLAW_BIND = prev;
        }
      }
    });
  });
