...
describe("config io write", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-config-io-"));
  });
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".aikaclaw", "aikaclaw.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".aikaclaw", "logs", "config-audit.jsonl");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withSuiteHome(async (home) => {
        const stateDir = path.join(home, ".aikaclaw");
  ...
  it('shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withSuiteHome(async (home) => {
      ...
      const invalidConfig: AikaClawConfig = {
        channels: {
          telegram: {
            dmPolicy: "open",
            allowFrom: [],
      ...
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        "aikaclaw config set channels.telegram.allowFrom '[\"*\"]'",
      );
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        'aikaclaw config set channels.telegram.dmPolicy "pairing"',
      );
    });
  });
  ...
  it("does not mutate caller config when unsetPaths is applied on first write", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".aikaclaw", "aikaclaw.json");
      const io = createConfigIO({
        env: {} as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    await withSuiteHome(async (home) => {
      ...
      expect(lines.length).toBeGreaterThan(0);
      await expect(
        fs.stat(path.join(home, ".aikaclaw", "logs", "config-audit.jsonl")),
      ).resolves.toBeDefined();
      await expect(
        fs.stat(path.resolve("undefined", ".aikaclaw", "logs", "config-audit.jsonl")),
      ).rejects.toThrow();
    });
  });
  ...
  it("records gateway watch session markers in config audit entries", async () => {
    await withSuiteHome(async (home) => {
      const { last } = await writeGatewayPatchAndReadLastAuditEntry({
        ...
        gatewayPatch: { bind: "loopback" },
        env: {
          AIKACLAW_WATCH_MODE: "1",
          AIKACLAW_WATCH_SESSION: "watch-session-1",
          AIKACLAW_WATCH_COMMAND: "gateway --force",
        } as NodeJS.ProcessEnv,
      });
