...
import { parseAikaClawVersion } from "./version.js";

async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-config-"));
  try {
    await run(home);
  } finally {
...
describe("config io paths", () => {
  it("uses ~/.aikaclaw/aikaclaw.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".aikaclaw", 19001);
  ...
  it("defaults to ~/.aikaclaw/aikaclaw.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".aikaclaw", "aikaclaw.json"));
    });
  });

  it("uses AIKACLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { AIKACLAW_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".aikaclaw", "aikaclaw.json"));
    });
  });

  it("honors explicit AIKACLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".aikaclaw", 20002, "custom.json");
      const io = createIoForHome(home, { AIKACLAW_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("does not warn when config was last touched by a same-base correction publish", async () => {
    const parsedVersion = parseAikaClawVersion(VERSION);
    if (!parsedVersion) {
    ...
    await withTempHome(async (home) => {
      ...
      expect(logger.warn).not.toHaveBeenCalledWith(
        expect.stringContaining("Config was last written by a newer AikaClaw"),
      );
      expect(io.configPath).toBe(configPath);
    });
  });
