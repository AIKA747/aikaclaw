...
function createSourceConfig(): AikaClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createRuntimeConfig(): AikaClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runtime config snapshot writes", () => {
  it("returns the source snapshot when runtime snapshot is active", async () => {
    await withTempHome("aikaclaw-config-runtime-source-", async () => {
  ...
  it("skips source projection for non-runtime-derived configs", async () => {
    await withTempHome("aikaclaw-config-runtime-projection-shape-", async () => {
      ...
      const independentConfig: AikaClawConfig = {
        models: {
          providers: {
            openai: {
              baseUrl: "https://api.openai.com/v1",
  ...
  it("preserves source secret refs when writeConfigFile receives runtime-resolved config", async () => {
    await withTempHome("aikaclaw-config-runtime-write-", async (home) => {
  ...
  it("refreshes the runtime snapshot after writes so follow-up reads see persisted changes", async () => {
    await withTempHome("aikaclaw-config-runtime-write-refresh-", async (home) => {
      const configPath = path.join(home, ".aikaclaw", "aikaclaw.json");
      const sourceConfig: AikaClawConfig = {
        models: {
          providers: {
      ...
      const nextRuntimeConfig: AikaClawConfig = {
        ...runtimeConfig,
  ...
  it("keeps the last-known-good runtime snapshot active while a specialized refresh is pending", async () => {
    await withTempHome("aikaclaw-config-runtime-refresh-pending-", async (home) => {
      const configPath = path.join(home, ".aikaclaw", "aikaclaw.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
      const nextRuntimeConfig: AikaClawConfig = {
