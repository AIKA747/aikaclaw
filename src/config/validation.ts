...
import type { AikaClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: AikaClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: AikaClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: AikaClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AikaClawSchema.safeParse(normalizedRaw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AikaClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AikaClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as AikaClawConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as AikaClawConfig,
  };
}
...
function validateConfigObjectWithPluginsBase(
  ...
  const warnings: ConfigValidationIssue[] = listLegacyWebSearchConfigPaths(raw).map((path) => ({
    ...
    message:
      ...
      "Move it under plugins.entries.<plugin>.config.webSearch.*; AikaClaw mapped it automatically for compatibility.",
  }));
  ...
  let compatConfig: AikaClawConfig | null | undefined;

  const ensureCompatConfig = (): AikaClawConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native AikaClaw config schema.
