...
describe("usage-format", () => {
  const originalAgentDir = process.env.AIKACLAW_AGENT_DIR;
  let agentDir: string;

  beforeEach(async () => {
    agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-usage-format-"));
  ...
  afterEach(async () => {
    if (originalAgentDir === undefined) {
      ...
      process.env.AIKACLAW_AGENT_DIR = originalAgentDir;
    }
    __resetUsageFormatCachesForTest();
    __resetGatewayModelPricingCacheForTest();
  ...
  it("prefers models.json pricing over aikaclaw config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          openai: {
  ...
  it("falls back to aikaclaw config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
          anthropic: {
