...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): AikaClawConfig {
    const defaults: NonNullable<NonNullable<AikaClawConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: AikaClawConfig) {
