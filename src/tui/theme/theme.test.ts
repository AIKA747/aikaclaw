...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      AIKACLAW_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when AIKACLAW_THEME=light", async () => {
    const mod = await importThemeWithEnv({ AIKACLAW_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when AIKACLAW_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ AIKACLAW_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats AIKACLAW_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ AIKACLAW_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("AIKACLAW_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      AIKACLAW_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ AIKACLAW_THEME: "light" });
    const darkMod = await importThemeWithEnv({ AIKACLAW_THEME: "dark" });
...
describe("light palette accessibility", () => {
  it("keeps light theme text colors at WCAG AA contrast or better", async () => {
    vi.resetModules();
    process.env.AIKACLAW_THEME = "light";
