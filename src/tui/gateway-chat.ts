...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set AIKACLAW_GATEWAY_TOKEN/AIKACLAW_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "aikaclaw-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.AIKACLAW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.AIKACLAW_GATEWAY_PASSWORD);
