...
describe("resolveInitialTuiAgentId", () => {
  const cfg: AikaClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/aikaclaw" },
        { id: "ops", workspace: "/tmp/aikaclaw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/aikaclaw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run aikaclaw devices list");
    expect(state.pairingHint).toContain("aikaclaw devices list");
  });
