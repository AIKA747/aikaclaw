...
type ConfiguredBinding = NonNullable<AikaClawConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<AikaClawConfig>,
): AikaClawConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: AikaClawConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/aikaclaw" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
