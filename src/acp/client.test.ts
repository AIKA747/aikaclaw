...
const createTempDir = () => tempDirs.make("aikaclaw-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets AIKACLAW_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "aikaclaw",
    });

    expect(env.AIKACLAW_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("aikaclaw");
  });

  it("overrides pre-existing AIKACLAW_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      AIKACLAW_SHELL: "wrong",
    });
    expect(env.AIKACLAW_SHELL).toBe("acp-client");
  });
  ...
  it("preserves AIKACLAW_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        AIKACLAW_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["AIKACLAW_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default AikaClaw bridge", () => {
    ...
    expect(env.AIKACLAW_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.AIKACLAW_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "aikaclaw",
        serverArgs: ["acp"],
        defaultServerCommand: "aikaclaw",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("AIKACLAW_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "aikaclaw", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "aikaclaw", "dist", "entry.js");
    const shimPath = path.join(dir, "aikaclaw.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\aikaclaw\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/aikaclaw-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/aikaclaw-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/aikaclaw-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
