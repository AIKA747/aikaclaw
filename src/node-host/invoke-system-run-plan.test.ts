...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  return withFakeRuntimeBins({
    ...
    tmpPrefix: `aikaclaw-${params.binName}-bin-`,
    run: params.run,
  });
}
...
function withFakeRuntimeBins<T>(params: {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), params.tmpPrefix ?? "aikaclaw-runtime-bins-"));
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "aikaclaw-ruby-require-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "aikaclaw-ruby-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "aikaclaw-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "aikaclaw-perl-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "aikaclaw-perl-preload-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "aikaclaw-inline-shell-node-",
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  for (const testCase of cases) {
    it.runIf(process.platform !== "win32")(testCase.name, () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-approval-hardening-"));
      const oldPath = process.env.PATH;
      ...
      withFakeRuntimeBins({
        ...
        run: () => {
          withScriptOperandPlanFixture(
            {
              tmpPrefix: "aikaclaw-approval-script-plan-",
              fixture: runtimeCase,
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-shell-option-value-"));
    try {
