...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  let testAikaClawHome = "";
  let previousAikaClawHome: string | undefined;

  beforeEach(() => {
    previousAikaClawHome = process.env.AIKACLAW_HOME;
    testAikaClawHome = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-node-host-home-"));
    process.env.AIKACLAW_HOME = testAikaClawHome;
  ...
  afterEach(() => {
    ...
    if (testAikaClawHome) {
      fs.rmSync(testAikaClawHome, { recursive: true, force: true });
      testAikaClawHome = "";
    }
  });
  ...
  async function withTempApprovalsHome<T>(params: {
    ...
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-exec-approvals-"));
    const previousAikaClawHome = process.env.AIKACLAW_HOME;
    process.env.AIKACLAW_HOME = tempHome;
  ...
  async function withFakeRuntimeOnPath<T>(params: {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), `aikaclaw-${params.runtime}-path-`));
    const binDir = path.join(tmp, "bin");
    fs.mkdirSync(binDir, { recursive: true });
    const runtimePath =
  ...
  for (const testCase of approvedEnvShellWrapperCases) {
    it.runIf(process.platform !== "win32")(testCase.name, async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-approved-wrapper-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withPathTokenCommand({
        tmpPrefix: "aikaclaw-approval-path-pin-",
        run: async ({ expected }) => {
          const { runCommand, sendInvokeResult } = await runSystemInvoke({
            preferMacAppExecHost: false,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withPathTokenCommand({
        tmpPrefix: "aikaclaw-prepare-run-path-pin-",
        run: async ({ expected }) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const sendInvokeResult = vi.fn(async () => {});
      await withPathTokenCommand({
        tmpPrefix: "aikaclaw-allowlist-path-pin-",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-approval-cwd-link-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-approval-cwd-parent-link-"));
  ...
  it("uses canonical executable path for approval-based relative command execution", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-approval-cwd-real-"));
  ...
  it("denies approval-based execution when cwd identity drifts before execution", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-approval-cwd-drift-"));
    const fallback = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-approval-cwd-drift-alt-"));
    const script = path.join(tmp, "run.sh");
    fs.writeFileSync(script, "#!/bin/sh\necho SAFE\n");
    fs.chmodSync(script, 0o755);
    const canonicalCwd = fs.realpathSync(tmp);
  ...
  it("denies approval-based execution when a script operand changes after approval", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-approval-script-drift-"));
  ...
  it("keeps approved shell script execution working when the script is unchanged", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-approval-script-stable-"));
    const fixture = createMutableScriptOperandFixture(tmp);
    fs.writeFileSync(fixture.scriptPath, fixture.initialBody);
    if (process.platform !== "win32") {
      fs.chmodSync(fixture.scriptPath, 0o755);
  ...
  for (const runtime of ["bun", "deno", "tsx", "jiti"] as const) {
      ...
      await withFakeRuntimeOnPath({
        ...
        run: async () => {
          const tmp = fs.mkdtempSync(
            path.join(os.tmpdir(), `aikaclaw-approval-${runtime}-script-drift-`),
          );
    ...
    it(`keeps approved ${runtime} script execution working when the script is unchanged`, async () => {
      await withFakeRuntimeOnPath({
        ...
        run: async () => {
          const tmp = fs.mkdtempSync(
            path.join(os.tmpdir(), `aikaclaw-approval-${runtime}-script-stable-`),
          );
          const fixture = createRuntimeScriptOperandFixture({ tmp, runtime });
          fs.writeFileSync(fixture.scriptPath, fixture.initialBody);
          try {
  ...
  it("denies approval-based execution when tsx is missing a required mutable script binding", async () => {
    await withFakeRuntimeOnPath({
      ...
      run: async () => {
        const tmp = fs.mkdtempSync(
          path.join(os.tmpdir(), "aikaclaw-approval-tsx-missing-binding-"),
        );
  ...
  it("denies ./sh wrapper spoof in allowlist on-miss mode before execution", async () => {
    const marker = path.join(os.tmpdir(), `aikaclaw-wrapper-spoof-${process.pid}-${Date.now()}`);
    const runCommand = vi.fn(async () => {
  ...
  it("denies semicolon-chained shell payloads in allowlist mode without explicit approval", async () => {
    const payloads = ["aikaclaw status; id", "aikaclaw status; cat /etc/passwd"];
