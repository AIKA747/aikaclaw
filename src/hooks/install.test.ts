...
const fixtureRoot = path.join(process.cwd(), ".tmp", `aikaclaw-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@aikaclaw/test-hooks",
      version: "0.0.1",
      aikaclaw: { hooks: params.hooks },
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "aikaclaw.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "aikaclaw.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@aikaclaw/test-hooks@0.0.1",
      hooksDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@aikaclaw/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@aikaclaw/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@aikaclaw/test-hooks@0.0.2-beta.1",
      name: "@aikaclaw/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@aikaclaw/test-hooks@beta"');
    }
  });
});
