import type { AikaClawConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "aikaclaw-bundled": {
    ...
    canOverride: ["aikaclaw-bundled"],
    canBeOverriddenBy: ["aikaclaw-managed", "aikaclaw-plugin"],
  },
  "aikaclaw-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["aikaclaw-bundled", "aikaclaw-plugin"],
    canBeOverriddenBy: ["aikaclaw-managed"],
  },
  "aikaclaw-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["aikaclaw-bundled", "aikaclaw-managed", "aikaclaw-plugin"],
    canBeOverriddenBy: ["aikaclaw-managed"],
  },
  "aikaclaw-workspace": {
    precedence: 40,
    ...
    canOverride: ["aikaclaw-workspace"],
    canBeOverriddenBy: ["aikaclaw-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "aikaclaw-plugin") {
    return { enabled: true };
  }
