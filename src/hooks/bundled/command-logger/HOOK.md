...
homepage: https://docs.aikaclaw.ai/automation/hooks#command-logger
metadata:
  {
    "aikaclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.aikaclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.aikaclaw/logs/commands.log ~/.aikaclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/aikaclaw`:
   ```
   /home/username/.aikaclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
