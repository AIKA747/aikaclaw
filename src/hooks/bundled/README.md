...
This directory contains hooks that ship with AikaClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
- **Managed hooks**: `~/.aikaclaw/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.aikaclaw/aikaclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'aikaclaw.*gateway' && pnpm aikaclaw gateway`
3. Enable the hook: `aikaclaw hooks enable my-hook`
