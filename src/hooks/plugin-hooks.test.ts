...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "aikaclaw-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.AIKACLAW_BUNDLED_HOOKS_DIR;
    process.env.AIKACLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });

  afterEach(() => {
    clearInternalHooks();
    if (previousBundledHooksDir === undefined) {
      delete process.env.AIKACLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.AIKACLAW_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  function createConfig(enabled: boolean): AikaClawConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("aikaclaw-plugin");
  ...
  it("does not treat Claude hooks.json bundles as AikaClaw hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".aikaclaw", "extensions", "claude-bundle");
