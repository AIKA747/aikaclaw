...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "aikaclaw-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "aikaclaw-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "aikaclaw-bundled");
      const plugin = makeHookEntry("shared", "aikaclaw-plugin");
      const managed = makeHookEntry("shared", "aikaclaw-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("aikaclaw-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "aikaclaw-managed");
      const workspace = makeHookEntry("shared", "aikaclaw-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "aikaclaw-workspace");
      const second = makeHookEntry("shared", "aikaclaw-workspace");
      second.hook.handlerPath = "/tmp/aikaclaw-workspace/shared/handler-2.js";
