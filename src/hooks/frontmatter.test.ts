...
`;
    ...
    expect(parsed.aikaclaw.emoji).toBe("💾");
    expect(parsed.aikaclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.aikaclaw.emoji).toBe("📝");
    expect(parsed.aikaclaw.events).toEqual(["command"]);
    expect(parsed.aikaclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.aikaclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"aikaclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"aikaclaw": {"events": ["test"]}}');
  });
...
describe("resolveAikaClawMetadata", () => {
  it("extracts aikaclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveAikaClawMetadata(frontmatter);
  ...
  it("returns undefined when aikaclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        aikaclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with AikaClaw" },
            { id: "npm", kind: "npm", package: "@aikaclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@aikaclaw/hook");
  });
    ...
    "aikaclaw":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with AikaClaw" }],
      },
  }
---
...
`;
    ...
    const aikaclaw = resolveAikaClawMetadata(frontmatter);
    expect(aikaclaw).toBeDefined();
    expect(aikaclaw?.emoji).toBe("💾");
    expect(aikaclaw?.events).toEqual(["command:new", "command:reset"]);
    expect(aikaclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(aikaclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const aikaclaw = resolveAikaClawMetadata(frontmatter);
    expect(aikaclaw?.emoji).toBe("disk");
    expect(aikaclaw?.events).toEqual(["command:new"]);
  });
});
