...
async function ensureAikaClawHooks(manifest: HookPackageManifest) {
  const hooks = manifest[MANIFEST_KEY]?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing aikaclaw.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json aikaclaw.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(
  ...
  try {
    hookEntries = await ensureAikaClawHooks(manifest);
  } catch (err) {
  ...
  for (const entry of hookEntries) {
    ...
    if (!runtime.isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `aikaclaw.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `aikaclaw.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromNpmSpec(params: {
  ...
  return await runtime.installFromValidatedNpmSpecArchive({
    tempDirPrefix: "aikaclaw-hook-pack-",
    spec,
    timeoutMs,
