...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "aikaclaw", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "aikaclaw", version: "2.3.4" });
  ...
  it("ignores non-aikaclaw package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers AIKACLAW_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        AIKACLAW_VERSION: "9.9.9",
        AIKACLAW_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        AIKACLAW_VERSION: "   ",
        AIKACLAW_SERVICE_VERSION: "  2.0.0  ",
