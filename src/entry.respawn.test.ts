...
import {
  ...
  AIKACLAW_NODE_EXTRA_CA_CERTS_READY,
  AIKACLAW_NODE_OPTIONS_READY,
...
describe("buildCliRespawnPlan", () => {
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    const plan = buildCliRespawnPlan({
      argv: ["node", "aikaclaw", "gateway", "run"],
      env: {},
      execArgv: [],
      autoNodeExtraCaCerts: "/etc/ssl/certs/ca-certificates.crt",
    });
    ...
    expect(plan?.env[AIKACLAW_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(plan?.env[AIKACLAW_NODE_OPTIONS_READY]).toBe("1");
  });
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "aikaclaw", "gateway", "run"],
        env: {
          [AIKACLAW_NODE_EXTRA_CA_CERTS_READY]: "1",
          [AIKACLAW_NODE_OPTIONS_READY]: "1",
        },
