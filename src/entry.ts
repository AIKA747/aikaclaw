...
import { ensureAikaClawExecMarkerOnProcess } from "./infra/aikaclaw-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "aikaclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "aikaclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.AIKACLAW_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureCliRespawnReady(): boolean {
    ...
    child.once("error", (error) => {
      console.error(
        "[aikaclaw] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `AikaClaw ${VERSION} (${commit})` : `AikaClaw ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[aikaclaw] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureCliRespawnReady()) {
    ...
    if (!parsedContainer.ok) {
      console.error(`[aikaclaw] ${parsedContainer.error}`);
      process.exit(2);
    }
    ...
    if (!parsed.ok) {
      ...
      console.error(`[aikaclaw] ${parsed.error}`);
      process.exit(2);
    }

    const containerTargetName = resolveCliContainerTarget(process.argv);
    if (containerTargetName && parsed.profile) {
      console.error("[aikaclaw] --container cannot be combined with --profile/--dev");
      process.exit(2);
    }
...
export function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[aikaclaw] Failed to display help:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
