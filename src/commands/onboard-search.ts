...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<AikaClawConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<AikaClawConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: AikaClawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: AikaClawConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

/** Build an env-backed SecretRef for a search provider. */
function buildSearchEnvRef(config: AikaClawConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: AikaClawConfig, result: AikaClawConfig): AikaClawConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as AikaClawConfig["plugins"],
  };
}
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with aikaclaw configure --section web",
      },
    ],
