...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "AIKACLAW_GATEWAY_TOKEN",
          placeholder: "AIKACLAW_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like AIKACLAW_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. AikaClaw will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: AikaClaw trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.aikaclaw.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
