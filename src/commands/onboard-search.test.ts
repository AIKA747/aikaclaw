...
function createPerplexityConfig(apiKey: string, enabled?: boolean): AikaClawConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: AikaClawConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): AikaClawConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: AikaClawConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: AikaClawConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
