...
describe("doctor config flow", () => {
  ...
  it("creates a Matrix migration snapshot before doctor repair mutates Matrix state", async () => {
    await withTempHome(async (home) => {
      ...
      const snapshotDir = path.join(home, "Backups", "aikaclaw-migrations");
      ...
      expect(marker.archivePath).toContain(path.join("Backups", "aikaclaw-migrations"));
    });
  });
  ...
  it("warns when Matrix is installed from a stale custom path", async () => {
    ...
    expect(
      doctorWarnings.some(
        (line) => line.includes("custom path") && line.includes("/tmp/aikaclaw-matrix-missing"),
      ),
    ).toBe(true);
  });
