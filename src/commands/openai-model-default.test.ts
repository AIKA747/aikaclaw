...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: AikaClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: AikaClawConfig) => { changed: boolean; next: AikaClawConfig };
  defaultModel: string;
  overrideConfig: AikaClawConfig;
  alreadyDefaultConfig: AikaClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: AikaClawConfig) => config,
      applyDefaultConfig: (config: AikaClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: AikaClawConfig = { agents: { defaults: {} } };
