...
describe("promptRemoteGatewayConfig", () => {
  const envSnapshot = captureEnv(["AIKACLAW_ALLOW_INSECURE_PRIVATE_WS"]);
  ...
  it("does not route from TXT-only discovery metadata", async () => {
    ...
    const next = await promptRemoteGatewayConfig({} as AikaClawConfig, prompter);
  ...
  it("allows ws:// hostname remote URLs when AIKACLAW_ALLOW_INSECURE_PRIVATE_WS=1", async () => {
    process.env.AIKACLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        expect(params.validate?.("ws://aikaclaw-gateway.ai:18789")).toBeUndefined();
        expect(params.validate?.("ws://1.1.1.1:18789")).toContain("Use wss://");
        return "ws://aikaclaw-gateway.ai:18789";
      }
      return "";
    }) as WizardPrompter["text"];
    ...
    expect(next.gateway?.mode).toBe("remote");
    expect(next.gateway?.remote?.url).toBe("ws://aikaclaw-gateway.ai:18789");
  });

  it("supports storing remote auth as an external env secret ref", async () => {
    process.env.AIKACLAW_GATEWAY_TOKEN = "remote-token-value";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        return "wss://remote.example.com:18789";
      }
      if (params.message === "Environment variable name") {
        return "AIKACLAW_GATEWAY_TOKEN";
      }
      return "";
    }) as WizardPrompter["text"];
