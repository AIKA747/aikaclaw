...
describe("doctor config flow steps", () => {
  it("collects legacy compatibility issue lines and preview fix hints", () => {
    const result = applyLegacyCompatibilityStep({
      ...
      shouldRepair: false,
      doctorFixCommand: "aikaclaw doctor --fix",
    });
    ...
    expect(result.state.fixHints).toContain(
      'Run "aikaclaw doctor --fix" to apply compatibility migrations.',
    );
  });
  ...
  it("removes unknown keys and adds preview hint", () => {
    const result = applyUnknownConfigKeyStep({
      state: {
        cfg: {},
        candidate: { bogus: true } as unknown as AikaClawConfig,
        pendingChanges: false,
        fixHints: [],
      },
      shouldRepair: false,
    ...
    expect(result.state.fixHints).toContain('Run "aikaclaw doctor --fix" to remove these keys.');
  });
});
