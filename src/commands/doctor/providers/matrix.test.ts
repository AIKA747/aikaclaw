...
describe("doctor matrix provider helpers", () => {
  ...
  it("warns on stale custom Matrix plugin paths", async () => {
    const missingPath = path.join(tmpdir(), "aikaclaw-matrix-missing-provider-test");
    ...
    expect(warnings[1]).toContain("aikaclaw plugins install @aikaclaw/matrix");
    expect(warnings[2]).toContain("aikaclaw plugins install ./extensions/matrix");
  });
  ...
  it("removes stale Matrix plugin config when install path is missing", async () => {
    const missingPath = path.join(tmpdir(), "aikaclaw-matrix-stale-cleanup-test-" + Date.now());
    await fs.rm(missingPath, { recursive: true, force: true });
