...
type EnvSnapshot = {
  HOME?: string;
  AIKACLAW_HOME?: string;
  AIKACLAW_STATE_DIR?: string;
  AIKACLAW_OAUTH_DIR?: string;
};
...
function captureEnv(): EnvSnapshot {
  return {
    ...
    AIKACLAW_OAUTH_DIR: process.env.AIKACLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: AikaClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: AikaClawConfig) {
...
async function runStateIntegrityText(cfg: AikaClawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.AIKACLAW_HOME = tempHome;
    process.env.AIKACLAW_STATE_DIR = path.join(tempHome, ".aikaclaw");
    delete process.env.AIKACLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.AIKACLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when AIKACLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.AIKACLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: AikaClawConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints aikaclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: AikaClawConfig = {};
    ...
    expect(text).toMatch(/aikaclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/aikaclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /aikaclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
