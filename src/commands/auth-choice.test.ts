...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    ...
    const env = await setupAuthTestEnv("aikaclaw-auth-");
      ...
      config: {
        secrets: {
          providers: {
            filemain: {
              source: "file",
              path: "/tmp/aikaclaw-missing-secrets.json",
              mode: "json",
            },
          },
        },
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireAikaClawAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
