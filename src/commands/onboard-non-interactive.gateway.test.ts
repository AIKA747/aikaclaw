    ...
    runNonInteractiveSetup(
      {
        nonInteractive: true,
        mode: "local",
        workspace: path.join(stateDir, "aikaclaw"),
        authChoice: "skip",
        skipSkills: true,
        skipHealth: false,
        installDaemon: true,
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("aikaclaw-onboard-");
  ...
  it("writes gateway token auth into config", async () => {
    await withStateDir("state-noninteractive-", async (stateDir) => {
      ...
      const workspace = path.join(stateDir, "aikaclaw");
  ...
  it("uses AIKACLAW_GATEWAY_TOKEN when --gateway-token is omitted", async () => {
    await withStateDir("state-env-token-", async (stateDir) => {
      const envToken = "tok_env_fallback_123";
      const workspace = path.join(stateDir, "aikaclaw");
      const prevToken = process.env.AIKACLAW_GATEWAY_TOKEN;
      process.env.AIKACLAW_GATEWAY_TOKEN = envToken;
      ...
      try {
        ...
        if (prevToken === undefined) {
          ...
          process.env.AIKACLAW_GATEWAY_TOKEN = prevToken;
        }
      }
    });
  }, 60_000);
        ...
        await runNonInteractiveSetup(
          {
            ...
            gatewayBind: "loopback",
            gatewayAuth: "token",
            gatewayTokenRefEnv: "AIKACLAW_GATEWAY_TOKEN",
          },
          runtime,
        );
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `aikaclaw gateway status --deep` for more detail.");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.AIKACLAW_STATE_DIR = stateDir;
      process.env.AIKACLAW_CONFIG_PATH = path.join(stateDir, "aikaclaw.json");
