...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.aikaclaw/aikaclaw.json",
...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.aikaclaw/workspace",
  applyWizardMetadata: (cfg: AikaClawConfig) => cfg,
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.applySearchProviderSelection.mockImplementation((cfg: AikaClawConfig) => cfg);
  });
  ...
  it("persists provider-owned web search config changes returned by applySearchKey", async () => {
    ...
    mocks.hasExistingKey.mockReturnValue(false);
    mocks.hasKeyInEnv.mockReturnValue(false);
    mocks.applySearchKey.mockImplementation((cfg: AikaClawConfig, provider: string, key: string) =>
  ...
  it("applies provider selection side effects when a key already exists via secret ref or env", async () => {
    ...
    mocks.hasKeyInEnv.mockReturnValue(false);
    mocks.applySearchProviderSelection.mockImplementation((cfg: AikaClawConfig, provider: string) =>
      createEnabledWebSearchConfig(provider, {
        enabled: true,
      })(cfg),
    );
