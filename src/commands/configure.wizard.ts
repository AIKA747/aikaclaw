...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.AIKACLAW_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.AIKACLAW_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from aikaclaw.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "AikaClaw update wizard" : "AikaClaw configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: AikaClawConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("aikaclaw doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: process.env.AIKACLAW_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password: process.env.AIKACLAW_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    });
