...
import { resolveAikaClawPackageRoot } from "../infra/aikaclaw-root.js";
...
function resolveMode(cfg: AikaClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("AikaClaw doctor");

  const root = await resolveAikaClawPackageRoot({
  ...
  let cfg: AikaClawConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("aikaclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("aikaclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("aikaclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("aikaclaw config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("aikaclaw config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("aikaclaw doctor --fix")}" to apply changes.`);
  }
