...
function resolveAgentName(cfg: AikaClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: AikaClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AikaClawConfig): AgentSummary[] {
