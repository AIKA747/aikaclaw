...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under AIKACLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("aikaclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-oauth-sync-"));
    ...
    process.env.AIKACLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-oauth-nosync-"));
    process.env.AIKACLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside AIKACLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-oauth-external-"));
    process.env.AIKACLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* AIKACLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to AIKACLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("aikaclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
