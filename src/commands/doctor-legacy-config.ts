...
export function normalizeCompatibilityConfigValues(cfg: AikaClawConfig): {
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as AikaClawConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as AikaClawConfig["browser"],
    };
    changes.push(
  ...
  const normalizeLegacyNanoBananaSkill = () => {
    type ModelProviderEntry = Partial<
      NonNullable<NonNullable<AikaClawConfig["models"]>["providers"]>[string]
    >;
    type ModelsConfigPatch = Partial<NonNullable<AikaClawConfig["models"]>>;
    ...
    if (!hasGoogleApiKey && legacyApiKey) {
      ...
      rawModels.providers = rawProviders as NonNullable<AikaClawConfig["models"]>["providers"];
      next = {
        ...next,
        models: rawModels as AikaClawConfig["models"],
      };
      changes.push(
  ...
  const normalizeLegacyTalkConfig = () => {
    ...
    const normalizedTalk = normalizeTalkSection(rawTalk as AikaClawConfig["talk"]);
    if (!normalizedTalk) {
      return;
    }
  ...
  const normalizeLegacyMediaProviderOptions = () => {
    ...
    next = {
      ...
      tools: {
        ...next.tools,
        media: nextMedia as NonNullable<AikaClawConfig["tools"]>["media"],
      },
    };
  };
  ...
  const normalizeLegacyMistralModelMaxTokens = () => {
    ...
    next = {
      ...
      models: {
        ...next.models,
        providers: nextProviders as NonNullable<AikaClawConfig["models"]>["providers"],
      },
    };
  };
