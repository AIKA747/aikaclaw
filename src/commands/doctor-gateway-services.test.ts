...
vi.mock("../daemon/service-audit.js", () => ({
  ...
  readEmbeddedGatewayToken: (
    ...
    command?.environmentValueSources?.AIKACLAW_GATEWAY_TOKEN === "file"
      ? undefined
      : command?.environment?.AIKACLAW_GATEWAY_TOKEN?.trim() || undefined,
  SERVICE_AUDIT_CODES: {
...
const originalUpdateInProgress = process.env.AIKACLAW_UPDATE_IN_PROGRESS;
...
async function runRepair(cfg: AikaClawConfig) {
...
async function runNonInteractiveRepair(params: {
  ...
  if (params.updateInProgress) {
    process.env.AIKACLAW_UPDATE_IN_PROGRESS = "1";
  } else {
    delete process.env.AIKACLAW_UPDATE_IN_PROGRESS;
  }
...
function setupGatewayTokenRepairScenario() {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      AIKACLAW_GATEWAY_TOKEN: "stale-token",
    },
  });
  mocks.auditGatewayServiceConfig.mockResolvedValue({
    ok: false,
    issues: [
      {
        ...
        message: "Gateway service AIKACLAW_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    fsMocks.realpath.mockImplementation(async (value: string) => value);
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: AikaClawConfig, env) => {
      const configToken =
        typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
      const envToken = env.AIKACLAW_GATEWAY_TOKEN?.trim() || undefined;
  ...
  afterEach(() => {
    ...
    if (originalUpdateInProgress === undefined) {
      ...
      process.env.AIKACLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
    }
  });
  ...
  it("uses AIKACLAW_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ AIKACLAW_GATEWAY_TOKEN: "env-token" }, async () => {
      setupGatewayTokenRepairScenario();
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/aikaclaw/dist/index.js",
      installEntrypoint:
        "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/aikaclaw@2026.3.12/node_modules/aikaclaw/dist/index.js",
      realpath: async (value: string) => {
        if (value.includes("/global/5/node_modules/aikaclaw/")) {
          return value.replace(
            "/global/5/node_modules/aikaclaw/",
            "/global/5/node_modules/.pnpm/aikaclaw@2026.3.12/node_modules/aikaclaw/",
          );
        }
        return value;
      },
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/opt/aikaclaw/../aikaclaw/dist/index.js",
      installEntrypoint: "/opt/aikaclaw/dist/index.js",
  ...
  it("still flags entrypoint mismatch when canonicalized paths differ", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint:
        "/Users/test/.nvm/versions/node/v22.0.0/lib/node_modules/aikaclaw/dist/index.js",
      installEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/aikaclaw/dist/index.js",
    });
  ...
  it("repairs entrypoint mismatch in non-interactive fix mode", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/npm/node_modules/aikaclaw/dist/entry.js",
      installEntrypoint: "/Users/test/Library/npm/node_modules/aikaclaw/dist/index.js",
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            AIKACLAW_GATEWAY_TOKEN: "env-file-token",
          },
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing AikaClaw gateway next.",
    );
  });
});
