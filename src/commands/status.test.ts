...
beforeAll(() => {
  envSnapshot = captureEnv(["AIKACLAW_PROFILE"]);
...
vi.mock("../infra/aikaclaw-root.js", () => ({
  resolveAikaClawPackageRoot: vi.fn().mockResolvedValue("/tmp/aikaclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/aikaclaw/pnpm-lock.yaml",
      markerPath: "/tmp/aikaclaw/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.aikaclaw.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.aikaclaw.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("aikaclaw status --all") ||
          line.includes("aikaclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  });
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("AIKACLAW_GATEWAY_TOKEN", "abcd1234", async () => {
