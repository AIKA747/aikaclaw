...
import { type AikaClawConfig, createConfigIO, writeConfigFile } from "../config/config.js";
...
async function readConfigFileRaw(configPath: string): Promise<{
  exists: boolean;
  parsed: AikaClawConfig;
}> {
  try {
    const raw = await fs.readFile(configPath, "utf-8");
    const parsed = JSON5.parse(raw);
    if (parsed && typeof parsed === "object") {
      return { exists: true, parsed: parsed as AikaClawConfig };
    }
