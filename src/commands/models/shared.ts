...
export async function loadValidConfigOrThrow(): Promise<AikaClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: AikaClawConfig) => AikaClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: AikaClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AikaClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("aikaclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
