...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAikaClawAgentDir: vi.fn().mockReturnValue("/tmp/aikaclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/aikaclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/aikaclaw-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/aikaclaw-dev/aikaclaw.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveAikaClawAgentDir: mocks.resolveAikaClawAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/aikaclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAikaClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/aikaclaw-dev/aikaclaw.json");
    expect(payload.auth.storePath).toBe("/tmp/aikaclaw-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/aikaclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/aikaclaw-agent-custom");
