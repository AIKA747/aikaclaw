...
describe("resolveDefaultChannelAccountContext", () => {
  it("uses enabled/configured defaults when hooks are missing", async () => {
    ...
    const result = await resolveDefaultChannelAccountContext(plugin, {} as AikaClawConfig);
  ...
  it("keeps strict mode fail-closed when resolveAccount throws", async () => {
    ...
    await expect(resolveDefaultChannelAccountContext(plugin, {} as AikaClawConfig)).rejects.toThrow(
      /missing secret/i,
    );
  });
