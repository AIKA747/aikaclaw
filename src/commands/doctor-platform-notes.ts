...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".aikaclaw", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: AikaClawConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [
    ["AIKACLAW_GATEWAY_TOKEN", await getenv("AIKACLAW_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["AIKACLAW_GATEWAY_PASSWORD", await getenv("AIKACLAW_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "AIKACLAW_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.AIKACLAW_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- AIKACLAW_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
