...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByAikaClaw ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByAikaClaw ? "installed · " : "";
  ...
  runtime.log(theme.heading("AikaClaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`aikaclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("aikaclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("aikaclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("aikaclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("aikaclaw security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("aikaclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("aikaclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("aikaclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("aikaclaw gateway probe")}`);
  }
}
