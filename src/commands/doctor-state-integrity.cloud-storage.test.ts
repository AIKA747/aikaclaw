...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "AikaClaw", ".aikaclaw");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/aikaclaw-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".aikaclaw");
  ...
  it("anchors cloud detection to OS homedir when AIKACLAW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".aikaclaw");
    const originalAikaClawHome = process.env.AIKACLAW_HOME;
    process.env.AIKACLAW_HOME = "/tmp/aikaclaw-home-override";
    ...
    try {
      ...
      if (originalAikaClawHome === undefined) {
