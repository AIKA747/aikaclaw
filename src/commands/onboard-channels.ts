...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("aikaclaw pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelSetupDmPolicy) => {
    ...
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`aikaclaw pairing approve ${policy.channel} <code>`)}`,
    ...
    if (!plugin) {
      if (adapter) {
        await prompter.note(
          ...
          )}\` and \`${formatCliCommand("aikaclaw plugins enable " + channel)}\`, then restart the gateway.`,
          "Channel setup",
        );
        await refreshStatus(channel);
        return true;
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("aikaclaw channels add")}\``,
        },
      ],
