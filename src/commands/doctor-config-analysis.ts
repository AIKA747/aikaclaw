...
import { AikaClawSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: AikaClawConfig): {
  config: AikaClawConfig;
  removed: string[];
} {
  const parsed = AikaClawSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: AikaClawConfig): void {
