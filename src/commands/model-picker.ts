...
type PromptDefaultModelResult = { model?: string; config?: AikaClawConfig };
...
function resolveConfiguredModelRaw(cfg: AikaClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: AikaClawConfig): string[] {
...
export function applyModelAllowlist(cfg: AikaClawConfig, models: string[]): AikaClawConfig {
