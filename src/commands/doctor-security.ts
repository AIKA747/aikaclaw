...
import type { AikaClawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: AikaClawConfig): string[] {
...
export async function noteSecurityWarnings(cfg: AikaClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("aikaclaw security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.aikaclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("aikaclaw approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("aikaclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("aikaclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("aikaclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("aikaclaw config set gateway.bind loopback")}`,
