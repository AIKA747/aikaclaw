...
export function hasExplicitMemorySearchConfig(cfg: AikaClawConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: AikaClawConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: AikaClawConfig, agentId: string) => { store: { path: string } } | null;
