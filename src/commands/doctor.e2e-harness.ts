...
export const resolveAikaClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadAikaClawPlugins: () => createEmptyPluginRegistry(),
}));
...
beforeEach(() => {
  ...
  resolveAikaClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.AIKACLAW_STATE_DIR;
  originalUpdateInProgress = process.env.AIKACLAW_UPDATE_IN_PROGRESS;
  process.env.AIKACLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-doctor-state-"));
