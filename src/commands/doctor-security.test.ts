...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    note.mockClear();
    pluginRegistry.list = [];
    prevToken = process.env.AIKACLAW_GATEWAY_TOKEN;
    prevPassword = process.env.AIKACLAW_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    ...
    if (prevPassword === undefined) {
      ...
      process.env.AIKACLAW_GATEWAY_PASSWORD = prevPassword;
    }
  });
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as AikaClawConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.AIKACLAW_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as AikaClawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats SecretRef token config as authenticated for exposure warning level", async () => {
    const cfg = {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: { source: "env", provider: "default", id: "AIKACLAW_GATEWAY_TOKEN" },
        },
      },
    } as AikaClawConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as AikaClawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as AikaClawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("aikaclaw approvals get --gateway");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as AikaClawConfig);
    const message = lastMessage();
    expect(message).toContain("[secrets]");
    expect(message).toContain("failed to resolve account");
    expect(message).toContain("Run: aikaclaw security audit --deep");
  });
