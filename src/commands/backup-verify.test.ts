...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-aikaclaw-backup";
...
describe("backupVerifyCommand", () => {
  ...
  async function resetTempHome() {
    ...
    await fs.mkdir(path.join(tempHome.home, ".aikaclaw"), { recursive: true });
    delete process.env.AIKACLAW_CONFIG_PATH;
  }

  beforeAll(async () => {
    tempHome = await createTempHomeEnv("aikaclaw-backup-verify-test-");
  });
  ...
  it("verifies an archive created by backup create", async () => {
    const stateDir = path.join(tempHome.home, ".aikaclaw");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-backup-verify-out-"));
    try {
      await fs.writeFile(path.join(stateDir, "aikaclaw.json"), JSON.stringify({}), "utf8");
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-aikaclaw-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.aikaclaw",
            archivePath: `${rootName}/payload/posix/tmp/.aikaclaw`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "aikaclaw-backup-traversal-",
  ...
  it("fails when archive paths contain backslashes", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "aikaclaw-backup-backslash-",
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".aikaclaw");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-workspace-"));
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.aikaclaw/payload.txt`;
    await withBrokenArchiveFixture(
      {
        tempPrefix: "aikaclaw-backup-duplicate-manifest-",
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "aikaclaw-backup-duplicate-payload-",
        manifestAssetArchivePath: payloadArchivePath,
        payloads: [
