...
import { readConfigFileSnapshot, writeConfigFile, type AikaClawConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("aikaclaw config set gateway.auth.mode token")} or ${formatCliCommand("aikaclaw config set gateway.auth.mode password")}.`,
  ].join(" ");
}
