...
describe("resetCommand", () => {
  ...
  beforeEach(() => {
    ...
    resolveCleanupPlanFromDisk.mockReturnValue({
      stateDir: "/tmp/.aikaclaw",
      configPath: "/tmp/.aikaclaw/aikaclaw.json",
      oauthDir: "/tmp/.aikaclaw/credentials",
      configInsideState: true,
      oauthInsideState: true,
      workspaceDirs: ["/tmp/.aikaclaw/workspace"],
    });
    removePath.mockResolvedValue({ ok: true });
    listAgentSessionDirs.mockResolvedValue(["/tmp/.aikaclaw/agents/main/sessions"]);
  ...
  it("recommends creating a backup before state-destructive reset scopes", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(expect.stringContaining("aikaclaw backup create"));
  });
  ...
  it("does not recommend backup for config-only reset", async () => {
    ...
    expect(runtime.log).not.toHaveBeenCalledWith(expect.stringContaining("aikaclaw backup create"));
  });
});
