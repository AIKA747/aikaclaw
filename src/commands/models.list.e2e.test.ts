...
const ensureAikaClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveAikaClawAgentDir = vi.fn().mockReturnValue("/tmp/aikaclaw-agent");
...
beforeEach(() => {
  ...
  ensureAikaClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureAikaClawModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureAikaClawModelsJson).toHaveBeenCalled();
    expect(ensureAikaClawModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
