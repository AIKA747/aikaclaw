...
export function resolveHooksConfig(cfg: AikaClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AikaClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-aikaclaw-token"] === "string"
      ? req.headers["x-aikaclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-aikaclaw-idempotency-key"]) ||
