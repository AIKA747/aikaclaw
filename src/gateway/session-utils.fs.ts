...
export function attachAikaClawTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__aikaclaw && typeof record.__aikaclaw === "object" && !Array.isArray(record.__aikaclaw)
      ? (record.__aikaclaw as Record<string, unknown>)
      : {};
  return {
    ...record,
    __aikaclaw: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachAikaClawTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "aikaclaw" && model === "delivery-mirror";
