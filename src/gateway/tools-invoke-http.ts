...
import { createAikaClawTools } from "../agents/aikaclaw-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-aikaclaw-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-aikaclaw-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-aikaclaw-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-aikaclaw-thread-id")?.trim() || undefined;
  ...
  const allTools = createAikaClawTools({
