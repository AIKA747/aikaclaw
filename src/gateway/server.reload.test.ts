...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.AIKACLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.AIKACLAW_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.AIKACLAW_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    prevGeminiApiKey = process.env.GEMINI_API_KEY;
    process.env.AIKACLAW_SKIP_CHANNELS = "0";
    delete process.env.AIKACLAW_SKIP_GMAIL_WATCHER;
    delete process.env.AIKACLAW_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.AIKACLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.AIKACLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.AIKACLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.AIKACLAW_SKIP_PROVIDERS;
    } else {
      process.env.AIKACLAW_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    ...
    if (!configPath) {
      throw new Error("AIKACLAW_CONFIG_PATH is not set");
    }
  ...
  async function writeAuthProfileEnvRefStore() {
    ...
    if (!stateDir) {
      throw new Error("AIKACLAW_STATE_DIR is not set");
    }
      ...
      `${JSON.stringify(
        {
          ...
          profiles: {
            missing: {
              type: "api_key",
              provider: "openai",
              keyRef: { source: "env", provider: "default", id: "MISSING_AIKACLAW_AUTH_REF" },
            },
          },
  ...
  it("fails startup when an active exec ref id contains traversal segments", async () => {
    ...
    const previousGatewayTokenEnv = process.env.AIKACLAW_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.AIKACLAW_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
    }
  });
  ...
  it("fails startup when auth-profile secret refs are unresolved", async () => {
    ...
    delete process.env.MISSING_AIKACLAW_AUTH_REF;
    try {
      await expect(withGatewayServer(async () => {})).rejects.toThrow(
        'Environment variable "MISSING_AIKACLAW_AUTH_REF" is missing or empty.',
      );
    } finally {
