...
describe("OpenAI-compatible models HTTP API (e2e)", () => {
  it("serves /v1/models when compatibility endpoints are enabled", async () => {
    ...
    expect(json.data?.map((entry) => entry.id)).toContain("aikaclaw");
    expect(json.data?.map((entry) => entry.id)).toContain("aikaclaw/default");
    expect(
      json.data?.every((entry) => typeof entry.id === "string" && entry.id?.startsWith("aikaclaw")),
    ).toBe(true);
  });
