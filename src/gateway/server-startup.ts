...
import type { loadAikaClawPlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureAikaClawModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: ReturnType<typeof loadConfig>;
  pluginRegistry: ReturnType<typeof loadAikaClawPlugins>;
  ...
  // Start AikaClaw browser control server (unless disabled via config).
  ...
  // Tests can opt out via AIKACLAW_SKIP_CHANNELS (or legacy AIKACLAW_SKIP_PROVIDERS).
  const skipChannels =
    isTruthyEnvValue(process.env.AIKACLAW_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.AIKACLAW_SKIP_PROVIDERS);
  if (!skipChannels) {
    try {
    ...
    params.logChannels.info(
      "skipping channel start (AIKACLAW_SKIP_CHANNELS=1 or AIKACLAW_SKIP_PROVIDERS=1)",
    );
  }
