...
describe("readFirstUserMessageFromTranscript", () => {
  ...
  registerTempSessionStore("aikaclaw-session-fs-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    const marker = out[1] as {
      ...
      __aikaclaw?: { kind?: string; id?: string };
      timestamp?: number;
    };
    expect(marker.role).toBe("system");
    expect(marker.content?.[0]?.text).toBe("Compaction");
    expect(marker.__aikaclaw?.kind).toBe("compaction");
    expect(marker.__aikaclaw?.id).toBe("comp-1");
  ...
  test("reads cross-agent absolute sessionFile across store-root layouts", () => {
    ...
    for (const testCase of cases) {
      ...
      expect((out[0] as { __aikaclaw?: { seq?: number } }).__aikaclaw?.seq).toBe(1);
    }
  });
});
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  registerTempSessionStore("aikaclaw-session-preview-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("readLatestSessionUsageFromTranscript", () => {
  ...
  registerTempSessionStore("aikaclaw-session-usage-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses AIKACLAW_HOME instead of os.homedir()", () => {
    vi.stubEnv("AIKACLAW_HOME", "/srv/aikaclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const candidates = resolveSessionTranscriptCandidates("sess-1", undefined);
    const fallback = candidates[candidates.length - 1];
    expect(fallback).toBe(
      path.join(path.resolve("/srv/aikaclaw-home"), ".aikaclaw", "sessions", "sess-1.jsonl"),
    );
  });
});
...
describe("resolveSessionTranscriptCandidates safety", () => {
  test("keeps cross-agent absolute sessionFile for standard and custom store roots", () => {
    const cases = [
      {
        storePath: "/tmp/aikaclaw/agents/main/sessions/sessions.json",
        sessionFile: "/tmp/aikaclaw/agents/ops/sessions/sess-safe.jsonl",
      },
      {
  ...
  test("drops unsafe session IDs instead of producing traversal paths", () => {
    const candidates = resolveSessionTranscriptCandidates(
      "../etc/passwd",
      "/tmp/aikaclaw/agents/main/sessions/sessions.json",
    );
  ...
  test("drops unsafe sessionFile candidates and keeps safe fallbacks", () => {
    const storePath = "/tmp/aikaclaw/agents/main/sessions/sessions.json";
    const candidates = resolveSessionTranscriptCandidates(
      "sess-safe",
      storePath,
      "../../etc/passwd",
  ...
  test("prefers the current sessionId transcript before a stale sessionFile candidate", () => {
    ...
    const candidates = resolveSessionTranscriptCandidates(
      ...
      storePath,
      "/tmp/aikaclaw/agents/main/sessions/22222222-2222-4222-8222-222222222222.jsonl",
    );

    expect(candidates[0]).toBe(
      path.resolve("/tmp/aikaclaw/agents/main/sessions/11111111-1111-4111-8111-111111111111.jsonl"),
    );
    expect(candidates).toContain(
      path.resolve("/tmp/aikaclaw/agents/main/sessions/22222222-2222-4222-8222-222222222222.jsonl"),
    );
  });

  test("keeps explicit custom sessionFile ahead of synthesized fallback", () => {
    const storePath = "/tmp/aikaclaw/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/aikaclaw/agents/main/sessions/custom-transcript.jsonl";
    const candidates = resolveSessionTranscriptCandidates(
      "11111111-1111-4111-8111-111111111111",
      storePath,
      sessionFile,
  ...
  test("keeps custom topic-like transcript paths ahead of synthesized fallback", () => {
    const storePath = "/tmp/aikaclaw/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/aikaclaw/agents/main/sessions/custom-topic-notes.jsonl";
    const candidates = resolveSessionTranscriptCandidates(
      "11111111-1111-4111-8111-111111111111",
      storePath,
      sessionFile,
  ...
  test("keeps forked transcript paths ahead of synthesized fallback", () => {
    ...
    const sessionFile =
      "/tmp/aikaclaw/agents/main/sessions/2026-03-23T16-30-00-000Z_11111111-1111-4111-8111-111111111111.jsonl";
  ...
  test("keeps timestamped custom transcript paths ahead of synthesized fallback", () => {
    const storePath = "/tmp/aikaclaw/agents/main/sessions/sessions.json";
    const sessionId = "11111111-1111-4111-8111-111111111111";
    const sessionFile = "/tmp/aikaclaw/agents/main/sessions/2026-03-23T16-30-00-000Z_notes.jsonl";
    const candidates = resolveSessionTranscriptCandidates(sessionId, storePath, sessionFile);
  ...
  test("still treats generated topic transcripts from another session as stale", () => {
    ...
    const staleSessionFile =
      "/tmp/aikaclaw/agents/main/sessions/22222222-2222-4222-8222-222222222222-topic-thread.jsonl";
...
describe("archiveSessionTranscripts", () => {
  ...
  registerTempSessionStore("aikaclaw-archive-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });

  beforeAll(() => {
    vi.stubEnv("AIKACLAW_HOME", tmpDir);
  });
