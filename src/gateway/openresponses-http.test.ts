...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("AIKACLAW_CONFIG_PATH is required for gateway config tests");
  }
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "aikaclaw", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `aikaclaw` or `aikaclaw/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "aikaclaw", input: "hi" },
        { "x-aikaclaw-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "aikaclaw/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "aikaclaw/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "aikaclaw", input: "hi" },
        { "x-aikaclaw-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-aikaclaw-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
