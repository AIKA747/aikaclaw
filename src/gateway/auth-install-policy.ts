...
type GatewayInstallAuthMode = NonNullable<NonNullable<AikaClawConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: AikaClawConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    durableServiceEnv.AIKACLAW_GATEWAY_PASSWORD?.trim() ||
