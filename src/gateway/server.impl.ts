...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("aikaclaw doctor")}" to repair, then retry.`
    : "";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.AIKACLAW_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.AIKACLAW_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "AIKACLAW_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "AIKACLAW_RAW_STREAM_PATH",
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `aikaclaw config set gateway.auth.mode token` and `aikaclaw config set gateway.auth.token <token>`.",
      );
    }
  }
    ...
    transcriptUnsub = minimalTestGateway
      ...
      : onSessionTranscriptUpdate((update) => {
          ...
          const sessionSnapshot = sessionRow
            ...
            : {};
          const message = attachAikaClawTranscriptMeta(update.message, {
