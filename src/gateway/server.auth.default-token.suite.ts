...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.AIKACLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.AIKACLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.AIKACLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.AIKACLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers AIKACLAW_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.AIKACLAW_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.AIKACLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.AIKACLAW_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.AIKACLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(75);
        process.env.AIKACLAW_HANDSHAKE_TIMEOUT_MS = "";
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.AIKACLAW_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.AIKACLAW_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.AIKACLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.AIKACLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            AIKACLAW_VERSION: " ",
            AIKACLAW_SERVICE_VERSION: "2.4.6-service",
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `aikaclaw-test-device-${randomUUID()}.json`),
        nonce,
      });
