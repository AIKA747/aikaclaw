...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "AIKACLAW_SKIP_BROWSER_CONTROL_SERVER",
  "AIKACLAW_SKIP_GMAIL_WATCHER",
  "AIKACLAW_SKIP_CANVAS_HOST",
  "AIKACLAW_BUNDLED_PLUGINS_DIR",
  "AIKACLAW_SKIP_CHANNELS",
  "AIKACLAW_SKIP_PROVIDERS",
  "AIKACLAW_SKIP_CRON",
  "AIKACLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.AIKACLAW_CONFIG_PATH) {
    configPaths.add(process.env.AIKACLAW_CONFIG_PATH);
  }
  if (process.env.AIKACLAW_STATE_DIR) {
    configPaths.add(path.join(process.env.AIKACLAW_STATE_DIR, "aikaclaw.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.AIKACLAW_STATE_DIR = path.join(tempHome, ".aikaclaw");
  delete process.env.AIKACLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.AIKACLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.AIKACLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.AIKACLAW_SKIP_CANVAS_HOST = "1";
  process.env.AIKACLAW_SKIP_CHANNELS = "1";
  process.env.AIKACLAW_SKIP_PROVIDERS = "1";
  process.env.AIKACLAW_SKIP_CRON = "1";
  process.env.AIKACLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.AIKACLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "aikaclaw-test-no-bundled-extensions")
    : "aikaclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  applyGatewaySkipEnv();
  const stateDir = process.env.AIKACLAW_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".aikaclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".aikaclaw-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".aikaclaw-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>aikaclaw-test-control-ui</title>\n",
    "utf-8",
  );
...
const CONNECT_CHALLENGE_NONCE_KEY = "__aikaclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__aikaclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["AIKACLAW_GATEWAY_TOKEN"]);
  const prev = process.env.AIKACLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.AIKACLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.AIKACLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
