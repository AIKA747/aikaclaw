...
import { resolveAikaClawAgentDir } from "../agents/agent-paths.js";
...
import { ensureAikaClawModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { clearRuntimeConfigSnapshot, loadConfig } from "../config/config.js";
import type { ModelsConfig, AikaClawConfig, ModelProviderConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.AIKACLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const PROVIDERS = parseFilter(process.env.AIKACLAW_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.AIKACLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.AIKACLAW_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const describeLive = isLiveTestEnabled(["AIKACLAW_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.AIKACLAW_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.AIKACLAW_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<AikaClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.AIKACLAW_CONFIG_PATH,
    token: process.env.AIKACLAW_GATEWAY_TOKEN,
    skipChannels: process.env.AIKACLAW_SKIP_CHANNELS,
    skipGmail: process.env.AIKACLAW_SKIP_GMAIL_WATCHER,
    skipCron: process.env.AIKACLAW_SKIP_CRON,
    skipCanvas: process.env.AIKACLAW_SKIP_CANVAS_HOST,
    agentDir: process.env.AIKACLAW_AGENT_DIR,
  ...
  process.env.AIKACLAW_SKIP_CHANNELS = "1";
  process.env.AIKACLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.AIKACLAW_SKIP_CRON = "1";
  process.env.AIKACLAW_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.AIKACLAW_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveAikaClawAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-live-state-"));
  process.env.AIKACLAW_STATE_DIR = tempStateDir;
  ...
  process.env.AIKACLAW_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.aikaclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveAikaClawAgentDir();
  const sanitizedCfg: AikaClawConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-live-"));
  const tempConfigPath = path.join(tempDir, "aikaclaw.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.AIKACLAW_CONFIG_PATH = tempConfigPath;
  ...
  try {
        ...
        try {
          ...
          for (
            ...
            toolText = await requestGatewayAgentText({
              ...
              modelKey,
              message: strictReply
                ? "AikaClaw live tool probe (local, safe): " +
    ...
    process.env.AIKACLAW_CONFIG_PATH = previous.configPath;
    process.env.AIKACLAW_GATEWAY_TOKEN = previous.token;
    process.env.AIKACLAW_SKIP_CHANNELS = previous.skipChannels;
    process.env.AIKACLAW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.AIKACLAW_SKIP_CRON = previous.skipCron;
    process.env.AIKACLAW_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.AIKACLAW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.AIKACLAW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureAikaClawModelsJson(cfg);
      ...
      const rawModels = process.env.AIKACLAW_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via AIKACLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.aikaclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
