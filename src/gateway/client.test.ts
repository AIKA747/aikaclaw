...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("aikaclaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with AIKACLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with AIKACLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.AIKACLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://aikaclaw-gateway.ai:18789",
      onConnectError,
    });
