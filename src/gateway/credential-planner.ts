...
type GatewaySecretDefaults = NonNullable<AikaClawConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${AIKACLAW_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.AIKACLAW_GATEWAY_TOKEN);
}

export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.AIKACLAW_GATEWAY_PASSWORD);
}
