...
const GATEWAY_TEST_PLUGIN_REGISTRY_STATE_KEY = Symbol.for(
  "aikaclaw.gatewayTestHelpers.pluginRegistryState",
);
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("aikaclaw.gatewayTestHelpers.configRoot");

const hoisted = vi.hoisted(() => {
  const key = Symbol.for("aikaclaw.gatewayTestHelpers.hoisted");
...
const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `aikaclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.AIKACLAW_CONFIG_PATH = path.join(root, "aikaclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "aikaclaw.json");
  ...
  const composeTestConfig = (baseConfig: Record<string, unknown>) => {
    ...
    const defaults = {
      ...
      workspace: path.join(os.tmpdir(), "aikaclaw-gateway-test"),
      ...fileDefaults,
  ...
  return {
    ...
    applyConfigOverrides: (cfg: AikaClawConfig) =>
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadAikaClawPlugins: () => pluginRegistryState.registry,
  };
});
