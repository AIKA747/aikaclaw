...
function cfg(input: Partial<AikaClawConfig>): AikaClawConfig {
  return input as AikaClawConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: AikaClawConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
const DEFAULT_ENV = {
  AIKACLAW_GATEWAY_TOKEN: "env-token",
  AIKACLAW_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when AIKACLAW env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when AIKACLAW env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when AIKACLAW env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
