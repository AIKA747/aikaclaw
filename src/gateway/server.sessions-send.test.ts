...
const { createAikaClawTools } = await import("../agents/aikaclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createAikaClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createAikaClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["AIKACLAW_GATEWAY_PORT", "AIKACLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.AIKACLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.AIKACLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("AIKACLAW_CONFIG_PATH missing in gateway test environment");
      }
