...
import type { AikaClawConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: AikaClawConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: AikaClawConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: AikaClawConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: AikaClawConfig) => void | Promise<void>;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: AikaClawConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: AikaClawConfig) => {
