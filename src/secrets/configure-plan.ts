...
export function buildConfigureCandidates(config: AikaClawConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredAikaClawConfig?: AikaClawConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredAikaClawConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const aikaclawCandidates = discoverConfigSecretTargets(params.config)
    ...
    .map((entry) => {
      ...
      return {
        ...
        label: entry.path,
        configFile: "aikaclaw.json" as const,
  ...
  return [...aikaclawCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "aikaclaw secrets configure",
