...
describe("secrets runtime snapshot integration", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "AIKACLAW_DISABLE_PLUGIN_DISCOVERY_CACHE",
      "AIKACLAW_VERSION",
    ]);
    delete process.env.AIKACLAW_BUNDLED_PLUGINS_DIR;
    process.env.AIKACLAW_DISABLE_PLUGIN_DISCOVERY_CACHE = "1";
    delete process.env.AIKACLAW_VERSION;
  });
  ...
  it("activates runtime snapshots for loadConfig and ensureAuthProfileStore", async () => {
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(
          ensureAuthProfileStore("/tmp/aikaclaw-agent-main").profiles["openai:default"],
        ).toMatchObject({
          type: "api_key",
          key: "sk-runtime",
        });
  ...
  it("keeps active secrets runtime snapshots resolved after config writes", async () => {
    ...
    await withTempHome("aikaclaw-secrets-runtime-write-", async (home) => {
  ...
  it("keeps last-known-good runtime snapshot active when refresh fails after a write", async () => {
    ...
    await withTempHome("aikaclaw-secrets-runtime-refresh-fail-", async (home) => {
      const configDir = path.join(home, ".aikaclaw");
      const secretFile = path.join(configDir, "secrets.json");
      const agentDir = path.join(configDir, "agents", "main", "agent");
      const authStorePath = path.join(agentDir, "auth-profiles.json");
  ...
  it("keeps last-known-good web runtime snapshot when reload introduces unresolved active web refs", async () => {
    await withTempHome("aikaclaw-secrets-runtime-web-reload-lkg-", async (home) => {
      const prepared = await prepareSecretsRuntimeSnapshot({
        config: asConfig({
          tools: {
            web: {
      ...
      const persistedConfig = JSON.parse(
        await fs.readFile(path.join(home, ".aikaclaw", "aikaclaw.json"), "utf8"),
      ) as AikaClawConfig;
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("aikaclaw-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".aikaclaw", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".aikaclaw", "agents", "ops", "agent");
