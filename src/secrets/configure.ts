...
function getSecretProviders(config: AikaClawConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: AikaClawConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: AikaClawConfig): Array<{ value: SecretRefSource; label: string }> {
...
function configureCandidateKey(candidate: {
  configFile: "aikaclaw.json" | "auth-profiles.json";
  path: string;
  agentId?: string;
}): string {
  ...
  return `aikaclaw:${candidate.path}`;
}
...
function resolveConfigureAgentId(config: AikaClawConfig, explicitAgentId?: string): string {
...
async function configureProvidersInteractive(config: AikaClawConfig): Promise<void> {
  while (true) {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    const candidates = buildConfigureCandidatesForScope({
      config: stagedConfig,
      authoredAikaClawConfig: snapshot.resolved,
      authProfiles: {
        agentId: configureAgentId,
        store: authStore,
      },
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          candidate.configFile === "auth-profiles.json" ? "auth-profiles.json" : "aikaclaw.json",
