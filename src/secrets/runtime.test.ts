...
function createOpenAiFileModelsConfig(): NonNullable<AikaClawConfig["models"]> {
  return {
    providers: {
      openai: {
...
describe("secrets runtime snapshot", () => {
  ...
  it("normalizes inline SecretRef object on token to tokenRef", async () => {
    const config: AikaClawConfig = { models: {}, secrets: {} };
    const snapshot = await prepareSecretsRuntimeSnapshot({
      config,
  ...
  it("resolves file refs via configured file provider", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-secrets-file-provider-"));
  ...
  it("fails when file provider payload is not a JSON object", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-secrets-file-provider-bad-"));
    const secretsPath = path.join(root, "secrets.json");
    try {
  ...
  it("does not write inherited auth stores during runtime secret activation", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-secrets-runtime-"));
    ...
    const prevStateDir = process.env.AIKACLAW_STATE_DIR;
    ...
    try {
      ...
      if (prevStateDir === undefined) {
        ...
        process.env.AIKACLAW_STATE_DIR = prevStateDir;
      }
      await fs.rm(root, { recursive: true, force: true });
    }
  });
