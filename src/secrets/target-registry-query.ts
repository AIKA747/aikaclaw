...
const AIKACLAW_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of AIKACLAW_COMPILED_SECRET_TARGETS) {
...
const AIKACLAW_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: AIKACLAW_COMPILED_SECRET_TARGETS,
    entriesById: AIKACLAW_TARGETS_BY_ID,
  });
