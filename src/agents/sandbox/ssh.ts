...
export async function createSshSandboxSessionFromConfigText(params: {
  ...
  const configDir = await fs.mkdtemp(path.join(resolveSshTmpRoot(), "aikaclaw-sandbox-ssh-"));
...
export async function createSshSandboxSessionFromSettings(
  ...
  try {
    ...
    const hostAlias = "aikaclaw-sandbox";
    const configPath = path.join(configDir, "config");
    const lines = [
      `Host ${hostAlias}`,
...
function resolveSshTmpRoot(): string {
  return path.resolve(resolvePreferredAikaClawTmpDir() ?? os.tmpdir());
}
