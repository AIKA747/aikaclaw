...
import { markAikaClawExecEnv } from "../../infra/aikaclaw-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "aikaclaw.sandbox=1");
  args.push("--label", `aikaclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `aikaclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `aikaclaw.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markAikaClawExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "aikaclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`aikaclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`aikaclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("aikaclaw sandbox recreate --all");
}
