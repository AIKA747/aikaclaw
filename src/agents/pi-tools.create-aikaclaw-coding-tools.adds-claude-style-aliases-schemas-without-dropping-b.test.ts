...
const defaultTools = createAikaClawCodingTools({ senderIsOwner: true });
...
describe("createAikaClawCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: AikaClawConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createAikaClawCodingTools({
    ...
    const anthropicTools = createAikaClawCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: AikaClawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createAikaClawCodingTools({
    ...
    const denied = createAikaClawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createAikaClawCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
