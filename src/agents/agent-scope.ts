...
type AgentEntry = NonNullable<NonNullable<AikaClawConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: AikaClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: AikaClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: AikaClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: AikaClawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: AikaClawConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: AikaClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
