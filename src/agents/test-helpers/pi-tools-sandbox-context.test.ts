...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("aikaclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("aikaclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("aikaclaw-sbx-");
