...
const fixtureSuite = createFixtureSuite("aikaclaw-skills-suite-");
...
async function writeEnvSkill(workspaceDir: string) {
  ...
  await writeSkill({
    ...
    metadata: '{"aikaclaw":{"requires":{"env":["ENV_KEY"]},"primaryEnv":"ENV_KEY"}}',
  });
}

beforeAll(async () => {
  await fixtureSuite.setup();
  tempHome = await createTempHomeEnv("aikaclaw-skills-home-");
  await fs.mkdir(path.join(tempHome.home, ".aikaclaw", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("buildWorkspaceSkillCommandSpecs", () => {
  ...
  it("includes enabled Claude bundle markdown commands as native AikaClaw slash commands", async () => {
    const workspaceDir = await makeWorkspace();
    const pluginRoot = path.join(tempHome!.home, ".aikaclaw", "extensions", "compound-bundle");
    ...
    ).toContain("/.aikaclaw/extensions/compound-bundle/commands/workflows-review.md");
  });
});
...
describe("applySkillEnvOverrides", () => {
  ...
  it("blocks unsafe env overrides but allows declared secrets", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata:
        '{"aikaclaw":{"requires":{"env":["OPENAI_API_KEY","NODE_OPTIONS"]},"primaryEnv":"OPENAI_API_KEY"}}',
    });
  ...
  it("blocks dangerous host env overrides even when declared", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"aikaclaw":{"requires":{"env":["BASH_ENV","SHELL"]}}}',
    });
  ...
  it("allows required env overrides from snapshots", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"aikaclaw":{"requires":{"env":["OPENAI_API_KEY"]}}}',
    });
