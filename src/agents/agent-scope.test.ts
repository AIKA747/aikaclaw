...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: AikaClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/aikaclaw" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: AikaClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: AikaClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: AikaClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: AikaClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: AikaClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: AikaClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: AikaClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/aikaclaw");
  });

  it("uses AIKACLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "aikaclaw-home");
    vi.stubEnv("AIKACLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as AikaClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".aikaclaw", "workspace"));
  });

  it("uses AIKACLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "aikaclaw-home");
    vi.stubEnv("AIKACLAW_HOME", home);
    // Clear state dir so it falls back to AIKACLAW_HOME
    vi.stubEnv("AIKACLAW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as AikaClawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".aikaclaw", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/aikaclaw-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/aikaclaw-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-agent-scope-"));
