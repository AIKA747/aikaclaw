    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "aikaclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "aikaclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "aikaclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("aikaclaw-");
  const acpxRoot = await tempDirs.make("aikaclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("aikaclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("aikaclaw-");
  const pluginRoot = await tempDirs.make("aikaclaw-plugin-");
  const outsideDir = await tempDirs.make("aikaclaw-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("aikaclaw-");
    const pluginRoot = await tempDirs.make("aikaclaw-claude-bundle-");
