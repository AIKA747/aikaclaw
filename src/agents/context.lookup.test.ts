...
function mockContextDeps(params: {
  ...
  const ensureAikaClawModelsJson = vi.fn(async () => {});
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveAikaClawAgentDir: () => "/tmp/aikaclaw-agent",
  }));
  ...
  return { ensureAikaClawModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real aikaclaw startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "aikaclaw", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "aikaclaw", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "aikaclaw", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureAikaClawModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureAikaClawModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
