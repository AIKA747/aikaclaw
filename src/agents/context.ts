...
let configuredConfig: AikaClawConfig | undefined;
...
function isLikelyAikaClawCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = path
    .basename(argv[1] ?? "")
    .trim()
    .toLowerCase();
  return (
    entryBasename === "aikaclaw" ||
    entryBasename === "aikaclaw.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real AikaClaw CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureAikaClawModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyAikaClawCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): AikaClawConfig | undefined {
  if (configuredConfig) {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureAikaClawModelsJson(cfg);
    } catch {
