...
function makeFallbacksOnlyCfg(): AikaClawConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): AikaClawConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
// https://github.com/aikaclaw/aikaclaw/issues/23440
const INSUFFICIENT_QUOTA_PAYLOAD =
  '{"type":"error","error":{"type":"insufficient_quota","message":"Your account has insufficient quota balance to run this request."}}';
// Internal AikaClaw compatibility marker, not a provider API contract.
...
describe("runWithModelFallback", () => {
  ...
  describe("fallback behavior with provider cooldowns", () => {
    async function makeAuthStoreWithCooldown(
      ...
      const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-test-"));
      const now = Date.now();
      const store: AuthProfileStore = {
        version: AUTH_STORE_VERSION,
        profiles: {
