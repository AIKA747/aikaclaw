...
let configOverride: AikaClawConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: AikaClawConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/aikaclaw/aikaclaw/issues/31298
    previousFastTestEnv = process.env.AIKACLAW_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.AIKACLAW_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // AIKACLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("AikaClaw runtime context (internal):");
