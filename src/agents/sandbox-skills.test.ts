...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "aikaclaw-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.AIKACLAW_BUNDLED_SKILLS_DIR = bundledDir;
