...
function prependInternalEventContext(
  ...
  if (body.includes("AikaClaw runtime context (internal):")) {
    return body;
  }
...
async function prepareAgentCommandExecution(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("aikaclaw agents list")}" to see configured agents.`,
      );
    }
  }
