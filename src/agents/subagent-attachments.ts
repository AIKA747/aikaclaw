...
function resolveAttachmentLimits(config: AikaClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".aikaclaw", "attachments");
  const relDir = path.posix.join(".aikaclaw", "attachments", attachmentId);
