...
function getTestConfigPath() {
  return path.join(process.env.HOME ?? "", ".aikaclaw", "aikaclaw.json");
}

async function writeAikaClawConfig(config: Record<string, unknown>, pretty = false) {
...
async function writeExecApprovalsConfig(config: Record<string, unknown>) {
  const approvalsPath = path.join(process.env.HOME ?? "", ".aikaclaw", "exec-approvals.json");
...
describe("exec approvals", () => {
  ...
  beforeEach(async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-test-"));
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-test-bin-"));
  ...
  it("denies gateway obfuscated command when approval request times out", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-test-obf-"));
