...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["AIKACLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-subagent-"));
  ...
  it("uses isolated temp state when AIKACLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "aikaclaw-test-state"));
  });
});
