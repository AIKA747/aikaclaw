...
function applyModelProviderToolPolicy(
  ...
  // Models with a native web_search tool cannot receive AikaClaw's
...
function resolveExecConfig(params: { cfg?: AikaClawConfig; agentId?: string }) {
...
export function createAikaClawCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createAikaClawReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
