...
const GOOGLE_LIVE_SINGLE_KEY = "AIKACLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "AIKACLAW_LIVE_ANTHROPIC_KEY",
    listVar: "AIKACLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "AIKACLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `AIKACLAW_LIVE_${base}_KEY`;
