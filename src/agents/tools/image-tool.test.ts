...
type CreateAikaClawCodingToolsArgs = Parameters<PiToolsModule["createAikaClawCodingTools"]>[0];
type MockAikaClawToolsOptions = {
...
vi.mock("../aikaclaw-tools.js", async () => {
  const { createImageTool } = await import("./image-tool.js");
  return {
    createAikaClawTools: vi.fn((options?: MockAikaClawToolsOptions) => {
...
async function createAikaClawCodingToolsWithFreshModules(options?: CreateAikaClawCodingToolsArgs) {
  vi.resetModules();
  const { createAikaClawCodingTools } = await import("../pi-tools.js");
  return createAikaClawCodingTools(options);
}

async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-image-"));
  try {
...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(parentDir, "aikaclaw-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): AikaClawConfig {
  return {
    agents: {
      defaults: {
...
async function withTempSandboxState(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-image-sandbox-"));
...
describe("image tool implicit imageModel config", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox image paths", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      ...
      await withTempAgentDir(async (agentDir) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-outside-"));
  ...
  it("allows workspace images via createAikaClawCodingTools when workspace root is explicit", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      const fetch = stubMinimaxOkFetch();
      await withTempAgentDir(async (agentDir) => {
        const cfg = createMinimaxImageConfig();

        const tools = await createAikaClawCodingToolsWithFreshModules({
          config: cfg,
          agentDir,
          workspaceDir,
        });
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      const res = await tool.execute("t1", {
        prompt: "Describe the image.",
        image: "@/Users/steipete/.aikaclaw/media/inbound/photo.png",
      });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  async function createMinimaxVlmFixture(baseResp: { status_code: number; status_msg: string }) {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg = createMinimaxImageConfig();
