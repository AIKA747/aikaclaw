...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    aikaclaw: process.env.AIKACLAW_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.aikaclaw === undefined) {
      delete process.env.AIKACLAW_GATEWAY_TOKEN;
    } else {
      process.env.AIKACLAW_GATEWAY_TOKEN = envSnapshot.aikaclaw;
    }
  });
  ...
  it("uses AIKACLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.AIKACLAW_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
