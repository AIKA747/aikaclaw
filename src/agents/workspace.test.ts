...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses AIKACLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      AIKACLAW_HOME: "/srv/aikaclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/aikaclaw-home"), ".aikaclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".aikaclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("aikaclaw-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("aikaclaw-workspace-");
    await fs.mkdir(path.join(tempDir, ".aikaclaw"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-workspace-hardlink-"));
    try {
