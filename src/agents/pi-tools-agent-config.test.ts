...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: AikaClawConfig) {
    return createAikaClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<AikaClawConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<AikaClawConfig["agents"]>["list"]>[number]["tools"];
  }): AikaClawConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createAikaClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createAikaClawCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createAikaClawCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createAikaClawCodingTools({
      config: cfg,
    ...
    const bobTools = createAikaClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createAikaClawCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createAikaClawCodingTools({
      config: cfg,
