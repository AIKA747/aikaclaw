...
describe("prepareCliBundleMcpConfig", () => {
  it("injects a merged --mcp-config overlay for claude-cli", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("aikaclaw-cli-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("aikaclaw-cli-bundle-mcp-workspace-");
