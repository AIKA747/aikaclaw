...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.AIKACLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".aikaclaw", `workspace-${profile}`);
  }
  return path.join(home, ".aikaclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".aikaclaw";
