...
describe("auth profile store cache", () => {
  ...
  it("reuses the synced auth store while auth-profiles.json is unchanged", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-auth-store-cache-"));
    const previousAgentDir = process.env.AIKACLAW_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      process.env.AIKACLAW_AGENT_DIR = agentDir;
  ...
  it("refreshes the cached auth store after auth-profiles.json changes", async () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-auth-store-refresh-"));
    const previousAgentDir = process.env.AIKACLAW_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      process.env.AIKACLAW_AGENT_DIR = agentDir;
  ...
  it("re-syncs external CLI credentials after the cache ttl when auth-profiles.json is absent", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-auth-store-missing-"));
    const previousAgentDir = process.env.AIKACLAW_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    vi.useFakeTimers();
