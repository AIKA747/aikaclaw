...
describe("exec PATH login shell merge", () => {
  ...
  it("sets AIKACLAW_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-aikaclaw-shell", {
      command: 'printf "%s" "${AIKACLAW_SHELL:-}"',
    });
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/aikaclaw-ssl-keys.log";
    try {
      ...
      expect(output).not.toContain("/tmp/aikaclaw-ssl-keys.log");
    } finally {
