...
async function runCustomProviderMergeTest(params: {
  ...
  await ensureAikaClawModelsJson({
    models: {
      mode: "merge",
      providers: {
...
function createOpenAiConfigWithResolvedApiKey(mergeMode = false): AikaClawConfig {
  return {
    models: {
...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureAikaClawModelsJson(validated.config);
  ...
  it("fills anthropic-vertex apiKey with the ADC sentinel when models exist", async () => {
    await withTempHome(async () => {
      const adcDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-adc-"));
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureAikaClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
