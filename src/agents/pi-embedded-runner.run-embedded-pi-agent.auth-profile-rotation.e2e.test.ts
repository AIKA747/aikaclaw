...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): AikaClawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): AikaClawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): AikaClawConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `aikaclaw-auth-rotation-${Date.now()}.log`),
    });
