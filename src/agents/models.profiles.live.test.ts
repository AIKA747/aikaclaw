...
import { ensureAikaClawModelsJson } from "./models-config.js";
import { isRateLimitErrorMessage } from "./pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "./pi-model-discovery.js";

const LIVE = isLiveTestEnabled();
const DIRECT_ENABLED = Boolean(process.env.AIKACLAW_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_HEARTBEAT_MS = Math.max(1_000, toInt(process.env.AIKACLAW_LIVE_HEARTBEAT_MS, 30_000));
const LIVE_SETUP_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.AIKACLAW_LIVE_SETUP_TIMEOUT_MS, 45_000),
);
  ...
  it(
    ...
    async () => {
      ...
      await withLiveStageTimeout(
        ensureAikaClawModelsJson(cfg),
        "[live-models] prepare models.json",
      );
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipping (set AIKACLAW_LIVE_MODELS=modern|all|<list>; all=modern)",
        );
        return;
      }
      ...
      const rawModels = process.env.AIKACLAW_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(process.env.AIKACLAW_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(process.env.AIKACLAW_LIVE_MODEL_TIMEOUT_MS, 30_000);
      const maxModels = toInt(process.env.AIKACLAW_LIVE_MAX_MODELS, 0);
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[live-models] capped to ${selectedCandidates.length}/${candidates.length} via AIKACLAW_LIVE_MAX_MODELS=${maxModels}`,
        );
      }
