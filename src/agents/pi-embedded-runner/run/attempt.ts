...
import { resolveAikaClawDocsPath } from "../../docs-path.js";
...
import { createAikaClawCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
const SESSIONS_YIELD_INTERRUPT_CUSTOM_TYPE = "aikaclaw.sessions_yield_interrupt";
const SESSIONS_YIELD_CONTEXT_CUSTOM_TYPE = "aikaclaw.sessions_yield";
const SESSIONS_YIELD_ABORT_SETTLE_TIMEOUT_MS = process.env.AIKACLAW_TEST_FAST === "1" ? 250 : 2_000;
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveAikaClawAgentDir();
    ...
    const docsPath = await resolveAikaClawDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/aikaclaw/aikaclaw/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("aikaclaw:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/aikaclaw/aikaclaw/issues/8643
