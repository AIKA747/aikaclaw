...
function createOllamaProviderConfig(injectNumCtxForOpenAICompat: boolean): AikaClawConfig {
  return {
    models: {
      providers: {
        ollama: {
...
describe("sessions_yield helpers", () => {
  ...
  it("queues a hidden interrupt steering message", () => {
    ...
    expect(steer).toHaveBeenCalledWith(
      expect.objectContaining({
        role: "custom",
        customType: "aikaclaw.sessions_yield_interrupt",
        display: false,
  ...
  it("persists a hidden yield context message without triggering a turn", async () => {
    ...
    expect(sendCustomMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        customType: "aikaclaw.sessions_yield",
        display: false,
  ...
  it("strips trailing yield interrupt artifacts from memory and transcript state", () => {
    ...
    const activeSession = {
      messages: [
        ...
        { role: "custom", customType: "aikaclaw.sessions_yield_interrupt" },
  ...
  it("includes resolved auth profile fields for context-engine afterTurn compaction", () => {
    const legacy = buildAfterTurnRuntimeContext({
      attempt: {
        ...
        agentAccountId: "acct-1",
        authProfileId: "openai:p1",
        config: { plugins: { slots: { contextEngine: "lossless-claw" } } } as AikaClawConfig,
        skillsSnapshot: undefined,
        senderIsOwner: true,
        provider: "openai-codex",
        modelId: "gpt-5.4",
