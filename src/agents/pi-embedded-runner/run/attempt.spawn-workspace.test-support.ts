...
vi.mock("../../docs-path.js", () => ({
  resolveAikaClawDocsPath: async () => undefined,
}));
...
vi.mock("../../pi-tools.js", () => ({
  createAikaClawCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) => [
    {
      name: "sessions_spawn",
      execute: async (
        _callId: string,
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("aikaclaw.cache-ttl", data),
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-ctx-engine-agent-"));
