...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("aikaclaw-bundled-");
  const workspaceDir = await createTempDir("aikaclaw-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: AikaClawConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.AIKACLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
