...
type CoreToolDefinition = {
  ...
  includeInAikaClawGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const aikaclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInAikaClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:aikaclaw": aikaclawTools,
