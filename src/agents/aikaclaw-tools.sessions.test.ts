...
import { __testing as aikaClawToolsTesting, createAikaClawTools } from "./aikaclaw-tools.js";
...
describe("sessions tools", () => {
  ...
  beforeEach(() => {
    ...
    aikaClawToolsTesting.setDepsForTest({
      callGateway: (opts: unknown) => callGatewayMock(opts),
      config: TEST_CONFIG,
    });
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = createAikaClawTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createAikaClawTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
