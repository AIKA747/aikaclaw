...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside aikaclaw tmp root",
      media: path.join(os.tmpdir(), "outside-aikaclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked AikaClaw tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked AikaClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("rejects symlinked AikaClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInAikaClawTmp(
