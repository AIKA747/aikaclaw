...
describe("createAikaClawCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-read-"));
    try {
      ...
      const contents = "Hello from aikaclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createAikaClawCodingTools({ sandbox });
