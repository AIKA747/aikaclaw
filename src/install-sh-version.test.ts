...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-install-sh-"));
  const cliPath = path.join(root, "aikaclaw");
...
AIKACLAW_BIN="$FAKE_AIKACLAW_BIN"
resolve_aikaclaw_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_AIKACLAW_BIN: cliPath,
        AIKACLAW_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("AikaClaw 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("AikaClaw dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("AikaClaw dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-install-stdin-"));
