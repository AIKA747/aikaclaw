...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        AIKACLAW_BASE_ENV: "base",
        AIKACLAW_TO_REMOVE: undefined,
      },
      env: {
        AIKACLAW_TEST_ENV: "ok",
      },
    });

    expect(resolved.AIKACLAW_BASE_ENV).toBe("base");
    expect(resolved.AIKACLAW_TEST_ENV).toBe("ok");
    expect(resolved.AIKACLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.AIKACLAW_CLI).toBe(AIKACLAW_CLI_ENV_VALUE);
  });
