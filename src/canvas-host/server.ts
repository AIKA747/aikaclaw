...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.aikaclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.aikaclawCanvasA2UIAction &&
        typeof window.aikaclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.aikaclawSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("aikaclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android AikaClaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.aikaclawSendUserAction === "function"
        ? window.aikaclawSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.AIKACLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.AIKACLAW_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>AikaClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
