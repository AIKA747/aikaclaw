...
describe("canvas host state dir defaults", () => {
  ...
  it("uses AIKACLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("aikaclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("AikaClaw Canvas");
      } finally {
        await handler.close();
      }
    });
