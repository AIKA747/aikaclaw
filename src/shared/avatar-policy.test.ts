...
describe("avatar policy", () => {
  it("classifies avatar URI and path helpers directly", () => {
    ...
    expect(isWindowsAbsolutePath("C:\\\\avatars\\\\aikaclaw.png")).toBe(true);
  });

  it("accepts workspace-relative avatar paths and rejects URI schemes", () => {
    expect(isWorkspaceRelativeAvatarPath("avatars/aikaclaw.png")).toBe(true);
    expect(isWorkspaceRelativeAvatarPath("C:\\\\avatars\\\\aikaclaw.png")).toBe(true);
  ...
  it("detects avatar-like path strings", () => {
    expect(looksLikeAvatarPath("avatars/aikaclaw.svg")).toBe(true);
    expect(looksLikeAvatarPath("aikaclaw.webp")).toBe(true);
