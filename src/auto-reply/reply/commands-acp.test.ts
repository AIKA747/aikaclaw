...
function createDiscordParams(commandBody: string, cfg: AikaClawConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: AikaClawConfig = baseCfg) {
  const params = createDiscordParams(commandBody, cfg);
  params.ctx.MessageThreadId = defaultThreadId;
  return params;
}

function createTelegramTopicParams(commandBody: string, cfg: AikaClawConfig = baseCfg) {
  const params = buildCommandTestParams(commandBody, cfg, {
    Provider: "telegram",
    Surface: "telegram",
...
function createTelegramDmParams(commandBody: string, cfg: AikaClawConfig = baseCfg) {
  const params = buildCommandTestParams(commandBody, cfg, {
    Provider: "telegram",
    Surface: "telegram",
    OriginatingChannel: "telegram",
...
async function runDiscordAcpCommand(commandBody: string, cfg: AikaClawConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: AikaClawConfig = baseCfg) {
  return handleAcpCommand(createThreadParams(commandBody, cfg), true);
}

async function runTelegramAcpCommand(commandBody: string, cfg: AikaClawConfig = baseCfg) {
  return handleAcpCommand(createTelegramTopicParams(commandBody, cfg), true);
}

async function runTelegramDmAcpCommand(commandBody: string, cfg: AikaClawConfig = baseCfg) {
  return handleAcpCommand(createTelegramDmParams(commandBody, cfg), true);
}

function createMatrixRoomParams(commandBody: string, cfg: AikaClawConfig = baseCfg) {
  const params = buildCommandTestParams(commandBody, cfg, {
    Provider: "matrix",
    Surface: "matrix",
...
function createMatrixThreadParams(commandBody: string, cfg: AikaClawConfig = baseCfg) {
  const params = createMatrixRoomParams(commandBody, cfg);
  params.ctx.MessageThreadId = "$thread-root";
  return params;
}

async function runMatrixAcpCommand(commandBody: string, cfg: AikaClawConfig = baseCfg) {
  return handleAcpCommand(createMatrixRoomParams(commandBody, cfg), true);
}

async function runMatrixThreadAcpCommand(commandBody: string, cfg: AikaClawConfig = baseCfg) {
  return handleAcpCommand(createMatrixThreadParams(commandBody, cfg), true);
}

function createFeishuDmParams(commandBody: string, cfg: AikaClawConfig = baseCfg) {
  const params = buildCommandTestParams(commandBody, cfg, {
    Provider: "feishu",
    Surface: "feishu",
...
async function runFeishuDmAcpCommand(commandBody: string, cfg: AikaClawConfig = baseCfg) {
