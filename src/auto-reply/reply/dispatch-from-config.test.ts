...
const ttsMocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveTtsConfig: vi.fn((_cfg: AikaClawConfig) => ({ mode: "final" })),
  };
});
...
vi.mock("../../tts/tts.js", () => ({
  ...
  resolveTtsConfig: (cfg: AikaClawConfig) => ttsMocks.resolveTtsConfig(cfg),
}));
...
vi.mock("../../tts/tts-config.js", () => ({
  normalizeTtsAutoMode: (value: unknown) => ttsMocks.normalizeTtsAutoMode(value),
  resolveConfiguredTtsMode: (cfg: AikaClawConfig) => ttsMocks.resolveTtsConfig(cfg).mode,
}));

const noAbortResult = { handled: false, aborted: false } as const;
const emptyConfig = {} as AikaClawConfig;
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("emits diagnostics when enabled", async () => {
    setNoAbort();
    const cfg = { diagnostics: { enabled: true } } as AikaClawConfig;
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
      Provider: "slack",
      Surface: "slack",
  ...
  it("routes plugin-owned bindings to the owning plugin before generic inbound claim broadcast", async () => {
    ...
    hookMocks.registry.plugins = [{ id: "aikaclaw-codex-app-server", status: "loaded" }];
    hookMocks.runner.runInboundClaimForPluginOutcome.mockResolvedValue({
      status: "handled",
      result: { handled: true },
    });
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        ...
        pluginRoot: "/Users/huntharo/github/aikaclaw-app-server",
      },
  ...
  it("falls back to AikaClaw once per startup when a bound plugin is missing", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const replyResolver = vi.fn(async () => ({ text: "aikaclaw fallback" }) satisfies ReplyPayload);
    ...
    const firstNotice = (firstDispatcher.sendToolResult as ReturnType<typeof vi.fn>).mock
      .calls[0]?.[0] as ReplyPayload | undefined;
    expect(firstNotice?.text).toContain("Routing this message to AikaClaw instead.");
  ...
  it("falls back to AikaClaw when the bound plugin is loaded but has no inbound_claim handler", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const notice = (dispatcher.sendToolResult as ReturnType<typeof vi.fn>).mock.calls[0]?.[0] as
      | ReplyPayload
      | undefined;
    expect(notice?.text).toContain("Routing this message to AikaClaw instead.");
    expect(replyResolver).toHaveBeenCalledTimes(1);
    expect(hookMocks.runner.runInboundClaim).not.toHaveBeenCalled();
  });
  ...
  it("passes configOverride to replyResolver when provided", async () => {
    ...
    let receivedCfg: AikaClawConfig | undefined;
    const replyResolver = async (
      _ctx: MsgContext,
      _opts?: GetReplyOptions,
      cfgArg?: AikaClawConfig,
    ) => {
      receivedCfg = cfgArg;
      return { text: "hi" } satisfies ReplyPayload;
    };
  ...
  it("does not pass cfg as implicit configOverride when configOverride is not provided", async () => {
    setNoAbort();
    const cfg = { agents: { defaults: { userTimezone: "UTC" } } } as AikaClawConfig;
    const dispatcher = createDispatcher();
