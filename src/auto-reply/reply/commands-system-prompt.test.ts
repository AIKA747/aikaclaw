...
const { createAikaClawCodingToolsMock } = vi.hoisted(() => ({
  createAikaClawCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    createAikaClawCodingToolsMock.mockClear();
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(createAikaClawCodingToolsMock).toHaveBeenCalledWith(
