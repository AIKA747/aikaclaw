...
const workspaceHarness = createCommandWorkspaceHarness("aikaclaw-command-plugins-install-");
...
describe("handleCommands /plugins install", () => {
  ...
  it("installs a plugin from a local path", async () => {
    ...
    await withTempHome("aikaclaw-command-plugins-home-", async () => {
  ...
  it("installs from an explicit clawhub: spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.2.3",
      extensions: ["index.js"],
      packageName: "@aikaclaw/clawhub-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@aikaclaw/clawhub-demo",
    ...
    await withTempHome("aikaclaw-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildCommandTestParams(
        "/plugins install clawhub:@aikaclaw/clawhub-demo@1.2.3",
        {
          commands: {
            text: true,
            plugins: true,
      ...
      expect(installPluginFromClawHubMock).toHaveBeenCalledWith(
        expect.objectContaining({
          spec: "clawhub:@aikaclaw/clawhub-demo@1.2.3",
        }),
      );
      expect(persistPluginInstallMock).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  it("treats /plugin add as an install alias", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.0.0",
      extensions: ["index.js"],
      packageName: "@aikaclaw/alias-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@aikaclaw/alias-demo",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        version: "1.0.0",
    ...
    await withTempHome("aikaclaw-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildCommandTestParams(
        "/plugin add clawhub:@aikaclaw/alias-demo@1.0.0",
        {
          commands: {
            text: true,
            plugins: true,
      ...
      expect(installPluginFromClawHubMock).toHaveBeenCalledWith(
        expect.objectContaining({
          spec: "clawhub:@aikaclaw/alias-demo@1.0.0",
        }),
      );
    });
  });
