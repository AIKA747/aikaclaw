...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop aikaclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@aikaclaw_bot", { botUsername: "aikaclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@aikaclaw_bot", { botUsername: "aikaclaw_bot" })).toBe(true);
