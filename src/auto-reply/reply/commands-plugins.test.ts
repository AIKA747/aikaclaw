...
const workspaceHarness = createCommandWorkspaceHarness("aikaclaw-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".aikaclaw", "extensions", params.pluginId);
...
function buildCfg(): AikaClawConfig {
  return {
    plugins: {
      enabled: true,
    },
