...
export function isCommandEnabled(cfg: AikaClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AikaClawConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: AikaClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AikaClawConfig) {
