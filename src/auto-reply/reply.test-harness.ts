...
type HomeEnvSnapshot = {
  ...
  AIKACLAW_STATE_DIR: string | undefined;
  AIKACLAW_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    AIKACLAW_AGENT_DIR: process.env.AIKACLAW_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".aikaclaw", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.AIKACLAW_STATE_DIR = path.join(home, ".aikaclaw");
    process.env.AIKACLAW_AGENT_DIR = path.join(home, ".aikaclaw", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".aikaclaw", "agent");
