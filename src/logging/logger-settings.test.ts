...
let originalAikaClawLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.AIKACLAW_TEST_FILE_LOG;
  originalAikaClawLogLevel = process.env.AIKACLAW_LOG_LEVEL;
  delete process.env.AIKACLAW_TEST_FILE_LOG;
  delete process.env.AIKACLAW_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.AIKACLAW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalAikaClawLogLevel === undefined) {
    delete process.env.AIKACLAW_LOG_LEVEL;
  } else {
    process.env.AIKACLAW_LOG_LEVEL = originalAikaClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
