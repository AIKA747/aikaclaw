...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "AIKACLAW_ENV_TEST_A";
    const keyB = "AIKACLAW_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "AIKACLAW_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "AIKACLAW_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "AIKACLAW_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "AIKACLAW_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "AIKACLAW_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "AIKACLAW_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "AIKACLAW_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "aikaclaw-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousAikaClawHome = process.env.AIKACLAW_HOME;
    const previousStateDir = process.env.AIKACLAW_STATE_DIR;
    const previousBundledDir = process.env.AIKACLAW_BUNDLED_PLUGINS_DIR;
    process.env.AIKACLAW_HOME = "/srv/aikaclaw-home";
    process.env.AIKACLAW_STATE_DIR = "/srv/aikaclaw-state";
    process.env.AIKACLAW_BUNDLED_PLUGINS_DIR = "/srv/aikaclaw-bundled";
    ...
    try {
      ...
      expect(env.AIKACLAW_HOME).toBeUndefined();
      expect(env.AIKACLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.AIKACLAW_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("AIKACLAW_HOME", previousAikaClawHome);
      restoreEnvKey("AIKACLAW_STATE_DIR", previousStateDir);
      restoreEnvKey("AIKACLAW_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processAikaClawHome: process.env.AIKACLAW_HOME,
          processBundledDir: process.env.AIKACLAW_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.AIKACLAW_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processAikaClawHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.AIKACLAW_HOME).toBe("/srv/aikaclaw-home");
    } finally {
      restoreEnvKey("AIKACLAW_HOME", previousAikaClawHome);
    }
  });
