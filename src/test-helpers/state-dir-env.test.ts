...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    aikaclaw: process.env.AIKACLAW_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.AIKACLAW_STATE_DIR).toBe(snapshot.aikaclaw);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips AIKACLAW_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/aikaclaw-state-dir-test");
    expect(process.env.AIKACLAW_STATE_DIR).toBe("/tmp/aikaclaw-state-dir-test");
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("aikaclaw-state-dir-env-", async ({ tempRoot, stateDir }) => {
      capturedTempRoot = tempRoot;
      capturedStateDir = stateDir;
      expect(process.env.AIKACLAW_STATE_DIR).toBe(stateDir);
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("aikaclaw-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
