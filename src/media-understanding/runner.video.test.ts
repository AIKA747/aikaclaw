...
describe("runCapability video provider wiring", () => {
  it("merges video baseUrl and headers with entry precedence", async () => {
    ...
    await withVideoFixture("aikaclaw-video-merge", async ({ ctx, media, cache }) => {
      const cfg = {
        models: {
          providers: {
            moonshot: {
  ...
  it("auto-selects moonshot for video when google is unavailable", async () => {
    const isolatedAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-video-agent-"));
    try {
      await withEnvAsync(
        {
        ...
        async () => {
          await withVideoFixture("aikaclaw-video-auto-moonshot", async ({ ctx, media, cache }) => {
            const cfg = {
              models: {
                providers: {
                  moonshot: {
