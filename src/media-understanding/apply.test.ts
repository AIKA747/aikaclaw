...
const TEMP_MEDIA_PREFIX = "aikaclaw-media-";
...
function createGroqAudioConfig(): AikaClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): AikaClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): AikaClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: AikaClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredAikaClawTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        AIKACLAW_AGENT_DIR: isolatedAgentDir,
