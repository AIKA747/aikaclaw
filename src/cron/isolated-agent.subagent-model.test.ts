...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "aikaclaw-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".aikaclaw", "sessions");
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<AikaClawConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<AikaClawConfig>,
