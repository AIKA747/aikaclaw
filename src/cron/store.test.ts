...
const { makeStorePath } = createCronStoreHarness({ prefix: "aikaclaw-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses AIKACLAW_HOME for tilde expansion", () => {
    vi.stubEnv("AIKACLAW_HOME", "/srv/aikaclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/aikaclaw-home", "cron", "jobs.json"));
  });
});
