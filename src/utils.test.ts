...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("aikaclaw-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("aikaclaw-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("aikaclaw-lid-a-", async (first) => {
      await withTempDir("aikaclaw-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.aikaclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "aikaclaw-config-dir-"));
    try {
  ...
  it("expands AIKACLAW_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/aikaclaw-home",
      AIKACLAW_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/aikaclaw-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers AIKACLAW_HOME over HOME", () => {
    vi.stubEnv("AIKACLAW_HOME", "/srv/aikaclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/aikaclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $AIKACLAW_HOME prefix when AIKACLAW_HOME is set", () => {
    vi.stubEnv("AIKACLAW_HOME", "/srv/aikaclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/aikaclaw-home")}/.aikaclaw/aikaclaw.json`)).toBe(
      "$AIKACLAW_HOME/.aikaclaw/aikaclaw.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $AIKACLAW_HOME replacement when AIKACLAW_HOME is set", () => {
    vi.stubEnv("AIKACLAW_HOME", "/srv/aikaclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/aikaclaw-home")}/.aikaclaw/aikaclaw.json`),
    ).toBe("config: $AIKACLAW_HOME/.aikaclaw/aikaclaw.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/aikaclaw", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "aikaclaw"),
    );
  });
  ...
  it("prefers AIKACLAW_HOME for tilde expansion", () => {
    vi.stubEnv("AIKACLAW_HOME", "/srv/aikaclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/aikaclaw")).toBe(path.resolve("/srv/aikaclaw-home", "aikaclaw"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/aikaclaw", env)).toBe(path.resolve("/srv/aikaclaw-home", "aikaclaw"));
  });
