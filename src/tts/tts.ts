...
import { resolvePreferredAikaClawTmpDir } from "../infra/tmp-aikaclaw-dir.ts";
import {
...
export function resolveTtsConfig(cfg: AikaClawConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.AIKACLAW_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: AikaClawConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: AikaClawConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredAikaClawTmpDir();
