...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __aikaclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__aikaclawDiagnosticEventsState) {
    globalStore.__aikaclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__aikaclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: AikaClawConfig): boolean {
