...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["AIKACLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.AIKACLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveAikaClawPackageRoot } = await import("./aikaclaw-root.js"));
    ...
    vi.mocked(resolveAikaClawPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveAikaClawPackageRoot).mockResolvedValue("/opt/aikaclaw");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/aikaclaw/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
