...
import { POSIX_AIKACLAW_TMP_DIR, resolvePreferredAikaClawTmpDir } from "./tmp-aikaclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredAikaClawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `aikaclaw-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredAikaClawTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_AIKACLAW_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredAikaClawTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredAikaClawTmpDir", () => {
  it("prefers /tmp/aikaclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_AIKACLAW_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/aikaclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_AIKACLAW_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/aikaclaw when /tmp/aikaclaw is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/aikaclaw when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/aikaclaw exists but is not writable", () => {
    const accessSync = vi.fn((target: string) => {
      if (target === POSIX_AIKACLAW_TMP_DIR) {
  ...
  it("falls back when /tmp/aikaclaw is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/aikaclaw is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/aikaclaw is group/other writable", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ mode: 0o40777 })) });
  });

  it("repairs existing /tmp/aikaclaw permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_AIKACLAW_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_AIKACLAW_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_AIKACLAW_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("tightened permissions on temp dir"));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/aikaclaw after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_AIKACLAW_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_AIKACLAW_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_AIKACLAW_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_AIKACLAW_TMP_DIR, 0o700);
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback AikaClaw temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "aikaclaw");
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredAikaClawTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_AIKACLAW_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback AikaClaw temp dir/);
  });
});
