...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AikaClawConfig | null = null;
  ...
  get(key: string, cfg: AikaClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AikaClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: AikaClawConfig): void {
