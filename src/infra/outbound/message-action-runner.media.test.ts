...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred AikaClaw tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideAikaClaw = path.join(os.tmpdir(), "outside-aikaclaw", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
