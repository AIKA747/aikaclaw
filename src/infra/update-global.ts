...
const PRIMARY_PACKAGE_NAME = "aikaclaw";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const AIKACLAW_MAIN_PACKAGE_SPEC = "github:aikaclaw/aikaclaw#main";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "AikaClaw", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.AIKACLAW_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.AIKACLAW_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return AIKACLAW_MAIN_PACKAGE_SPEC;
  }
