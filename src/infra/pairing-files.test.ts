...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/aikaclaw-state", "devices")).toEqual({
      dir: path.join("/tmp/aikaclaw-state", "devices"),
      pendingPath: path.join("/tmp/aikaclaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/aikaclaw-state", "devices", "paired.json"),
    });
  });
