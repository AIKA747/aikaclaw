...
async function makeTempRoot() {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "aikaclaw-state-dir-"));
  tempRoot = root;
  return root;
}
...
describe("legacy state dir auto-migration", () => {
  it("follows legacy symlink when it points at another legacy dir (clawdbot -> moldbot)", async () => {
    ...
    const targetMarker = path.join(root, ".aikaclaw", "marker.txt");
  ...
  it("skips state-dir migration when AIKACLAW_STATE_DIR is explicitly set", async () => {
    const root = await makeTempRoot();
    const legacyDir = path.join(root, ".clawdbot");
    fs.mkdirSync(legacyDir, { recursive: true });

    const result = await autoMigrateLegacyStateDir({
      env: { AIKACLAW_STATE_DIR: path.join(root, "custom-state") } as NodeJS.ProcessEnv,
      homedir: () => root,
    });
