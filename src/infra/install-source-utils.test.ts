...
const TEMP_DIR_PREFIX = "aikaclaw-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: {
  ...
  const cwd = await createTempDir("aikaclaw-install-source-utils-");
  const archivePath = path.join(cwd, "aikaclaw-plugin-1.2.3.tgz");
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("aikaclaw-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-aikaclaw-archive.tgz");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "aikaclaw-plugin@1.2.3",
          name: "aikaclaw-plugin",
          version: "1.2.3",
          filename: "aikaclaw-plugin-1.2.3.tgz",
    ...
    const result = await runPack("aikaclaw-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "aikaclaw-plugin",
        version: "1.2.3",
        resolvedSpec: "aikaclaw-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "aikaclaw-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "aikaclaw-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\naikaclaw-plugin-1.2.3.tgz\n",
    });
  ...
  it.each([
    ...
    },
    {
      ...
      stdout:
        'npm notice using cache\n[{"id":"aikaclaw-plugin@1.2.3","name":"aikaclaw-plugin","version":"1.2.3","integrity":"sha512-test-integrity","shasum":"abc123"}]\n',
      expectedMetadata: {
        name: "aikaclaw-plugin",
        version: "1.2.3",
        resolvedSpec: "aikaclaw-plugin@1.2.3",
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@aikaclaw/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@aikaclaw/whatsapp", cwd);
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("aikaclaw-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "aikaclaw-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@aikaclaw/plugin-demo@2.0.0",
            filename: "aikaclaw-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@aikaclaw/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "aikaclaw-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@aikaclaw/plugin-demo@2.0.0",
      },
    });
  });
