...
vi.mock("./aikaclaw-root.js", () => ({
  resolveAikaClawPackageRoot: vi.fn(async () => null),
  resolveAikaClawPackageRootSync: vi.fn(() => null),
}));
...
let aikaclawRoot: typeof import("./aikaclaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    aikaclawRoot = await import("./aikaclaw-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/aikaclaw");
    const wrapperArgv1 = abs("fixtures/bin/aikaclaw");
  ...
  it("uses resolveAikaClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/aikaclaw");
    (
      aikaclawRoot.resolveAikaClawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/aikaclaw"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "aikaclaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "aikaclaw.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-aikaclaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/aikaclaw-bundle");
    (
      aikaclawRoot.resolveAikaClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/AikaClaw.app/Contents/MacOS/AikaClaw");
    const bundledUiDir = abs("fixtures/AikaClaw.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/aikaclaw-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
