  ...
  it("includes the cross-platform supervisor hint env vars", () => {
    expect(SUPERVISOR_HINT_ENV_VARS).toEqual(
      expect.arrayContaining([
        "LAUNCH_JOB_LABEL",
        "INVOCATION_ID",
        "AIKACLAW_WINDOWS_TASK_NAME",
        "AIKACLAW_SERVICE_MARKER",
        "AIKACLAW_SERVICE_KIND",
      ]),
    );
  });
});
...
describe("detectRespawnSupervisor", () => {
  it("detects launchd and systemd only from non-blank platform-specific hints", () => {
    expect(detectRespawnSupervisor({ LAUNCH_JOB_LABEL: " ai.aikaclaw.gateway " }, "darwin")).toBe(
      "launchd",
    );
  ...
  it("detects scheduled-task supervision on Windows from either hint family", () => {
    expect(
      detectRespawnSupervisor({ AIKACLAW_WINDOWS_TASK_NAME: "AikaClaw Gateway" }, "win32"),
    ).toBe("schtasks");
    expect(
  ...
  it("ignores service markers on non-Windows platforms and unknown platforms", () => {
    ...
    expect(
      detectRespawnSupervisor({ LAUNCH_JOB_LABEL: "ai.aikaclaw.gateway" }, "freebsd"),
    ).toBeNull();
  });
});
