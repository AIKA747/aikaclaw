...
export type ClawHubPackageCompatibility = {
  ...
  builtWithAikaClawVersion?: string;
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    process.env.AIKACLAW_CLAWHUB_CONFIG_PATH?.trim() ||
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    process.env.AIKACLAW_CLAWHUB_TOKEN?.trim() ||
...
export async function downloadClawHubPackageArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-clawhub-package-"));
...
export async function downloadClawHubSkillArchive(params: {
  ...
  const bytes = new Uint8Array(await response.arrayBuffer());
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-clawhub-skill-"));
