...
describe("detectPackageManager", () => {
  it("prefers packageManager from package.json when supported", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-detect-pm-"));
    await fs.writeFile(
  ...
  it("falls back to lockfiles when package.json is missing or unsupported", async () => {
    const bunRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-detect-pm-"));
    await fs.writeFile(path.join(bunRoot, "bun.lock"), "", "utf8");
    await expect(detectPackageManager(bunRoot)).resolves.toBe("bun");

    const legacyBunRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-detect-pm-"));
    ...
    const npmRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-detect-pm-"));
    await fs.writeFile(
