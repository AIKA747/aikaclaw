  ...
  if (!channel) {
    return {
      warning:
        ...
        'Configure Matrix, then rerun "aikaclaw doctor --fix" or restart the gateway.',
    };
  }
  if (requiresExplicitMatrixDefaultAccount(params.cfg)) {
    return {
      warning:
        ...
        'Set "channels.matrix.defaultAccount" to the intended target account before rerunning "aikaclaw doctor --fix" or restarting the gateway.',
    };
  }
  ...
  if (!target) {
    ...
    return {
      warning:
        ...
        'Start the gateway once with a working Matrix login, or rerun "aikaclaw doctor --fix" after cached credentials are available.',
    };
  }
