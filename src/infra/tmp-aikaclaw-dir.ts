...
export const POSIX_AIKACLAW_TMP_DIR = "/tmp/aikaclaw";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredAikaClawTmpDirOptions = {
...
export function resolvePreferredAikaClawTmpDir(
  options: ResolvePreferredAikaClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "aikaclaw" : `aikaclaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[aikaclaw] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback AikaClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback AikaClaw temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_AIKACLAW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_AIKACLAW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_AIKACLAW_TMP_DIR)) {
      return POSIX_AIKACLAW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_AIKACLAW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_AIKACLAW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_AIKACLAW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_AIKACLAW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_AIKACLAW_TMP_DIR;
