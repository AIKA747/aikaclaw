...
async function withOutsideHardlinkAlias(params: {
  ...
  const outside = await tempDirs.make("aikaclaw-fs-safe-outside-");
...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): Promise<{
  ...
  const root = await tempDirs.make("aikaclaw-fs-safe-root-");
...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("aikaclaw-fs-safe-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("aikaclaw-fs-safe-root-");
    const sourceDir = await tempDirs.make("aikaclaw-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("aikaclaw-fs-safe-root-");
    const outside = await tempDirs.make("aikaclaw-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  it("keeps tilde expansion behavior aligned", async () => {
    ...
    const originalAikaClawHome = process.env.AIKACLAW_HOME;
    const fakeHome = path.resolve(path.sep, "tmp", "fake-home-test");
    process.env.HOME = fakeHome;
    process.env.AIKACLAW_HOME = fakeHome;
    try {
      const result = expandHomePrefix("~/file.txt");
      expect(path.normalize(result)).toBe(path.join(fakeHome, "file.txt"));
    } finally {
      process.env.HOME = originalHome;
      process.env.AIKACLAW_HOME = originalAikaClawHome;
    }

    const root = await tempDirs.make("aikaclaw-tilde-test-");
    process.env.HOME = root;
    process.env.AIKACLAW_HOME = root;
    try {
    ...
    const outsideRoot = await tempDirs.make("aikaclaw-tilde-outside-");
    await expect(
      openFileWithinRoot({
        rootDir: outsideRoot,
        relativePath: "~/escape.txt",
