...
export async function loadMatrixLegacyCryptoInspector(params: {
  ...
  if (resolution.status === "unsafe") {
    throw new Error(
      `Matrix plugin helper path is unsafe: ${resolution.candidatePath}. Reinstall @aikaclaw/matrix and try again.`,
    );
  }
  ...
  const pending = (async () => {
    ...
    if (!inspectLegacyMatrixCryptoStore) {
      throw new Error(
        `Matrix plugin helper at ${helperPath} does not export inspectLegacyMatrixCryptoStore(). Reinstall @aikaclaw/matrix and try again.`,
      );
    }
