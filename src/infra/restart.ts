...
export function triggerAikaClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.AIKACLAW_SYSTEMD_UNIT,
      process.env.AIKACLAW_PROFILE,
    );
  ...
  const label =
    process.env.AIKACLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.AIKACLAW_PROFILE);
