...
const DEFAULT_PACKAGE_NAME = "aikaclaw";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["aikaclaw", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "aikaclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("aikaclaw doctor", doctorArgv, gitRoot, { AIKACLAW_UPDATE_IN_PROGRESS: "1" }),
    );
