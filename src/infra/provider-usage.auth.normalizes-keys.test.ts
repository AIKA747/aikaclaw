...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-provider-auth-suite-"));
  });
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    nodeFs.mkdirSync(path.join(base, ".aikaclaw", "agents", "main", "sessions"), {
      recursive: true,
    });
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".aikaclaw", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      AIKACLAW_STATE_DIR: path.join(home, ".aikaclaw"),
      ...env,
    };
