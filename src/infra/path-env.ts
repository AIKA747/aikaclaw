...
function candidateBinDirs(opts: EnsureAikaClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `aikaclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "aikaclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.AIKACLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "aikaclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `aikaclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAikaClawCliOnPath(opts: EnsureAikaClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.AIKACLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
