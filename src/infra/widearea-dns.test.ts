...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "aikaclaw.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (AikaClaw)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "aikaclaw.internal", expected: "aikaclaw.internal." },
    { value: "aikaclaw.internal.", expected: "aikaclaw.internal." },
    { value: "  aikaclaw.internal  ", expected: "aikaclaw.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { AIKACLAW_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { AIKACLAW_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("aikaclaw.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "aikaclaw.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      ...
      instanceLabel: "studio-london",
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/aikaclaw",
    });

    expect(txt).toContain(`$ORIGIN aikaclaw.internal.`);
    ...
    expect(txt).toContain(`_aikaclaw-gw._tcp IN PTR studio-london._aikaclaw-gw._tcp`);
    expect(txt).toContain(`studio-london._aikaclaw-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (AikaClaw)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/aikaclaw`);
  });
  ...
  it("includes gateway TLS TXT fields and trims display metadata", () => {
    ...
    expect(txt).toContain(`$ORIGIN aikaclaw.internal.`);
    expect(txt).toContain(`studio-london IN A 100.123.224.76`);
    expect(txt).toContain(`studio-london._aikaclaw-gw._tcp IN TXT`);
    expect(txt).toContain(`displayName=Mac Studio (AikaClaw)`);
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("aikaclaw.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("aikaclaw.internal."),
