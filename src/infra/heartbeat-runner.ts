...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: AikaClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: AikaClawConfig) {
...
function resolveHeartbeatAgents(cfg: AikaClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: AikaClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: AikaClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: AikaClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
