...
function writeMatrixPluginFixture(rootDir: string): void {
  ...
  fs.writeFileSync(
    path.join(rootDir, "aikaclaw.plugin.json"),
    JSON.stringify({
      id: "matrix",
      configSchema: {
        type: "object",
...
const matrixHelperEnv = {
  AIKACLAW_BUNDLED_PLUGINS_DIR: (home: string) => path.join(home, "bundled"),
...
describe("matrix legacy encrypted-state migration", () => {
    ...
    await withTempHome(async (home) => {
      ...
      expect(detection.warnings).toContain(
        "Legacy Matrix encrypted state detected at " +
          ...
          ', but multiple Matrix accounts are configured and channels.matrix.defaultAccount is not set. Set "channels.matrix.defaultAccount" to the intended target account before rerunning "aikaclaw doctor --fix" or restarting the gateway.',
      );
    });
  });
  ...
  it("warns instead of throwing when a legacy crypto path is a file", async () => {
    await withTempHome(async (home) => {
      ...
      expect(detection.plans).toHaveLength(0);
      expect(detection.warnings).toContain(
        `Legacy Matrix encrypted state path exists but is not a directory: ${path.join(stateDir, "matrix", "crypto")}. AikaClaw skipped automatic crypto migration for that path.`,
      );
    });
  });
