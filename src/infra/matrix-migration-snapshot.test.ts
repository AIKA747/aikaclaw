...
describe("matrix migration snapshots", () => {
  ...
  it("creates a backup marker after writing a pre-migration snapshot", async () => {
    await withTempHome(async (home) => {
      const archivePath = path.join(home, "Backups", "aikaclaw-migrations", "snapshot.tar.gz");
      fs.mkdirSync(path.dirname(archivePath), { recursive: true });
      fs.writeFileSync(path.join(home, ".aikaclaw", "aikaclaw.json"), "{}\n", "utf8");
      fs.writeFileSync(path.join(home, ".aikaclaw", "state.txt"), "state\n", "utf8");
    ...
    await withTempHome(async (home) => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          version: 1,
          createdAt: "2026-03-10T18:00:00.000Z",
          archivePath: path.join(home, "Backups", "aikaclaw-migrations", "missing.tar.gz"),
          trigger: "older-run",
          includeWorkspace: false,
        }),
        "utf8",
  ...
  it("treats legacy Matrix crypto as warning-only until the plugin helper is available", async () => {
    await withTempHome(
      ...
      {
        env: {
          AIKACLAW_BUNDLED_PLUGINS_DIR: (home) => path.join(home, "empty-bundled"),
        },
      },
    );
  });
