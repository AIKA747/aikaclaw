...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers AIKACLAW_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for AIKACLAW_HOME", () => {
    const result = resolveRequiredHomeDir(
      { AIKACLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when AIKACLAW_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ AIKACLAW_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("resolveOsHomeDir", () => {
  it("ignores AIKACLAW_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          AIKACLAW_HOME: "/srv/aikaclaw-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { AIKACLAW_HOME: "/srv/aikaclaw-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/aikaclaw-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/aikaclaw-home " },
      expected: "/srv/aikaclaw-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/aikaclaw-home/docs"));
  });
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of AIKACLAW_HOME", () => {
    expect(
