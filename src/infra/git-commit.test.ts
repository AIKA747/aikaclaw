...
async function makeTempDir(label: string): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), `aikaclaw-${label}-`));
}
...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the aikaclaw package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "aikaclaw");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "aikaclaw", version: "2026.3.10" }),
      "utf-8",
    );
