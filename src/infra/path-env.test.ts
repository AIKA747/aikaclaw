...
let ensureAikaClawCliOnPath: typeof import("./path-env.js").ensureAikaClawCliOnPath;

describe("ensureAikaClawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "AIKACLAW_PATH_BOOTSTRAPPED",
    "AIKACLAW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureAikaClawCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/aikaclaw-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "aikaclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureAikaClawCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling aikaclaw exists", () => {
    const { tmp, appBinDir, appCli } = setupAppCliRoot("case-bundled");
    process.env.PATH = "/usr/bin";
    delete process.env.AIKACLAW_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.AIKACLAW_PATH_BOOTSTRAPPED = "1";
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "aikaclaw");
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.AIKACLAW_PATH_BOOTSTRAPPED;
      delete process.env.AIKACLAW_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.AIKACLAW_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/aikaclaw-path/case-linuxbrew");
