...
const GATEWAY_DEVICE_ID_HEADER = "x-aikaclaw-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-aikaclaw-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-aikaclaw-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.AIKACLAW_APNS_RELAY_BASE_URL);
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.AIKACLAW_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require AIKACLAW_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.AIKACLAW_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
