...
import { resolveAikaClawPackageRoot, resolveAikaClawPackageRootSync } from "./aikaclaw-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveAikaClawPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "aikaclaw" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1Dir) {
    // aikaclaw.mjs or dist/<bundle>.js
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/aikaclaw -> .../dist/index.js)
