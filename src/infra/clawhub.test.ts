...
describe("clawhub helpers", () => {
  ...
  afterEach(() => {
    delete process.env.AIKACLAW_CLAWHUB_TOKEN;
    delete process.env.CLAWHUB_TOKEN;
    delete process.env.CLAWHUB_AUTH_TOKEN;
    delete process.env.AIKACLAW_CLAWHUB_CONFIG_PATH;
  ...
  it("checks min gateway versions with loose host labels", () => {
    ...
    expect(satisfiesGatewayMinimum("AikaClaw 2026.3.22", "2026.3.0")).toBe(true);
  ...
  it("resolves ClawHub auth token from config.json", async () => {
    const configRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-clawhub-config-"));
    const configPath = path.join(configRoot, "clawhub", "config.json");
    process.env.AIKACLAW_CLAWHUB_CONFIG_PATH = configPath;
  ...
  it("resolves ClawHub auth token from the legacy config path override", async () => {
    const configRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-clawdhub-config-"));
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      const fakeHome = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-clawhub-home-"));
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      const fakeHome = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-clawhub-home-"));
      const xdgRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-clawhub-xdg-"));
  ...
  it("injects resolved auth token into ClawHub requests", async () => {
    process.env.AIKACLAW_CLAWHUB_TOKEN = "env-token-123";
