...
describe("normalizePackageTagInput", () => {
  const packageNames = ["aikaclaw", "@aikaclaw/plugin"] as const;
  ...
  it("strips known package-name prefixes before returning the tag", () => {
    expect(normalizePackageTagInput("aikaclaw@beta", packageNames)).toBe("beta");
    expect(normalizePackageTagInput("@aikaclaw/plugin@2026.2.24", packageNames)).toBe("2026.2.24");
    expect(normalizePackageTagInput("aikaclaw@   ", packageNames)).toBeNull();
  });

  it("treats exact known package names as an empty tag", () => {
    expect(normalizePackageTagInput("aikaclaw", packageNames)).toBeNull();
    expect(normalizePackageTagInput(" @aikaclaw/plugin ", packageNames)).toBeNull();
  });
  ...
  it("returns trimmed raw values when no package prefix matches", () => {
    ...
    expect(normalizePackageTagInput("aikaclawer@beta", packageNames)).toBe("aikaclawer@beta");
  });
});
