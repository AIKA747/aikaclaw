...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.AIKACLAW_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "AikaClaw";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(AikaClaw\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw = process.env.AIKACLAW_MDNS_HOSTNAME?.trim() || "aikaclaw";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
