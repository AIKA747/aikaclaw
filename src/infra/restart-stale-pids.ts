 ...
 * returning control to the caller (typically `triggerAikaClawRestart` →
...
/**
 * Parse aikaclaw gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("aikaclaw")) {
 ...
 * Returns only PIDs that belong to aikaclaw gateway processes (not the current process).
 */
