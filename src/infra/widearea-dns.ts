...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.AIKACLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*aikaclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "aikaclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "aikaclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "aikaclaw-gw");
  ...
  records.push(`_aikaclaw-gw._tcp IN PTR ${instanceLabel}._aikaclaw-gw._tcp`);
  records.push(`${instanceLabel}._aikaclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._aikaclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; aikaclaw-content-hash: ${contentHash}\n${contentBody}`;
}
