...
const REQUIRE_PROFILE_KEYS = isTruthyEnvValue(process.env.AIKACLAW_LIVE_REQUIRE_PROFILE_KEYS);
...
function withPluginsEnabled(cfg: AikaClawConfig): AikaClawConfig {
  return {
    ...cfg,
    plugins: {
      ...cfg.plugins,
...
describeLive("image generation live (provider sweep)", () => {
  it("generates images for every configured image-generation variant with available auth", async () => {
    ...
    const providerFilter = parseCsvFilter(process.env.AIKACLAW_LIVE_IMAGE_GENERATION_PROVIDERS);
    const caseFilter = parseCaseFilter(process.env.AIKACLAW_LIVE_IMAGE_GENERATION_CASES);
    const envModelMap = parseProviderModelMap(process.env.AIKACLAW_LIVE_IMAGE_GENERATION_MODELS);
