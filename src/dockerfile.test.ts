...
describe("Dockerfile", () => {
  it("uses shared multi-arch base image refs for all root Node stages", async () => {
    ...
    expect(dockerfile).toContain("FROM ${AIKACLAW_NODE_BOOKWORM_IMAGE} AS ext-deps");
    expect(dockerfile).toContain("FROM ${AIKACLAW_NODE_BOOKWORM_IMAGE} AS build");
    expect(dockerfile).toContain("FROM ${AIKACLAW_NODE_BOOKWORM_IMAGE} AS base-default");
    expect(dockerfile).toContain("FROM ${AIKACLAW_NODE_BOOKWORM_SLIM_IMAGE} AS base-slim");
  ...
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG AIKACLAW_INSTALL_BROWSER");
  ...
  it("pins bundled plugin discovery to copied source extensions in runtime images", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
    expect(dockerfile).toContain("ENV AIKACLAW_BUNDLED_PLUGINS_DIR=/app/extensions");
  });
  ...
  it("Docker GPG fingerprint awk uses correct quoting for AIKACLAW_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
