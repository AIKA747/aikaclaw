...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".aikaclaw");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.AIKACLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$AIKACLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via AIKACLAW_STATE_DIR.
