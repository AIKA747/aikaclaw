...
const resolveExistingKey = vi.hoisted(() =>
  vi.fn<(config: AikaClawConfig, provider: string) => string | undefined>(() => undefined),
);
const hasExistingKey = vi.hoisted(() =>
  vi.fn<(config: AikaClawConfig, provider: string) => boolean>(() => false),
);
...
const listConfiguredWebSearchProviders = vi.hoisted(() =>
  vi.fn<(params?: { config?: AikaClawConfig }) => PluginWebSearchProviderEntry[]>(() => []),
);
...
function createEnabledFirecrawlSearchConfig(): AikaClawConfig {
  return {
    tools: {
      web: {
        search: {
...
describe("finalizeSetupWizard", () => {
  ...
  it("resolves gateway password SecretRef for probe and TUI", async () => {
    ...
    process.env.AIKACLAW_GATEWAY_PASSWORD = "resolved-gateway-password"; // pragma: allowlist secret
