...
const readConfigFileSnapshot = vi.hoisted(() =>
  vi.fn(async () => ({
    path: "/tmp/.aikaclaw/aikaclaw.json",
    exists: false,
...
vi.mock("../commands/onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "/tmp/aikaclaw-workspace",
...
describe("runSetupWizard", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-onboard-suite-"));
  });
  ...
  it("resolves gateway.auth.password SecretRef for local setup probe", async () => {
    ...
    process.env.AIKACLAW_GATEWAY_PASSWORD = "gateway-ref-password"; // pragma: allowlist secret
