...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "AikaClaw is a hobby project and still in beta. Expect sharp edges.",
      "By default, AikaClaw is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "AikaClaw is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run AikaClaw.",
...
export async function runSetupWizard(
  ...
  await prompter.intro("AikaClaw setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: AikaClawConfig = snapshot.valid ? (snapshot.exists ? snapshot.config : {}) : {};
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("aikaclaw doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("aikaclaw doctor")}`,
        `Inspect: ${formatCliCommand("aikaclaw plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("aikaclaw configure")}.`;
  ...
  let localGatewayToken = process.env.AIKACLAW_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.AIKACLAW_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: AikaClawConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
