...
function localAikaClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "aikaclaw",
...
function createLocalAikaClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localAikaClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveAikaClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-reset-"));
    const profileDir = path.join(tempRoot, "aikaclaw");
    ...
    const ops = createLocalAikaClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveAikaClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "aikaclaw");
    fs.mkdirSync(profileDir, { recursive: true });
