...
vi.mock("./chrome.js", () => ({
  resolveAikaClawUserDataDir: vi.fn(() => "/tmp/aikaclaw-test/aikaclaw/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveAikaClawUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "aikaclaw-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveAikaClawUserDataDir).mockReturnValue(userDataDir);
