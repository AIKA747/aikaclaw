import type { BrowserConfig, BrowserProfileConfig, AikaClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_AIKACLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_AIKACLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "aikaclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_AIKACLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_AIKACLAW_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  rootConfig?: AikaClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_AIKACLAW_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_AIKACLAW_BROWSER_PROFILE_NAME]
        ? DEFAULT_AIKACLAW_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "aikaclaw";
