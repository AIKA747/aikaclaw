...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    ...
    if (!aikaclaw) {
      throw new Error("expected aikaclaw profile");
    }
    ...
    const state: BrowserServerState = {
      ...
      profiles: new Map([
        [
          "stale-removed",
          {
            profile: { ...aikaclaw, name: "stale-removed" },
            running: null,
          },
        ],
      ]),
    ...
    expect(listKnownProfileNames(state).toSorted()).toEqual(["aikaclaw", "stale-removed", "user"]);
  });
});
