  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          aikaclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  ...
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.AIKACLAW_GATEWAY_PORT;
    process.env.AIKACLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.AIKACLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.AIKACLAW_GATEWAY_PASSWORD;
