...
import type { BrowserProfileConfig, AikaClawConfig } from "../config/config.js";
...
export function createBrowserProfilesService(ctx: BrowserRouteContext) {
  ...
  const createProfile = async (params: CreateProfileParams): Promise<CreateProfileResult> => {
    ...
    const nextConfig: AikaClawConfig = {
      ...cfg,
      browser: {
        ...cfg.browser,
        profiles: {
  ...
  const deleteProfile = async (nameRaw: string): Promise<DeleteProfileResult> => {
    ...
    if (resolved?.cdpIsLoopback && resolved.driver === "aikaclaw") {
      try {
      ...
      const userDataDir = resolveAikaClawUserDataDir(name);
