...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredAikaClawTmpDir: vi.fn(() => "/tmp/aikaclaw"),
}));
vi.mock("../infra/tmp-aikaclaw-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredAikaClawTmpDir.mockReturnValue("/tmp/aikaclaw");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aikaclaw-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".aikaclaw-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredAikaClawTmpDir.mockReturnValue("/tmp/aikaclaw-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "aikaclaw-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "aikaclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredAikaClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "aikaclaw-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "aikaclaw-preferred", "downloads")}${path.sep}`),
    );
  });
