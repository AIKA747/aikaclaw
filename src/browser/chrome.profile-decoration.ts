...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".aikaclaw-profile-decorated");
}
 ...
 */
export function decorateAikaClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_AIKACLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_AIKACLAW_BROWSER_COLOR).toUpperCase();
