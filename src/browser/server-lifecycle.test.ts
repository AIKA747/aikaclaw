...
const { stopAikaClawChromeMock } = vi.hoisted(() => ({
  stopAikaClawChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopAikaClawChrome: stopAikaClawChromeMock,
}));
...
beforeEach(async () => {
  ...
  stopAikaClawChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["aikaclaw", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      aikaclaw: vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap.aikaclaw).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopAikaClawChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("aikaclaw browser stop failed: Error: oops");
  });
});
