...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: AikaClawConfig, policy: ChannelAccessPolicy) => AikaClawConfig;
  resolveAllowlist: (params: { cfg: AikaClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: AikaClawConfig; resolved: TResolved }) => AikaClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: AikaClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: AikaClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: AikaClawConfig, policy: ChannelAccessPolicy): AikaClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: AikaClawConfig, policy: ChannelAccessPolicy): AikaClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: AikaClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: AikaClawConfig; resolved: string[] }) => {
      calls.push("apply");
