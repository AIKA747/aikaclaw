...
import type { AikaClawConfig } from "../../config/types.aikaclaw.js";
import {
...
describe("group policy warning builders", () => {
  ...
  it("projects cfg-only warning collector inputs", () => {
    const collect = projectConfigWarningCollector<{ cfg: AikaClawConfig; accountId: string }>(
      ({ cfg }) => [cfg.channels ? "configured" : "none"],
    );

    expect(
      collect({
        cfg: { channels: { slack: {} } } as AikaClawConfig,
        accountId: "acct-1",
      }),
    ).toEqual(["configured"]);
  });
  ...
  it("projects account+cfg warning collector inputs", () => {
    const collect = projectAccountConfigWarningCollector<
      ...
      { account: { accountId: string }; cfg: AikaClawConfig }
    >(
      (cfg: AikaClawConfig) => cfg.channels ?? {},
