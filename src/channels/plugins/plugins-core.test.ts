...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@aikaclaw/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-catalog-"));
  ...
  it("preserves plugin ids when they differ from channel ids", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-channel-catalog-state-"));
  ...
  it("uses the provided env for external catalog path resolution", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-catalog-home-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@aikaclaw/env-demo-channel",
            aikaclaw: {
              channel: {
                id: "env-demo-channel",
              ...
              install: {
                npmSpec: "@aikaclaw/env-demo-channel",
              },
            },
          },
        ],
    ...
    const ids = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        AIKACLAW_PLUGIN_CATALOG_PATHS: "~/catalog.json",
        AIKACLAW_HOME: home,
        HOME: home,
      },
    }).map((entry) => entry.id);
  ...
  it("uses the provided env for default catalog paths", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-catalog-state-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@aikaclaw/default-env-demo",
            aikaclaw: {
              channel: {
                id: "default-env-demo",
              ...
              install: {
                npmSpec: "@aikaclaw/default-env-demo",
              },
            },
          },
        ],
  ...
  it("includes bundled metadata-only channel entries even when the runtime entrypoint is omitted", () => {
    const packageRoot = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-bundled-catalog-"));
    const bundledDir = path.join(packageRoot, "dist", "extensions", "whatsapp");
    fs.mkdirSync(bundledDir, { recursive: true });
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "aikaclaw" }),
      "utf8",
    );
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(bundledDir, "aikaclaw.plugin.json"),
    ...
    const entry = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        AIKACLAW_BUNDLED_PLUGINS_DIR: path.join(packageRoot, "dist", "extensions"),
      },
    }).find((item) => item.id === "whatsapp");

    expect(entry?.install.npmSpec).toBe("@aikaclaw/whatsapp");
    expect(entry?.pluginId).toBe("whatsapp");
  });

  it("includes shipped official channel catalog entries when bundled metadata is omitted", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-official-catalog-"));
  ...
  it("lets external catalogs override shipped fallback channel metadata", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-fallback-catalog-"));
    ...
    const entry = listChannelPluginCatalogEntries({
      ...
      env: {
        ...process.env,
        AIKACLAW_BUNDLED_PLUGINS_DIR: path.join(dir, "dist", "extensions"),
      },
    }).find((item) => item.id === "whatsapp");
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: AikaClawConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
