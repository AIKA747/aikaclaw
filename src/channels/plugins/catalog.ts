...
import { discoverAikaClawPlugins } from "../../plugins/discovery.js";
import { loadPluginManifest } from "../../plugins/manifest.js";
import type { AikaClawPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, AikaClawPackageManifest>>;
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = [
    resolveAikaClawPackageRootSync({ cwd: process.cwd() }),
    resolveAikaClawPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<AikaClawPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: AikaClawPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function loadBundledMetadataCatalogEntries(options: CatalogOptions): ChannelPluginCatalogEntry[] {
  ...
  for (const dirent of fs.readdirSync(bundledDir, { withFileTypes: true })) {
    ...
    const entry = buildCatalogEntry({
      ...
      packageManifest: packageJson.aikaclaw,
    });
    if (entry) {
      entries.push(entry);
    }
