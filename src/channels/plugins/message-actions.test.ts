...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(listChannelMessageCapabilities({} as AikaClawConfig).toSorted()).toEqual([
      "buttons",
      "cards",
      "interactive",
    ]);
    expect(channelSupportsMessageCapability({} as AikaClawConfig, "interactive")).toBe(true);
    expect(channelSupportsMessageCapability({} as AikaClawConfig, "buttons")).toBe(true);
    expect(channelSupportsMessageCapability({} as AikaClawConfig, "cards")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as AikaClawConfig, channel: "discord" },
        "interactive",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as AikaClawConfig, channel: "telegram" },
        "interactive",
      ),
    ).toBe(false);
    expect(
    ...
    expect(channelSupportsMessageCapabilityForChannel({ cfg: {} as AikaClawConfig }, "cards")).toBe(
      false,
    );
  });
  ...
  it("uses unified message tool discovery for actions, capabilities, and schema", () => {
    ...
    expect(listChannelMessageActions({} as AikaClawConfig)).toEqual(["send", "broadcast", "react"]);
    expect(listChannelMessageCapabilities({} as AikaClawConfig)).toEqual(["interactive"]);
    expect(
      resolveChannelMessageToolSchemaProperties({
        cfg: {} as AikaClawConfig,
        channel: "discord",
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(listChannelMessageActions({} as AikaClawConfig)).toEqual(["send", "broadcast"]);
    expect(listChannelMessageCapabilities({} as AikaClawConfig)).toEqual([]);
