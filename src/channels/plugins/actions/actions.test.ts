...
function telegramCfg(): AikaClawConfig {
  return { channels: { telegram: { botToken: "tok" } } } as AikaClawConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: AikaClawConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as AikaClawConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as AikaClawConfig;
...
function createSignalAccountOverrideCfg(): AikaClawConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("derives discord action listings from channel and moderation gates", () => {
    const cases = [
      {
        name: "defaults",
        cfg: { channels: { discord: { token: "d0" } } } as AikaClawConfig,
        expectUploads: true,
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as AikaClawConfig,
        accountId: undefined,
        params: {
