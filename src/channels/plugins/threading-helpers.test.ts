...
describe("createStaticReplyToModeResolver", () => {
  it("always returns the configured mode", () => {
    expect(createStaticReplyToModeResolver("off")({ cfg: {} as AikaClawConfig })).toBe("off");
    expect(createStaticReplyToModeResolver("all")({ cfg: {} as AikaClawConfig })).toBe("all");
  });
});
...
describe("createTopLevelChannelReplyToModeResolver", () => {
  it("reads the top-level channel config", () => {
    ...
    expect(
      resolver({
        cfg: { channels: { discord: { replyToMode: "first" } } } as AikaClawConfig,
      }),
    ).toBe("first");
  });
  ...
  it("falls back to off", () => {
    const resolver = createTopLevelChannelReplyToModeResolver("discord");
    expect(resolver({ cfg: {} as AikaClawConfig })).toBe("off");
  });
});
...
describe("createScopedAccountReplyToModeResolver", () => {
  ...
  it("passes chatType through", () => {
    ...
    expect(resolver({ cfg: {} as AikaClawConfig, chatType: "group" })).toBe("first");
