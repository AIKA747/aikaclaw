...
describe("channel action capability matrix", () => {
  ...
  function getCapabilities(plugin: Pick<ChannelPlugin, "actions">, cfg: AikaClawConfig) {
  ...
  it("forwards Telegram action capabilities through the channel wrapper", () => {
    ...
    const result = getCapabilities(telegramPlugin, {} as AikaClawConfig);
    ...
    const discordResult = getCapabilities(discordPlugin, {} as AikaClawConfig);
