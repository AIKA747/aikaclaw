...
type SetupContractEntry = {
  ...
  cases: Array<{
    ...
    beforeTest?: () => void;
    assertPatchedConfig?: (cfg: AikaClawConfig) => void;
    assertResolvedAccount?: (account: unknown, cfg: AikaClawConfig) => void;
  }>;
};
...
bundledChannelRuntimeSetters.setLineRuntime({
  channel: {
    line: {
      listLineAccountIds,
      resolveDefaultLineAccountId,
      resolveLineAccount: ({ cfg, accountId }: { cfg: AikaClawConfig; accountId?: string }) =>
...
const matrixSessionBindingStateDir = fs.mkdtempSync(
  path.join(os.tmpdir(), "aikaclaw-matrix-session-binding-contract-"),
);
