...
describe("docker build cache layout", () => {
  ...
  it("uses pnpm cache mounts in Dockerfiles that install repo dependencies", async () => {
    for (const path of [
      ...
      expect(
        ...
        /--mount=type=cache,id=aikaclaw-pnpm-store,target=\/(?:root|home\/appuser)\/\.local\/share\/pnpm\/store,sharing=locked/,
      );
    }
  });
  ...
  it("copies only install inputs before pnpm install in the e2e image", async () => {
    ...
    expect(
      indexOfPattern(
        dockerfile,
        /^COPY(?:\s+--chown=\S+)?\s+tsconfig\.json tsconfig\.plugin-sdk\.dts\.json tsdown\.config\.ts vitest\.config\.ts vitest\.e2e\.config\.ts vitest\.performance-config\.ts aikaclaw\.mjs \.\/$/m,
      ),
