...
# AikaClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://aikaclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one AikaClaw."
...
GUM_VERSION="${AIKACLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 AikaClaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://aikaclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$AIKACLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.aikaclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_aikaclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.aikaclaw-* "$npm_root"/aikaclaw 2>/dev/null || true
}

extract_aikaclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_aikaclaw_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/aikaclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/aikaclaw"|"/usr/local/bin/aikaclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/aikaclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale aikaclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing aikaclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing AikaClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_aikaclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "AikaClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*aikaclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_aikaclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_aikaclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "AikaClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an aikaclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${AIKACLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${AIKACLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((AIKACLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${AIKACLAW_NO_ONBOARD:-0}
NO_PROMPT=${AIKACLAW_NO_PROMPT:-0}
DRY_RUN=${AIKACLAW_DRY_RUN:-0}
INSTALL_METHOD=${AIKACLAW_INSTALL_METHOD:-}
AIKACLAW_VERSION=${AIKACLAW_VERSION:-latest}
USE_BETA=${AIKACLAW_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/aikaclaw"
GIT_DIR=${AIKACLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${AIKACLAW_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${AIKACLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${AIKACLAW_VERBOSE:-0}"
VERIFY_INSTALL="${AIKACLAW_VERIFY_INSTALL:-0}"
AIKACLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
AikaClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://aikaclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/aikaclaw)
...
Environment variables:
  AIKACLAW_INSTALL_METHOD=git|npm
  AIKACLAW_VERSION=latest|next|main|<semver>|<spec>
  AIKACLAW_BETA=0|1
  AIKACLAW_GIT_DIR=...
  AIKACLAW_GIT_UPDATE=0|1
  AIKACLAW_NO_PROMPT=1
  AIKACLAW_VERIFY_INSTALL=1
  AIKACLAW_DRY_RUN=1
  AIKACLAW_NO_ONBOARD=1
  AIKACLAW_VERBOSE=1
  AIKACLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://aikaclaw.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://aikaclaw.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://aikaclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected AikaClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a AikaClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_aikaclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"aikaclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://aikaclaw.ai/install.sh | bash"
}
...
ensure_aikaclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/aikaclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/aikaclaw" ]]; then
        ln -sf "$npm_root/aikaclaw/dist/entry.js" "${npm_bin}/aikaclaw"
        ui_info "Created aikaclaw bin link at ${npm_bin}/aikaclaw"
    fi
    return 0
}

# Check for existing AikaClaw installation
check_existing_aikaclaw() {
    if [[ -n "$(type -P aikaclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing AikaClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make aikaclaw show as \"command not found\" in new terminals."
...
warn_aikaclaw_not_found() {
    ui_warn "Installed, but aikaclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t aikaclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named aikaclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_aikaclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P aikaclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/aikaclaw" ]]; then
        echo "${npm_bin}/aikaclaw"
        return 0
    fi
...
install_aikaclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/aikaclaw/aikaclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing AikaClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing AikaClaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning AikaClaw" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building AikaClaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/aikaclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/aikaclaw"
    ui_success "AikaClaw wrapper installed to \$HOME/.local/bin/aikaclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view aikaclaw dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:aikaclaw/aikaclaw#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_aikaclaw() {
    local package_name="aikaclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            AIKACLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="aikaclaw"
        else
            AIKACLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${AIKACLAW_VERSION}" ]]; then
        AIKACLAW_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${AIKACLAW_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${AIKACLAW_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing AikaClaw v${resolved_version}"
    else
        ui_info "Installing AikaClaw (${AIKACLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${AIKACLAW_VERSION}")"

    if ! install_aikaclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_aikaclaw_paths
        install_aikaclaw_npm "${install_spec}"
    fi

    if [[ "${AIKACLAW_VERSION}" == "latest" && "${package_name}" == "aikaclaw" ]]; then
        if ! resolve_aikaclaw_bin &> /dev/null; then
            ui_warn "npm install aikaclaw@latest failed; retrying aikaclaw@next"
            cleanup_npm_aikaclaw_paths
            install_aikaclaw_npm "aikaclaw@next"
        fi
    fi

    ensure_aikaclaw_bin_link || true

    ui_success "AikaClaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${AIKACLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_aikaclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (aikaclaw not on PATH yet)"
        warn_aikaclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${AIKACLAW_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.aikaclaw/workspace-${profile}"
    else
        echo "${HOME}/.aikaclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${AIKACLAW_CONFIG_PATH:-$HOME/.aikaclaw/aikaclaw.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run aikaclaw onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but aikaclaw not on PATH; skipping onboarding"
        warn_aikaclaw_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run aikaclaw onboard to retry"
        return
    }
}
...
resolve_aikaclaw_version() {
    local version=""
    local raw_version_output=""
    local claw="${AIKACLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v aikaclaw &> /dev/null; then
        claw="$(command -v aikaclaw)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_aikaclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/aikaclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/aikaclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: aikaclaw not on PATH yet"
        warn_aikaclaw_not_found
        return 1
    fi

    run_quiet_step "Checking AikaClaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: aikaclaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_aikaclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found AikaClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set AIKACLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_aikaclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing AikaClaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g aikaclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g aikaclaw 2>/dev/null || true
        ...
        install_aikaclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/aikaclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/aikaclaw"
    ...
    AIKACLAW_BIN="$(resolve_aikaclaw_bin || true)"
    ...
    installed_version=$(resolve_aikaclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 AikaClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 AikaClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/aikaclaw"
        ui_kv "Update command" "aikaclaw update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://aikaclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running aikaclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                AIKACLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                AIKACLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                AIKACLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run aikaclaw doctor and aikaclaw plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run aikaclaw onboard later"
        else
            local config_path="${AIKACLAW_CONFIG_PATH:-$HOME/.aikaclaw/aikaclaw.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (aikaclaw not on PATH yet)"
                    warn_aikaclaw_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run aikaclaw onboard to finish setup"
            return 0
        fi
    fi

    if command -v aikaclaw &> /dev/null; then
        local claw="${AIKACLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_aikaclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (aikaclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if AIKACLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: aikaclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${AIKACLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
