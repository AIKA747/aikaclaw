...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "aikaclaw.plugin.json");
    ...
    if (packageJson.aikaclaw && "extensions" in packageJson.aikaclaw) {
      packageJson.aikaclaw = {
        ...packageJson.aikaclaw,
        extensions: rewritePackageExtensions(packageJson.aikaclaw.extensions),
        ...(typeof packageJson.aikaclaw.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.aikaclaw.setupEntry) }
          : {}),
      };
    }
