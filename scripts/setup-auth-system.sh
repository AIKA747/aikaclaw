#!/bin/bash
# Setup AikaClaw Auth Management System
...
echo "=== AikaClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'aikaclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  AikaClaw message: Send warning via AikaClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/aikaclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/aikaclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/aikaclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now aikaclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status aikaclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
