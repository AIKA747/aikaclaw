...
# Build and bundle AikaClaw into a minimal .app we can open.
# Outputs to dist/AikaClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/AikaClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="AikaClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.aikaclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/aikaclaw/aikaclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/AikaClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AikaClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AikaClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/AikaClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/AikaClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/AikaClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/AikaClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/AikaClaw/Resources/AikaClaw.icns" "$APP_ROOT/Contents/Resources/AikaClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/AikaClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AikaClawKit resources"
AIKACLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AikaClawKit_AikaClawKit.bundle"
if [ -d "$AIKACLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AikaClawKit_AikaClawKit.bundle"
  cp -R "$AIKACLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AikaClawKit_AikaClawKit.bundle"
else
  echo "WARN: AikaClawKit resource bundle not found at $AIKACLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running AikaClaw"
killall -q AikaClaw 2>/dev/null || true
