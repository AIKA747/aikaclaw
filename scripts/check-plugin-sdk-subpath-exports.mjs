...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("aikaclaw/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("aikaclaw/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced aikaclaw/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced aikaclaw/plugin-sdk/<subpath> must exist in the public package exports.",
  );
