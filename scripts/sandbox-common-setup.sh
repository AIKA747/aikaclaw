...
BASE_IMAGE="${BASE_IMAGE:-aikaclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-aikaclaw-sandbox-common:bookworm-slim}"
...
AIKACLAW_DOCKER_BUILD_USE_BUILDX="${AIKACLAW_DOCKER_BUILD_USE_BUILDX:-0}"
AIKACLAW_DOCKER_BUILD_CACHE_FROM="${AIKACLAW_DOCKER_BUILD_CACHE_FROM:-}"
AIKACLAW_DOCKER_BUILD_CACHE_TO="${AIKACLAW_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${AIKACLAW_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${AIKACLAW_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${AIKACLAW_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${AIKACLAW_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${AIKACLAW_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=aikaclaw.sandbox=1)
NOTE
