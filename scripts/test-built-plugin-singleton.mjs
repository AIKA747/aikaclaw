...
const warningFilterKey = Symbol.for("aikaclaw.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadAikaClawPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadAikaClawPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@aikaclaw/build-smoke-plugin",
      type: "module",
      aikaclaw: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'aikaclaw/plugin-sdk';",
...
const registry = loadAikaClawPlugins({
  ...
  env: {
    ...process.env,
    AIKACLAW_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
