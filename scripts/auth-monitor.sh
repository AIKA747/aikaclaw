...
#
# Suggested cron: */30 * * * * /home/admin/aikaclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send AikaClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., aikaclaw-alerts)
...
STATE_FILE="$HOME/.aikaclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via AikaClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use aikaclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via AikaClaw to $NOTIFY_PHONE..."
            aikaclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! AikaClaw is down. Run: ssh l36 '~/aikaclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
