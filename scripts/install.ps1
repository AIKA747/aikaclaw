...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 AikaClaw Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one AikaClaw.$NC" -Level info
    Write-Host ""
}
...
function Install-AikaClawGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing AikaClaw from git..." -Level info
    ...
    @"
@echo off
node "%~dp0..\aikaclaw\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\aikaclaw.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install AikaClaw from git to $GitDir" -Level info
        } else {
            Install-AikaClawGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install AikaClaw via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-AikaClawNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'aikaclaw onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 AikaClaw installed successfully!" -Level success
}
