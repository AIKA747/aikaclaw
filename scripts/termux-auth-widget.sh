...
# AikaClaw Auth Widget for Termux
...
SERVER="${AIKACLAW_SERVER:-l36}"

# Check auth status
termux-toast "Checking AikaClaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/aikaclaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/aikaclaw/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|AIKACLAW_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/aikaclaw/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|AIKACLAW_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: AikaClaw won't work until re-auth"
                ;;
        esac
        ;;
