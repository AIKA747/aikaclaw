...
import { discoverAikaClawPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']aikaclaw\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']aikaclaw\/plugin-sdk\/compat["']/;
...
function main() {
  ...
  if (monolithicOffenders.length > 0 || legacyCompatOffenders.length > 0) {
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic aikaclaw/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy aikaclaw/plugin-sdk/compat.",
      );
    ...
    if (monolithicOffenders.length > 0 || legacyCompatOffenders.length > 0) {
      console.error(
        "Use aikaclaw/plugin-sdk/<domain> or aikaclaw/plugin-sdk/<channel> subpaths for bundled plugins; root and compat are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
