...
- regenerates apps/ios/AikaClaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${AIKACLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid AIKACLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid AIKACLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid AIKACLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid AIKACLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing AIKACLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(AIKACLAW_URL_SLASH)$(AIKACLAW_URL_SLASH)#g'
)"
...
AIKACLAW_CODE_SIGN_STYLE = Automatic
AIKACLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
AIKACLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
AIKACLAW_APP_BUNDLE_ID = ai.aikaclaw.client
AIKACLAW_SHARE_BUNDLE_ID = ai.aikaclaw.client.share
AIKACLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.aikaclaw.client.activitywidget
AIKACLAW_WATCH_APP_BUNDLE_ID = ai.aikaclaw.client.watchkitapp
AIKACLAW_WATCH_EXTENSION_BUNDLE_ID = ai.aikaclaw.client.watchkitapp.extension
AIKACLAW_APP_PROFILE =
AIKACLAW_SHARE_PROFILE =
AIKACLAW_PUSH_TRANSPORT = relay
AIKACLAW_PUSH_DISTRIBUTION = official
AIKACLAW_URL_SLASH = /
AIKACLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
AIKACLAW_PUSH_APNS_ENVIRONMENT = production
EOF
