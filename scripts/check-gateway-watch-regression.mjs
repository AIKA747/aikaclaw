...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir) {
  const shellSource = [
    'echo "$$" > "$AIKACLAW_WATCH_PID_FILE"',
    "exec node scripts/watch-node.mjs gateway --force --allow-unconfigured",
  ].join("\n");
  const env = {
    AIKACLAW_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    AIKACLAW_HOME: isolatedHomeDir,
  };
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "aikaclaw-gateway-watch-"));
