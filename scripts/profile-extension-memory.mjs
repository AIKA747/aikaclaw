...
const RSS_MARKER = "__AIKACLAW_MAX_RSS_KB__=";
...
async function main() {
  ...
  const tmpHome = mkdtempSync(path.join(os.tmpdir(), "aikaclaw-extension-memory-"));
  const hookPath = path.join(tmpHome, "measure-rss.mjs");
  const jsonPath = options.jsonPath ?? path.join(os.tmpdir(), "aikaclaw-extension-memory.json");
