 ...
 * `aikaclaw update`. Use it to verify the completion installation flow
...
\`aikaclaw update\`, \`aikaclaw doctor\`, and \`aikaclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# AikaClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`aikaclaw completion --install\` later to enable.`));
    return;
  }
