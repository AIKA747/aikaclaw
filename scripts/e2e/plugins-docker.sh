...
IMAGE_NAME="aikaclaw-plugins-e2e"
...
if [ -f dist/index.mjs ]; then
  AIKACLAW_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  AIKACLAW_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export AIKACLAW_ENTRY

home_dir=$(mktemp -d "/tmp/aikaclaw-plugins-e2e.XXXXXX")
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$AIKACLAW_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$AIKACLAW_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$AIKACLAW_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
JS
  cat > "$dir/aikaclaw.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/aikaclaw.plugin.json" "$id"
const fs = require("node:fs");
...
mkdir -p "$HOME/.aikaclaw/extensions/demo-plugin"

cat > "$HOME/.aikaclaw/extensions/demo-plugin/index.js" <<'JS'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
cat > "$HOME/.aikaclaw/extensions/demo-plugin/aikaclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$AIKACLAW_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/aikaclaw-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@aikaclaw/demo-plugin-tgz",
  "version": "0.0.1",
  "aikaclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/aikaclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$AIKACLAW_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$AIKACLAW_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/aikaclaw-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@aikaclaw/demo-plugin-dir",
  "version": "0.0.1",
  "aikaclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/aikaclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$AIKACLAW_ENTRY" plugins install "$dir_plugin"
node "$AIKACLAW_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/aikaclaw-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@aikaclaw/demo-plugin-file",
  "version": "0.0.1",
  "aikaclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/aikaclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$AIKACLAW_ENTRY" plugins install "file:$file_pack_dir/package"
node "$AIKACLAW_ENTRY" plugins list --json > /tmp/plugins4.json
...
bundle_root="$HOME/.aikaclaw/extensions/claude-bundle-e2e"
...
const configPath = path.join(process.env.HOME, ".aikaclaw", "aikaclaw.json");
...
gateway_log="/tmp/aikaclaw-plugin-command-e2e.log"
start_gateway "$gateway_log"
wait_for_gateway_health

echo "Testing /plugin install with auto-restart..."
slash_install_dir="$(mktemp -d "/tmp/aikaclaw-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@aikaclaw/slash-install-plugin",
  "version": "0.0.1",
  "aikaclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/aikaclaw.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "aikaclaw/fixture-marketplace"
    }
  }
}
JSON

node "$AIKACLAW_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$AIKACLAW_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$AIKACLAW_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$AIKACLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$AIKACLAW_ENTRY" plugins update marketplace-shortcut --dry-run
node "$AIKACLAW_ENTRY" plugins update marketplace-shortcut
node "$AIKACLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
