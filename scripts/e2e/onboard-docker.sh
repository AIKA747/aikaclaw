...
IMAGE_NAME="aikaclaw-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/aikaclaw-bin:$PATH"
  mkdir -p /tmp/aikaclaw-bin
  cat > /tmp/aikaclaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/aikaclaw-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$AIKACLAW_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_aikaclaw_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/aikaclaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/aikaclaw-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$AIKACLAW_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/aikaclaw-e2e-$1.XXXXXX"
  }

  set_isolated_aikaclaw_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export AIKACLAW_HOME="$home_dir"
    export AIKACLAW_STATE_DIR="$home_dir/.aikaclaw"
    export AIKACLAW_CONFIG_PATH="$AIKACLAW_STATE_DIR/aikaclaw.json"
    mkdir -p "$AIKACLAW_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$AIKACLAW_STATE_DIR/workspace"
    config_path="$AIKACLAW_CONFIG_PATH"
    sessions_dir="$AIKACLAW_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_aikaclaw_env "$home_dir"
	    ...
	    node "$AIKACLAW_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$AIKACLAW_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$AIKACLAW_ENTRY\" configure --section skills" send_skills_flow
