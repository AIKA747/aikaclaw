...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/aikaclaw-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/aikaclaw-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/aikaclaw-install.sh
  guest_exec /usr/bin/env AIKACLAW_NO_ONBOARD=1 bash /tmp/aikaclaw-install.sh "${version_args[@]}" --no-onboard
  guest_exec aikaclaw --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec aikaclaw --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" aikaclaw onboard \
    --non-interactive \
    --mode local \
...
verify_local_turn() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" aikaclaw agent \
    --local \
    --agent main \
    --message ping \
    --json
