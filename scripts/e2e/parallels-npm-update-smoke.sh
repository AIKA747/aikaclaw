...
PACKAGE_SPEC=""
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/aikaclaw-parallels-npm-update.XXXXXX)"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: aikaclaw@latest
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/aikaclaw-main-$CURRENT_HEAD_SHORT.tgz"
...
start_server() {
  ...
  ) >/tmp/aikaclaw-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
run_windows_script_via_log() {
  ...
  runner_name="aikaclaw-update-$RANDOM-$RANDOM.ps1"
  log_name="aikaclaw-update-$RANDOM-$RANDOM.log"
  done_name="aikaclaw-update-$RANDOM-$RANDOM.done"
...
run_macos_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$MACOS_VM" --current-user /usr/bin/tee /tmp/aikaclaw-main-update.sh >/dev/null
set -euo pipefail
...
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/aikaclaw-main-update.tgz
/opt/homebrew/bin/npm install -g /tmp/aikaclaw-main-update.tgz
version="\$(/opt/homebrew/bin/aikaclaw --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
/opt/homebrew/bin/aikaclaw models set openai/gpt-5.4
/opt/homebrew/bin/aikaclaw gateway status --deep --require-rpc
/opt/homebrew/bin/aikaclaw agent --agent main --session-id parallels-npm-update-macos-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$MACOS_VM" --current-user /bin/bash /tmp/aikaclaw-main-update.sh
}
...
\$env:PATH = "\$env:LOCALAPPDATA\AikaClaw\deps\portable-git\cmd;\$env:LOCALAPPDATA\AikaClaw\deps\portable-git\mingw64\bin;\$env:LOCALAPPDATA\AikaClaw\deps\portable-git\usr\bin;\$env:PATH"
\$tgz = Join-Path \$env:TEMP 'aikaclaw-main-update.tgz'
...
& \$aikaclaw gateway restart >> \$log 2>&1
Start-Sleep -Seconds 5
& \$aikaclaw gateway status --deep --require-rpc >> \$log 2>&1
\$output = & \$aikaclaw agent --agent main --session-id parallels-npm-update-windows-$head_short --message 'Reply with exact ASCII text OK only.' --json 2>&1
if (\$null -ne \$output) {
...
run_linux_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/aikaclaw-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/aikaclaw-main-update.tgz
npm install -g /tmp/aikaclaw-main-update.tgz --no-fund --no-audit
version="\$(aikaclaw --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
...
aikaclaw models set openai/gpt-5.4
aikaclaw agent --local --agent main --session-id parallels-npm-update-linux-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" /bin/bash /tmp/aikaclaw-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="aikaclaw@$LATEST_VERSION"
fi
