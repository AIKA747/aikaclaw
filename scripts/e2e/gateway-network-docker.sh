...
IMAGE_NAME="aikaclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="aikaclaw-net-e2e-$$"
GW_NAME="aikaclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "AIKACLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "AIKACLAW_SKIP_CHANNELS=1" \
  -e "AIKACLAW_SKIP_GMAIL_WATCHER=1" \
  -e "AIKACLAW_SKIP_CRON=1" \
  -e "AIKACLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
