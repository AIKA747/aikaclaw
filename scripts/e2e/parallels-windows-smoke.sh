...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-aikaclaw-native-e2e-2026-03-12"
MODE="both"
OPENAI_API_KEY_ENV="OPENAI_API_KEY"
INSTALL_URL="https://aikaclaw.ai/install.ps1"
HOST_PORT="18426"
...
MINGIT_ZIP_NAME=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/aikaclaw-parallels-windows.XXXXXX)"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-aikaclaw-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --openai-api-key-env <var> Host env var name for OpenAI API key.
                             Default: OPENAI_API_KEY
  --install-url <url>        Installer URL for latest release. Default: https://aikaclaw.ai/install.ps1
...
\$aikaclaw = Join-Path \$env:APPDATA 'npm\aikaclaw.cmd'
\$args = $args_literal
if ('${env_name_q}' -ne '') {
  Set-Item -Path ('Env:' + '${env_name_q}') -Value '${env_value_q}'
}
# aikaclaw.cmd preserves multi-word --message args reliably here; Start-Process
# against the shim can re-split argv and make Commander reject the turn.
\$output = & \$aikaclaw @args 2>&1
if (\$null -ne \$output) {
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "aikaclaw-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_exec cmd.exe /d /s /c "if exist \"%LOCALAPPDATA%\\AikaClaw\\deps\\portable-git\" rmdir /s /q \"%LOCALAPPDATA%\\AikaClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "if not exist \"%LOCALAPPDATA%\\AikaClaw\\deps\" mkdir \"%LOCALAPPDATA%\\AikaClaw\\deps\""
  guest_exec cmd.exe /d /s /c "mkdir \"%LOCALAPPDATA%\\AikaClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "curl.exe -fsSL \"$mingit_url\" -o \"%TEMP%\\$MINGIT_ZIP_NAME\""
  guest_exec cmd.exe /d /s /c "tar.exe -xf \"%TEMP%\\$MINGIT_ZIP_NAME\" -C \"%LOCALAPPDATA%\\AikaClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "del /q \"%TEMP%\\$MINGIT_ZIP_NAME\" & set \"PATH=%LOCALAPPDATA%\\AikaClaw\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\AikaClaw\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\AikaClaw\\deps\\portable-git\\usr\\bin;%PATH%\" && git.exe --version"
}
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/aikaclaw-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
& (Join-Path \$env:APPDATA 'npm\aikaclaw.cmd') --version
EOF
)"
...
install_main_tgz() {
  ...
  run_windows_retry "main tgz install" 2 \
    guest_exec cmd.exe /d /s /c "set \"PATH=%LOCALAPPDATA%\\AikaClaw\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\AikaClaw\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\AikaClaw\\deps\\portable-git\\usr\\bin;%PATH%\" && curl.exe -fsSL \"$tgz_url\" -o \"%TEMP%\\$temp_name\" && npm.cmd install -g \"%TEMP%\\$temp_name\" --no-fund --no-audit && \"%APPDATA%\\npm\\aikaclaw.cmd\" --version"
}

verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_aikaclaw "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  ...
  runner_name="aikaclaw-onboard-$RANDOM-$RANDOM.ps1"
  log_name="aikaclaw-onboard-$RANDOM-$RANDOM.log"
  done_name="aikaclaw-onboard-$RANDOM-$RANDOM.done"
...
try {
  \$aikaclaw = Join-Path \$env:APPDATA 'npm\aikaclaw.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice openai-api-key --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --accept-risk --json > "{1}" 2>&1' -f \$aikaclaw, \$log)
...
verify_gateway() {
  guest_run_aikaclaw "" "" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_run_aikaclaw "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_aikaclaw "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_aikaclaw "" "" gateway status --deep
}

verify_turn() {
  guest_run_aikaclaw "" "" agent --agent main --message "Reply with exact ASCII text OK only." --json
}
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "aikaclaw-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "aikaclaw-main-upgrade.tgz" || return $?
