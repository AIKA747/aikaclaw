...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.aikaclaw) ? packageJson.aikaclaw : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.aikaclaw?.channel?.id) || left.name;
    const rightId = trimString(right.aikaclaw?.channel?.id) || right.name;
