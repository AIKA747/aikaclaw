...
function stripWorkspaceAikaclawDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.aikaclaw !== "workspace:*") {
    return false;
  }
  delete devDeps.aikaclaw;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceAikaclawDevDependency(pkg);
