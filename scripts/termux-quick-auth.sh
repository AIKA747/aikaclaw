...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/aikaclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "AikaClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/aikaclaw/scripts/mobile-reauth.sh'" --id aikaclaw-auth
        ;;
    *)
