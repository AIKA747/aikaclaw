#!/usr/bin/env bash
# ClawDock - Docker helpers for AikaClaw
# Inspired by Simon Willison's "Running AikaClaw in Docker"
# https://til.simonwillison.net/llms/aikaclaw-docker
#
# Installation:
...
# Common paths to check for AikaClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/aikaclaw"
  "${HOME}/workspace/aikaclaw"
  "${HOME}/projects/aikaclaw"
  "${HOME}/dev/aikaclaw"
  "${HOME}/code/aikaclaw"
  "${HOME}/src/aikaclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found AikaClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/aikaclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ AikaClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^AIKACLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d aikaclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart aikaclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f aikaclaw-gateway
}
...
clawdock-workspace() {
  cd ~/.aikaclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec aikaclaw-gateway \
    bash -c 'echo "alias aikaclaw=\"./aikaclaw.mjs\"" > /tmp/.bashrc_aikaclaw && bash --rcfile /tmp/.bashrc_aikaclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm aikaclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build aikaclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "AIKACLAW_GATEWAY_TOKEN=$token" aikaclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" aikaclaw-gateway \
    bash -c './aikaclaw.mjs config set gateway.remote.token "$TOKEN" && ./aikaclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec aikaclaw-gateway \
    bash -c "./aikaclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart aikaclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm aikaclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec aikaclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'aikaclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for AikaClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (aikaclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to aikaclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.aikaclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'aikaclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'aikaclaw status')"
  echo ""
