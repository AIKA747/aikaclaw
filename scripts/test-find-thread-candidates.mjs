...
export function parseArgs(argv) {
  const envLimit = readEnvNumber("AIKACLAW_TEST_THREAD_CANDIDATE_LIMIT");
  return parseFlagArgs(
    argv,
    {
      config: "vitest.unit.config.ts",
      limit: Number.isFinite(envLimit) ? Math.max(1, Math.floor(envLimit)) : 20,
      minDurationMs: readEnvNumber("AIKACLAW_TEST_THREAD_CANDIDATE_MIN_DURATION_MS") ?? 250,
      minGainMs: readEnvNumber("AIKACLAW_TEST_THREAD_CANDIDATE_MIN_GAIN_MS") ?? 100,
      minGainPct: readEnvNumber("AIKACLAW_TEST_THREAD_CANDIDATE_MIN_GAIN_PCT") ?? 10,
      json: false,
      files: [],
    },
    [
