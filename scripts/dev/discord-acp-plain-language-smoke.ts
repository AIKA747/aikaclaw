...
type DriverMode = "token" | "webhook" | "aikaclaw";
...
function resolveStateDir(): string {
  const override = process.env.AIKACLAW_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.AIKACLAW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".aikaclaw");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver aikaclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) AikaClaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|aikaclaw> Driver transport mode (default: token)\n" +
    ...
    "  --aikaclaw-bin <path>        AikaClaw CLI binary for driver=aikaclaw (default: aikaclaw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  AIKACLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  AIKACLAW_DISCORD_SMOKE_DRIVER\n" +
    "  AIKACLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  AIKACLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  AIKACLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  AIKACLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  AIKACLAW_DISCORD_SMOKE_AGENT\n" +
    "  AIKACLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  AIKACLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  AIKACLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  AIKACLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  AIKACLAW_DISCORD_SMOKE_AIKACLAW_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.AIKACLAW_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.AIKACLAW_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "aikaclaw"
        ? "aikaclaw"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.AIKACLAW_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.AIKACLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.AIKACLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.AIKACLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.AIKACLAW_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.AIKACLAW_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.AIKACLAW_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.AIKACLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.AIKACLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.AIKACLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const aikaclawBin =
    resolveArg("--aikaclaw-bin") || process.env.AIKACLAW_DISCORD_SMOKE_AIKACLAW_BIN || "aikaclaw";
...
async function aikaclawCliJson<T>(params: { aikaclawBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.aikaclawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`aikaclaw ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await aikaclawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    aikaclawBin: params.aikaclawBin,
    args: [
      "message",
      "read",
      "--channel",
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `aikaclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await aikaclawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("aikaclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from AikaClaw.`,
        diagnostics: {
          bindingCandidates: [
            {
