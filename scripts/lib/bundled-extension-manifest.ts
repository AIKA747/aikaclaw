...
export function collectBundledExtensionManifestErrors(extensions: BundledExtension[]): string[] {
  ...
  for (const extension of extensions) {
    const install = extension.packageJson.aikaclaw?.install;
    if (install !== undefined && !isRecord(install)) {
      errors.push(
        `bundled extension '${extension.id}' manifest invalid | aikaclaw.install must be an object`,
      );
      continue;
    }
    ...
    if (
      ...
      errors.push(
        `bundled extension '${extension.id}' manifest invalid | aikaclaw.install.npmSpec must be a non-empty string`,
      );
    }
