...
import { parseReleaseVersion } from "../aikaclaw-npm-release-check.ts";
...
export function collectPublishablePluginPackageErrors(
  ...
  const extensions = packageJson.aikaclaw?.extensions ?? [];

  if (!packageName.startsWith("@aikaclaw/")) {
    errors.push(
      `package name must start with "@aikaclaw/"; found "${packageName || "<missing>"}".`,
    );
  }
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("aikaclaw.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("aikaclaw.extensions must contain only non-empty strings.");
  }
...
export function collectPublishablePluginPackages(
  ...
  for (const dir of dirs) {
    ...
    if (packageJson.aikaclaw?.release?.publishToNpm !== true) {
      continue;
    }
    ...
    publishable.push({
      ...
      installNpmSpec: packageJson.aikaclaw?.install?.npmSpec?.trim() || undefined,
    });
  }
...
export function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "aikaclaw-plugin-npm-view-"));
