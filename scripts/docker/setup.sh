...
EXTRA_MOUNTS="${AIKACLAW_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${AIKACLAW_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${AIKACLAW_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${AIKACLAW_DOCKER_SOCKET:-}"
TIMEZONE="${AIKACLAW_TZ:-}"
...
read_config_gateway_token() {
  local config_path="$AIKACLAW_CONFIG_DIR/aikaclaw.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == AIKACLAW_GATEWAY_TOKEN=* ]]; then
      token="${line#AIKACLAW_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
ensure_control_ui_allowed_origins() {
  if [[ "${AIKACLAW_GATEWAY_BIND}" == "loopback" ]]; then
    return 0
  fi

  local allowed_origin_json
  local current_allowed_origins
  allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$AIKACLAW_GATEWAY_PORT" "$AIKACLAW_GATEWAY_PORT")"
...
sync_gateway_mode_and_bind() {
  ...
  run_prestart_cli config set gateway.bind "$AIKACLAW_GATEWAY_BIND" >/dev/null
  echo "Pinned gateway.mode=local and gateway.bind=$AIKACLAW_GATEWAY_BIND for Docker setup."
}
...
run_prestart_cli() {
  # During setup, avoid the shared-network aikaclaw-cli service because it
  ...
  run_prestart_gateway --entrypoint node aikaclaw-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" aikaclaw-cli "$@"
}
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "AIKACLAW_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "AIKACLAW_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
AIKACLAW_CONFIG_DIR="${AIKACLAW_CONFIG_DIR:-$HOME/.aikaclaw}"
AIKACLAW_WORKSPACE_DIR="${AIKACLAW_WORKSPACE_DIR:-$HOME/.aikaclaw/workspace}"

validate_mount_path_value "AIKACLAW_CONFIG_DIR" "$AIKACLAW_CONFIG_DIR"
validate_mount_path_value "AIKACLAW_WORKSPACE_DIR" "$AIKACLAW_WORKSPACE_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "AIKACLAW_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "AIKACLAW_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "AIKACLAW_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "AIKACLAW_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "AIKACLAW_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "AIKACLAW_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$AIKACLAW_CONFIG_DIR"
mkdir -p "$AIKACLAW_WORKSPACE_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$AIKACLAW_CONFIG_DIR/identity"
mkdir -p "$AIKACLAW_CONFIG_DIR/agents/main/agent"
mkdir -p "$AIKACLAW_CONFIG_DIR/agents/main/sessions"

export AIKACLAW_CONFIG_DIR
export AIKACLAW_WORKSPACE_DIR
export AIKACLAW_GATEWAY_PORT="${AIKACLAW_GATEWAY_PORT:-18789}"
export AIKACLAW_BRIDGE_PORT="${AIKACLAW_BRIDGE_PORT:-18790}"
export AIKACLAW_GATEWAY_BIND="${AIKACLAW_GATEWAY_BIND:-lan}"
export AIKACLAW_IMAGE="$IMAGE_NAME"
export AIKACLAW_DOCKER_APT_PACKAGES="${AIKACLAW_DOCKER_APT_PACKAGES:-}"
export AIKACLAW_EXTENSIONS="${AIKACLAW_EXTENSIONS:-}"
export AIKACLAW_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export AIKACLAW_HOME_VOLUME="$HOME_VOLUME_NAME"
export AIKACLAW_ALLOW_INSECURE_PRIVATE_WS="${AIKACLAW_ALLOW_INSECURE_PRIVATE_WS:-}"
export AIKACLAW_SANDBOX="$SANDBOX_ENABLED"
export AIKACLAW_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export AIKACLAW_TZ="$TIMEZONE"
...
if [[ -z "${AIKACLAW_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    AIKACLAW_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $AIKACLAW_CONFIG_DIR/aikaclaw.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      AIKACLAW_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      AIKACLAW_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      AIKACLAW_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${AIKACLAW_CONFIG_DIR}:/home/node/.aikaclaw"
    gateway_workspace_mount="${AIKACLAW_WORKSPACE_DIR}:/home/node/.aikaclaw/workspace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${AIKACLAW_INSTALL_DOCKER_CLI:-}" ]]; then
    export AIKACLAW_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  AIKACLAW_CONFIG_DIR \
  AIKACLAW_WORKSPACE_DIR \
  AIKACLAW_GATEWAY_PORT \
  AIKACLAW_BRIDGE_PORT \
  AIKACLAW_GATEWAY_BIND \
  AIKACLAW_GATEWAY_TOKEN \
  AIKACLAW_IMAGE \
  AIKACLAW_EXTRA_MOUNTS \
  AIKACLAW_HOME_VOLUME \
  AIKACLAW_DOCKER_APT_PACKAGES \
  AIKACLAW_EXTENSIONS \
  AIKACLAW_SANDBOX \
  AIKACLAW_DOCKER_SOCKET \
  DOCKER_GID \
  AIKACLAW_INSTALL_DOCKER_CLI \
  AIKACLAW_ALLOW_INSECURE_PRIVATE_WS \
  AIKACLAW_TZ

if [[ "$IMAGE_NAME" == "aikaclaw:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  docker build \
    --build-arg "AIKACLAW_DOCKER_APT_PACKAGES=${AIKACLAW_DOCKER_APT_PACKAGES}" \
    --build-arg "AIKACLAW_EXTENSIONS=${AIKACLAW_EXTENSIONS}" \
    --build-arg "AIKACLAW_INSTALL_DOCKER_CLI=${AIKACLAW_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the AikaClaw metadata subdirectory
# (.aikaclaw/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh aikaclaw-gateway -c \
  'find /home/node/.aikaclaw -xdev -exec chown node:node {} +; \
   [ -d /home/node/.aikaclaw/workspace/.aikaclaw ] && chown -R node:node /home/node/.aikaclaw/workspace/.aikaclaw || true'

echo ""
echo "==> Onboarding (interactive)"
echo "Docker setup pins Gateway mode to local."
echo "Gateway runtime bind comes from AIKACLAW_GATEWAY_BIND (default: lan)."
echo "Current runtime bind: $AIKACLAW_GATEWAY_BIND"
echo "Gateway token: $AIKACLAW_GATEWAY_TOKEN"
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm aikaclaw-cli channels login"
...
# --- Sandbox setup (opt-in via AIKACLAW_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -f "$ROOT_DIR/Dockerfile.sandbox" ]]; then
    echo "Building sandbox image: aikaclaw-sandbox:bookworm-slim"
    docker build \
      -t "aikaclaw-sandbox:bookworm-slim" \
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker aikaclaw-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg AIKACLAW_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (AIKACLAW_IMAGE=aikaclaw:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
YAML
    ...
    echo "WARNING: AIKACLAW_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in AikaClaw config.
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate aikaclaw-gateway
  fi
else
...
echo "Config: $AIKACLAW_CONFIG_DIR"
echo "Workspace: $AIKACLAW_WORKSPACE_DIR"
echo "Token: $AIKACLAW_GATEWAY_TOKEN"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f aikaclaw-gateway"
echo "  ${COMPOSE_HINT} exec aikaclaw-gateway node dist/index.js health --token \"$AIKACLAW_GATEWAY_TOKEN\""
