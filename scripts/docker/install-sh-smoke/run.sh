...
SMOKE_PREVIOUS_VERSION="${AIKACLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${AIKACLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="aikaclaw"
PACKAGE_NAME="${AIKACLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (AIKACLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${AIKACLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${AIKACLAW_INSTALL_LATEST_OUT:-}"
fi
