...
const extensionRestartMetadataFiles = new Set(["aikaclaw.plugin.json", "package.json"]);
...
const shouldBuild = (deps) => {
  if (deps.env.AIKACLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.AIKACLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[aikaclaw] ${message}\n`);
};

const runAikaClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["aikaclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
