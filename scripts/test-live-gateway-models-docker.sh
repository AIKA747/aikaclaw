...
elif [[ -n "${AIKACLAW_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(aikaclaw_live_collect_auth_dirs_from_csv "${AIKACLAW_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
docker run --rm -t \
  ...
  -e AIKACLAW_LIVE_GATEWAY_MODELS="${AIKACLAW_LIVE_GATEWAY_MODELS:-modern}" \
  -e AIKACLAW_LIVE_GATEWAY_PROVIDERS="${AIKACLAW_LIVE_GATEWAY_PROVIDERS:-}" \
  -e AIKACLAW_LIVE_GATEWAY_MAX_MODELS="${AIKACLAW_LIVE_GATEWAY_MAX_MODELS:-24}" \
  -e AIKACLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${AIKACLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-}" \
  -v "$ROOT_DIR":/src:ro \
