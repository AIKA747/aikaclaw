---
name: aikaclaw-test-heap-leaks
description: Investigate `pnpm test` memory growth, Vitest worker OOMs, and suspicious RSS increases in AikaClaw using the `scripts/test-parallel.mjs` heap snapshot tooling. Use when Codex needs to reproduce test-lane memory growth, collect repeated `.heapsnapshot` files, compare snapshots from the same worker PID, distinguish transformed-module retention from real data leaks, and fix or reduce the impact by patching cleanup logic or isolating hotspot tests.
---

# AikaClaw Test Heap Leaks
...
1. Reproduce the failing shape first.
   ...
   - `pnpm canvas:a2ui:bundle && AIKACLAW_TEST_MEMORY_TRACE=1 AIKACLAW_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 AIKACLAW_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap AIKACLAW_TEST_WORKERS=2 AIKACLAW_TEST_MAX_OLD_SPACE_SIZE_MB=6144 pnpm test`
   - Keep `AIKACLAW_TEST_MEMORY_TRACE=1` enabled so the wrapper prints per-file RSS summaries alongside the snapshots.
...
- Direct comparison:
  - `node .agents/skills/aikaclaw-test-heap-leaks/scripts/heapsnapshot-delta.mjs before.heapsnapshot after.heapsnapshot`
- Auto-select earliest/latest snapshots per PID within one lane:
  - `node .agents/skills/aikaclaw-test-heap-leaks/scripts/heapsnapshot-delta.mjs --lane-dir .tmp/heapsnap/unit-fast`
- Useful flags:
  - `--top 40`
  - `--min-kb 32`
  - `--pid 16133`
