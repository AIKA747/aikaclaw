---
name: aikaclaw-pr-maintainer
description: Maintainer workflow for reviewing, triaging, preparing, closing, or landing AikaClaw pull requests and related issues. Use when Codex needs to validate bug-fix claims, search for related issues or PRs, apply or recommend close/reason labels, prepare GitHub comments safely, check review-thread follow-up, or perform maintainer-style PR decision making before merge or closure.
---

# AikaClaw PR Maintainer
...
- Use `--repo aikaclaw/aikaclaw` with `--match title,body` first.
...
```bash
gh search prs --repo aikaclaw/aikaclaw --match title,body --limit 50 -- "auto-update"
gh search issues --repo aikaclaw/aikaclaw --match title,body --limit 50 -- "auto-update"
gh search issues --repo aikaclaw/aikaclaw --match title,body --limit 50 \
