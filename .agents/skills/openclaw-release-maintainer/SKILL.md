---
name: aikaclaw-release-maintainer
description: Maintainer workflow for AikaClaw releases, prereleases, changelog release notes, and publish validation. Use when Codex needs to prepare or verify stable or beta release steps, align version naming, assemble release notes, check release auth requirements, or validate publish-time commands and artifacts.
---

# AikaClaw Release Maintainer
...
- Core `aikaclaw` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- Version locations include:
  ...
  - `apps/macos/Sources/AikaClaw/Resources/Info.plist`
...
- Every AikaClaw release ships the npm package and macOS app together.
- The production Sparkle feed lives at `https://raw.githubusercontent.com/aikaclaw/aikaclaw/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `aikaclaw` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `aikaclaw YYYY.M.D-beta.N`
...
```bash
  AIKACLAW_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/aikaclaw-npm-postpublish-verify.ts <published-version>
```
...
- Always validate the AikaClaw npm release path before creating the tag.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `aikaclaw/aikaclaw` and the real mac preflight in
  `aikaclaw/releases-private` for every release.
...
- AikaClaw publish uses GitHub trusted publishing.
...
- `.github/workflows/macos-release.yml` in `aikaclaw/aikaclaw` is now a
...
- Real mac preflight and real mac publish both use
  `aikaclaw/releases-private/.github/workflows/aikaclaw-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@aikaclaw/aikaclaw-release-managers` before publish continues.
- Mac publish uses
  `aikaclaw/releases-private/.github/workflows/aikaclaw-macos-publish.yml` for
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `aikaclaw/aikaclaw`
  automatically when `AIKACLAW_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for AikaClaw releases.
- `@aikaclaw/*` plugin publishes use a separate maintainer-only flow.
...
9. Start `.github/workflows/aikaclaw-npm-release.yml` with `preflight_only=true`
   and wait for it to pass.
10. Start `.github/workflows/macos-release.yml` in `aikaclaw/aikaclaw` and wait
...
13. Start `.github/workflows/aikaclaw-npm-release.yml` with the same tag for
    the real publish.
14. Wait for `npm-release` approval from `@aikaclaw/aikaclaw-release-managers`.
15. Start
    `aikaclaw/releases-private/.github/workflows/aikaclaw-macos-publish.yml`
...
- Use `aikaclaw-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
