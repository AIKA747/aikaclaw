...
```bash
export AIKACLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.aikaclaw/aikaclaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env AIKACLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `aikaclaw` wrapper for guest `message send/read`; `node aikaclaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/aikaclaw-parallels-smoke.*`
...
- host inbound nonce appears in `aikaclaw message read` output
