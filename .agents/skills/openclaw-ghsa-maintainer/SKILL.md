---
name: aikaclaw-ghsa-maintainer
description: Maintainer workflow for AikaClaw GitHub Security Advisories (GHSA). Use when Codex needs to inspect, patch, validate, or publish a repo advisory, verify private-fork state, prepare advisory Markdown or JSON payloads safely, handle GHSA API-specific publish constraints, or confirm advisory publish success.
---

# AikaClaw GHSA Maintainer

Use this skill for repo security advisory workflow only. Keep general release work in `aikaclaw-release-maintainer`.
...
```bash
fork=$(gh api /repos/aikaclaw/aikaclaw/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/aikaclaw/aikaclaw/security-advisories/<GHSA> \
